...
usage(int verbose)
{
	...
	fprintf(output, "fvwm-root version %s with support for: XBM"
#ifdef HAVE_PNG
	    ", PNG"
#endif
	    "\n", VERSION);
...
SetRootWindow(char *tline)
{
	...
	if (use_our_color_limit) {
		...
		PictureInitCMapRoot(dpy, !NoColorLimit, &colorLimitop, True,
		    True);
	} else {
		/*
	...
	if (depth == Pdepth) {
		...
		gc = fvwmlib_XCreateGC(dpy, root, GCForeground | GCBackground,
		    &gcv);
...
main(int argc, char **argv)
{
	...
	for (i = 1; i < argc - 1; i++) {
		if (strcasecmp(argv[i], "-r") == 0 ||
			...
			fprintf(stderr, "fvwm-root: unknown option '%s'\n",
			    argv[i]);
			fprintf(stderr, "Run '%s --help' to get the usage.\n",
			    argv[0]);
			exit(1);
		}
	}
	...
	(void) XGetWindowProperty(dpy, root, prop, 0L, 1L, True,
	...
	if (RetainPixmap && !Dummy) {
		...
		XChangeProperty(dpy, root, prop, XA_PIXMAP, 32,
		    PropModeReplace, (unsigned char *) &dp, 1);
	}
	XCloseDisplay(dpy);
