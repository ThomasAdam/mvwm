...
static void
PCopyArea(Display *dpy, Pixmap pixmap, Pixmap mask, int depth,
    Drawable d, GC gc,
    int src_x, int src_y, int src_w, int src_h, int dest_x, int dest_y)
{
	...
	if (depth == Pdepth) {
		...
		XCopyPlane(dpy, pixmap, d,
		    (my_gc != None) ? my_gc : gc,
		    src_x, src_y, src_w, src_h, dest_x, dest_y, 1);
	}
	if (my_gc != None) {
		XFreeGC(dpy, my_gc);
	} else {
...
static void
PTileRectangle(Display *dpy, Window win, Pixmap pixmap, Pixmap mask,
    int depth,
    int src_x, int src_y,
    ...
    Bool d_is_a_window)
{
	...
	if (try_to_grab) {
		fim =
		    FGetFImage(dpy, d, Pvisual, Pdepth, x, y, w, h, AllPlanes,
		    ZPixmap);
		if (PGrabImageError) {
#if 0
...
PCreateRenderPixmap(Display *dpy, Window win, Pixmap pixmap, Pixmap mask,
    ...
    int dest_y, int dest_w, int dest_h, Bool do_repeat, int *new_w,
    int *new_h, Bool *new_do_repeat, Pixmap *new_mask)
{
	...
	*new_do_repeat = do_repeat;
	...
	if (depth != Pdepth) {
		...
		if (pixmap_copy && gc) {
			XCopyPlane(dpy, pixmap, pixmap_copy, gc,
	...
	if (src_pix == ParentRelative) {
		pixmap_fim =
		    PGrabXImage(dpy, d, dest_x, dest_y, dest_w, dest_h,
		    d_is_a_window);
	} else {
		pixmap_fim =
		    FGetFImage(dpy, src_pix, Pvisual, Pdepth, src_x, src_y,
	...
	if (mask != None) {
		mask_fim =
		    FGetFImage(dpy, mask, Pvisual, 1, src_x, src_y, src_w,
	...
	*new_h = h;
							...
							if (am[m] > 0) {
								...
								if (am[m] <
									...
									c.blue
									    ...
									    (unsigned
									    short)
									    (((255 - am[m]) * dest_colors[l].blue + c.blue * am[m]) / 255);
									c.green = (unsigned short)
									    (((255 - am[m]) * dest_colors[l].green + c.green * am[m]) / 255);
									c.red
									    =
									    (unsigned
									    short)
									    (((255 - am[m]) * dest_colors[l].red + c.red * am[m]) / 255);
									l++;
									PictureAllocColor
									    (Pdpy,
									    Pcmap,
	...
	if (*new_mask && mono_gc) {
		FPutFImage(dpy, *new_mask, mono_gc, new_mask_fim,
		    0, 0, 0, 0, w, h);
	}
    ...
    int out_height)
{
	...
	if (!(src_fim =
		FGetFImage(dpy, src, Pvisual, depth, 0, 0, in_width,
		    in_height, AllPlanes, ZPixmap))) {
		return None;
	}
	if (mask != None) {
		mask_fim =
		    FGetFImage(dpy, mask, Pvisual, 1, 0, 0, in_width,
		    in_height, AllPlanes, ZPixmap);
		if (!mask_fim) {
			FDestroyFImage(dpy, mask_fim);
			return None;
	...
	out_fim =
	    FCreateFImage(dpy, Pvisual, Pdepth, ZPixmap, out_width,
	    out_height);
	if (gc == None) {
		gc = PictureDefaultGC(dpy, win);
	}
	...
	for (j = 0; j < out_height; j++) {
		for (i = 0; i < out_width; i++) {
			...
			if (cm[m] > 0) {
				c = colors[k++];
				PictureAllocColorAllProp(Pdpy, Pcmap, &c, i,
				    j, False, False, True);
			} else {
    ...
    int dest_x, int dest_y, int dest_w, int dest_h, int do_repeat)
{
	...
	if (t_fra.tint_percent > 0 || t_fra.added_alpha_percent < 100
		...
		xrs_pixmap =
		    PCreateRenderPixmap(dpy, win, pixmap, mask, alpha, depth,
		    t_fra.added_alpha_percent, t_fra.tint, t_fra.tint_percent,
		    d_is_a_window, d, gc, mono_gc, alpha_gc, src_x, src_y,
		    src_w, src_h, dest_x, dest_y, dest_w, dest_h, do_repeat,
	...
	if (do_repeat) {
		PTileRectangle(dpy, win, tmp_pixmap, tmp_mask, depth,
		    src_x, src_y, d, gc, mono_gc, dest_x, dest_y, dest_w,
		    dest_h);
	} else {
		PCopyArea(dpy, tmp_pixmap, tmp_mask, depth, d, gc,
		    src_x, src_y, src_w, src_h, dest_x, dest_y);
	}
	if (xrs_pixmap) {
    ...
    int dest_w, int dest_h, int do_repeat)
{
	PGraphicsRenderPixmaps(dpy, win, p->picture, p->mask, p->alpha,
	    p->depth, fra, d, gc, mono_gc, alpha_gc, src_x, src_y, src_w,
	    src_h, dest_x, dest_y, dest_w, dest_h, do_repeat);
}
...
{
	PGraphicsRenderPixmaps(dpy, None, pixmap, mask, alpha, depth, 0, d,
	    gc, None, None, src_x, src_y, src_w, src_h, dest_x, dest_y, src_w,
	    src_h, False);
}
    ...
    int src_x, int src_y, int src_w, int src_h, int dest_x, int dest_y)
{
	PGraphicsRenderPicture(dpy, None, p, 0, d, gc, None, None, src_x,
	    src_y, src_w, src_h, dest_x, dest_y, src_w, src_h, False);
}
    ...
    int dest_h)
{
	PGraphicsRenderPixmaps(dpy, win, pixmap, mask, alpha, depth, 0, d, gc,
	    mono_gc, None, src_x, src_y, dest_w, dest_h, dest_x, dest_y,
	    dest_w, dest_h, True);
}
    ...
    int dest_width, int dest_height, GC gc, GC mono_gc, GC alpha_gc)
{
	...
	pixmap =
	    CreateStretchPixmap(dpy, src->picture, src->width, src->height,
	...
	if (src->mask) {
		mask =
		    CreateStretchPixmap(dpy, src->mask, src->width,
		    src->height, 1, dest_width, dest_height, mono_gc);
	}
	if (src->alpha) {
		alpha =
		    CreateStretchPixmap(dpy, src->alpha, src->width,
    ...
    int dest_width, int dest_height, GC gc, GC mono_gc, GC alpha_gc)
{
	...
	pixmap =
	    CreateTiledPixmap(dpy, src->picture, src->width, src->height,
	...
	if (src->mask) {
		mask =
		    CreateTiledPixmap(dpy, src->mask, src->width, src->height,
		    dest_width, dest_height, 1, mono_gc);
	}
	if (src->alpha) {
		alpha =
		    CreateTiledPixmap(dpy, src->alpha, src->width,
    ...
    Bool parent_relative)
{
	...
	if (parent_relative) {
		 ...
		 */
		if (!XGetGeometry(dpy, win, &root, (int *) &junk,
			(int *) &junk, (unsigned int *) &sw,
			(unsigned int *) &sh, (unsigned int *) &junk,
			(unsigned int *) &junk)) {
			goto bail;
		}
...
#if 0
	fprintf(stderr, "Geo: %i,%i,%i,%i / %i,%i,%i,%i / %i,%i,%i,%i\n",
	...
	if (XRenderSupport && FRenderGetExtensionSupported()) {
		...
		if (FRenderRender(dpy, win, ParentRelative, None, None,
			Pdepth, 100, fra->tint, fra->tint_percent, r, gc,
			None, gx, gy, gw, gh, 0, 0, gw, gh, False)) {
			goto bail;
		}
		XFreePixmap(dpy, r);
	}
    ...
    int dest_x, int dest_y, int dest_w, int dest_h)
{
	 ...
	 */
	if (FRenderTintRectangle(dpy, win, None, tint, tint_percent, dest,
		dest_x, dest_y, dest_w, dest_h)) {
		return;
	}
#else
...
Pixmap
PGraphicsCreateDitherPixmap(Display *dpy, Window win, Drawable src,
    Pixmap mask, int depth, GC gc, int in_width, int in_height, int out_width,
    int out_height)
{
	return PCreateDitherPixmap(dpy, win, src, mask, depth, gc,
