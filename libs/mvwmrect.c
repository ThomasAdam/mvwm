...
/*
** mvwmrect.c:
** This file supplies routines for mvwm internal rectangle handling.
*/
...
static int
mvwmrect_do_intervals_intersect(int x1, int width1, int x2, int width2)
{
...
int
mvwmrect_do_rectangles_intersect(rectangle *r, rectangle *s)
{
	if (r->x + r->width <= s->x) {
		return 0;
	}
 ...
 * diff_g. */
void
mvwmrect_subtract_rectangles(rectangle *rdiff, rectangle *r1, rectangle *r2)
{
	rdiff->x = r1->x - r2->x;
	rdiff->y = r1->y - r2->y;
...
int
mvwmrect_rectangles_equal(rectangle *r1, rectangle *r2)
{
	if (r1 == r2) {
		return 1;
	}
...
int
mvwmrect_move_into_rectangle(rectangle *move_rec, rectangle *target_rec)
{
	int             has_changed = 0;

	if (!mvwmrect_do_intervals_intersect(move_rec->x, move_rec->width,
	...
	if (!mvwmrect_do_intervals_intersect(move_rec->y, move_rec->height,
...
int
mvwmrect_intersect_xrectangles(XRectangle * r1, XRectangle * r2)
{
