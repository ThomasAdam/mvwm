...
set_default_iconv_charsets(FlocaleCharset * fc)
{
	...
	while (FLC_GET_LOCALE_CHARSET(FLCIconvUtf8Charset, i) != NULL) {
		...
		while (FLC_GET_LOCALE_CHARSET(fc, j) != NULL) {
			if (is_iconv_supported(FLC_GET_LOCALE_CHARSET
...
set_iconv_charset_index(FlocaleCharset * fc)
{
	...
	while (FLC_GET_LOCALE_CHARSET(fc, i) != NULL) {
		if (is_iconv_supported(FLC_GET_ICONV_CHARSET
			(FLCIconvUtf8Charset), FLC_GET_LOCALE_CHARSET(fc,
			    i))) {
    ...
    const char *in, unsigned int in_size)
{
	...
	for (is_finished = 0; is_finished == 0;) {
		nconv =
		    Ficonv(cd, (ICONV_ARG_CONST char **) &inptr, &insize,
...
char           *
FiconvUtf8ToCharset(Display *dpy, FlocaleCharset * fc,
    const char *in, unsigned int in_size)
{
	char           *out = NULL;
	FlocaleCharset *my_fc = NULL;
	...
	if (FLC_ENCODING_TYPE_IS_UTF_8(my_fc)) {
		...
		out =
		    convert_charsets(FLC_GET_ICONV_CHARSET
		    (FLCIconvUtf8Charset), to_cs, in, in_size);
	}
...
char           *
FiconvCharsetToUtf8(Display *dpy, FlocaleCharset * fc,
    const char *in, unsigned int in_size)
{
	char           *out = NULL;
	FlocaleCharset *my_fc = NULL;
    ...
    FlocaleCharset * out_fc, const char *in, unsigned int in_size)
{
	...
	if (!FLC_ENCODING_TYPE_IS_UTF_8(my_in_fc)) {
		tmp =
		    FiconvCharsetToUtf8(dpy, my_in_fc, (const char *) in,
		    in_size);
		if (tmp != NULL) {
			free_tmp = True;
			tmp_len = strlen(tmp);
	...
	if (!FLC_ENCODING_TYPE_IS_UTF_8(my_out_fc)) {
		out =
		    FiconvUtf8ToCharset(dpy, my_out_fc, (const char *) tmp,
		    tmp_len);
		if (free_tmp) {
			free(tmp);
		}
