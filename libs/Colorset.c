...
static int
get_aspect_dimensions(int *ret_w, int *ret_h, int dest_w, int dest_h,
    int src_w, int src_h)
{
	double          ax;
	double          ay;
    ...
    unsigned int depth, GC gc, Bool clear_area)
{
	...
	if (FHaveShapeExtension && colorset->shape_mask) {
		mask =
		    CreateBackgroundPixmap(dpy, None, width, height, colorset,
		    1, None, True);
		if (mask != None) {
			FShapeCombineMask(dpy, win, FShapeBounding, 0, 0,
			    mask, FShapeSet);
	...
	if (cs_t->pixmap == None) {
		...
		*h = cs_t->height;
		...
		case PIXMAP_STRETCH_ASPECT:
			get_aspect_dimensions(w, h, width, height,
			    cs_t->width, cs_t->height);
			break;
 ...
 * window */
Pixmap
CreateOffsetBackgroundPixmap(Display *dpy, Window win, int x, int y,
    ...
    Bool is_shape_mask)
{
	...
	if (colorset->pixmap == ParentRelative && !is_shape_mask &&
		...
		if (is_bad_gc == 0 &&
			...
			PGraphicsRenderPixmaps(dpy, win, pixmap, None, None,
			    Pdepth, &fra, pixmap,
			    fill_gc, None, None,
			    0, 0, width, height, 0, 0, width, height, False);
		}
		XSync(dpy, False);
	...
	if (!is_shape_mask) {
		...
		cs_keep_aspect =
		    (colorset->shape_type == SHAPE_STRETCH_ASPECT);
	...
	if (cs_pixmap == None) {
		...
		trim_side =
		    ...
		    height, cs_width, cs_height);
		big_pixmap =
		    CreateStretchPixmap(dpy, cs_pixmap, cs_width, cs_height,
		    depth, big_width, big_height, gc);
 ...
 * drawable. */
void
SetClippedRectangleBackground(Display *dpy, Window win, int x, int y,
    ...
    unsigned int depth, GC gc)
{
	...
	Bool            keep_aspect =
	    (colorset->pixmap_type == PIXMAP_STRETCH_ASPECT);
	Bool            stretch_x =
	    (colorset->pixmap_type == PIXMAP_STRETCH_X)
	    || (colorset->pixmap_type == PIXMAP_STRETCH);
	Bool            stretch_y =
	    (colorset->pixmap_type == PIXMAP_STRETCH_Y)
	    || (colorset->pixmap_type == PIXMAP_STRETCH);
	...
	if (!colorset->pixmap) {
		...
		if (stretch_x || stretch_y) {
			if (!stretch_x && colorset->width != width) {
				pixmap2 =
				    CreateStretchXPixmap(dpy, pixmap,
				    colorset->width, height, depth, width,
				    gc);
				XFreePixmap(dpy, pixmap);
				pixmap = pixmap2;
			}
			if (!stretch_y && colorset->height != height) {
				pixmap2 =
				    CreateStretchYPixmap(dpy, pixmap, width,
		...
		if (pixmap) {
			 ...
			 */
			XCopyArea(dpy, pixmap, win, draw_gc,
