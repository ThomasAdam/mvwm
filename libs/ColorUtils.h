...
int             pixel_to_color_string(Display *dpy, Colormap cmap,
    Pixel pixel, char *output, Bool use_hash);
...
Pixel           mvwmlib_clone_color(Pixel p);
void            mvwmlib_free_colors(Display *dpy, Pixel *pixels, int n,
    Bool no_limit);
void            mvwmlib_copy_color(Display *dpy, Pixel *dst_color,
    Pixel *src_color, Bool do_free_dest, Bool do_copy_src);
