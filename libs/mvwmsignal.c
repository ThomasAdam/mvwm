...
#include <setjmp.h>
#include <errno.h>
#include "mvwmsignal.h"

#define true   1
#define false  0

typedef enum
{ SIG_INIT = 0, SIG_DONE } SIG_STATUS;
...
/*
 * mvwmSetSignalMask - store the set of mutually exclusive signals
 ...
 */
void
mvwmSetSignalMask(int sigmask)
{
	term_sigs = sigmask;
}

/*
 * mvwmGetSignalMask - get the set of signals that will terminate mvwm
 *
 * NOTE: We don't need this if we have POSIX.1 since we can install
 *       a signal mask automatically using sigaction()
 */
int
mvwmGetSignalMask(void)
{
	return term_sigs;
}
...
/*
 * mvwmSetTerminate - set the "end-of-execution" flag.
...
#ifdef HAVE_SELECT
/*
 * mvwmSelect - wrapper around the select() system call.
