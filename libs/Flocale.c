...
FlocaleUtf8ToUnicodeStr2b(char *str, int len, int *nl)
{
	...
	while (i < len && str[i] != 0) {
		if ((str[i] & 0x80) == 0) {
			...
			t = ((str[i] & 0x0f) << 12) + ((str[i +
				    1] & 0x3f) << 6) + (str[i + 2] & 0x3f);
			str2b[j].byte2 = (unsigned char) (t & 0xff);
			str2b[j].byte1 = (unsigned char) (t >> 8);
			i += 2;
		}
    ...
    int *nl)
{
	...
	if (flf->fc && StrEquals(flf->fc->x, "jisx0208.1983-0")) {
		tmp =
		    FiconvCharsetToCharset(dpy, flf->fc,
    ...
    int **l_to_v)
{
	...
	*nl = len;
	...
	if (FiconvSupport) {
		 ...
		 */
		tmp_str =
		    FiconvCharsetToUtf8(dpy, flf->str_fc, (const char *) str,
		    len);
		/*
		...
		if (tmp_str != NULL) {
			 ...
			 */
			str1 =
			    FiconvUtf8ToCharset(dpy, flf->str_fc,
	...
	if (FiconvSupport && do_iconv) {
		str2 =
		    FiconvCharsetToCharset(dpy, flf->str_fc, flf->fc,
		    (const char *) str1, len);
		if (str2 == NULL) {
			/*
			 * fail to convert
    ...
    int *do_free, int *len, superimpose_char_t **comb_chars, int **l_to_v)
{
	int             len2b;
	fws->e_str =
	    FlocaleEncodeString(dpy, flf, fws->str, do_free, *len, len, NULL,
	    comb_chars, l_to_v);
	fws->str2b = NULL;
	...
	if (flf->font != None) {
		if (FLC_ENCODING_TYPE_IS_UTF_8(flf->fc)) {
			fws->str2b =
			    FlocaleUtf8ToUnicodeStr2b(fws->e_str, *len,
			    &len2b);
		} else if (flf->flags.is_mb) {
			fws->str2b =
			    FlocaleStringToString2b(dpy, flf, fws->e_str,
			    *len, &len2b);
		}
	}
}
    ...
    FlocaleWinString *fws, int len, Bool image)
{
	...
	is_string16 = (FLC_ENCODING_TYPE_IS_UTF_8(flf->fc)
	    || flf->flags.is_mb);
    ...
    superimpose_char_t *comb_chars, int *pixel_pos)
{
	...
	if (flf->font != NULL) {
		...
		FlocaleFontStructDrawString(dpy, flf, canvas_pix, font_gc, 0,
		    height - descent,
		    fg, fgsh, has_fg_pixels, fws, len, True);
	...
	if (comb_chars != NULL) {
		while (comb_chars[i].c.byte1 != 0
			...
			int             curr_len =
			    ...
			    buf);
			int             out_len;
			char           *buf2 = FiconvUtf8ToCharset(dpy,
			    flf->str_fc,
			...
			if (flf->fontset != None) {
				...
				if (FLC_ENCODING_TYPE_IS_UTF_8(flf->fc)) {
					...
					tmp_fws.str2b =
					    FlocaleStringToString2b(dpy, flf,
	 ...
	 */
	if ((image =
		XCreateImage(dpy, Pvisual, 1, XYBitmap, 0,
		    (char *) normal_data, normal_w, normal_h, 8,
		    0)) == NULL) {
		return;
	}
	 ...
	 */
	XGetSubImage(dpy, canvas_pix, 0, 0, normal_w, normal_h,
	...
	if ((rotated_image =
		XCreateImage(dpy, Pvisual, 1, XYBitmap, 0,
		    (char *) rotated_data, rotated_w, rotated_h, 8,
		    0)) == NULL) {
		return;
	}

	rotated_image->byte_order = rotated_image->bitmap_bit_order =
	    MSBFirst;
	...
	XSetTSOrigin(dpy, my_gc, xpsh, ypsh);
	XFillRectangle(dpy, fws->win, my_gc, xpsh, ypsh, rotated_w,
	    rotated_h);
...
FlocaleFixNameForFontSet(Display *dpy, char *name, char *module)
{
	...
	if (charset == NULL && !strchr(name, '*') && !strchr(name, '?')) {
		...
		if ((test_font = XLoadQueryFont(dpy, name))) {
			charset =
			    FlocaleGetCharsetOfFontStruct(dpy, test_font);
...
FlocaleGetFont(Display *dpy, char *fontname, char *encoding, char *module)
{
	...
	while (!font && fn) {
		...
		if (!font && str && *str) {
			str =
			    GetQuotedString(str, &fn, ",", NULL, NULL, NULL);
		}
	}
	if (font == NULL) {
		if (fn != NULL && fn != fallback_font) {
    ...
    char *fullname, char *module)
{
	...
	if (fontname && strlen(fontname) > 3 &&
		...
		if (FftSupport) {
			flf =
			    FlocaleGetFftFont(dpy, fontname + 4, encoding,
			    module);
		}
		if (flf) {
...
FlocaleLoadFont(Display *dpy, char *fontname, char *module)
{
	...
	while (!flf && (fn && *fn)) {
		flf =
		    FlocaleGetFontOrFontSet(dpy, fn, encoding, fontname,
		    module);
		if (fn != NULL && fn != mb_fallback_font
		    && fn != fallback_font) {
			free(fn);
	...
	if (flf == NULL) {
		...
		if (Flocale != NULL) {
			...
			if ((flf =
				FlocaleGetFontSet(dpy, mb_fallback_font, NULL,
				    module)) != NULL) {
				flf->name = mb_fallback_font;
			}
		}
		if (flf == NULL) {
			...
			if ((flf =
				FlocaleGetFont(dpy, fallback_font, NULL,
				    module)) != NULL) {
				flf->name = fallback_font;
			} else if (!ask_default) {
				fprintf(stderr,
    ...
    unsigned long flags)
{
	...
	curr_str = fws->e_str;
	for (char_len = 0, i = 0;
	    i < len && curr_str[i] != 0; char_len++, i += curr_len) {
	...
	if (comb_chars != NULL && (comb_chars[0].c.byte1 != 0
		...
		for (i = 0, curr_pixel_pos = 0;
		    i < char_len; i++, curr_str += curr_len) {
	...
	if (fws->flags.text_rotation != ROTATION_0 &&
		 ...
		 */
		FlocaleRotateDrawString(dpy, flf, fws, fg, fgsh,
		    has_fg_pixels, len, comb_chars, pixel_pos);
		...
		FlocaleFontStructDrawString(dpy, flf, fws->win, fws->gc,
		    fws->x, fws->y, fg, fgsh, has_fg_pixels, fws, len, False);
	}
...
FlocaleTextWidth(FlocaleFont *flf, char *str, int sl)
{
	...
	if (sl == 1) {
		...
		tmp_str =
		    FlocaleEncodeString(Pdpy, flf, str, &do_free, sl, &new_l,
		    NULL, &comb_chars, NULL);
	}
	/*
	...
	if (strlen(tmp_str) == 0 && comb_chars &&
		...
		if (FLC_ENCODING_TYPE_IS_UTF_8(flf->fc) || flf->flags.is_mb) {
			...
			if (FLC_ENCODING_TYPE_IS_UTF_8(flf->fc))
				str2b =
				    FlocaleUtf8ToUnicodeStr2b(tmp_str, new_l,
				    &nl);
			else
				str2b =
				    FlocaleStringToString2b(Pdpy, flf,
				    tmp_str, new_l, &nl);
			if (str2b != NULL) {
    ...
    XICCEncodingStyle style, XTextProperty * text_prop_return)
{
	...
	if (Flocale != NULL) {
		ret =
		    XmbTextListToTextProperty(dpy, list, count, style,
		    text_prop_return);
		if (ret == XNoMemory) {
			ret = False;
		} else {
	...
	if (!ret) {
		if (XStringListToTextProperty(list, count,
			text_prop_return) == 0) {
			ret = False;
		} else {
			ret = True;
			...
			if (verbose >= 2) {
				if (flf->fftf.fftfont != NULL) {
					...
					fprintf(stderr, "    Xft info:\n"
					    "      - Vertical font:");
					FftPrintPatternInfo(fftf->fftfont,
					    False);
					fprintf(stderr,
					    "      " "- Rotated font 90:");
