    ...
    unsigned int width, unsigned int height)
{
	...
	if (!fim->im) {
		if ((fim->im =
			XCreateImage(dpy, visual, depth, ZPixmap, 0, 0, width,
			    height, Pdepth > 16 ? 32 : (Pdepth > 8 ? 16 : 8),
			    0))) {
			fim->im->data =
			    mvwm_malloc(fim->im->bytes_per_line * height);
		} else {
			free(fim);
		}
	}
...
FImage         *
FGetFImage(Display *dpy, Drawable d, Visual * visual,
...
void
FPutFImage(Display *dpy, Drawable d, GC gc, FImage * fim, int src_x,
