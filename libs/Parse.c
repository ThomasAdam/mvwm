...
static char    *
CopyToken(char *src, char *dest, char *spaces, int snum, char *delims,
    ...
    char *out_delim)
{
	...
	indata = SkipSpaces(indata, spaces, snum);
	    ...
	    CopyToken(indata, tmptok, spaces, snum, delims, dnum, out_delim);
...
#if 0
int
CheckNTokens(char *indata, unsigned int n)
{
	unsigned int    i;
	char           *token;
...
{
	return _get_suffixed_integer_arguments(action, ret_action, retvals,
	    num, suffixlist, ret_suffixnum, "%d%n");
}
