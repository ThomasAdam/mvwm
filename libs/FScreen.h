...
/* Screen info */
Bool FScreenGetScrRect(fscreen_scr_arg *arg, int *x, int *y, int *w, int *h);
Bool FScreenGetScrId(fscreen_scr_arg *arg);
void FScreenTranslateCoordinates(
	fscreen_scr_arg *arg_src, const char *screen_src,
	fscreen_scr_arg *arg_dest, const char *screen_dest,
	int *x, int *y);
void FScreenGetResistanceRect(
	int wx, int wy, unsigned int ww, unsigned int wh, int *x0, int *y0,
	int *x1, int *y1);
Bool FScreenIsRectangleOnScreen(fscreen_scr_arg *arg, rectangle *rec);
const char	*FScreenOfPointerXY(int, int);


/* Clipping/positioning */
int FScreenClipToScreen(fscreen_scr_arg *arg, int *x, int *y, int w, int h);
void FScreenCenterOnScreen(fscreen_scr_arg *arg, int *x, int *y, int w, int h);
...
void FScreenMangleScreenIntoUSPosHints(const char *screen, XSizeHints *hints);
