...
static void
FftSetupEncoding(Display *dpy, FftFontType * fftf, char *encoding,
    char *module)
{
	FlocaleCharset *fc;
...
FftGetRotatedFont(Display *dpy, FftFont * f, rotation_t rotation)
{
	...
	if ((FftPatternGetMatrix(rotated_pat, FFT_MATRIX, 0,
		    &pm) == FftResultMatch) && pm) {
		/*
		 * rotate the matrice
		 */
...
FftGetFontWidths(FlocaleFont *flf, int *max_char_width)
{
	...
	if (FftUtf8Support && FLC_ENCODING_TYPE_IS_UTF_8(flf->fc)) {
		FftTextExtentsUtf8(fftdpy, flf->fftf.fftfont,
		    (FftChar8 *) "W", 1, &extents);
	} else {
		FftTextExtents8(fftdpy, flf->fftf.fftfont, (FftChar8 *) "W",
		    1, &extents);
	}
...
FftGetFont(Display *dpy, char *fontname, char *module)
{
	...
	if ((load_pat =
		FftFontMatch(dpy, fftscreen, src_pat, &result)) == NULL) {
		goto bail;
	}
	/*
	 * safety check
...
void
FftDrawString(Display *dpy, FlocaleFont *flf, FlocaleWinString *fws,
    Pixel fg, Pixel fgsh, Bool has_fg_pixels, int len, unsigned long flags)
{
	FftDraw        *fftdraw = NULL;
	typedef void    (*DrawStringFuncType) (FftDraw * fftdraw,
	    FftColor * fft_fg, FftFont * uf, int xt, int yt, char *str,
	    int len);
	...
	if (flf->shadow_size != 0 && has_fg_pixels) {
		while (FlocaleGetShadowTextPosition(&xt, &yt, &gstp_args)) {
			DrawStringFunc(fftdraw, &fft_fgsh, uf, xt, yt, str,
			    len);
		}
	}
	xt = gstp_args.orig_x;
...
FftTextWidth(FlocaleFont *flf, char *str, int len)
{
	...
	if (FftUtf8Support && FLC_ENCODING_TYPE_IS_UTF_8(flf->fc)) {
		...
		if (new != NULL) {
			FftTextExtents16(fftdpy, flf->fftf.fftfont, new, nl,
			    &extents);
			result = extents.xOff;
			free(new);
		}
	} else if (FLC_ENCODING_TYPE_IS_USC_2(flf->fc)) {
		FftTextExtents16(fftdpy, flf->fftf.fftfont, (FftChar16 *) str,
		    len, &extents);
		result = extents.xOff;
	} else if (FLC_ENCODING_TYPE_IS_USC_4(flf->fc)) {
		FftTextExtents32(fftdpy, flf->fftf.fftfont, (FftChar32 *) str,
		    len, &extents);
		result = extents.xOff;
	} else {
		FftTextExtents8(fftdpy, flf->fftf.fftfont, (FftChar8 *) str,
		    len, &extents);
		result = extents.xOff;
	}
...
FftPrintPatternInfo(FftFont * f, Bool vertical)
{
	...
	if (!vertical) {
		...
		if (FftPatternGetMatrix(f->pattern, FFT_MATRIX, 0,
			&pm) == FftResultMatch && pm) {
