    ...
    int src_depth, int dest_width, int dest_height, GC gc)
{
	...
	pixmap =
	    CreateStretchYPixmap(dpy, temp_pixmap, dest_width, src_height,
    ...
    int dest_width, int dest_height, int depth, GC gc)
{
	...
	xgcv.ts_x_origin = 0;
	xgcv.ts_y_origin = 0;
	XChangeGC(dpy, gc, GCFillStyle | GCTile | GCTileStipXOrigin |
    ...
    int depth, GC gc, int rotation)
{
	...
	if (!(src_fim =
		FGetFImage(dpy, src, Pvisual, depth, 0, 0, src_width,
		    src_height, AllPlanes, ZPixmap))) {
		error = True;
		goto bail;
	}
	if (!(fim =
		FCreateFImage(dpy, Pvisual, depth, ZPixmap, dest_width,
		    dest_height))) {
		error = True;
		goto bail;
	}
...
AllocLinearGradient(char *s_from, char *s_to, int npixels,
    int skip_first_color, int dither)
{
	XColor         *xcs;
	XColor          from, to, c;
	float           r;
    ...
    int dither)
{
	...
	for (i = 0; i < nsegs; ++i) {
		...
		if (n > 1) {
			c = AllocLinearGradient(s_colors[i], s_colors[i + 1],
			    n, skip_first_color, dither);
...
XColor         *
AllocAllGradientColors(char *color_names[], int perc[], int nsegs,
    int ncolors, int dither)
{
	 ...
	 */
	xcs =
	    AllocNonlinearGradient(color_names, perc, nsegs, ncolors, dither);
	for (i = 0; i <= nsegs; i++) {
		if (color_names[i]) {
			free(color_names[i]);
		}
    ...
    int **perc_return, int *nsegs_return)
{
	...
	if (GetIntegerArguments(item, NULL, &nsegs, 1) != 1) {
		...
		if (s_colors[nsegs] == NULL) {
			fprintf(stderr, "ParseGradient: too few gradient"
			    " segments: '%s'\n", orig);
	...
	for (i = 0, sum = 0; !is_syntax_error && i < nsegs; ++i) {
		...
		if (sum < old_sum) {
			 ...
			 * integer overflow
			 */
			fprintf(stderr, "ParseGradient: multi gradient"
			    " overflow: '%s'", orig);
			is_syntax_error = 1;
			break;
		}
    ...
    int d_width, int d_height, XRectangle * rclip)
{
	...
	if (d_pixels != NULL && *d_pixels != NULL) {
		if (d_npixels != NULL && *d_npixels > 0) {
			PictureFreeColors(dpy, Pcmap, *d_pixels, *d_npixels,
			    0, False);
		}
		free(*d_pixels);
		*d_pixels = NULL;
	...
	if (dither) {
		pica =
		    PictureOpenImageColorAllocator(dpy, Pcmap, t_width,
	...
	case S_GRADIENT:
		{
			...
			for (i = 0; i < t_width; i++) {
				register int    pi =
				    min(i, t_width - 1 - i) * t_height;
				for (j = 0; j < t_height; j++) {
					register int    pj =
					    min(j,
					    t_height - 1 - j) * t_width;
					c = xcs[(min(pi,
						    pj) * myncolors -
						1) / t_scale];
					if (dither) {
	...
	case C_GRADIENT:
		{
			...
			for (i = 0; i < t_width; i++) {
				for (j = 0; j < t_height; j++) {
					register double x =
					    (double) ((2 * i -
						t_width) * t_height) / 4.0;
					register double y =
					    (double) ((t_height -
						2 * j) * t_width) / 4.0;
					register double rad =
					    sqrt(x * x + y * y);
					c = xcs[(int) ((rad * ncolors -
						    0.5) / t_scale)];
					if (dither) {
						PictureAllocColorImage(dpy,
						    pica, &c, i, j);
	...
	xgcv.clip_mask = None;
	XChangeGC(dpy, gc,
	    GCFunction | GCPlaneMask | GCFillStyle | GCClipMask, &xgcv);
	if (rclip) {
...
CreateGradientPixmapFromString(Display *dpy, Drawable d, GC gc, int type,
    char *action, int *width_return, int *height_return,
    Pixel **pixels_return, int *nalloc_pixels, int dither)
{
	...
	if (CalculateGradientDimensions(dpy, d, ncolors, type, dither,
		...
		pixmap =
		    CreateGradientPixmap(dpy, d, gc, type, *width_return,
		    *height_return, ncolors, xcs, dither, &d_pixels,
		    &d_npixels, None, 0, 0, 0, 0, NULL);
	}
