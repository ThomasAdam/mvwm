		...
		if (cs.charset_count > 0) {
			...
			for (i = 0; i < FLCXOMCharsetList_num; i++) {
				...
				fprintf(stderr,
				    ...
				    "%i: %s, bidi:%s\n",
				    i,
				    FLC_DEBUG_GET_X_CHARSET(FLCXOMCharsetList
					[i]),
    ...
    char *encoding, char *module)
{
	...
	if (hints && *hints) {
		iconv =
		    GetQuotedString(hints, &charset, "/", NULL, NULL, NULL);
	...
	if (flf->fc == NULL) {
		if (FftSupport && flf->fftf.fftfont != NULL) {
			flf->fc =
			    FlocaleCharsetOfXCharset(flf->fftf.encoding);
	...
	if (flf->fc != NULL && iconv && *iconv) {
		...
		while (!iconv_found &&
			...
			if (strcmp(iconv,
				FLC_GET_LOCALE_CHARSET(flf->fc, i)) == 0) {
				iconv_found = True;
				/*
				 * Trust the user? yes ...
				 */
	...
	if (encoding != NULL) {
		...
		if (flf->fftf.str_encoding != NULL) {
			...
			if (flf->str_fc == NULL) {
				flf->str_fc =
				    FlocaleCharsetOfLocaleCharset(flf->fftf.
				    str_encoding);
			}
			if (flf->str_fc == NULL) {
				flf->str_fc = &UnknownCharset;
