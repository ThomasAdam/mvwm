...
FRenderCreateShadePicture(Display *dpy, Window win, int alpha_percent)
{
	...
	if (!shade_pixmap || !shade_picture) {
		...
		if (shade_pixmap) {
			shade_picture =
			    FRenderCreatePicture(dpy, shade_pixmap,
	...
	if (shade_picture &&
		...
		FRenderFillRectangle(dpy, FRenderPictOpSrc, shade_picture,
		    &frc, 0, 0, 1, 1);
    ...
    int dest_h)
{
	...
	if (!(dest_picture =
		FRenderCreatePicture(dpy, d, PFrenderVisualFormat, val, &pa))) {
		return (0);
	}
    ...
    Bool do_repeat)
{
	...
	if (Pdepth != depth && pixmap) {
		pixmap_copy =
		    PictureBitmapToPixmap(dpy, win, pixmap, Pdepth, gc, src_x,
		    src_y, src_w, src_h);
		src_x = src_y = 0;
	}
	...
	if (pixmap == ParentRelative) {
		...
		if (pixmap_copy && gc) {
			XCopyArea(dpy, win, pixmap_copy, gc,
		...
		pixmap_copy = XCreatePixmap(dpy, win, src_w, src_h, Pdepth);
		if (pixmap_copy && gc) {
			XCopyArea(dpy, pixmap, pixmap_copy, gc,
			    src_x, src_y, src_w, src_h, 0, 0);
		}
		src_x = src_y = 0;
	} else if (!pixmap_copy) {
		src_picture =
		    FRenderCreatePicture(dpy, pixmap, PFrenderVisualFormat,
		    pam, &pa);
	}

	if (!src_picture && pixmap_copy) {
		src_picture =
		    FRenderCreatePicture(dpy, pixmap_copy,
	...
	if (added_alpha_percent >= 100) {
		if (alpha != None) {
			alpha_picture =
			    FRenderCreatePicture(dpy, alpha,
			    PFrenderAlphaFormat, pam, &pa);
		} else if (mask != None) {
			alpha_picture =
			    FRenderCreatePicture(dpy, mask,
		...
		if (alpha != None) {
			...
			if (alpha_copy && alpha_gc) {
				...
				alpha_picture =
				    FRenderCreatePicture(dpy, alpha_copy,
				    PFrenderAlphaFormat, pam, &pa);
			}
			...
			if (alpha_picture) {
				frc.red = frc.green = frc.blue = frc.alpha =
				    0;
		...
		if (!(shade_picture =
			FRenderCreateShadePicture(dpy, win,
			    added_alpha_percent))) {
			goto bail;
		}
