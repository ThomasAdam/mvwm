 ...
 * function in one atomic operation. This ensures that mvwm will not
...
#define BSD_BLOCK_SIGNALS      int old_mask = sigblock( mvwmGetSignalMask() )
 ...
 * Module prototypes
 */
RETSIGTYPE      mvwmReapChildren(int sig);
extern void     mvwmSetTerminate(int sig);

#ifdef USE_BSD_SIGNALS
extern void     mvwmSetSignalMask(int);
extern int      mvwmGetSignalMask(void);
#endif

#ifdef  HAVE_SELECT
extern int      mvwmSelect(fd_set_size_t nfds, fd_set * readfds,
