...
/* This module, MvwmProxy, is an original work by Jason Weber.
 *
...
static MvwmPicture **pictureArray=NULL;
...
static void ExpandSlots(int slots)
{
	...
	if(slots>numSlots)
	{
		pictureArray=(MvwmPicture**)realloc(pictureArray,
			sizeof(MvwmPicture*)*(slots));
...
static void LinkSlotAction(char *string)
{
	...
	if (strncasecmp(token, "Click", 5) == 0)
	{
		...
		if (i > 0 && b >=1 && b <= NUMBER_OF_EXTENDED_MOUSE_BUTTONS)
		{
			...
			slot_action_list[b-1][slot] = mvwm_strdup(string);
...
static void LinkAction(char *string)
{
	...
	else if (StrEquals(token, "Select"))
	{
		...
		action_list[PROXY_ACTION_SELECT] = mvwm_strdup(string);
	}
	else if (StrEquals(token, "Show"))
	{
		...
		action_list[PROXY_ACTION_SHOW] = mvwm_strdup(string);
	}
	else if (StrEquals(token, "Hide"))
	{
		...
		action_list[PROXY_ACTION_HIDE] = mvwm_strdup(string);
	}
	else if (StrEquals(token, "Abort"))
	{
		...
		action_list[PROXY_ACTION_ABORT] = mvwm_strdup(string);
	}
	else if (StrEquals(token, "Mark"))
	{
		...
		action_list[PROXY_ACTION_MARK] = mvwm_strdup(string);
	}
	else if (StrEquals(token, "Unmark"))
	{
		...
		action_list[PROXY_ACTION_UNMARK] = mvwm_strdup(string);
	}
	else if (StrEquals(token, "ModifierRelease"))
	{
		...
		action_list[PROXY_ACTION_MODIFIER_RELEASE] = mvwm_strdup(string);
	}
...
MvwmPicture* loadPicture(char* name)
{
	MvwmPicture *picture=NULL;
	...
	picture = PGetMvwmPicture(dpy, RootWindow(dpy,screen),
		ImagePath, name, fpa);
	if (!picture)
	{
...
static Bool parse_options(void)
{
		...
		GetConfigLine(fd, &tline))
	{
		...
		else if (StrEquals(resource, "SmallFont"))
		{
			...
			small_font_name = mvwm_strdup(tline);
		}
...
{
	ProxyWindow *p = mvwm_malloc(sizeof(ProxyWindow));
...
static WindowName *new_WindowName(void)
{
	WindowName *p = mvwm_malloc(sizeof(WindowName));
...
{
	ProxyGroup *p = mvwm_malloc(sizeof(ProxyGroup));
...
static void send_command_to_mvwm(char *command, Window w)
{
...
static void DrawPicture(
	Window window, int x, int y, MvwmPicture *picture, int cset)
{
	...
	ProxyWindow *proxy, int x,int y,int w,int h)
{
	int texty,maxy;
	int cset;
	MvwmPicture *picture = &proxy->picture;
...
{
	SendMvwmPipe(fd, "SendToModule MvwmProxy Resolve",
		proxy->window);
}
	...
	for (proxy=firstProxy; proxy != NULL; proxy=proxy->next)
	{
			...
			proxy->group==instigator->group)
		{
			sprintf(commandBuffer,"Iconify %s",
				iconify? "On": "Off");
			send_command_to_mvwm(commandBuffer,proxy->window);
		}
	}
}
...
void RestackAtomic(int raise)
{
	...
	windows = mvwm_malloc (count * sizeof (Window));
	for (proxy=firstProxy; proxy != NULL; proxy=proxy->next)
	{
...
void ReadStack(unsigned long *body, unsigned long length)
{
	...
	fprintf(stderr, "ReadStack %p %d-%d\n",
		body, (int)length, MvwmPacketHeaderSize);
#endif

	for (i = 0; i < (length - MvwmPacketHeaderSize); i += 3)
	{
...
static ProxyWindow* FindNeighborForProxy(ProxyWindow* instigator)
{
	...
	for (proxy=firstProxy; proxy != NULL; proxy=proxy->next)
	{
			...
			proxy->proxy_group==instigator->proxy_group)
		{
			...
			{
				SendMvwmPipe(fd,
					"SendToModule MvwmProxy IsolateToggle",
					proxy->window);
			}
			if(proxy->group)
			{
...
static void ConfigureWindow(MvwmPacket *packet)
{
	...
	else
	{
			...
			proxy->w!=wsx || proxy->h!=wsy))
		{
			...
			sprintf(commandBuffer,
				"SendToModule MvwmProxy Stamp %d %d %d %d",
				proxy->x,proxy->y,proxy->w,proxy->h);
			SendMvwmPipe(fd, commandBuffer, proxy->window);
		}
		...
		else
		{
			...
			else
			{
				...
				if(neighbor && force_other)
				{
					...
					send_command_to_mvwm(commandBuffer,
						neighbor->window);
				}
			}
			AdhereGroup(neighbor);
...
static void StartProxies(void)
{
	...
	send_command_to_mvwm(action_list[PROXY_ACTION_SHOW], None);
	are_windows_shown = 1;
	CloseWindows();
	ReshuffleWindows();
	OpenWindows();
...
static void MarkProxy(ProxyWindow *new_proxy)
{
	...
	if (old_proxy != NULL)
		send_command_to_mvwm(action_list[PROXY_ACTION_UNMARK],
				old_proxy->window);
	if (selectProxy != NULL)
		send_command_to_mvwm(action_list[PROXY_ACTION_MARK],
				selectProxy->window);
...
static void SelectProxy(void)
{
	...
	HideProxies();
	if (selectProxy)
		send_command_to_mvwm(action_list[PROXY_ACTION_SELECT],
				selectProxy->window);

	send_command_to_mvwm(action_list[PROXY_ACTION_HIDE], None);
...
{
	HideProxies();
	send_command_to_mvwm(action_list[PROXY_ACTION_ABORT], None);
	selectProxy = NULL;
...
static void RotateIsolated(ProxyWindow *instigator,int direction)
{
	...
	{
		sprintf(commandBuffer,"Iconify Off");
		send_command_to_mvwm(commandBuffer,adjacent->window);
	}
...
static void ProcessMessage(MvwmPacket* packet)
{
	unsigned long type = packet->type;
	unsigned long length = packet->size;
	unsigned long* body = packet->body;
	MvwmWinPacketBodyHeader *bh = (void *)body;
	ProxyWindow *proxy;
	int x=0;
	int y=0;
	int w=0;
	...
	case M_CONFIGURE_WINDOW:
		...
		fprintf(stderr, "MvwmProxy ProcessMessage M_CONFIGURE_WINDOW\n");
#endif
	case M_ADD_WINDOW:
#if PROXY_EVENT_DEBUG
		...
		{
			fprintf(stderr,
				"MvwmProxy ProcessMessage M_ADD_WINDOW\n");
		}
#endif
...
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy ProcessMessage M_DESTROY_WINDOW\n");
#endif
		DestroyWindow(bh->w);
		break;
	case M_ICONIFY:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy ProcessMessage M_ICONIFY\n");
#endif
		IconifyWindow(FindProxy(bh->w), 1);
		break;
	case M_DEICONIFY:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy ProcessMessage M_DEICONIFY\n");
#endif
		IconifyWindow(FindProxy(bh->w), 0);
		break;
	case M_RAISE_WINDOW:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy ProcessMessage M_RAISE_WINDOW\n");
#endif
		RaiseLowerGroup(bh->w,1);
		break;
	case M_LOWER_WINDOW:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy ProcessMessage M_LOWER_WINDOW\n");
#endif
		RaiseLowerGroup(bh->w,0);
		break;
	case M_RESTACK:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy ProcessMessage M_RESTACK\n");
#endif
		ReadStack(body,length);
		break;
	case M_WINDOW_NAME:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy ProcessMessage M_WINDOW_NAME\n");
#endif
		...
		if (proxy != NULL)
		{
			...
			proxy->name = mvwm_strdup((char*)&body[3]);
#if PROXY_GROUP_DEBUG
	...
	case M_ICON_NAME:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy ProcessMessage M_ICON_NAME\n");
#endif
		proxy = FindProxy(bh->w);
		if (proxy != NULL)
		{
			...
			proxy->iconname = mvwm_strdup((char*)&body[3]);
#if PROXY_GROUP_DEBUG
...
*/
	...
	case M_NEW_DESK:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy ProcessMessage M_NEW_DESK\n");
#endif
	...
	case M_NEW_PAGE:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy ProcessMessage M_NEW_PAGE\n");
#endif
		deskNumber=body[2];
		ReshuffleWindows();
		break;
	case M_MINI_ICON:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy ProcessMessage M_MINI_ICON\n");
#endif
		proxy = FindProxy(bh->w);
		if (proxy != NULL)
		{
	...
	case M_FOCUS_CHANGE:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy ProcessMessage M_FOCUS_CHANGE\n");
#endif
	{
		focusWindow=bh->w;
		if(bh->w != 0)
	...
	case M_STRING:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy ProcessMessage M_STRING\n");
#endif
	{
		...
		else if (StrEquals(token, "Circulate"))
		{
			...
			strcat(commandBuffer," SendToModule MvwmProxy Mark");
			if (next)
				SendMvwmPipe(fd,commandBuffer,w);
		}
		else if (StrEquals(token, "Show"))
		{
			StartProxies();
	...
	case M_CONFIG_INFO:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy ProcessMessage M_CONFIG_INFO\n");
#endif
	{
		char *tline, *token;
...
static int My_XNextEvent(Display *dpy,XEvent *event)
{
	...
	if ( mvwmSelect(fd_width, &in_fdset, 0, 0, timeout) > 0)
	{
		...
		{
			MvwmPacket* packet = ReadMvwmPacket(fd[1]);
			if (!packet)
			{
				exit(0);
			}
...
static void DispatchEvent(XEvent *pEvent)
{
	...
	case Expose:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy DispatchEvent Expose\n");
#endif
		proxy = FindProxy(window);
		if (proxy != NULL)
		{
	...
	case ButtonPress:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy DispatchEvent ButtonPress\n");
#endif
		proxy = FindProxy(window);
		if (proxy)
		{
	...
	case MotionNotify:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy DispatchEvent MotionNotify\n");
#endif
		proxy = FindProxy(window);
	...
	case EnterNotify:
#if PROXY_EVENT_DEBUG
		fprintf(stderr, "MvwmProxy DispatchEvent EnterNotify\n");
#endif
...
int main(int argc, char **argv)
{
	...
	if (argc < 6)
	{
		fprintf(
			stderr,
			"MvwmProxy should only be executed by mvwm!\n");
		exit(1);
	}

	FlocaleInit(LC_CTYPE, "", "", "MvwmProxy");
	...
	miniIconGC=mvwmlib_XCreateGC(dpy,rootWindow,GCPlaneMask,&xgcv);
	fg_gc = mvwmlib_XCreateGC(dpy,rootWindow,GCPlaneMask,&xgcv);
	hi_gc = mvwmlib_XCreateGC(dpy,rootWindow,GCPlaneMask,&xgcv);
	sh_gc = mvwmlib_XCreateGC(dpy,rootWindow,GCPlaneMask,&xgcv);
	...
	if (parse_options() == False)
		exit(1);
	stampQueue = mvwm_malloc(sizeof(GeometryStamp) * stampLimit);
