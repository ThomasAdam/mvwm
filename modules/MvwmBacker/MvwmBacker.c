 ...
 * The functions in this source file that are based on part of the MvwmIdent
 * module for mvwm are noted by a small copyright atop that function, all others
...
#include "MvwmBacker.h"

/* migo (22-Nov-1999): Temporarily until mvwm_msg is moved to libs */
#define ERR
#define mvwm_msg(t,l,f) fprintf(stderr, "[mvwm][MvwmBacker]: <<ERROR>> %s\n", f)
...
char           *Module;		/* i.e. "MvwmBacker" */
char           *configPrefix;	/* i.e. "*MvwmBacker" */
...
main(int argc, char **argv)
{
	 ...
	 */
	SendInfo(mvwm_fd, "Send_WindowList", 0);

	/*
	 * tell mvwm we're running
	 */
	SendFinishedStartupNotification(mvwm_fd);

	/*
	 * Recieve all messages from mvwm
	 */
	EndLessLoop();
...
/*
  ReadMvwmPipe - Read a single message from the pipe from mvwm
*/
void
ReadMvwmPipe(void)
{
	MvwmPacket     *packet = ReadMvwmPacket(mvwm_fd[1]);
	if (packet == NULL) {
		exit(0);
	} else {
...
SetRootAtoms(Display *dpy2, Window root2, Pixmap RootPix)
{
	...
	if (RetainPixmap && RootPix != None) {
		XChangeProperty(dpy2, root2, XA_ESETROOT_PMAP_ID, XA_PIXMAP,
		    32, PropModeReplace, (unsigned char *) &l_root_pix, 1);
...
SetDeskPageBackground(const Command * c)
{
	...
	case 2:	/* colorset */
		...
		case 2:
			...
			if (CSET_IS_TRANSPARENT(c->colorset)) {
				mvwm_msg(ERR, "MvwmBacker", "You cannot "
				...
				pix =
				    CreateBackgroundPixmap(dpy2, root2,
				    MyDisplayWidth, MyDisplayHeight,
				    &Colorset[c->colorset], DefaultDepth(dpy2,
					screen2), DefaultGC(dpy2, screen2),
				    False);
				if (pix != None) {
					XSetWindowBackgroundPixmap(dpy2,
					    root2, pix);
					XClearWindow(dpy2, root2);
				}
			} else {
				SetWindowBackground(dpy2, root2,
				    MyDisplayWidth, MyDisplayHeight,
				    &Colorset[c->colorset], DefaultDepth(dpy2,
					screen2), DefaultGC(dpy2, screen2),
				    True);
		...
		case 1:	/* Process a solid color request */
			if (RetainPixmap) {
				...
				gc = mvwmlib_XCreateGC(dpy2, root2,
				    GCForeground, &xgcv);
				pix =
				    XCreatePixmap(dpy2, root2, 1, 1,
	...
	default:
		if (c->cmdStr != NULL) {
			SendMvwmPipe(mvwm_fd, c->cmdStr, (unsigned long) 0);
		}
		break;
	}
}
...
/*
  ProcessMessage - Process the message coming from mvwm
*/
void
...
/*
  ParseConfigLine - Parse the configuration line mvwm to us to use
*/
int
...
/*
  ParseConfig - Parse the configuration file mvwm to us to use
*/
void
ParseConfig(void)
{
	 ...
	 * TA:  FIXME!  xasprintf()
	 */
	line_start = mvwm_malloc(strlen(Module) + 2);
	strcpy(line_start, "*");
	strcat(line_start, Module);

	InitGetConfigLine(mvwm_fd, line_start);
	GetConfigLine(mvwm_fd, &tline);
	...
	while (*parens_run &&
		...
		if (StrEquals(name, "Desk")) {
			...
			*do_ignore_desk = False;
			...
			if ((option_val =
				GetNextToken(option_val, &value)) == NULL) {
				mvwm_msg(ERR, "MvwmBacker",
...
AddCommand(char *line)
{
	...
	this = mvwm_malloc(sizeof(Command));
	this->desk = 0;
	...
	if (strncasecmp(line, "Desk", 4) == 0) {
		...
		if (*line == '(') {
			...
			line++;
			line =
			    GetQuotedString(line, &parens, ")", NULL, NULL,
			    NULL);
			if (line == NULL) {
				mvwm_msg(ERR, "MvwmBacker",
	...
	if (strncasecmp(line, "-solid", 6) == 0) {
		...
		this->solidColor = (!token || !*token) ?
		    BlackPixel(dpy, screen) : BackerGetColor(token);
		free(token);
	} else if (strncasecmp(line, "colorset", 8) == 0) {
		/*
		 * Colorset command
		...
		this->type = 0;
		this->cmdStr = mvwm_malloc(strlen(line) + 1);
