...
volatile sig_atomic_t Bg;	/* MvwmCommand in background */
...
main(int argc, char *argv[])
{
	...
	mvwmSetSignalMask(sigmask(SIGINT) | sigmask(SIGHUP) | sigmask(SIGQUIT)
	...
	if (f_stem == NULL) {
		if ((f_stem = fifos_get_default_name()) == NULL) {
			fprintf(stderr, "\n%s can't decide on fifo-name. "
			    "Make sure that $MVWM_USERDIR is set.\n", MYNAME);
			exit(1);
		}
	}
	...
	if (Fdrun < 0) {
		...
		if ((f = fopen(s, "r")) != NULL) {
			...
			*cmd = 0;
			...
			fclose(f);
			fprintf(stderr,
			    "\nMvwmCommand lock file %sR is detected. "
			    "This may indicate another MvwmCommand is running. "
			    "It appears to be running under process ID:\n%s\n",
			    f_stem, (*cmd) ? cmd : "(unknown)");
			fprintf(stderr,
			    "You may either kill the process or run MvwmCommand "
	...
	if (Opt_stdin) {
		...
		ncnt = mvwmSelect(FD_SETSIZE, &fdset, 0, 0, &tv2);
		...
		while (!isTerminated) {
			...
			ncnt = mvwmSelect(FD_SETSIZE, &fdset, 0, 0, NULL);
...
receive(void)
{
	...
	if (Opt_reply) {
		...
		FD_SET(Fdr, &fdset);
		ncnt =
		    select(FD_SETSIZE, SELECT_FD_SET_CAST & fdset, 0, 0,
		    NULL);
	}
	...
	while (1) {
		...
		ncnt =
		    select(FD_SETSIZE, SELECT_FD_SET_CAST & fdset, 0, 0, &tv);
...
usage(void)
{
	...
	fprintf(stderr, "Send commands to mvwm via %sS\n\n", MYNAME);
	fprintf(stderr,
	...
	fprintf(stderr,
	    ...
	    MYNAME);
	fprintf(stderr, "  -i <info level>     0 - error only\n");
	fprintf(stderr,
	...
	fprintf(stderr, "  -F <flag info>      0 - no flag info\n");
	fprintf(stderr,
	    "                      2 - full flag info (default)\n");
	fprintf(stderr,
	    "  -m                  monitor mvwm message transaction\n");
	fprintf(stderr,
	    "  -r                  "
	    "wait for a reply (overrides waiting time)\n");
	fprintf(stderr, "  -v                  print version number\n");
	fprintf(stderr,
	    "  -w <micro sec>      waiting time for the reponse from mvwm\n");
...
list_configure(unsigned long *body)
{
	...
	printf("0x%08lx %-20s width %ld, height %ld\n", cfgpacket->w,
	    "base size", cfgpacket->hints_base_width,
	    cfgpacket->hints_base_height);
	printf("0x%08lx %-20s width %ld, height %ld\n", cfgpacket->w,
	    "size increment", cfgpacket->hints_width_inc,
	    cfgpacket->hints_height_inc);
	printf("0x%08lx %-20s width %ld, height %ld\n", cfgpacket->w,
	    "min size", cfgpacket->hints_min_width,
	    cfgpacket->hints_min_height);
	printf("0x%08lx %-20s width %ld, height %ld\n", cfgpacket->w,
	    "max size", cfgpacket->hints_max_width,
	    cfgpacket->hints_max_height);
	...
	printf("0x%08lx %-20s text 0x%lx, back 0x%lx\n", cfgpacket->w,
	    "pixel", cfgpacket->TextPixel, cfgpacket->BackPixel);
}
 ...
 */
void
list_icon_loc(unsigned long *body)
{
	printf("0x%08lx %-20s x %ld, y %ld, width %ld, height%ld\n",
 ...
 */
void
list(unsigned long *body, char *text)
{
...
{
	printf
	    ("           %-20s x %ld, y %ld, desk %ld, max x %ld, max y %ld\n",
	    "new page", body[0], body[1], body[2], body[3], body[4]);
}
 ...
 */
void
list_header(unsigned long *body, char *text)
{
	printf("0x%08lx %s\n", body[0], text);
}
 ...
 */
void
list_focus_change(unsigned long *body)
{
	printf
	    ("0x%08lx %-20s highlight 0x%lx, foreground 0x%lx, background 0x%lx\n",
