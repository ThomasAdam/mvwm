...
fifos_get_default_name(void)
{
	 ...
	 * * user or non-existing. If not: use MVWM_USERDIR as base instead.
	 */
	...
	for (c = file_suffix; *c != 0 && is_path_valid; c++) {
		...
		if (rc == 0) {
			 ...
			 * stat successful
			 */
			if (stat_buf.st_uid != owner ||
			    stat_buf.st_nlink > 1 ||
			    S_ISDIR(stat_buf.st_mode) ||
			    MVWM_S_ISLNK(stat_buf.st_mode) ||
			    (stat_buf.st_mode & MVWM_S_IFLNK) != 0) {
				is_path_valid = False;
			}
			break;
