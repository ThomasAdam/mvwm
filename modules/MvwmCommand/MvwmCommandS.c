...
#define MYNAME   "MvwmCommandS"

static int      Fd[2];		/* pipes to mvwm */
...
/* a queue of messages for MvwmCommand to receive */
typedef struct Q
{
...
{
	mvwmSetTerminate(signo);
	SIGNAL_RETURN;
}

/*
 * setup server and communicate with mvwm and the client
 */
void
server(char *name)
{
	...
	while (!isTerminated) {
		...
		ret =
		    mvwmSelect(FD_SETSIZE, &fdrset, &fdwset, 0,
		    queueing ? &tv : NULL);
		...
		if (queueing && ret == 0) {
			/*
			 * a timeout has occurred, this means the pipe to MvwmCommand is full
			...
			if (!Qstart->sent) {
				...
				fprintf(stderr,
				    "MvwmCommandS: leaving a partially sent message in the queue\n");
			...
			for (ix = 0; ix < len; ix++) {
				...
				if (cmd[cix] == '\n') {
					...
					if (StrHasPrefix(bugger_off, cmd))
						/*
						 * mvwm will close our pipes when it has processed this
						 */
		...
		if (queueing && FD_ISSET(FfdM, &fdwset)) {
			...
			/*
			 * send one packet to MvwmCommand, try to send it all but cope
			 * * with partial success
			 */
			sent =
			    write(FfdM, q->body + q->sent,
			    q->length - q->sent);
...
close_fifos(void)
{
	 ...
	 */
	if ((stat(FfdC_name, &stat_buf) == 0)
	    && (stat_buf.st_ino == FfdC_ino))
		unlink(FfdC_name);
	/*
	 * only unlink this file if it is the same as FfdM
	 */
	if ((stat(FfdM_name, &stat_buf) == 0)
	    && (stat_buf.st_ino == FfdM_ino))
		unlink(FfdM_name);
	/*
...
open_fifos(const char *f_stem)
{
	...
	/*
	 * check to see if another MvwmCommandS is running by trying to talk to it
	 */
	...
	unlink(FfdC_name);
	/*
	 * If a previous MvwmCommandS is lingering tell it to go away
	 ...
	 * * and any lingering MvwmCommandS will not share them or remove them
	 */
	if (mkfifo(FfdM_name, MVWM_S_IRUSR | MVWM_S_IWUSR) < 0) {
		err_msg(FfdM_name);
		return -1;
	}
	if (mkfifo(FfdC_name, MVWM_S_IRUSR | MVWM_S_IWUSR) < 0) {
		err_msg(FfdC_name);
		return -1;
	}
...
process_message(unsigned long type, unsigned long *body)
{
	...
	case M_CONFIGURE_WINDOW:
		relay_packet(type, sizeof(struct ConfigWinPacket) * SOL,
		    body);
		break;
 ...
 * the front end. In this way MvwmCommandS is always responsive to commands
 * from the input pipes. (it will also die a lot faster when mvwm quits)
 */
void
relay_packet(unsigned long type, unsigned long length, unsigned long *body)
{
	Q              *new;
