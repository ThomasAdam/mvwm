...
system( "MvwmCommand 'MvwmCommandS $fifo'");
#for slow machine
...
# start monitoring (-m option ) all mvwm transaction (-i3 option )
open( FCM, "MvwmCommand -f $fifo -m -i3 |" ) || die "FCM $fifo";
			...
			if( /^$id frame\s+x -?\d+, y (-?\d+), width (\d+)/ ) {
				...
				if( /^$id resource/ ) {
					...
					if( /\s+\w+popup/ ) {
						...
						# mvwm doesn't like commands from modules too fast
