...
#
#   AddToFunc "InitFunction" "I" Module MvwmBanner
#   + "I" Module	MvwmPager 0 8
...
#    window creation. A shell is invoked to avoid mvwm itself sleeps for
#    2 seconds.
#
...
$MVWMCOMMAND = "/usr/X11/lib/X11/mvwm/MvwmCommand";
# if not there, try this
if (! -x $MVWMCOMMAND) {
  $MVWMCOMMAND = "$ENV{HOME}/usr/X11/lib/X11/mvwm/MvwmCommand";
}
...
sub kill_server {
  send_cmd ("killme"); # kill MvwmCommandS
  close (FCM);
  close (FCC);
}
...
sub init {
  ...
  # start a client monitoring (-m option ) all mvwm transaction (-i3 option )
  # unlinking and verifying M FIFO ensures that new MvwmCommand is running
  unlink( "${FIFO}M" );
  open( FCM, "$main::MVWMCOMMAND -S $FIFO -f $FIFO  -m -i3 </dev/null|" )
    || die "FCM $FIFO";
  while (! -p "${FIFO}M") {
    sleep(1);
    if ($count++ > 5) {
