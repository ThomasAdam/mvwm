#! xPERLx
# MvwmCommand example - move a group of windows
...
open( FCM, "MvwmCommand -m -i3 |" ) || die "$! MvwmCommand";
open( FCC, ">$ENV{'HOME'}/.MvwmCommandC" ) || die "FCC";
select( FCC ); $| = 1;
select( STDOUT ); $| = 1;
...
sub ask {
	...
	# list up windows
	open( FCM, "MvwmCommand -i2 send_windowlist |" ) || die "$! MvwmCommand";
	while( <FCM> ) {
		if( /^0x(\S+) (id|window) +(.*)/ ) {
			$Config{$1}{$2} = $3;
