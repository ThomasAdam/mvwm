...
extern FvwmPagerScreenInfo FPScr;
extern Display *dpy;
...
initialize_viz_pager(void)
{
	...
	if (Pdefault) {
		...
		FPScr.NormalGC =
		    fvwmlib_XCreateGC(dpy, FPScr.Pager_w, 0, &xgcv);
	}
	FPScr.NormalGC = fvwmlib_XCreateGC(dpy, FPScr.Pager_w, 0, NULL);

	xgcv.plane_mask = AllPlanes;
	FPScr.MiniIconGC =
	    fvwmlib_XCreateGC(dpy, FPScr.Pager_w, GCPlaneMask, &xgcv);
...
initialize_balloon_window(void)
{
	...
	FPScr.balloon_w =
	    ...
	    &attributes);
	FPScr.balloon_gc = fvwmlib_XCreateGC(dpy, FPScr.balloon_w, 0, &xgcv);
	/*
...
initialize_pager(void)
{
	...
	if (windowcolorset >= 0) {
		...
		FPScr.whGC =
		    fvwmlib_XCreateGC(dpy, FPScr.Pager_w, GCForeground, &gcv);
	...
	for (i = 0; i < ndesks; i++) {
		...
		if (uselabel && Ffont && Ffont->font) {
			...
			Desks[i].NormalGC =
			    fvwmlib_XCreateGC(dpy, FPScr.Pager_w,
