/* -*-c-*- */
/* This module, and the entire MvwmAuto program, and the concept for
...
main(int argc, char **argv)
{
...
#ifdef DEBUGTOFILE
	freopen(".MvwmAutoDebug","w",stderr);
#endif

	module = ParseModuleArgs(argc,argv,0); /* no alias in this module */
	if (module==NULL)
	{
		fprintf(stderr,"MvwmAuto Version "VERSION" should only be executed by mvwm!\n");
		exit(1);
	}
	...
	{
		fprintf(stderr,"MvwmAuto can use one to five arguments.\n");
		exit(1);
	}
...
# else
	...
	*/
	...
	/* tell mvwm that we want to be lock on send */
	...
	while (!isTerminated)
	{
...
#endif
		if (mvwmSelect(fd_width, &in_fdset, NULL, NULL,
		...
		if (raise_window_now)
		{
			...
			/* force mvwm to synchronise on slow X connections to
