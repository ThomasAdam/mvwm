...
/*
 * This module is based on MvwmModuleDebugger which has the following
 * copyright:
 *
...
static event_entry extended_message_event_table[] = {
	...
	EVENT_ENTRY("reply", 0),	/* MvwmEvent will never receive MX_REPLY */
	EVENT_ENTRY(NULL, 0)
};
...
{
	char           *s;
	unsigned long   header[MvwmPacketHeaderSize];
	unsigned long   body[MvwmPacketBodyMaxSize];
	...
	*MyName = '*';
	...
	while (!isTerminated) {
		 ...
		 */
		total = 0;
		remaining = (header[2] - MvwmPacketHeaderSize) *
		    sizeof(unsigned long);
		while (remaining) {
			if ((count =
				read(fd[1], &body[total], remaining)) < 0) {
				isTerminated = 1;
				unlock_event(header[1]);
				continue;
			}
...
void
handle_config_line(char *buf, char **phost)
{
	char           *event;
	char           *action;
	char           *p;
...
config(void)
{
	...
	while (GetConfigLine(fd, &buf), buf != NULL && *buf != 0) {
		...
		/*
		 * Search for MyName (normally *MvwmEvent)
		 */
