...
main_loop(void)
{
	 ...
	 */
	if (fd_width < mvwm_fd[1]) {
		fd_width = mvwm_fd[1];
	}
	++fd_width;

	FD_ZERO(&saveset);
	FD_SET(mvwm_fd[1], &saveset);
	FD_SET(x_fd, &saveset);
	...
	while (!isTerminated) {
		...
		if (mvwmSelect(fd_width, &readset, NULL, NULL, &tv) < 0) {
			ConsoleMessage
...
main(int argc, char **argv)
{
	...
	FlocaleInit(LC_CTYPE, "", "", "MvwmIconMan");
	...
	*Module = '*';
	...
	mvwmSetSignalMask(sigmask(SIGPIPE) |
	    sigmask(SIGINT) |
	    sigmask(SIGHUP) | sigmask(SIGTERM) | sigmask(SIGQUIT));
#endif
	 ...
	 * extended messages
	 */
	SetMessageMask(mvwm_fd, MX_VISIBLE_ICON_NAME | MX_PROPERTY_CHANGE);

	SendText(mvwm_fd, "Send_WindowList", 0);
	 ...
	 */
	SetSyncMask(mvwm_fd, M_DEICONIFY | M_ICONIFY);
	SetNoGrabMask(mvwm_fd, M_DEICONIFY | M_ICONIFY);
