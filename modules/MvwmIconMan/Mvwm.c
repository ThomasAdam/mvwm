...
static WinData *mvwm_focus_win = NULL;
...
static void set_draw_mode(WinManager *man, int flag)
{
	...
	else if (man->we_are_drawing && !flag)
	{
		...
		ConsoleDebug(
			MVWM, "SetDrawMode on 0x%lx, num = %d\n",
...
static void got_configure(WinManager *man)
{
	if (man && !man->we_are_drawing)
	{
		...
		ConsoleDebug(
			MVWM, "got_configure on 0x%lx, num_expected now = %d\n",
...
static void set_win_configuration(WinData *win, MvwmPacketBody *body)
{
	...
	win->state = (win == mvwm_focus_win) ? FOCUS_CONTEXT : PLAIN_CONTEXT;
	win->geometry_set = 1;
...
static void configure_window(MvwmPacketBody *body)
{
	Ulong app_id = body->add_config_data.w;
	WinData *win;
	ConsoleDebug(MVWM, "configure_window: %ld\n", app_id);
...
static void focus_change(MvwmPacketBody *body)
{
	Ulong app_id = body->minimal_data.app_id;
	WinData *win = id_to_win(app_id);

	ConsoleDebug(MVWM, "Focus Change\n");
	ConsoleDebug(MVWM, "\tID: %ld\n", app_id);

	if (mvwm_focus_win &&
	    mvwm_focus_win->button &&
	    mvwm_focus_win->manager->showonlyfocused)
	  delete_windows_button(mvwm_focus_win);

	if (mvwm_focus_win && win != mvwm_focus_win)
	{
		del_win_state(mvwm_focus_win, FOCUS_CONTEXT);
		if (mvwm_focus_win->manager &&
			mvwm_focus_win->manager->focus_button)
		{
			mvwm_focus_win->manager->focus_button = NULL;
		}
		mvwm_focus_win = NULL;
	}
	...
	mvwm_focus_win = win;
	ConsoleDebug(MVWM, "leaving focus_change\n");
}

static void res_name(MvwmPacketBody *body)
{
	Ulong app_id = body->name_data.app_id;
	Uchar *name = body->name_data.name.name;
	WinData *win;

	ConsoleDebug(MVWM, "In res_name\n");
	...
	ConsoleDebug(MVWM, "Exiting res_name\n");
}

static void class_name(MvwmPacketBody *body)
{
	Ulong app_id = body->name_data.app_id;
	Uchar *name = body->name_data.name.name;
	WinData *win;

	ConsoleDebug(MVWM, "In class_name\n");
	...
	ConsoleDebug(MVWM, "Exiting class_name\n");
}

static void icon_name(MvwmPacketBody *body)
{
	WinData *win;
	Ulong app_id;
	Uchar *name = body->name_data.name.name;

	ConsoleDebug(MVWM, "In icon_name\n");
	...
	{
		ConsoleDebug(
			MVWM, "No icon change: %s %s\n", win->iconname, name);
		return;
	}

	copy_string(&win->iconname, (char *)name);
	ConsoleDebug(MVWM, "new icon name: %s\n", win->iconname);

	ConsoleDebug(MVWM, "Exiting icon_name\n");
}

static void visible_icon_name(MvwmPacketBody *body)
{
	WinData *win;
	Ulong app_id;
	Uchar *name = body->name_data.name.name;

	ConsoleDebug(MVWM, "In visible_icon_name\n");
...
static void window_name(MvwmPacketBody *body)
{
	WinData *win;
	Ulong app_id;
	Uchar *name = body->name_data.name.name;

	ConsoleDebug(MVWM, "In window_name\n");
	...
	{
		ConsoleDebug(
			MVWM, "No name change: %s %s\n", win->titlename, name);
		return;
	}

	copy_string(&win->titlename, (char *)name);

	ConsoleDebug(MVWM, "Exiting window_name\n");
}

static void visible_name(MvwmPacketBody *body)
{
	WinData *win;
	Ulong app_id;
	Uchar *name = body->name_data.name.name;

	ConsoleDebug(MVWM, "In visible_name\n");
	...
	{
		ConsoleDebug(
			MVWM, "No visible name change: %s %s\n",
	...
	ConsoleDebug(MVWM, "Exiting visible_name\n");
}


static void new_window(MvwmPacketBody *body)
{
	WinData *win;
...
static void destroy_window(MvwmPacketBody *body)
{
	WinData *win;
	Ulong app_id;
	...
	if (win->button)
	{
		ConsoleDebug(MVWM, "destroy_window: deleting windows_button\n");
		delete_windows_button(win);
	}
	if (win == mvwm_focus_win)
	{
		mvwm_focus_win = NULL;
	}
	free_windata(win);
...
static void mini_icon(MvwmPacketBody *body)
{
...
static void iconify(MvwmPacketBody *body, int dir)
{
	Ulong app_id = body->minimal_data.app_id;
	WinData *win;
...
static void window_shade(MvwmPacketBody *body, int do_shade)
{
	Window w;
	WinManager *man;
...
static void new_desk(MvwmPacketBody *body)
{
	globals.desknum = body->new_desk_data.desknum;
	remanage_winlist();
}

static void sendtomodule(MvwmPacketBody *body)
{
	extern void execute_function(char *);
	Uchar *string = body->name_data.name.name;

	ConsoleDebug(MVWM, "Got string: %s\n", string);

	execute_function((char *)string);
}

static void property_change(MvwmPacketBody *body)
{
	WinManager *man = NULL;
	int j;
...
static void ProcessMessage(Ulong type, MvwmPacketBody *body)
{
	int i;

	ConsoleDebug(MVWM, "mvwm message type: %ld\n", type);

	switch(type)
	{
	case M_CONFIG_INFO:
		ConsoleDebug(MVWM, "DEBUG::M_CONFIG_INFO\n");
		handle_config_info((unsigned long*)body);
		break;

	case M_CONFIGURE_WINDOW:
		ConsoleDebug(MVWM, "DEBUG::M_CONFIGURE_WINDOW\n");
		configure_window(body);
		break;

	case M_FOCUS_CHANGE:
		ConsoleDebug(MVWM, "DEBUG::M_FOCUS_CHANGE\n");
		focus_change(body);
		break;

	case M_RES_NAME:
		ConsoleDebug(MVWM, "DEBUG::M_RES_NAME\n");
		res_name(body);
		break;

	case M_RES_CLASS:
		ConsoleDebug(MVWM, "DEBUG::M_RES_CLASS\n");
		class_name(body);
		break;

	case M_MAP:
		ConsoleDebug(MVWM, "DEBUG::M_MAP\n");
		break;

	case M_ADD_WINDOW:
		ConsoleDebug(MVWM, "DEBUG::M_ADD_WINDOW\n");
		new_window(body);
		break;

	case M_DESTROY_WINDOW:
		ConsoleDebug(MVWM, "DEBUG::M_DESTROY_WINDOW\n");
		destroy_window(body);
		break;

	case M_MINI_ICON:
		ConsoleDebug(MVWM, "DEBUG::M_MINI_ICON\n");
		mini_icon(body);
		break;

	case M_WINDOW_NAME:
		ConsoleDebug(MVWM, "DEBUG::M_WINDOW_NAME\n");
		window_name(body);
		break;

	case M_VISIBLE_NAME:
		ConsoleDebug(MVWM, "DEBUG::M_VISIBLE_NAME\n");
		visible_name(body);
		break;

	case M_ICON_NAME:
		ConsoleDebug(MVWM, "DEBUG::M_ICON_NAME\n");
		icon_name(body);
		break;

	case MX_VISIBLE_ICON_NAME:
		ConsoleDebug(MVWM, "DEBUG::MX_VISIBLE_ICON_NAME\n");
		visible_icon_name(body);
		break;

	case M_DEICONIFY:
		ConsoleDebug(MVWM, "DEBUG::M_DEICONIFY\n");
		iconify(body, 0);
		SendUnlockNotification(mvwm_fd);
		break;

	case M_ICONIFY:
		ConsoleDebug(MVWM, "DEBUG::M_ICONIFY\n");
		iconify(body, 1);
		SendUnlockNotification(mvwm_fd);
		break;
	...
	case M_END_WINDOWLIST:
		ConsoleDebug(MVWM, "DEBUG::M_END_WINDOWLIST\n");
		ConsoleDebug(
			MVWM, "+++++ End window list +++++\n");
	...
	case M_NEW_DESK:
		ConsoleDebug(MVWM, "DEBUG::M_NEW_DESK\n");
		new_desk(body);
		break;

	case M_NEW_PAGE:
		ConsoleDebug(MVWM, "DEBUG::M_NEW_PAGE\n");
		if (globals.x == body->new_page_data.x &&
			globals.y == body->new_page_data.y &&
			globals.desknum == body->new_page_data.desknum)
		{
			ConsoleDebug(MVWM, "Useless NEW_PAGE received\n");
			break;
		}
		globals.x = body->new_page_data.x;
		globals.y = body->new_page_data.y;
		globals.desknum = body->new_page_data.desknum;
		if (mvwm_focus_win && mvwm_focus_win->manager &&
			mvwm_focus_win->manager->followFocus)
		{
			/* need to set the focus on a page change */
			add_win_state(mvwm_focus_win, FOCUS_CONTEXT);
		}
		for (i = 0; i < globals.num_managers; i++)
		{
	...
	case M_STRING:
		ConsoleDebug(MVWM, "DEBUG::M_STRING\n");
		sendtomodule(body);
		break;

	case MX_PROPERTY_CHANGE:
		ConsoleDebug(MVWM, "DEBUG::MX_PROPERTY_CHANGE\n");
...
void ReadMvwmPipe(void)
{
	MvwmPacket* packet;

	PrintMemuse();
	ConsoleDebug(MVWM, "DEBUG: entering ReadMvwmPipe\n");

	if ((packet = ReadMvwmPacket(mvwm_fd[1])) == NULL)
	{
		exit(0);
	}
	else
	{
		ProcessMessage(packet->type, (MvwmPacketBody *)packet->body);
	}

	ConsoleDebug(MVWM, "DEBUG: leaving ReadMvwmPipe\n");
}
