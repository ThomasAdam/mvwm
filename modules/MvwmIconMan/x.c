...
grab_pointer(WinManager *man)
{
	...
	if (grab_state == NEED_TO_GRAB) {
		if (XGrabPointer(theDisplay, man->theWindow, True,
			GRAB_EVENTS, GrabModeAsync, GrabModeAsync, None, None,
...
xevent_loop(void)
{
	...
	while (FPending(theDisplay)) {
		...
		if (!man && theEvent.type == ConfigureNotify) {
			 ...
			 */
			man =
			    find_windows_manager(theEvent.xconfigure.window);
		}
		if (!man) {
			ConsoleDebug(X11,
		...
		case KeyPress:
			...
			theEvent.xkey.keycode =
			    ...
			    mvwm_KeycodeToKeysym(theDisplay,
...
#if 0
			fprintf(stderr, "\tExpose Done: %i,%i,%i,%i %i\n",
			    ex, ey, ex2 - ex, ey2 - ey,
			    theEvent.xexpose.count);
#endif
		...
		case ConfigureNotify:
			{
				 ...
				 */
				while (FPending(theDisplay) &&
				    FCheckTypedEvent(theDisplay,
					ConfigureNotify, &theEvent)) {
					saveEvent = theEvent;
					/*
				...
				fprintf(stderr, "Configure: %i,%i,%i,%i %i\n",
				    theEvent.xconfigure.x,
				    theEvent.xconfigure.y,
				    theEvent.xconfigure.width,
				    theEvent.xconfigure.height, moved);
#endif
				if ((man->geometry.width !=
					theEvent.xconfigure.width
					|| man->geometry.height !=
					theEvent.xconfigure.height || (moved
					    && CSET_IS_TRANSPARENT(man->
						colorsets[DEFAULT])))
				    && !CSET_IS_TRANSPARENT_PR_PURE(man->
					colorsets[DEFAULT])) {
					recreate_bg = True;
					if (moved) {
						man->geometry.x =
...
static void
set_window_properties(Window win, char *name, char *icon,
...
X_init_manager(int man_id)
{
	...
	if (man->button_geometry_str) {
		...
		x = y = width = height = 0;
		val =
		    XParseGeometry(man->button_geometry_str, &x, &y, &width,
		    &height);
	...
	if (man->geometry_str) {
		...
		geometry_mask =
		    FScreenParseGeometryWithScreen(man->geometry_str,
		    &man->geometry.x, &man->geometry.y, &man->geometry.cols,
		    &man->geometry.rows, &scr);
	...
	if ((geometry_mask & YValue) && (geometry_mask & YNegative))
		man->geometry.y +=
		    man->managed_g.height - man->geometry.height;
	if (globals.transient) {
		...
		if (FQueryPointer(theDisplay, theRoot, &dummyroot,
			&dummychild, &man->geometry.x, &man->geometry.y,
			&junk, &junk, &ujunk) == False) {
			/*
...
create_manager_window(int man_id)
{
	...
	unsigned long   winattrmask =
	    CWBackPixmap | CWBorderPixel | CWEventMask | CWBackingStore |
	...
	man->theWindow =
	    ...
	    man->geometry.width, man->geometry.height, 0, Pdepth, InputOutput,
	 ...
	 * the window so that mvwm would place the window correctly if it has
	...
	for (i = 0; i < NUM_CONTEXTS; i++) {
		if (man->colorsets[i] != -2) {
			man->backContext[i] =
			    mvwmlib_XCreateGC(theDisplay, man->theWindow,
			    gcmask, &gcval);
			XSetLineAttributes(theDisplay, man->backContext[i],
			    line_width, line_style, cap_style, join_style);
		}
		man->hiContext[i] =
		    mvwmlib_XCreateGC(theDisplay, man->theWindow, gcmask,
		    &gcval);
		...
		if (man->colorsets[i] >= 0) {
			...
			XSetFillStyle(theDisplay, man->backContext[i],
			    FillSolid);
			if (i == DEFAULT) {
				XSetWindowBackground(theDisplay,
				    man->theWindow, man->backcolor[i]);
			}
		}
	}
