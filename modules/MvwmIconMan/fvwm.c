...
set_draw_mode(WinManager *man, int flag)
{
	...
	if (man->we_are_drawing == 0 && flag) {
		...
		ConsoleDebug(MVWM, "SetDrawMode on 0x%lx, num = %d\n",
...
static void
set_win_configuration(WinData *win, MvwmPacketBody * body)
{
...
static void
configure_window(MvwmPacketBody * body)
{
...
static void
focus_change(MvwmPacketBody * body)
{
...
static void
class_name(MvwmPacketBody * body)
{
	Ulong           app_id = body->name_data.app_id;
	Uchar          *name = body->name_data.name.name;
	WinData        *win;
...
icon_name(MvwmPacketBody * body)
{
	...
	if (win->iconname && !strcmp(win->iconname, (char *) name)) {
		ConsoleDebug(MVWM, "No icon change: %s %s\n", win->iconname,
		    name);
		return;
	}
...
static void
visible_icon_name(MvwmPacketBody * body)
{
	WinData        *win;
	Ulong           app_id;
	Uchar          *name = body->name_data.name.name;
	...
	ConsoleDebug(MVWM, "new visible icon name: %s\n",
...
static void
window_name(MvwmPacketBody * body)
{
	WinData        *win;
	Ulong           app_id;
	Uchar          *name = body->name_data.name.name;
	...
	if (win->titlename && !strcmp(win->titlename, (char *) name)) {
		ConsoleDebug(MVWM, "No name change: %s %s\n", win->titlename,
		    name);
		return;
	}
...
static void
visible_name(MvwmPacketBody * body)
{
	WinData        *win;
	Ulong           app_id;
	Uchar          *name = body->name_data.name.name;
	...
	if (win->visible_name && !strcmp(win->visible_name, (char *) name)) {
		ConsoleDebug(MVWM, "No visible name change: %s %s\n",
...
static void
new_window(MvwmPacketBody * body)
{
	WinData        *win;
...
static void
destroy_window(MvwmPacketBody * body)
{
	WinData        *win;
	Ulong           app_id;
	...
	if (win->button) {
		ConsoleDebug(MVWM,
		    "destroy_window: deleting windows_button\n");
...
static void
iconify(MvwmPacketBody * body, int dir)
{
	Ulong           app_id = body->minimal_data.app_id;
	WinData        *win;
...
static void
window_shade(MvwmPacketBody * body, int do_shade)
{
	Window          w;
	WinManager     *man;
...
static void
sendtomodule(MvwmPacketBody * body)
{
...
static void
property_change(MvwmPacketBody * body)
{
	WinManager     *man = NULL;
	int             j;
...
static void
ProcessMessage(Ulong type, MvwmPacketBody * body)
{
	int             i;
	...
	case M_END_WINDOWLIST:
		...
		ConsoleDebug(MVWM, "+++++ End window list +++++\n");
