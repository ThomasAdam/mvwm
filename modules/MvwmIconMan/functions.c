	...
	case NextButton:
		if (man) {
			for (i = man->index + 1, new = man + 1;
			    ...
			    new->buttons.num_windows == 0; i++, new++) {
				;
			}
...
builtin_sendcommand(int numargs, BuiltinArg * args)
{
	...
	tmp =
	    module_expand_action(theDisplay, theScreen, command, &r, NULL,
	    NULL);
	if (tmp) {
		command = tmp;
	}

	SendMvwmPipe(mvwm_fd, command, win->app_id);
	if (tmp) {
		free(tmp);
	}
...
do_jmp(int off)
{
	...
	if (off < 0) {
		ConsoleMessage
		    ("Can't have a negative relative jump offset\n");
		return;
	}
	for (i = 0; i < off; i++) {
		if (function_context.fp) {
