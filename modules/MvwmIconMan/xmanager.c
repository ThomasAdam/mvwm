...
box_to_index(WinManager *man, int box)
{
	...
	if (man->geometry.dir & GROW_DOWN) {
		...
		index =
		    box - (man->geometry.rows - num_visible_rows(n,
			cols)) * cols;
...
resize_window(WinManager *man)
{
	...
	if ((dir & GROW_DOWN) && (dir & GROW_RIGHT)) {
		...
		if (dir & GROW_LEFT) {
			man->geometry.x =
			    g->x + g->width - man->geometry.width;
			x_changed = 1;
		} else {
			man->geometry.x = g->x;
		}
		...
		ConsoleDebug(X11, "queried: y: %d, h: %d, queried: %d\n",
		    g->y, man->geometry.height, g->height);
...
{
	int             n = man->buttons.num_windows, cols =
	    man->geometry.cols;
	int             i = -1;
...
button_below(WinManager *man, Button * b)
{
	...
	if (b) {
		i = box_to_index(man, index_to_box(man,
			b->index) + man->geometry.cols);
	}
	if (i < 0 || i >= n)
		return b;
	else
...
set_win_displaystring(WinData *win)
{
	...
	if (man->window_up) {
		...
		maxlen =
		    man->geometry.width / man->fontwidth +
		    2 /* fudge factor */ ;
	} else {
		maxlen = 0;
	}
...
{
	XRectangle      rects[3];
	int             num_rects = 0, n = man->buttons.num_windows, cols =
	    man->geometry.cols;
	...
	if (n != 0 && rows * cols != 0) {
		...
		rects[num_rects].y = 0;
		rects[num_rects].width =
		    man->geometry.width - cols * boxwidth;
...
{
	ManGeometry    *new;
	int             oldwidth, oldheight, oldrows, oldcols, old_boxwidth,
	    old_boxheight;
	int             dir;
	int             i;
...
static void
draw_3d_icon(WinManager *man, int box, ButtonGeometry * g,
    int iconified, Contexts contextId)
{
   ...
   */
static void
iconify_box(WinManager *man, WinData *win, int box,
...
static void
draw_relief(WinManager *man, int button_state, ButtonGeometry * g,
    GC context1, GC context2)
{
	...
	if (state == BUTTON_EDGEUP || state == BUTTON_EDGEDOWN) {
		...
		RelieveRectangle(theDisplay, man->theWindow, g->button_x + 2,
		    g->button_y + 2, g->button_w - 5, g->button_h - 5,
		    context2, context1, relief);
	} else {
		RelieveRectangle(theDisplay, man->theWindow, g->button_x,
		    g->button_y, g->button_w - 1, g->button_h - 1, context1,
		    context2, relief);
...
draw_button(WinManager *man, int button, int force)
{
	...
	if (draw_background || draw_icon || draw_string) {
		...
		b_region =
		    GetRegion(bounding.x, bounding.y, bounding.width,
		    bounding.height);
		...
		if (draw_string) {
			...
			tr = GetRegion(g.text_x, g.text_y, g.text_w,
			    g.text_h);
...
{
	int             i, force_draw = 0, update_geometry = 0, redraw_all =
	    0;
	int             shape_changed = 0;
...
draw_transparent_buttons(WinManager *man, Bool only_moved, Bool clear_only)
{
	...
	for (i = 0; i < man->buttons.num_windows; i++) {
		...
		if (clear_only) {
			XClearArea(theDisplay, man->theWindow,
			    bp[i]->x, bp[i]->y, bp[i]->w, bp[i]->h, True);
			r = True;
		} else {
...
find_windows_spot(WinData *win)
{
	...
	if (man->sort != SortNone) {
		...
		for (i = start;
		    i != finish && bp[i]->drawn_state.win
		    && cmp_dir * compare_windows(man->sort, win,
			bp[i]->drawn_state.win) > 0; i = i + cmp_dir);
		i += correction;
...
man_exposed(WinManager *man, XEvent *theEvent)
{
	...
	if (man->buttons.num_windows) {
		for (i = 0; i < man->buttons.num_windows; i++) {
			...
			if (mvwmrect_do_rectangles_intersect(&r1, &r2)) {
...
tips_on(WinManager *man, Button * b)
{
	...
	if ((tips_str = get_tips(man, b)) != NULL) {
		FTipsOn(theDisplay, man->theWindow, man->tips_conf,
		    (void *) b, tips_str, b->x, b->y, b->w, b->h);
		man->tipped_button = b;
	} else {
		tips_cancel(man);
	}
