...
add_to_stringlist(StringList * list, char *s)
{
	...
	new = mvwm_malloc(sizeof(StringEl));
	new->string = mvwm_malloc((strlen(pat) + 1) * sizeof(char));
	new->type = type;
...
static int
matches_string(NameType type, char *pattern, char *tname,
...
check_resolution(WinManager *manager, WinData *win)
{
	...
	case SHOW_PAGE:
		if (IS_STICKY_ACROSS_PAGES(win)) {
			 ...
			 */
			flag =
			    mvwmrect_do_rectangles_intersect(&g,
			    &manager->managed_g);
		}
		break;
...
static int
iconmanager_show(WinManager *man, char *tname, char *iname,
	...
	if (!in_dontshowlist) {
		if (man->show.list == NULL) {
			...
			for (string = man->show.list; string;
				...
				ConsoleDebug(WINLIST, "Matching: %s\n",
				    string->string);
				if (matches_string(string->type,
					string->string, tname, iname, rname,
					cname)) {
					ConsoleDebug(WINLIST, "Show\n");
					in_showlist = 1;
					break;
...
check_win_complete(WinData *p)
{
	...
	if (p->geometry_set &&
	    p->resname &&
	    p->classname && p->iconname && p->titlename && p->app_id_set) {
		p->complete = 1;
