...
static char *alloc_string(char *string)
{
	...
	char *ret = mvwm_malloc((len + 1) * sizeof(char));
	strcpy(ret, string);
	return ret;
}
...
static void init_win_manager(int id)
{
	...
	globals.managers[id].titlename = alloc_string("MvwmIconMan");
	globals.managers[id].iconname = alloc_string("MvwmIconMan");
...
int allocate_managers(int num)
{
	...
	globals.managers = mvwm_malloc(num * sizeof(WinManager));
