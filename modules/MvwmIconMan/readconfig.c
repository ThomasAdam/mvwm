...
/* This is only used for printing out the .mvwmrc line if an error
   occured */
...
parse_function(char **line, char *pstop_char)
{
	...
	if (builtin_functions_i) {
		...
		for (j = 0;
			...
			case ManagerArg:
				ptr =
				    parse_button(ptr, &ftype->args[j], &flag,
				    pstop_char);
				if (!flag) {
					ConsoleMessage
					    ("%s: too few arguments\n",
			...
			case JmpArg:
				...
				if (extract_int(tok,
					&ftype->args[j].value.int_value) ==
				    0) {
					ftype->args[j].value.string_value =
					    tok;
...
{
	char           *action, modifiers[20], key[20], *ptr, *token,
	    *actionstring, *keystring;
	...
	for (i = min; i <= max; i++) {
		if (mvwm_KeycodeToKeysym(theDisplay, i, 0, 0) == keysym) {
			if (!func) {
...
read_next_cmd(ReadOption flag)
{
	...
	switch (flag) {
	case READ_LINE:
		while (GetConfigLineWrapper(mvwm_fd, &buffer)) {
			cur_pos = buffer;
			skip_space(&cur_pos);
...
read_in_resources(void)
{
	...
	InitGetConfigLine(mvwm_fd, Module);
			...
			if (!strcasecmp(option1, "action")) {
				...
				if (manager == -1) {
					...
					for (j = 0; j < globals.num_managers;
						...
						add_to_binding(&globals.
						    managers[j].bindings[i],
						    binding);
					}
				} else if (manager < globals.num_managers) {
					add_to_binding(&globals.
					    managers[manager].bindings[i],
					    binding);
				} else {
					ConsoleMessage("Bad line: %s\n",
					    current_line);
				...
				for (i = 0; i < NUM_CONTEXTS; i++)
					SET_MANAGER(manager, backColorName[i],
					    conditional_copy_string(&globals.
						managers[id].backColorName[i],
						p));
				...
				if (!strcasecmp(p, "name")) {
					 ...
					 */
					ConsoleMessage
					    ("MvwmIconMan: sort option no longer takes "
					    "true or false value\n"
				...
				while (p) {
					if (!strncasecmp(p, "resource=", 9)) {
						copy_string(&weighted_sort.
						    resname, p + 9);
					} else if (!strncasecmp(p, "class=",
						6)) {
						copy_string(&weighted_sort.
						    classname, p + 6);
					} else if (!strncasecmp(p, "title=",
						6)) {
						copy_string(&weighted_sort.
						    titlename, p + 6);
					} else if (!strncasecmp(p, "icon=",
						5)) {
						copy_string(&weighted_sort.
						    iconname, p + 5);
					} else {
...
process_dynamic_config_line(char *line)
{
	...
	if (strcasecmp(token, "resolution") == 0) {
		...
		if (!strcasecmp(token, "global"))
			...
			fprintf(stderr, "%s: unknown resolution %s.\n",
			    MyName, token);
			free(token);
			return;
		}
