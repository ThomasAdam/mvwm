...
/* FIXME: default layer should be received from mvwm */
...
main(int argc, char **argv)
{
	...
	if (module == NULL) {
		fprintf(stderr, "MvwmIdent Version %s should only be executed"
		    " by mvwm!\n", VERSION);
		exit(1);
	}
	...
	while (tline != (char *) 0) {
		...
		if (strncasecmp(tline,
			...
			} else if (strncasecmp(tline, "MinimalLayer",
				12) == 0) {
				char           *layer_str =
				    PeekToken(&tline[12], NULL);
				if (layer_str == NULL) {
					minimal_layer = default_layer;
				} else if (sscanf(layer_str, "%d",
					&minimal_layer) != 1) {
					if (strncasecmp(layer_str, "none",
						4) == 0) {
						minimal_layer = -1;
					} else {
						minimal_layer = default_layer;
	...
	if (module->window == 0) {
		mvwmlib_get_target_window(dpy, screen, module->name,
		    &(module->window), True);
	}
...
/*
 *
 * Read the entire window list from mvwm
 *
 */
void
Loop(int *fd)
...
list_property_change(unsigned long *body)
{
	if (body[0] == MX_PROPERTY_CHANGE_BACKGROUND && body[2] == 0 &&
		...
		if (UsePixmapDrawing) {
			...
			UpdateBackgroundTransparency(dpy, main_win,
			    main_width, main_height, &Colorset[(colorset)],
			    Pdepth, gc, True);
		}
	}
}
...
ProcessXEvent(int x, int y)
{
	...
	while (FPending(dpy)) {
		...
		case Expose:
			...
			ey2 =
			    max(ey2, Event.xexpose.y + Event.xexpose.height);
		...
		case ButtonRelease:
			if (is_button_pressed) {
				if (is_button_pressed == 2 &&
					...
					DestroyList();
					mvwmlib_get_target_window(dpy, screen,
		...
		case ConfigureNotify:
			...
			if ((Event.xconfigure.send_event ||
				...
				if (UsePixmapDrawing) {
					...
					main_win, main_width, main_height,
...
list_end(void)
{
	...
	/*
	 * tell mvwm to only send config messages
	 */
	...
	mysizehints.flags =
	    USSize | USPosition | PWinGravity | PResizeInc | PBaseSize |
	    PMinSize | PMaxSize;
	...
	main_win =
	    XCreateWindow(dpy, Root, x, y, mysizehints.width,
	...
	gc = mvwmlib_XCreateGC(dpy, main_win, gcm, &gcv);
	if (UsePixmapDrawing) {
		PixmapDrawWindow(main_width, main_height);
	} else if (colorset >= 0) {
	...
	while (1) {
		 ...
		 */
		select(fd_width, SELECT_FD_SET_CAST & fdset, NULL, NULL,
		    NULL);
...
PixmapDrawWindow(int w, int h)
{
	...
	if (colorset >= 0) {
		...
		if (cs_pix == ParentRelative) {
			...
			pix =
			    CreateTiledPixmap(dpy, cs_pix, 0, 0, w, h, Pdepth,
			    gc);
...
change_window_name(char *str)
{
	...
	myclasshints.res_class = "MvwmIdent";
...
AddToList(char *s1, char *s2)
{
	...
	item = mvwm_malloc(sizeof(struct Item));
	item->col1 = s1;
	item->col2 = s2;
	item->next = NULL;
...
MakeList(void)
{
	...
	/*
	 * FIXME: we should use the mvwm default layers
	 */
