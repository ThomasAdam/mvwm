...
my_get_font(char **s)
{
	...
	*s = GetNextFullOption(*s, &option);
	if (option) {
		for (len = strlen(option) - 1;
		    len >= 0 && isspace(option[len]); len--) {
			/*
...
**/
static void
ParseSwallow(char **ss, unsigned int *flags, unsigned int *mask,
    button_info *b)
{
	char           *swallowopts[] = {
		"nohints", "hints",
...
**/
static void
ParsePanel(char **ss, unsigned int *flags, unsigned int *mask,
    char *direction, int *steps, int *delay, panel_flags_type *panel_flags,
		...
		case 21:	/* position */
			...
			*rela_x = *rela_y = 0;
			while (*s != ',' && *s != ')' && *s) {
				...
				switch (GetTokenIndex(s, positionopts, -1,
					&s)) {
				case 0:	/* button */
...
*** *MvwmButtons: (option[ options]) title iconname command
**/
/*#define DEBUG_PARSER*/
static void
ParseButton(button_info **uberb, char *s)
{
	...
	if (*s == '(' && s++) {
		...
		while (*s && *s != ')') {
			...
			case 11:	/* Container */
				/*
				 * SS: This looks very buggy! The MvwmButtons
...
/**
*** ParseConfigFile()
*** Parses optional separate configuration file for MvwmButtons
**/
static void
