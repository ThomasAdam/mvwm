...
static Window   GetRealGeometry(Display *, Window, int *, int *,
...
DeadPipeCleanup(void)
{
		...
		if ((buttonSwallowCount(b) == 3) && swin) {
			...
			if (!IsThereADestroyEvent(b)) {	/* Has someone destroyed it? */
				if (!(buttonSwallow(b) & b_NoClose)) {
					...
					XSetWindowBorderWidth(Dpy, swin,
					    b->bw);
					if (b->flags.b_Panel) {
						XMapWindow(Dpy, swin);
					}
	...
	while (NextButton(&ub, &b, &button, 1)) {
		...
		if (b->flags.b_Icon && b->icon != NULL) {
			PDestroyMvwmPicture(Dpy, b->icon);
		}
		...
		if (b->flags.b_Container && b->c->flags.b_IconBack &&
			...
			PDestroyMvwmPicture(Dpy, b->c->backicon);
		}
	}
}
...
GetButtonAction(button_info *b, int n)
{
	...
	act =
	    module_expand_action(Dpy, screen, b->action[n], &r,
...
SetTransparentBackground(button_info *ub, int w, int h)
{
	if (FShapesSupported) {
		...
		if (transGC == NULL) {
			transGC = mvwmlib_XCreateGC(Dpy, shapeMask, 0, &gcv);
		}
...
/**
*** myErrorHandler()
*** Shows X errors made by MvwmButtons.
**/
...
main(int argc, char **argv)
{
	...
	FlocaleInit(LC_CTYPE, "", "", "MvwmButtons");
	...
	if (argc < 6 || argc > 10) {
		fprintf(stderr,
		    "%s version %s should only be executed by mvwm!\n",
		    MyName, VERSION);
		exit(1);
	}
	...
	if (!(Dpy = XOpenDisplay(NULL))) {
		fprintf(stderr,
		    "%s: Can't open display %s", MyName, XDisplayName(NULL));
		exit(1);
	}
	...
	MyWindow =
	    XCreateWindow(Dpy, Root, 0, 0, 1, 1, 0, Pdepth, InputOutput,
...
handle_new_position(button_info *b, int pos_x, int pos_y)
{
	...
	if (pos_x < 0 || pos_x >= Width || pos_y < 0 || pos_y >= Height) {
		/*
		 * cursor is outside of MvwmButtons window.
		 */
		return b;
	}
...
/* Start of TO-BE-REFORMATTED-BLOCK */
			...
			case ConfigureNotify:
				{
					XEvent          event;
					unsigned int    depth, tw, th,
					    border_width;
					Window          root;

					while (FCheckTypedWindowEvent(Dpy,
						MyWindow, ConfigureNotify,
						&event)) {
						if (!event.xconfigure.
						    send_event
						    && Event.xconfigure.
					...
					if (tw != Width || th != Height) {
						...
						for (i = 0; i < nColorsets;
						    i++)
							change_colorset(i,
							    &Event);
						if (!UberButton->c->flags.
						    b_Colorset) {
							XClearWindow(Dpy,
							    MyWindow);
							RedrawWindow();
			...
			case LeaveNotify:
				if (Event.xcrossing.x < 0
				     ...
				     * * a swallowed window of MvwmButtons, but we're not
				     * * interested in these situations.
				     */
				    !select_button(UberButton, x,
					y)->flags.b_Swallow ||
				    /*
			...
			case KeyPress:
				XLookupString(&Event.xkey, buffer, 10,
				    &keysym, 0);
				if (keysym != XK_Return
				    && keysym != XK_KP_Enter
				    && keysym != XK_Linefeed)
			...
			case ButtonPress:
				...
				if (Event.xbutton.
				    state & DEFAULT_ALL_BUTTONS_MASK) {
					break;
				}
			...
			case ButtonRelease:
				...
				act =
				    GetButtonAction(b, Event.xbutton.button);
				...
				while (NextButton(&ub, &b, &button, 0)) {
					...
					if ((buttonSwallowCount(b) == 3)
						...
						if (buttonSwallow(b) &
							...
							p = module_expand_action(Dpy, screen, b->spawn, NULL, UberButton->c->fore, UberButton->c->back);
							if (p) {
								exec_swallow
								    (p, b);
								free(p);
...
**/
int
LoadIconFile(const char *s, MvwmPicture **p, int cset)
{
	...
	*p = PCacheMvwmPicture(Dpy, MyWindow, imagePath, s, fpa);
	if (*p) {
		return 1;
	}
	return 0;
...
{
	int             i, x = 0, y =
	    0, ix, iy, tx, ty, hix, hiy, htx, hty, pix, piy, ptx, pty;
	FlocaleFont    *Ffont;
	...
	if (b->flags.b_Font) {
		...
		if (strncasecmp(b->font_string, "none", 4) == 0) {
			...
			FlocaleLoadFont(Dpy, b->font_string, MyName))) {
			b->flags.b_Font = 0;
		}
	}
	...
	if (b->flags.b_Container && b->c->flags.b_Font) {
		...
		if (strncasecmp(b->c->font_string, "none", 4) == 0) {
			...
			FlocaleLoadFont(Dpy, b->c->font_string, MyName))) {
			b->c->flags.b_Font = 0;
		}
	}
...
#ifdef DEBUG_LOADDATA
		fprintf(stderr, "%s: Loading: Back to container 0x%06x",
		    MyName, (ushort) b);
#endif
	...
	if (b->flags.b_Title && (Ffont = buttonFont(b))) {
		...
		if (buttonJustify(b) & b_Horizontal) {
			tx = buttonXPad(b) + FlocaleTextWidth(Ffont, b->title,
			    strlen(b->title));
			ty = Ffont->height;
		} else {
	...
	if (b->flags.b_ActiveTitle && (Ffont = buttonFont(b))) {
		...
		if (buttonJustify(b) & b_Horizontal) {
			htx =
			    buttonXPad(b) + FlocaleTextWidth(Ffont,
			    b->activeTitle, strlen(b->activeTitle));
			hty = Ffont->height;
		} else {
			htx =
	...
	if (b->flags.b_PressTitle && (Ffont = buttonFont(b))) {
		...
		if (buttonJustify(b) & b_Horizontal) {
			ptx =
			    buttonXPad(b) + FlocaleTextWidth(Ffont,
			    b->pressTitle, strlen(b->pressTitle));
			pty = Ffont->height;
		} else {
			ptx =
...
HandlePanelPress(button_info *b)
{
	...
	if (is_mapped &&
		 ...
		 */
		XTranslateCoordinates(Dpy, b->PanelWin, Root, b->x, b->y,
		    &b->x, &b->y, &JunkW);
	} else {
		XWMHints        wmhints;
		 ...
		 */
		FScreenMangleScreenIntoUSPosHints(FSCREEN_XYPOS,
		    &mysizehints);
		XSetWMNormalHints(Dpy, b->PanelWin, &mysizehints);
		/*
	...
	if (!b->panel_flags.ignore_lrborder
		...
		if (ancestor) {
			if (XGetGeometry(Dpy, ancestor, &JunkW, &ax, &ay,
				...
				(unsigned int *) &abw, &d)
			    && XGetGeometry(Dpy, b->PanelWin, &JunkW, &px,
				&py, (unsigned int *) &pw,
				(unsigned int *) &ph, (unsigned int *) &pbw,
				&d)
			    && XTranslateCoordinates(Dpy, b->PanelWin,
				ancestor, 0, 0, &px, &py, &JunkW)) {
				/*
	...
	/*
	 * to force mvwm to map the window where we want
	 */
	if (is_mapped) {
		/*
	...
	XSync(Dpy, 0);
	/*
	 * Give mvwm a chance to update the window.  Otherwise the window may end up
...
CreateUberButtonWindow(button_info *ub, int maxx, int maxy)
{
	...
	if (x > -100000) {
		if (xneg) {
			wx = DisplayWidth(Dpy,
			    screen) + x - mysizehints.width;
			gravity = NorthEastGravity;
		} else {
			wx = x;
		}
		if (yneg) {
			wy = DisplayHeight(Dpy,
			    screen) + y - mysizehints.height;
	...
	NormalGC = mvwmlib_XCreateGC(Dpy, MyWindow, gcm, &gcv);
	gcv.foreground = shadow_pix;
	gcv.background = fore_pix;
	ShadowGC = mvwmlib_XCreateGC(Dpy, MyWindow, gcm, &gcv);
...
My_FNextEvent(Display *Dpy, XEvent *event)
{
	...
	if (mvwmSelect(fd_width, SELECT_FD_SET_CAST & in_fdset, 0, 0,
		NULL) > 0) {
		...
		if ((buttonSwallowCount(b) == 1) && b->flags.b_Hangon &&
			...
			if (b->spawn) {
				...
				fprintf(stderr,
				    ...
				    "spawning own\n");
#endif
				p = module_expand_action(Dpy, screen,
				    b->spawn, NULL, UberButton->c->fore,
				    UberButton->c->back);
				if (p) {
					exec_swallow(p, b);
					free(p);
...
change_swallowed_window_colorset(button_info *b, Bool do_clear)
{
	...
	nchildren =
	    GetEqualSizeChildren(Dpy, w, Pdepth, XVisualIDFromVisual(Pvisual),
	    Pcmap, &children);
...
send_bg_change_to_module(button_info *b, XEvent *Event)
{
	...
	if (Event != NULL) {
		...
		FSendEvent(Dpy, SwallowedWindow(b), False, NoEventMask,
		    Event);
	} else {
		char            cmd[256];
...
update_root_transparency(XEvent *Event)
{
	...
	Bool            bg_is_transparent =
	    CSET_IS_TRANSPARENT(UberButton->c->colorset);
	...
	if (!bg_is_transparent) {
		...
		while (NextButton(&ub, &b, &button, 1)) {
			...
			while (tmpb->parent != NULL && !redraw) {
				if (CSET_IS_TRANSPARENT_ROOT(buttonColorset
					(tmpb))) {
					redraw = True;
				}
				tmpb = tmpb->parent;
		...
		while (NextButton(&ub, &b, &button, 0)) {
			if (b->flags.b_Swallow && buttonSwallowCount(b) == 3) {
				if (b->swallow & b_MvwmModule) {
				    ...
				    && !buttonBackgroundButton(b->parent,
					NULL)) {
					/*
    ...
    int *x, int *y, int *w, int *h)
{
	...
	/*
	 * take in acount or not the MvwmButtons borders width
	 */
	...
	case SLIDE_CONTEXT_PB:	/* slide relatively to the panel button */
		...
		XTranslateCoordinates(Dpy, MyWindow, Root, bx, by, &bx, &by,
		    &JunkWin);
		break;
	case SLIDE_CONTEXT_MODULE:	/* slide relatively to MvwmButtons */
		{
			...
			if (XGetGeometry(Dpy, MyWindow, &JunkWin, &bx, &by,
				...
				(unsigned int *) &dum, &dum2)) {
				XTranslateCoordinates(Dpy, MyWindow, Root, bx,
				    by, &bx, &by, &JunkWin);
				break;
			} else {
				/*
...
swallow(unsigned long *body)
{
	...
	while (NextButton(&ub, &b, &button, 0)) {
		...
		if ((swin == (Window) body[0])
			...
			if (p == None)
				/*
				 * This means the window is no more
				 *//*
				 * NO! wrong
				 */
			{
			...
			if (b->flags.b_Swallow) {
				...
				if (buttonSwallow(b) & b_UseTitle) {
					...
					b->flags.b_Title = 1;
					FlocaleGetNameProperty(XGetWMName,
					    Dpy, swin, &temp);
					if (temp.name != NULL) {
						CopyString(&b->title,
						    temp.name);
