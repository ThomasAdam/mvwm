 ...
 */
Bool
GetIconPosition(button_info *b,
    MvwmPicture *pic, int *r_x, int *r_y, int *r_w, int *r_h)
{
#ifdef NO_ICONS
	return False;
#else
...
DrawForegroundIcon(button_info *b, XEvent *pev)
{
	...
	if (pev) {
		if (!frect_get_intersection(x, y, w, h,
	...
	PGraphicsRenderPicture(Dpy, MyWindow, pic, &fra, MyWindow,
	    NormalGC, None, None,
