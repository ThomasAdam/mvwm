	...
	if (buttonSwallowCount(b) == 3 && b->flags.b_Swallow) {
		...
		if (iw > 0 && ih > 0) {
			if (!(buttonSwallow(b) & b_NoHints)) {
				if (!XGetWMNormalHints(Dpy, b->IconWin,
					b->hints, &supplied))
					b->hints->flags = 0;
				ConstrainSize(b->hints, &b->icon_w,
				    &b->icon_h);
			}
			if (b->flags.b_Right)
				ix += iw - b->icon_w;
...
RedrawButton(button_info *b, int draw, XEvent *pev)
{
	...
	if (b->flags.b_Action) {
		...
		for (n = 0; n < 4; n++) {
			if (b->action[n]) {
				if (strncasecmp(b->action[n],
					"GotoDeskAndPage", 15)
				    == 0) {
					k = sscanf(&b->action[n][15], "%d",
					    &i);
					if (k == 1 && i == new_desk) {
						rev = 1;
					}
					...
					4) == 0) {
					k = sscanf(&b->action[n][4], "%d%d",
					    &i, &j);
					if (k == 2 && i == 0 && j == new_desk) {
						rev = 1;
					}
					break;
				} else if (strncasecmp(b->action[n],
					"GotoDesk", 8) == 0) {
					k = sscanf(&b->action[n][8], "%d%d",
					    &i, &j);
					if (k == 2 && i == 0 && j == new_desk) {
						rev = 1;
					}
		...
		if (b->flags.b_Container) {
			if (b->c->flags.b_Colorset) {
				...
				if ((lw > 0 && lh > 0) &&
					...
					XFillRectangle(Dpy, MyWindow,
					    NormalGC, clip.x, clip.y,
			...
			if (do_draw) {
				...
				SetClippedRectangleBackground(Dpy, MyWindow,
				    ...
				    &clip, &Colorset[b->colorset], Pdepth,
				    NormalGC);
			}
#if 0
			...
			if (UberButton->c->flags.b_TransBack &&
			    FShapesSupported) {
				int             w = buttonWidth(b), h =
				    buttonHeight(b);
				MvwmPicture    *icon = buttonIcon(b);
				XGCValues       gcv;

				if (transGC == NULL) {
					transGC =
					    mvwmlib_XCreateGC(Dpy, shapeMask,
					    0, &gcv);
				}
				XSetClipMask(Dpy, transGC, None);
				XSetForeground(Dpy, transGC, 0);
				XFillRectangle(Dpy, shapeMask, transGC, x, y,
				    w, h);
				XSetForeground(Dpy, transGC, 1);

				if (!icon || icon->mask == None || cs >= 0) {
					XFillRectangle(Dpy, shapeMask,
					    transGC, x, y, w, h);
				} else {
					int             xx, yy, ww, hh;
					GetIconPosition(b,
				...
				if (buttonTitle(b)) {
					DrawTitle(b, shapeMask, transGC,
					    NULL);
				}
				FShapeCombineMask(Dpy, MyWindow,
				    FShapeBounding, 0, 0, shapeMask,
				    FShapeSet);
			}
		}
	}
	...
	if (title == NULL && b->flags.b_Panel &&
		...
		if (b->slide_direction == SLIDE_GEOMETRY) {
			...
			RelieveRectangle(Dpy, MyWindow, ix, iy, iw - 1,
			    ih - 1, NormalGC, ShadowGC, 1);
		} else {
			...
			DrawTrianglePattern(Dpy, MyWindow, NormalGC, ShadowGC,
			    None, ix, iy, iw, ih, 0, dir, 1, 0, 0);
		}
	}
	/*
	 * panel indicator
	...
	if (*s && l > 0 && BH >= Ffont->height) {	/* Clip it somehow? */
		...
		if (pev) {
			if (!frect_get_intersection(FwinString.x,
				FwinString.y - Ffont->ascent, i,
				Ffont->height, pev->xexpose.x, pev->xexpose.y,
