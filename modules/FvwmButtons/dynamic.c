...
parse_button_id(char **line)
{
	...
	*line = NULL;
	...
	if (*s == '+') {
		...
		if (x < 0 || y < 0) {
			show_error
			    ("Button column/row must not be negative\n");
			return NULL;
		}
...
parse_message_line(char *line)
{
	...
	case 1:
		...
		while (rest && rest[0] != '\0') {
			 ...
			 */
			rest =
			    GetQuotedString(rest, &option_pair, ",", NULL,
			    NULL, NULL);
			while (isspace(*rest)) {
				rest++;
			}
			...
			option =
			    GetTokenIndex(option_pair, button_options, -1,
			    &value0);
			if (option < 0) {
				show_error
