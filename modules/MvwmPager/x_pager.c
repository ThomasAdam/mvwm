...
extern MvwmPagerScreenInfo FPScr;
extern Display *dpy;
...
extern MvwmPicture *PixmapBack;
extern MvwmPicture *HilightPixmap;
...
DeskInfo       *Desks;
int             Wait = 0;
int             MvwmErrorHandler(Display *, XErrorEvent *);
...
static int      MyVx, MyVy;	/* copy of FPScr.Vx/y for drag logic */
 ...
 * sent back to mvwm. The function shall be called with is_message_recieved
 * True when the Scroll command has been processed by mvwm. This is checked
 ...
 */
static void
CalcGeom(PagerWindow *t, int win_w, int win_h,
    int *x_ret, int *y_ret, int *w_ret, int *h_ret)
{
	 ...
	 */
	virt2 = FPScr.VWidth - 1 - virt;
	edge2 = (virt2 * win_w) / FPScr.VWidth;
	...
	if (size < MinSize) {
		 ...
		 */
		page = virt / FPScr.MyDisplayWidth;

		/*
		 * if the left edge is on the same page then possibly move it left
		 */
		if (page == ((virt - t->width + 1) / FPScr.MyDisplayWidth)) {
			/*
	...
	*w_ret = size;
	 ...
	 */
	virt = FPScr.Vy + t->y;
	edge = (virt * win_h) / FPScr.VHeight;
	virt += t->height - 1;
	virt2 = FPScr.VHeight - 1 - virt;
	edge2 = (virt2 * win_h) / FPScr.VHeight;
	edge2 = win_h - 1 - edge2;
	size = edge2 - edge + 1;
	if (size < MinSize) {
		size = MinSize;
		page = virt / FPScr.MyDisplayHeight;
		if (page == ((virt - t->height + 1) / FPScr.MyDisplayHeight)) {
			over =
			    edge + size - ((page +
				1) * win_h * FPScr.MyDisplayHeight) /
			    FPScr.VHeight;
			if (over > 0)
				edge -= over;
		}
...
initialize_viz_pager(void)
{
	 ...
	 * * DefaultDepth(dpy, FPScr.screen) to use the FPScr.Root for FPScr.Pager_w
	 */
	if (Pdefault) {
		FPScr.Pager_w = FPScr.Root;
	} else {
		attr.background_pixmap = None;
		attr.border_pixel = 0;
		attr.colormap = Pcmap;
		FPScr.Pager_w =
		    XCreateWindow(dpy, FPScr.Root, -10, -10, 10, 10, 0,
		    Pdepth, InputOutput, Pvisual,
		    CWBackPixmap | CWBorderPixel | CWColormap, &attr);
		FPScr.NormalGC =
		    mvwmlib_XCreateGC(dpy, FPScr.Pager_w, 0, &xgcv);
	}
	FPScr.NormalGC = mvwmlib_XCreateGC(dpy, FPScr.Pager_w, 0, NULL);

	xgcv.plane_mask = AllPlanes;
	FPScr.MiniIconGC =
	    mvwmlib_XCreateGC(dpy, FPScr.Pager_w, GCPlaneMask, &xgcv);
	FPScr.black = GetColor("Black");

	/*
	 * Transparent background are only allowed when the depth matched the
	 * * root
	 */
	if (Pdepth == DefaultDepth(dpy, FPScr.screen)) {
...
initialize_balloon_window(void)
{
	 ...
	 */
	FPScr.balloon_w =
	    XCreateWindow(dpy, FPScr.Root, 0, 0, /* coords set later */ 1, 1,
	    BalloonBorderWidth, Pdepth, InputOutput, Pvisual, valuemask,
	    &attributes);
	FPScr.balloon_gc = mvwmlib_XCreateGC(dpy, FPScr.balloon_w, 0, &xgcv);
	/*
...
initialize_pager(void)
{
	 ...
	 */
	XSetErrorHandler(MvwmErrorHandler);
	...
	if (Pdepth < 2) {
		FPScr.gray_pixmap =
		    XCreatePixmapFromBitmapData(dpy, FPScr.Pager_w, g_bits,
		    g_width, g_height, fore_pix, back_pix, Pdepth);
		FPScr.light_gray_pixmap =
		    XCreatePixmapFromBitmapData(dpy, FPScr.Pager_w, l_g_bits,
		    l_g_width, l_g_height, fore_pix, back_pix, Pdepth);
		FPScr.sticky_gray_pixmap =
		    XCreatePixmapFromBitmapData(dpy, FPScr.Pager_w, s_g_bits,
		    s_g_width, s_g_height, fore_pix, back_pix, Pdepth);
	}

	n = FPScr.VxMax / FPScr.MyDisplayWidth;
	m = FPScr.VyMax / FPScr.MyDisplayHeight;
	...
	if (window_w > 0) {
		...
		window_w =
		    Columns * (FPScr.VWidth / FPScr.VScale + n) + Columns - 1;
	}
	if (window_h > 0) {
		window_h =
		...
		window_h =
		    Rows * (FPScr.VHeight / FPScr.VScale + m + label_h + 1) -
		    1;
	}
	...
	if (is_transient) {
		...
		FScreenGetScrRect(&fscr, FSCREEN_XYPOS,
		    &screen_g.x, &screen_g.y, &screen_g.width,
		    &screen_g.height);
		if (window_w + window_x > screen_g.x + screen_g.width) {
			window_x =
			    screen_g.x + screen_g.width -
			    FPScr.MyDisplayWidth;
			xneg = 1;
		}
		if (window_h + window_y > screen_g.y + screen_g.height) {
			window_y =
			    screen_g.y + screen_g.height -
			    FPScr.MyDisplayHeight;
			yneg = 1;
		}
	}
	if (xneg) {
		sizehints.win_gravity = NorthEastGravity;
		window_x = FPScr.MyDisplayWidth - window_w + window_x;
	}
	if (yneg) {
		window_y = FPScr.MyDisplayHeight - window_h + window_y;
	 ...
	 */
	if (FPScr.Pager_w != FPScr.Root)
		XDestroyWindow(dpy, FPScr.Pager_w);
	FPScr.Pager_w =
	    XCreateWindow(dpy, FPScr.Root, window_x, window_y, window_w,
	    window_h, 0, Pdepth, InputOutput, Pvisual, valuemask,
	    &attributes);
	XSetWMProtocols(dpy, FPScr.Pager_w, &wm_del_win, 1);
	/*
	 * hack to prevent mapping on wrong screen with StartsOnScreen
	 */
	FScreenMangleScreenIntoUSPosHints(FSCREEN_XYPOS, &sizehints);
	XSetWMNormalHints(dpy, FPScr.Pager_w, &sizehints);
	if (is_transient) {
		XSetTransientForHint(dpy, FPScr.Pager_w, FPScr.Root);
	}
	...
	icon_win =
	    XCreateWindow(dpy, FPScr.Root, window_x, window_y, icon_w, icon_h,
	    0, Pdepth, InputOutput, Pvisual, valuemask, &attributes);
	...
	if (icon_x != -10000) {
		if (icon_xneg)
			icon_x = FPScr.MyDisplayWidth + icon_x - icon_w;
		if (icon_y != -10000) {
			if (icon_yneg)
				icon_y =
				    FPScr.MyDisplayHeight + icon_y - icon_h;
		} else {
			icon_y = 0;
		}
		icon_xneg = 0;
	...
	class1.res_class = "MvwmPager";

	XSetWMProperties(dpy, FPScr.Pager_w, &name, &name, NULL, 0,
	 ...
	 */
	XSetForeground(dpy, FPScr.NormalGC, focus_fore_pix);

	if (FwindowFont != NULL && FwindowFont->font != NULL)
		XSetFont(dpy, FPScr.NormalGC, FwindowFont->font->fid);
	...
	if (windowcolorset >= 0) {
		...
		FPScr.whGC =
		    mvwmlib_XCreateGC(dpy, FPScr.Pager_w, GCForeground, &gcv);
	...
	for (i = 0; i < ndesks; i++) {
		...
		if (uselabel && Ffont && Ffont->font) {
			...
			Desks[i].NormalGC =
			    mvwmlib_XCreateGC(dpy, FPScr.Pager_w,
		...
		if (Desks[i].colorset >= 0
			...
			attributes.background_pixel =
			    (Desks[i].colorset <
			    0) ? (Desks[i].Dcolor ? GetColor(Desks[i].
				Dcolor) : back_pix)
		...
		Desks[i].title_w =
		    XCreateWindow(dpy, FPScr.Pager_w, x - 1, y - 1, w, h, 1,
		...
		if (Desks[i].colorset > -1 &&
			...
			attributes.background_pixel =
			    (Desks[i].colorset <
			    0) ? back_pix : Colorset[Desks[i].colorset].bg;
		}

		Desks[i].w =
		    XCreateWindow(dpy, Desks[i].title_w, x - 1,
		...
		if (HilightDesks) {
			...
			h = desk_h / (m + 1);
			Desks[i].CPagerWin =
			    XCreateWindow(dpy, Desks[i].w, -32768, -32768, w,
			    h, 0, CopyFromParent, InputOutput, CopyFromParent,
...
UpdateWindowShape(void)
{
	if (FHaveShapeExtension) {
		...
		shape_count =
		    ndesks + ((FPScr.CurrentDesk < desk1
			|| FPScr.CurrentDesk > desk2) ? 0 : 1);

		shape =
		    (XRectangle *) alloca(shape_count * sizeof(XRectangle));
		...
		for (i = 0; i < Rows; ++i) {
			...
			for (j = 0; j < Columns; ++j) {
				if (cnt < ndesks) {
					...
					if (cnt == FPScr.CurrentDesk - desk1) {
		...
		FShapeCombineRectangles(dpy, FPScr.Pager_w, FShapeBounding, 0,
		    0, shape, shape_count, FShapeSet, 0);
	}
}
			...
			for (i = 0; i < ndesks; i++) {
				if (Event->xany.window == Desks[i].w) {
					...
					Scroll(desk_w, desk_h, x, y,
					    FPScr.CurrentDesk, False);
					if (FPScr.CurrentDesk != i + desk1) {
						Wait = 0;
						SwitchToDesk(i);
					}
					break;
...
HandleExpose(XEvent *Event)
{
	...
	if (Event->xany.window == FPScr.balloon_w) {
		DrawInBalloonWindow(FPScr.balloon_desk);
		return;
	}
	if (Event->xany.window == icon_win)
		DrawIconGrid(0);
...
ReConfigure(void)
{
	...
	n1 = FPScr.Vx / FPScr.MyDisplayWidth;
	m1 = FPScr.Vy / FPScr.MyDisplayHeight;
	n = FPScr.VxMax / FPScr.MyDisplayWidth;
	m = FPScr.VyMax / FPScr.MyDisplayHeight;
	...
	x = (desk_w - n) * FPScr.Vx / FPScr.VWidth + n1;
	y = (desk_h - m) * FPScr.Vy / FPScr.VHeight + m1;
		...
		for (j = 0; j < Columns; j++) {
			...
			if (i < ndesks) {
				...
				XMoveResizeWindow(dpy, Desks[i].w, -1,
				    ...
				    desk_h);
				if (!is_size_changed) {
					if (CSET_IS_TRANSPARENT(Desks[i].
						colorset)) {
						draw_desk_background(i, w, h);
					}
				}
				if (HilightDesks) {
					if (i == FPScr.CurrentDesk - desk1)
						XMoveResizeWindow(dpy,
						    Desks[i].CPagerWin, x, y,
						    w, h);
					else
						XMoveResizeWindow(dpy,
						    Desks[i].CPagerWin,
	...
	for (t = Start; t != NULL; t = t->next) {
		...
		if (t->PagerView != None) {
			SetWindowBackground(dpy, t->PagerView,
			    ...
			    &Colorset[cset], Pdepth, FPScr.NormalGC, True);
		}
		if (t->IconView) {
...
update_pr_transparent_windows(void)
{
		...
		for (j = 0; j < Columns; j++) {
			...
			if (i < ndesks) {
				if (CSET_IS_TRANSPARENT_PR(Desks[i].colorset)) {
					draw_desk_background(i, w, h);
				} else if (CSET_IS_TRANSPARENT_PR(Desks[i].
					highcolorset) && HilightDesks) {
	 ...
	 */
	if (BalloonView != None) {
		cset = Desks[FPScr.balloon_desk].ballooncolorset;
		if (CSET_IS_TRANSPARENT_PR(cset)) {
			XClearArea(dpy, FPScr.balloon_w, 0, 0, 0, 0, True);
		}
	}
}
...
MovePage(Bool is_new_desk)
{
	...
	if (FPScr.CurrentDesk != icon_desk_shown) {
		icon_desk_shown = FPScr.CurrentDesk;

		if ((FPScr.CurrentDesk >= desk1)
		    && (FPScr.CurrentDesk <= desk2))
			sptr = Desks[FPScr.CurrentDesk - desk1].label;
		else {
			sprintf(str, "GotoDesk %d", FPScr.CurrentDesk);
			sptr = &str[0];
		}
		...
		XSetWMIconName(dpy, FPScr.Pager_w, &name);
		XFree(name.value);
	}
}
...
ReConfigureIcons(Bool do_reconfigure_desk_only)
{
	...
	for (t = Start; t != NULL; t = t->next) {
		if (do_reconfigure_desk_only && t->desk != FPScr.CurrentDesk)
			continue;
		...
		if (FPScr.CurrentDesk == t->desk)
			XMoveResizeWindow(dpy, t->IconView, x, y, w, h);
		else
			XMoveResizeWindow(dpy, t->IconView, -32768, -32768, w,
			    h);
	}
}
...
DrawGrid(int desk, int erase, Window ew, XRectangle * r)
{
	...
	if (!ew || ew == Desks[desk].w) {
		x = FPScr.MyDisplayWidth;
		y1 = 0;
		y2 = desk_h;
		while (x < FPScr.VWidth) {
			x1 = (x * desk_w) / FPScr.VWidth;
		...
		y = FPScr.MyDisplayHeight;
		x1 = 0;
		x2 = desk_w;
		while (y < FPScr.VHeight) {
			y1 = (y * desk_h) / FPScr.VHeight;
			if (!use_no_separators) {
				XDrawLine(dpy, Desks[desk].w,
	...
	if (((FPScr.CurrentDesk - desk1) == desk) && !ShapeLabels) {
		if (uselabel) {
			XFillRectangle(dpy, Desks[desk].title_w,
			    Desks[desk].HiliteGC, bound.x, bound.y,
	...
	if ((w <= desk_w) && (uselabel)) {
		...
		if (desk == (FPScr.CurrentDesk - desk1)) {
...
DrawIconGrid(int erase)
{
	...
	if (erase) {
		int             tmp = (FPScr.CurrentDesk - desk1);
	...
	y2 = icon_h;
	while (x < FPScr.VWidth) {
		x1 = x * icon_w / FPScr.VWidth;
		if (!use_no_separators)
			for (i = 0; i < ndesks; i++)
				XDrawLine(dpy, icon_win, Desks[i].DashedGC,
				    x1, y1, x1, y2);
		x += FPScr.MyDisplayWidth;
	}
	...
	x = (icon_w - n) * FPScr.Vx / FPScr.VWidth + n1;
	y = (icon_h - m) * FPScr.Vy / FPScr.VHeight + m1;

	if (HilightDesks) {
		if (HilightPixmap) {
			for (i = 0; i < ndesks; i++)
				XCopyArea(dpy, HilightPixmap->picture,
				    icon_win, FPScr.NormalGC, 0, 0, w, h, x,
				    y);
		} else {
			for (i = 0; i < ndesks; i++)
				XFillRectangle(dpy, icon_win,
				    Desks[i].HiliteGC, x, y, w, h);
		}
	}
}
...
SwitchToDeskAndPage(int Desk, XEvent *Event)
{
	...
	if (FPScr.CurrentDesk != (Desk + desk1)) {
		int             vx, vy;
		/*
		 * patch to let mouse button 3 change desks and do not cling to a page
		 */
		vx = Event->xbutton.x * FPScr.VWidth / (desk_w *
		    FPScr.MyDisplayWidth);
		vy = Event->xbutton.y * FPScr.VHeight / (desk_h *
		    FPScr.MyDisplayHeight);
		FPScr.Vx = vx * FPScr.MyDisplayWidth;
		FPScr.Vy = vy * FPScr.MyDisplayHeight;
		sprintf(command, "GotoDeskAndPage %d %d %d", Desk + desk1, vx,
		    vy);
		SendText(fd, command, 0);

	} else {
		int             x =
		    Event->xbutton.x * FPScr.VWidth / (desk_w *
		    FPScr.MyDisplayWidth);
		int             y =
		    Event->xbutton.y * FPScr.VHeight / (desk_h *
		    FPScr.MyDisplayHeight);
		...
		if (y < 0)
			y = 0;
		if (x * FPScr.MyDisplayWidth > FPScr.VxMax)
			x = FPScr.VxMax / FPScr.MyDisplayWidth;
		if (y * FPScr.MyDisplayHeight > FPScr.VyMax)
			y = FPScr.VyMax / FPScr.MyDisplayHeight;
...
IconSwitchPage(XEvent *Event)
{
	...
	sprintf(command, "GotoPage %d %d",
	    Event->xbutton.x * FPScr.VWidth / (icon_w * FPScr.MyDisplayWidth),
	    Event->xbutton.y * FPScr.VHeight / (icon_h *
		FPScr.MyDisplayHeight));
	SendText(fd, command, 0);
	Wait = 1;
}
...
AddNewWindow(PagerWindow *t)
{
	...
	if ((i >= 0) && (i < ndesks)) {
		t->PagerView =
		    XCreateWindow(dpy, Desks[i].w, x, y, w, h, 0,
		    CopyFromParent, InputOutput, CopyFromParent, valuemask,
		    &attributes);
		if (windowcolorset > -1) {
	...
	if (windowcolorset > -1) {
		SetWindowBackground(dpy, t->IconView, w, h,
		    &Colorset[windowcolorset], Pdepth, FPScr.NormalGC, True);
	}
	if (FPScr.CurrentDesk == t->desk) {
...
ChangeDeskForWindow(PagerWindow *t, long newdesk)
{
	...
	CalcGeom(t, desk_w, desk_h, &x, &y, &w, &h);
	size_changed = (t->pager_view_width != w
	    || t->pager_view_height != h);
	t->pager_view_x = x;
	t->pager_view_y = y;
	t->pager_view_width = w;
...
MoveResizePagerView(PagerWindow *t, Bool do_force_redraw)
{
	...
	if (t->PagerView != None) {
		if (size_changed || position_changed || do_force_redraw) {
			...
			if (cset > -1 && (size_changed
				|| CSET_IS_TRANSPARENT(cset))) {
				SetWindowBackground(dpy, t->PagerView,
				    t->pager_view_width, t->pager_view_height,
				    &Colorset[cset], Pdepth, FPScr.NormalGC,
				    True);
			}
		}
...
MoveStickyWindow(Bool is_new_page, Bool is_new_desk)
{
	...
	for (t = Start; t != NULL; t = t->next) {
		if (is_new_desk && t->desk != FPScr.CurrentDesk &&
		    ((IS_ICONIFIED(t) && IS_ICON_STICKY_ACROSS_DESKS(t)) ||
			IS_STICKY_ACROSS_DESKS(t))) {
			ChangeDeskForWindow(t, FPScr.CurrentDesk);
		} else if (is_new_page &&
		    ((IS_ICONIFIED(t) &&
	...
	if (Pdepth < 2) {
		...
		if (cset < 0) {
			if (t->PagerView != None) {
				...
				XClearArea(dpy, t->PagerView, 0, 0, 0, 0,
				    True);
			}
    ...
    Bool do_scroll_icon)
{
	 ...
	 */
	if (Desk >= 0 && Desk + desk1 != FPScr.CurrentDesk) {
		return;
	}
	 ...
	 * center around mouse
	 */
	adjx = (desk_w / (1 + FPScr.VxMax / FPScr.MyDisplayWidth));
	adjy = (desk_h / (1 + FPScr.VyMax / FPScr.MyDisplayHeight));
	x -= adjx / 2;
	y -= adjy / 2;
	...
	sy = 0;
	if (window_w != 0) {
		sx = (x * FPScr.VWidth / window_w - MyVx);
	}
	if (window_h != 0) {
		sy = (y * FPScr.VHeight / window_h - MyVy);
	}
	...
	if (Wait == 0 || last_sx != sx || last_sy != sy) {
		 ...
		 */
		/*
		 * We don't use FPScr.Vx/y since they lag the true position.
		 */
		MyVx += sx;
		if (MyVx < 0) {
			MyVx = 0;
...
MoveWindow(XEvent *Event)
{
	...
	n = FPScr.VxMax / FPScr.MyDisplayWidth;
	m = FPScr.VyMax / FPScr.MyDisplayHeight;
	n1 = (FPScr.Vx + t->x) / FPScr.MyDisplayWidth;
	m1 = (FPScr.Vy + t->y) / FPScr.MyDisplayHeight;
	wx = (FPScr.Vx + t->x) * (desk_w - n) / FPScr.VWidth + n1;
	wy = (FPScr.Vy + t->y) * (desk_h - m) / FPScr.VHeight + m1;
	...
	XReparentWindow(dpy, t->PagerView, FPScr.Pager_w, wx1, wy1);
	...
	while (!finished) {
		...
		if (Event->type == MotionNotify) {
			XTranslateCoordinates(dpy, Event->xany.window,
			    FPScr.Pager_w, Event->xmotion.x,
			    Event->xmotion.y, &x, &y, &dumwin);
			...
			XTranslateCoordinates(dpy, Event->xany.window,
			    FPScr.Pager_w, Event->xbutton.x,
			    Event->xbutton.y, &x, &y, &dumwin);
			XMoveWindow(dpy, t->PagerView, x - x1, y - y1);
			finished = 1;
	...
	if (KeepMoving) {
		...
		if (FQueryPointer(dpy, FPScr.Root, &JunkRoot, &JunkChild,
		...
		if (NewDesk != t->desk) {
			 ...
			 * * is current desk, and if mvwm is on another
		...
		XTranslateCoordinates(dpy, FPScr.Pager_w, Desks[NewDesk].w,
		    x - x1, y - y1, &x2, &y2, &dumwin);

		n1 = x2 * FPScr.VWidth / (desk_w * FPScr.MyDisplayWidth);
		m1 = y2 * FPScr.VHeight / (desk_h * FPScr.MyDisplayHeight);
		x = (x2 - n1) * FPScr.VWidth / (desk_w - n) - FPScr.Vx;
		y = (y2 - m1) * FPScr.VHeight / (desk_h - m) - FPScr.Vy;
		/*
		 * force onto desk
		 */
		if (x + t->frame_width + FPScr.Vx < 0) {
			x = -FPScr.Vx - t->frame_width;
		}
		if (y + t->frame_height + FPScr.Vy < 0) {
			y = -FPScr.Vy - t->frame_height;
		}
		if (x + FPScr.Vx >= FPScr.VWidth) {
			x = FPScr.VWidth - FPScr.Vx - 1;
		}
		if (y + FPScr.Vy >= FPScr.VHeight) {
			y = FPScr.VHeight - FPScr.Vy - 1;
		}
		if ((IS_ICONIFIED(t) && IS_ICON_STICKY_ACROSS_DESKS(t)) ||
		    (IS_STICKY_ACROSS_DESKS(t))) {
			NewDesk = FPScr.CurrentDesk - desk1;
			if (x > FPScr.MyDisplayWidth - 16) {
				x = FPScr.MyDisplayWidth - 16;
			}
			if (y > FPScr.MyDisplayHeight - 16) {
				y = FPScr.MyDisplayHeight - 16;
			}
			if (x + t->width < 16) {
				x = 16 - t->width;
			}
		...
		if (NewDesk + desk1 != t->desk) {
			if ((IS_ICONIFIED(t)
			    ...
			    || (IS_STICKY_ACROSS_DESKS(t))) {
				NewDesk = FPScr.CurrentDesk - desk1;
				if (t->desk != FPScr.CurrentDesk) {
					ChangeDeskForWindow(t,
					    FPScr.CurrentDesk);
				}
			} else if (NewDesk + desk1 != FPScr.CurrentDesk) {
 ...
 *
 *  Procedure:
 *	MvwmErrorHandler - displays info on internal errors
 *
 */
int
MvwmErrorHandler(Display *dpy, XErrorEvent * event)
{
#if 1
...
draw_window_border(PagerWindow *t, Window w, int width, int height)
{
	if (w != None) {
		if (!WindowBorders3d || windowcolorset < 0
		    || activecolorset < 0) {
			XSetForeground(dpy, FPScr.NormalGC, FPScr.black);
			RelieveRectangle(dpy, w, 0, 0, width - 1,
			    t->pager_view_height - 1, FPScr.NormalGC,
			    FPScr.NormalGC, WindowBorderWidth);
		} else if (t == FocusWin) {
			RelieveRectangle(dpy, w, 0, 0, width - 1, height - 1,
			    FPScr.ahGC, FPScr.asGC, WindowBorderWidth);
		} else {
			RelieveRectangle(dpy, w, 0, 0, width - 1, height - 1,
			    FPScr.whGC, FPScr.wsGC, WindowBorderWidth);
		}
	}
}
...
label_window_wrap(PagerWindow *t)
{
	...
	while (*cur) {
		while (*next) {
			...
			if (width >
			    t->pager_view_width - cur_width - space_width -
			    2 * label_border)
				break;
		...
		if (cur == next) {
			...
			while (*next) {
				...
				len =
				    FlocaleStringNumberOfBytes(FwindowFont,
				    next);
				width =
				    FlocaleTextWidth(FwindowFont, next, len);

				if (width >
				    t->pager_view_width - cur_width -
...
do_label_window(PagerWindow *t, Window w)
{
	...
	if (t == FocusWin) {
		...
		XSetForeground(dpy, FPScr.NormalGC, t->text);
		cs = windowcolorset;
	}
	...
	if (MiniIcons) {
		if (t->mini_icon.picture && w != None) {
			...
			PGraphicsRenderPicture(dpy, w, &t->mini_icon, &fra, w,
			    FPScr.MiniIconGC, None, None, src_x, src_y,
...
IconMoveWindow(XEvent *Event, PagerWindow *t)
{
	...
	if (KeepMoving) {
		...
		x = x - x1;
		y = y - y1;
		n1 = x * FPScr.VWidth / (icon_w * FPScr.MyDisplayWidth);
		m1 = y * FPScr.VHeight / (icon_h * FPScr.MyDisplayHeight);
		x = (x - n1) * FPScr.VWidth / (icon_w - n) - FPScr.Vx;
		y = (y - m1) * FPScr.VHeight / (icon_h - m) - FPScr.Vy;
		/*
		 * force onto desk
		 */
		if (x + t->icon_width + FPScr.Vx < 0) {
			x = -FPScr.Vx - t->icon_width;
		}
		if (y + t->icon_height + FPScr.Vy < 0) {
			y = -FPScr.Vy - t->icon_height;
		}
		if (x + FPScr.Vx >= FPScr.VWidth) {
			x = FPScr.VWidth - FPScr.Vx - 1;
		}
...
setup_balloon_window(int i)
{
	...
	if (!ShowBalloons && FPScr.balloon_w == None) {
		return;
	}
	XUnmapWindow(dpy, FPScr.balloon_w);
	...
	XChangeWindowAttributes(dpy, FPScr.balloon_w, valuemask, &xswa);
...
setup_balloon_gc(int i, PagerWindow *t)
{
	...
	if (Desks[i].ballooncolorset < 0) {
		...
		if (BalloonFore == NULL) {
			XSetForeground(dpy, FPScr.balloon_gc, t->text);
		}
	} else {
	...
	XChangeGC(dpy, FPScr.balloon_gc, valuemask, &xgcv);
...
MapBalloonWindow(PagerWindow *t, Bool is_icon_view)
{
	...
	FPScr.balloon_desk = i;
	/*
	 * get the label for this balloon
	 */
	if (FPScr.balloon_label) {
		free(FPScr.balloon_label);
	}
	FPScr.balloon_label = GetBalloonLabel(t, BalloonFormatString);
#if 0
	if (FPScr.balloon_label == NULL) {
		/*
	...
	if (FPScr.balloon_label) {
		new_g.width +=
		    FlocaleTextWidth(Desks[i].balloon.Ffont,
		    FPScr.balloon_label, strlen(FPScr.balloon_label));
	}
	/*
	 ...
	 */
	XTranslateCoordinates(dpy, view, FPScr.Root, x, y, &new_g.x, &new_g.y,
	    &dummy);
	/*
	...
	if (new_g.y < 2) {
		...
		XTranslateCoordinates(dpy, view, FPScr.Root, x, y, &new_g.x,
		    &new_g.y, &dummy);
	}
	/*
	 ...
	 */
	    ...
	    FPScr.MyDisplayHeight - (2 * BalloonBorderWidth) - 2) {
	 ...
	 */
	    ...
	    FPScr.MyDisplayWidth - (2 * BalloonBorderWidth) - 2) {
		new_g.x = FPScr.MyDisplayWidth - new_g.width -
		    (2 * BalloonBorderWidth) - 2;
	}
	/*
	 * make changes to window
	 */
	XMoveResizeWindow(dpy, FPScr.balloon_w, new_g.x, new_g.y, new_g.width,
	    new_g.height);
	/*
	 * if background not set in config make it match pager window
	 */
	if (BalloonBack == NULL) {
		XSetWindowBackground(dpy, FPScr.balloon_w, t->back);
	}
	if (Desks[i].ballooncolorset > -1) {
		SetWindowBackground(dpy, FPScr.balloon_w, new_g.width,
		    new_g.height, &Colorset[Desks[i].ballooncolorset], Pdepth,
		    FPScr.NormalGC, True);
		XSetWindowBorder(dpy, FPScr.balloon_w,
		    Colorset[Desks[i].ballooncolorset].fg);
	}
	XMapRaised(dpy, FPScr.balloon_w);
...
{
	FwinString->str = FPScr.balloon_label;
	FwinString->win = FPScr.balloon_w;
	FwinString->gc = FPScr.balloon_gc;
	if (Desks[i].ballooncolorset >= 0) {
...
set_window_colorset_background(PagerWindow *t, colorset_t *csetp)
{
	if (t->PagerView != None) {
		SetWindowBackground(dpy, t->PagerView, t->pager_view_width,
		    t->pager_view_height, csetp, Pdepth, FPScr.NormalGC,
		    True);
	}
	SetWindowBackground(dpy, t->IconView, t->icon_view_width,
	    t->icon_view_height, csetp, Pdepth, FPScr.NormalGC, True);
...
change_colorset(int colorset)
{
	...
	XSetWindowBackgroundPixmap(dpy, FPScr.Pager_w, default_pixmap);
	for (i = 0; i < ndesks; i++) {
		if (Desks[i].highcolorset == colorset) {
			...
			if (HilightDesks) {
				if (uselabel && (i == FPScr.CurrentDesk)) {
					SetWindowBackground(dpy,
					    Desks[i].title_w, desk_w, desk_h,
					    &Colorset[colorset], Pdepth,
					    FPScr.NormalGC, True);
				}
				SetWindowBackground(dpy, Desks[i].CPagerWin,
				    0, 0, &Colorset[colorset], Pdepth,
				    FPScr.NormalGC, True);
				XLowerWindow(dpy, Desks[i].CPagerWin);
			}
		}
	...
	if (ShowBalloons
	    && Desks[FPScr.balloon_desk].ballooncolorset == colorset) {
		XSetWindowBackground(dpy, FPScr.balloon_w,
		    Colorset[colorset].fg);
		XSetForeground(dpy, FPScr.balloon_gc, Colorset[colorset].fg);
		XClearArea(dpy, FPScr.balloon_w, 0, 0, 0, 0, True);
	}
	...
	if (windowcolorset == colorset) {
		...
		XSetForeground(dpy, FPScr.whGC, csetp->hilite);
		XSetForeground(dpy, FPScr.wsGC, csetp->shadow);
	...
	if (activecolorset == colorset) {
		...
		XSetForeground(dpy, FPScr.ahGC, csetp->hilite);
		XSetForeground(dpy, FPScr.asGC, csetp->shadow);
