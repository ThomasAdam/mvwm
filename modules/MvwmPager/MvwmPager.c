...
MvwmPicture    *HilightPixmap = NULL;
int             HilightDesks = 1;
...
main(int argc, char **argv)
{
	...
	FlocaleInit(LC_CTYPE, "", "", "MvwmPager");
	...
	SetMessageMask(fd,
	    MX_VISIBLE_ICON_NAME | MX_PROPERTY_CHANGE | MX_REPLY);
	ParseOptions();
	if (is_transient) {
		if (FQueryPointer(dpy, FPScr.Root, &JunkRoot, &JunkChild,
			&window_x, &window_y, &JunkX, &JunkY,
			&JunkMask) == False) {
			/*
	...
	if (is_transient) {
		...
		for (i = 0;
			...
			if (!is_pointer_grabbed
			    && XGrabPointer(dpy, FPScr.Root, True,
			...
			if (!is_keyboard_grabbed &&
			    XGrabKeyboard(dpy, FPScr.Root, True,
	...
	while (!isTerminated) {
		...
		if (error_occured) {
			...
			if (XGetGeometry(dpy, FPScr.Pager_w, &root, &x, &y,
				(unsigned *) &window_w,
				(unsigned *) &window_h, &border_width,
				&depth) == 0) {
				/*
				 * does not return
				 */
 ...
 */
void
process_message(MvwmPacket * packet)
{
...
list_new_desk(unsigned long *body)
{
	...
	if (fAlwaysCurrentDesk && oldDesk != FPScr.CurrentDesk) {
		...
		if (Desks[0].bgPixmap != NULL) {
			PDestroyMvwmPicture(dpy, Desks[0].bgPixmap);
...
list_deiconify(unsigned long *body, unsigned long length)
{
	...
	if (t == NULL) {
		...
		if (length >= 11 + MvwmPacketHeaderSize) {
			t->frame_x = body[7];
			t->frame_y = body[8];
...
list_property_change(unsigned long *body)
{
	if (body[0] == MX_PROPERTY_CHANGE_BACKGROUND) {
		...
		if (((!Swallowed && body[2] == 0) || (Swallowed
			    && body[2] == FPScr.Pager_w))) {
...
My_XNextEvent(Display *dpy, XEvent *event)
{
	...
	if (mvwmSelect(fd_width, &in_fdset, 0, 0, NULL) > 0) {
...
static void
ParseColorset(char *arg1, char *arg2, void *offset_deskinfo,
...
static void
SetDeskLabel(int desk, const char *label)
{
	PagerStringList *item;
	...
	for (GetConfigLine(fd, &tline); tline != NULL;
		...
		if (StrEquals(token, "Colorset")) {
			...
			if (!MoveThresholdSetForModule) {
				int             val;
				if (GetIntegerArguments(next, NULL, &val,
					1) > 0) {
					if (val >= 0)
						MoveThreshold = val;
					else
		...
		if (StrEquals(resource, "Colorset")) {
			...
			usposition = 0;
			flags =
			    FScreenParseGeometry(arg1, &g_x, &g_y, &width,
			    &height);
			if (flags & WidthValue) {
				window_w = width;
			}
			...
			if (fAlwaysCurrentDesk) {
				...
				if (item->next != NULL) {
					...
					item->next->bgPixmap =
					    PCacheMvwmPicture(dpy,
					    FPScr.Pager_w, ImagePath, arg2,
					    fpa);
				} else {
					/*
					 * new Dcolor and desktop
				...
				Desks[dNr].bgPixmap =
				    PCacheMvwmPicture(dpy, FPScr.Pager_w,
				    ImagePath, arg2, fpa);
			}
			...
			if (Pdepth > 1) {
				if (PixmapBack) {
					PDestroyMvwmPicture(dpy, PixmapBack);
					PixmapBack = NULL;
				}
		 ...
		 */
			...
			if (strncasecmp(BalloonTypeString, "Pager", 5) == 0) {
				ShowPagerBalloons = 1;
				ShowIconBalloons = 0;
			} else if (strncasecmp(BalloonTypeString, "Icon",
				4) == 0) {
				ShowPagerBalloons = 0;
				ShowIconBalloons = 1;
			} else {
