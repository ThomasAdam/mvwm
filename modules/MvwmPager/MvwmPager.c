 ...
 *
 */
void process_message( MvwmPacket* packet )
{
...
void ParseOptions(void)
{
  ...
  for (GetConfigLine(fd,&tline); tline != NULL; GetConfigLine(fd,&tline))
  {
	...
	if (item->next != NULL)
	{
	  ...
	  {
	    PDestroyMvwmPicture(dpy, item->next->bgPixmap);
	    item->next->bgPixmap = NULL;
	  }
	  item->next->bgPixmap = PCacheMvwmPicture(
	...
	else
	{
	  /* new Dcolor and desktop */
	  item = NewPagerStringItem(item, desk);
	  item->bgPixmap = PCacheMvwmPicture(
		  dpy, FPScr.Pager_w, ImagePath, arg2, fpa);
	}
	if (desk == FPScr.CurrentDesk)
	{
	  ...
	  Desks[0].bgPixmap = PCacheMvwmPicture(
		  dpy, FPScr.Pager_w, ImagePath, arg2, fpa);
	}
      }
      else if((desk >= desk1)&&(desk <=desk2))
	...
	{
	  PDestroyMvwmPicture(dpy, Desks[dNr].bgPixmap);
	  Desks[dNr].bgPixmap = NULL;
	}
	Desks[dNr].bgPixmap = PCacheMvwmPicture(
		dpy, FPScr.Pager_w, ImagePath, arg2, fpa);
      }
    ...
    else if (StrEquals(resource, "HilightPixmap"))
    {
      ...
      {
	if (HilightPixmap) {
	  PDestroyMvwmPicture (dpy, HilightPixmap);
	  HilightPixmap = NULL;
	}

	HilightPixmap = PCacheMvwmPicture (
		dpy, FPScr.Pager_w, ImagePath, arg1, fpa);
