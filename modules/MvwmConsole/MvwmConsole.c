...
int main(int argc, char *argv[])
{
	...
	{
		fprintf(
			stderr, "MvwmConsole version "VERSION" should only be"
			" executed by mvwm!\n");
		exit(1);
	}
...
void server(void)
{
	...
	S_name = mvwm_malloc(strlen(home) + sizeof(S_NAME) + 1);
	strcpy(S_name, home);
	...
	else
	{
		...
		alarm(MVWMCONSOLE_CONNECTION_TO_SECS);
	...
	tline = NULL;
	mvwm_send(Ns, C_BEG, strlen(C_BEG), 0);
	GetConfigLine(Fd, &tline);
	while (tline != NULL)
	{
		if (strlen(tline) > 1)
		{
			mvwm_send(Ns, tline, strlen(tline), 0);
			mvwm_send(Ns, "\n", 1, 0);
		}
		GetConfigLine(Fd, &tline);
	}
	mvwm_send(Ns, C_END, strlen(C_END), 0);
	strcpy(ver, "*");
	strcpy(ver, module->name);
	strcat(ver, " version ");
	strcat(ver, VERSION VERSIONINFO);
	strcat(ver, "\n");
	mvwm_send(Ns, ver, strlen(ver), 0);
	...
	while (1)
	{
		...
		mvwmSelect(FD_SETSIZE, SELECT_FD_SET_CAST &fdset, 0, 0, NULL);
		...
		if (FD_ISSET(Ns, &fdset))
		{
			...
			rc = mvwm_recv(Ns, buf, MAX_COMMAND_SIZE, 0);
			if (rc == 0)
			{
