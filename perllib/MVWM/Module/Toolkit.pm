...
sub import ($@) {
	...
	while (@_) {
		...
		if ($name eq 'base') {
			...
			eval "
				...
				use MVWM::Constants;
				\$VERSION = \$MVWM::Module::Toolkit::VERSION;
				\@ISA = qw(MVWM::Module::Toolkit);
			";
			if ($@) {
				die "Internal error:\n$@";
			}
	...
	if ($error) {
		...
		my $error_title = 'MVWM Perl library error';
...
sub add_default_error_handler ($) {
	...
	$self->add_handler(M_ERROR, sub {
		my ($self, $event) = @_;
		$self->show_error($event->_text, "mvwm error");
	});
}
...
MVWM::Module::Toolkit - MVWM::Module with abstract widget toolkit attached
...
the nice dialog if absent:
    ...
    use MVWM::Module::Toolkit qw(Tk X11::Protocol Tk::Balloon);

    use MVWM::Module::Toolkit qw(Tk=804.024,catch X11::Protocol>=0.52);
...
MVWM::Module::Toolkit->show_message() is used to show the dialog and the
application dies.
...
A new toolkit subclass implementation may look like this:
    ...
    package MVWM::Module::SomeToolkit;
    # this automatically sets the base class and tries "use SomeToolkit;"
    use MVWM::Module::Toolkit qw(base SomeToolkit);
...
The B<MVWM::Module::Toolkit> package is a sub-class of B<MVWM::Module> that
...
has been reported by mvwm. The M_ERROR handler then calls C<show_error()>
...
For more information, see L<mvwm>, L<MVWM::Module>, L<MVWM::Module::Gtk>,
L<MVWM::Module::Gtk2>, L<MVWM::Module::Tk>.
