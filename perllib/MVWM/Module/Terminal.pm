...
BEGIN {
	if ($ENV{MVWM_MODULE_TERMINAL_CLIENT}) {
		my ($out_fd, $in_fd, $name, $prompt, $ornaments) = @ARGV;
		$name ||= "MVWM::Module::Terminal";
		$prompt ||= "";
		$ornaments ||= 0;
		$name .= " Client";
...
package MVWM::Module::Terminal;

use 5.004;
use strict;

use MVWM::Module::Toolkit qw(base IO::Select);
use General::Parse;
...
sub new ($@) {
	...
	my $xterm = delete $params{XTerm} || "xterm -name MvwmPerlTerminal -e";
	...
	if ($pid == 0) {
		...
		$ENV{MVWM_MODULE_TERMINAL_CLIENT} = 1;
...
MVWM::Module::Terminal - MVWM::Module with X terminal based solutions
...
Name this module TestModuleTerminal, make it executable and place in ModulePath:
    ...
    use MVWM::Module::Terminal;

    my $module = new MVWM::Module::Terminal(
...
The B<MVWM::Module::Terminal> class is a sub-class of
B<MVWM::Module::Toolkit> that overloads the methods B<wait_packet>,
...
Listen to the terminal read-line while waiting for the packet from mvwm.
...
For more information, see L<mvwm>, L<MVWM::Module> and L<Term::ReadLine>.
