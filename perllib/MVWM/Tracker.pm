...
sub new ($$) {
	my $class = shift;
	my $module = shift;
	die "$class: no MVWM::Module object given in constructor\n"
		unless UNIVERSAL::isa($module, "MVWM::Module");
	my $self = {
		module => $module,
		data   => undef,
		active => 0,
...
sub start ($) {
	...
	$self->{module}->MVWM::Module::event_loop(1)
		if %{$self->{handler_ids}};
...
Tracker is an object that automatically listens to certain mvwm events and
...
Using B<MVWM::Module> $module object:
...
In the future this syntax will probably work too:
    ...
    my $tracker = new MVWM::Tracker::TrackerName($module, @params);
...
Makes the tracker actually work, i.e. listen to I<mvwm> events,
...
Stops the tracker activity. The corresponding I<mvwm> events are not listened,
...
following parameters: $module (B<MVWM::Module> object), $tracker (this object),
...
I<module> is an B<MVWM::Module> instance.
I<param-hash> is specific to the concrete Tracker class.

=item B<add_handler> I<type> I<handler>

A wrapper to B<MVWM::Module>::B<add_handler>, has the same syntax, but stores
...
Subclasses that work using I<mvwm> events sent in responce to
...
This is a wrapper to B<MVWM::Module>::B<internal_die>.

=item B<to_be_disconnected>

Does nothing by default. Subclasses may implement this method if something
should be sent to I<mvwm> just before the module disconnects itself.
...
For more information, see L<MVWM::Module> and the concrete tracker
implementations: L<MVWM::Tracker::Colorsets>, L<MVWM::Tracker::GlobalConfig>,
L<MVWM::Tracker::ModuleConfig>, L<MVWM::Tracker::PageInfo>,
L<MVWM::Tracker::Scheduler>, L<MVWM::Tracker::WindowList>.
