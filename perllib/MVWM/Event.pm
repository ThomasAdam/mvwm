...
sub dump ($) {
	...
	while (@arg_names) {
		...
		my $text;
		if ($type == MVWM::EventNames::number) {
			$text = $value;
			$text = "*undefined*" unless defined $value;
		} elsif ($type == MVWM::EventNames::bool) {
			$text = $value? "True": "False";
		} elsif ($type == MVWM::EventNames::window) {
			$text = sprintf("0x%07lx", $value);
		} elsif ($type == MVWM::EventNames::pixel) {
			$text = "rgb:" . join('/',
				sprintf("%06lx", $value) =~ /(..)(..)(..)/);
		} elsif ($type == MVWM::EventNames::string) {
			$value =~ s/"/\\"/g;
			$text = qq("$value");
		} elsif ($type == MVWM::EventNames::looped) {
		...
		} elsif ($type == MVWM::EventNames::wflags) {
...
MVWM::Event - the mvwm event object passed to event handlers

=head1 SYNOPSIS

  use lib `mvwm-perllib dir`;
  use MVWM::Module;

  my $module = new MVWM::Module(Mask => M_FOCUS_CHANGE);
...
has its own argument list, see L<MVWM::EventNames>) or a packed string of
these arguments as received from the I<mvwm> pipe.
...
For technical reasons there are 2 categories of mvwm events, regular and
...
argument. See L<MVWM::EventNames> for names of all event argument names.
...
For more information, see L<mvwm>, L<MVWM::Module>, L<MVWM::Constants> and
L<MVWM::EventNames>.
