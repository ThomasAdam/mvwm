...
package MVWM::Tracker::Scheduler;

use strict;

use MVWM::Tracker qw(base);
...
sub new ($$%) {
	...
	my $self = $class->MVWM::Tracker::new($module);

	$self->{random_base_number} = 8000000 + int(rand(900000));
	$self->{sent_string_prefix} = "MVWM::Tracker::Scheduler alarm ";
...
sub send_schedule ($$) {
	...
	if ($self->{use_alarm}) {
		...
		$self->{module}->send("Schedule $mseconds $sd->{mvwm_id} "
...
sub schedule ($$$;$) {
	...
	my $mvwm_id = $self->{random_base_number} + $schedule_id;
	my $schedule_data = {
		time_sent => 0,
		seconds   => $seconds,
		mvwm_id   => $mvwm_id,
...
sub deschedule ($$) {
	...
	my $mvwm_id = $data->{$schedule_id}->{mvwm_id};

	$self->{module}->send("Deschedule $mvwm_id")
...
sub dump ($;$) { 
	...
	foreach (@ids) {
		...
		$string .= "Schedule $_: $time_str + $sd->{seconds} sec, use 'Deschedule $sd->{mvwm_id}'\n";
	}
	return $string;
}
...
This is a subclass of B<MVWM::Tracker> that enables to define alarm callback,
...
This tracker uses the I<mvwm> command B<Schedule> to get a notification.
...
I<sheduled-id> is given. The hash keys are:
    ...
    mvwm_id    - internal I<mvwm>'s Schedule id
...
For more information, see L<MVWM::Module> and L<MVWM::Tracker>.
