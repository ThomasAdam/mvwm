...
sub all_event_type_names () {
	if (!defined $EVENT_TYPE_NAMES) {
		...
		$EVENT_TYPE_NAMES = {};
		my ($type, $name);
		foreach $name (@MVWM::Constants::EXPORT) {
...
sub event_info ($) {
	...
	unless (defined $EVENTS_INFO->{$type}) {
		die "MVWM::EventNames: Unknown event type (" .
...
MVWM::EventNames - names and types of all mvwm event arguments
...
Every event send by I<mvwm> consist of arguments. The argument names and
...
This class provides information about all mvwm events. It provides such
...
B<MVWM::Event> objects. Hovewer, you may need this class source as a
...
from the I<packed_str> (as received from I<mvwm>).
