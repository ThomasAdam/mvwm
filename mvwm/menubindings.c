 ...
 */
static int
get_selectable_item_index(MenuRoot *mr, MenuItem *mi_target,
    int *ret_sections)
{
	int             i = 0;
	int             s = 0;
	...
	*ret_cmd = NULL;
	...
	case 4:	/* MenuMoveCursor */
		...
		if (num == 2) {
			if (suffix[0] != 0 || count[0] != 0) {
				mvwm_msg(ERR, "parse_menu_action",
	...
	case 8:	/* MenuScroll */
		if (MST_MOUSE_WHEEL(mr) == MMW_OFF) {
			*saction = SA_SELECT;
			...
			num =
			    GetSuffixedIntegerArguments(options, NULL, count,
			    1, "s", suffix);
			if (num == 1) {
				*saction = SA_SCROLL;
				*items_to_move = count[0];
	...
	case 10:	/* MenuCloseAndExecute */
		...
		*ret_cmd = options;
		break;
	default:
		mvwm_msg(ERR, "parse_menu_action", "unknown action '%s'",
		    action);
		*saction = SA_NONE;
	}
    ...
    int context, int modifier, char *action, char *menu_style)
{
	...
	if (menu_style != NULL) {
		 ...
		 */
		mvwm_msg(ERR, "menu_binding", "a window name may not be"
	...
	if ((modifier & AnyModifier) && (modifier & (~AnyModifier))) {
		mvwm_msg(WARN, "menu_binding", "Binding specified AnyModifier"
	...
	if (keysym == 0 && button != 0 && modifier == 0 &&
		...
		mvwm_msg(OLD, "menu_binding",
		    "The syntax for disabling the tear off button has "
		    "changed. The TearOff action is no longer possible "
		    "in menu bindings.");
	}
	rc = AddBinding(dpy, menu_bindings, type, button, keysym,
    ...
    double_keypress * pdkp, int *ret_menu_x, int *ret_menu_y)
{
	...
	if (index == 1 && isascii(ikeychar) && isgraph(ikeychar) &&
		...
		do {
			if (MI_HAS_HOTKEY(mi) && !MI_IS_TITLE(mi) &&
				...
				key =
				    (MI_LABEL(mi)[(int) MI_HOTKEY_COLUMN(mi)])
	...
	case SA_MOVE_ITEMS:
		if (do_skip_section) {
			...
			index =
			    get_selectable_item_index(mr, mi_current, NULL);
			if (index == 0)
				/*
				 * wraparound
				 */
			...
			index =
			    get_selectable_item_index(mr, mi_current,
			    NULL) + items_to_move;
			/*
		...
		if (new_item) {
			*pmi_current = new_item;
			...
			if (MST_MOUSE_WHEEL(mr) == MMW_POINTER) {
				...
				menu_y +=
				    my - menuitem_middle_y_offset(new_item,
				    MR_STYLE(mr));
