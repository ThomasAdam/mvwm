...
menustyle_free_face(MenuFace *mf)
{
	...
	case GradientMenu:
		if (Pdepth <= 8 && mf->u.grad.npixels > 0 &&
			...
			PictureFreeColors(dpy, Pcmap, p, mf->u.grad.npixels,
			    0, False);
			free(p);
		}
		free(mf->u.grad.xcs);
	...
	case TiledPixmapMenu:
		if (mf->u.p) {
			PDestroyMvwmPicture(dpy, mf->u.p);
		}
		mf->u.p = NULL;
		break;
	case SolidMenu:
		mvwmlib_free_colors(dpy, &mf->u.back, 1, True);
	default:
		break;
	}
	mf->type = SimpleMenu;
...
menustyle_copy_face(MenuFace *destmf, MenuFace *origmf)
{
	...
	switch (origmf->type) {
	case SolidMenu:
		mvwmlib_copy_color(dpy, &destmf->u.back, &origmf->u.back,
		    False, True);
	...
	case GradientMenu:
		...
		for (i = 0; i < origmf->u.grad.npixels; i++) {
			mvwmlib_clone_color(origmf->u.grad.xcs[i].pixel);
		}
...
menustyle_parse_face(char *s, MenuFace *mf, int verbose)
{
	...
	if (!style) {
		...
		if (token) {
			...
			if (verbose) {
				mvwm_msg(ERR, "menustyle_parse_face",
		 ...
		 */
		xcs =
		    AllocAllGradientColors(s_colors, perc, nsegs, npixels,
		    mf->u.grad.do_dither);
		if (xcs == None) {
			return False;
		}
...
static void
parse_vertical_spacing_line(char *args, signed char *above,
    signed char *below, signed char above_default, signed char below_default)
{
	int             val[2];
...
parse_vertical_margins_line(char *args, unsigned char *top,
    unsigned char *bottom, signed char top_default,
...
menustyle_parse_old_style(F_CMD_ARGS)
{
	...
	if (!fore || !back || !stipple || !font || !style) {
		mvwm_msg(ERR, "menustyle_parse_old_style",
		...
		mvwm_msg(OLD, "menustyle_parse_old_style",
...
change_or_make_gc(GC *gc, unsigned long gcm, XGCValues * gcv)
{
	if (*gc != None) {
		...
		*gc = mvwmlib_XCreateGC(dpy, Scr.NoFocusWin, gcm, gcv);
	}
...
menustyle_free(MenuStyle *ms)
{
	...
	if (ST_SIDEPIC(ms)) {
		PDestroyMvwmPicture(dpy, ST_SIDEPIC(ms));
	}
	if (ST_HAS_SIDE_COLOR(ms) == 1) {
		mvwmlib_free_colors(dpy, &ST_SIDE_COLOR(ms), 1, True);
	}
	...
	mvwmlib_free_colors(dpy, &ST_MENU_COLORS(ms).back, 1, True);
	mvwmlib_free_colors(dpy, &ST_MENU_COLORS(ms).fore, 1, True);
	if (ST_HAS_STIPPLE_FORE(ms)) {
		mvwmlib_free_colors(dpy, &ST_MENU_STIPPLE_COLORS(ms).fore, 1,
		    True);
	}
	if (ST_HAS_ACTIVE_BACK(ms)) {
		mvwmlib_free_colors(dpy, &ST_MENU_ACTIVE_COLORS(ms).back, 1,
		    True);
	}
	if (ST_HAS_ACTIVE_FORE(ms)) {
		mvwmlib_free_colors(dpy, &ST_MENU_ACTIVE_COLORS(ms).fore, 1,
		    True);
	}
...
menustyle_update(MenuStyle *ms)
{
	...
	if (ST_USAGE_COUNT(ms) != 0) {
		mvwm_msg(ERR, "menustyle_update", "menu style %s is in use",
		    ST_NAME(ms));
		return;
	}
	ST_IS_UPDATED(ms) = 1;
...
menustyle_parse_style(F_CMD_ARGS)
{
	...
	if (ms && ST_USAGE_COUNT(ms) != 0) {
		mvwm_msg(ERR, "NewMenuStyle", "menu style %s is in use",
		    name);
		return ms;
	}
	...
	while (!is_initialised || (action && *action)) {
		...
		if (!is_initialised) {
			...
			args = action;
			action =
			    GetQuotedString(action, &optstring, ",", NULL,
			    NULL, NULL);
			if (!optstring) {
				break;
			}
		...
		case 2:	/* win */
			...
			if (ST_HAS_SIDE_COLOR(tmpms) == 1) {
				mvwmlib_free_colors(dpy,
				    &ST_SIDE_COLOR(tmpms), 1, True);
				ST_HAS_SIDE_COLOR(tmpms) = 0;
			}
			ST_HAS_SIDE_COLOR(tmpms) = 0;
			if (ST_SIDEPIC(tmpms)) {
				PDestroyMvwmPicture(dpy, ST_SIDEPIC(tmpms));
		...
		case 3:	/* Foreground */
			mvwmlib_free_colors(dpy, &ST_MENU_COLORS(tmpms).fore,
			    1, True);
			if (arg1) {
		...
		case 4:	/* Background */
			mvwmlib_free_colors(dpy, &ST_MENU_COLORS(tmpms).back,
			    1, True);
			if (arg1) {
		...
		case 15:	/* Font */
			if (arg1 != NULL &&
			    !(new_font = FlocaleLoadFont(dpy, arg1, "mvwm")))
			{
				mvwm_msg(ERR, "NewMenuStyle",
		...
		case 18:	/* PopupOffset */
			if ((n = GetIntegerArguments(args, NULL, val,
				    2)) == 0) {
				mvwm_msg(ERR, "NewMenuStyle",
		...
		case 31:	/* SidePic */
			...
			if (arg1) {
				fpa.mask = (Pdepth <= 8) ? FPAM_DITHER : 0;
				ST_SIDEPIC(tmpms) =
				    PCacheMvwmPicture(dpy, Scr.NoFocusWin,
				    NULL, arg1, fpa);
				if (!ST_SIDEPIC(tmpms)) {
					mvwm_msg(WARN, "NewMenuStyle",
		...
		default:
			mvwm_msg(ERR, "NewMenuStyle", "unknown option '%s'",
			    poption);
			break;
		}	/* switch */
...
menustyle_copy(MenuStyle *origms, MenuStyle *destms)
{
	 ...
	 * menu colors
	 */
	mvwmlib_copy_color(dpy, &ST_MENU_COLORS(destms).fore,
	    &ST_MENU_COLORS(origms).fore, True, True);
	mvwmlib_copy_color(dpy, &ST_MENU_COLORS(destms).back,
	    &ST_MENU_COLORS(origms).back, True, True);
	/*
	 * Greyed
	 */
	mvwmlib_copy_color(dpy, &ST_MENU_STIPPLE_COLORS(destms).fore,
	 ...
	 * HilightBack
	 */
	mvwmlib_copy_color(dpy, &ST_MENU_ACTIVE_COLORS(destms).back,
	 ...
	 * ActiveFore
	 */
	mvwmlib_copy_color(dpy, &ST_MENU_ACTIVE_COLORS(destms).fore,
	...
	if (ST_PSTDFONT(origms) && !ST_USING_DEFAULT_FONT(origms)) {
		if (!(ST_PSTDFONT(destms) =
			FlocaleLoadFont(dpy, ST_PSTDFONT(origms)->name,
			    "mvwm"))) {
			ST_PSTDFONT(destms) = Scr.DefaultFont;
			ST_USING_DEFAULT_FONT(destms) = True;
			mvwm_msg(ERR, "CopyMenuStyle",
	...
	if (ST_PTITLEFONT(origms) && !ST_USING_DEFAULT_TITLEFONT(origms)) {
		if (!(ST_PTITLEFONT(destms) =
			FlocaleLoadFont(dpy, ST_PTITLEFONT(origms)->name,
			    "mvwm"))) {
	...
	if (ST_SIDEPIC(destms)) {
		PDestroyMvwmPicture(dpy, ST_SIDEPIC(destms));
	...
	if (ST_SIDEPIC(origms)) {
		...
		ST_SIDEPIC(destms) =
		    PCacheMvwmPicture(dpy, Scr.NoFocusWin, NULL,
		    ST_SIDEPIC(origms)->name, fpa);
	}

	/*
	 * side color
	 */
	mvwmlib_copy_color(dpy, &ST_SIDE_COLOR(destms),
	    &ST_SIDE_COLOR(origms), ST_HAS_SIDE_COLOR(destms),
...
CMD_CopyMenuStyle(F_CMD_ARGS)
{
	...
	if (origname == NULL) {
		mvwm_msg(ERR, "CopyMenuStyle", "need two arguments");
		return;
	}

	origms = menustyle_find(origname);
	if (!origms) {
		mvwm_msg(ERR, "CopyMenuStyle", "%s: no such menu style",
		    origname);
		return;
	}
	...
	if (action && *action) {
		mvwm_msg(ERR, "CopyMenuStyle", "too many arguments");
		return;
	}
	...
	if (ST_USAGE_COUNT(destms) != 0) {
		mvwm_msg(ERR, "CopyMenuStyle", "menu style %s is in use",
		    destname);
		return;
	}
