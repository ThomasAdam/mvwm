...
SetupICCCM2(Bool replace_wm)
{
	...
	_XA_WM_COLORMAP_NOTIFY =
	    XInternAtom(dpy, "WM_COLORMAP_NOTIFY", False);
...
convertProperty(Window w, Atom target, Atom property)
{
	if (target == _XA_TARGETS) {
		XChangeProperty(dpy, w, property, XA_ATOM, 32,
		...
		XChangeProperty(dpy, w, property, XA_INTEGER, 32,
		    PropModeReplace, (unsigned char *) &local_managing_since,
		    1);
