...
/* ---------------------------- local types -------------------------------- */
typedef enum
{ FakeMouseEvent, FakeKeyEvent } FakeEventType;
...
char           *ModulePath = MVWM_MODULEDIR;
		...
		if (append) {
			...
			for (i = bs_start + 1; i <= bs_end; ++i) {
				...
				DFS_FACE_TYPE(tail->next->style) =
				    SimpleButton;
				tail->next->next = NULL;
				ReadDecorFace(spec, tail->next, button,
				    False);
				if (DFS_FACE_TYPE(tail->next->style) ==
				    VectorButton
				    && DFS_FACE_TYPE((&TB_STATE(*tb)[i])->
					style) == DefaultVectorButton) {
					/*
					 * override the default vector style
					 */
...
static void
__remove_window_decors(F_CMD_ARGS, MvwmDecor * d)
{
...
ReadMultiPixmapDecor(char *s, DecorFace *df)
{
	...
	while (pm_id >= 0) {
		...
		if (pm[pm_id] || acs[pm_id].cs >= 0 ||
			...
			mvwm_msg(WARN, "ReadMultiPixmapDecor",
		...
		if (strncasecmp(token, "Colorset", 8) == 0) {
			...
			if (!GetIntegerArguments(token, NULL, &val, 1) ||
			    val < 0) {
				mvwm_msg(ERR, "ReadMultiPixmapDecor",
		...
		if (pm_id == TBMP_BUTTONS) {
			...
			if (pm[TBMP_BUTTONS]) {
				pm[TBMP_LEFT_BUTTONS] =
				    PCloneMvwmPicture(pm[TBMP_BUTTONS]);
			...
			if (pm[TBMP_BUTTONS]) {
				PDestroyMvwmPicture(dpy, pm[TBMP_BUTTONS]);
...
/*
 *
 *  DestroyMvwmDecor -- frees all memory assocated with an MvwmDecor
 *      structure, but does not free the MvwmDecor itself
 *
 */
static void
DestroyMvwmDecor(MvwmDecor * decor)
{
	int             i;
	/*
...
static void
SetLayerButtonFlag(int layer, int multi, int set, MvwmDecor * decor,
    TitleButton *tb)
{
	int             i;
	int             start = 0;
 ...
 */
static void
SetMWMButtonFlag(mwm_flags flag, int multi, int set, MvwmDecor * decor,
    TitleButton *tb)
{
	int             i;
	int             start = 0;
...
do_button_style(F_CMD_ARGS, Bool do_add)
{
	...
	if (parm == NULL || button >= NUMBER_OF_TITLE_BUTTONS || button < 0) {
		mvwm_msg(ERR, "ButtonStyle",
		    "Bad button style (1) in line %s", action);
		return;
	}
		...
		if (!do_add && strcmp(parm, "-") == 0) {
			...
			while (tok) {
				...
				if (StrEquals(tok, "Clear")) {
					...
					SetMWMButtonFlag(MWM_DECOR_MENU,
					    multi, set, decor, tb);
					...
					SetMWMButtonFlag(MWM_DECOR_SHADE,
					    multi, set, decor, tb);
				} else if (StrEquals(tok, "MWMDecorStick")) {
					SetMWMButtonFlag(MWM_DECOR_STICK,
					    multi, set, decor, tb);
			...
			if (multi) {
				...
				ReadTitleButton(prev, tb, do_add, button))) {
				break;
			}
		}
	}
	...
	for (decor = &Scr.DefaultDecor; decor != NULL; decor = decor->next) {
		for (i = 0; i < NUMBER_OF_TITLE_BUTTONS; i++) {
			decor->flags.has_changed |=
			    update_titlebutton_colorset(&(decor->buttons[i]),
			    cset);
		}
		decor->flags.has_changed |=
		    update_titlebutton_colorset(&(decor->titlebar), cset);
		decor->flags.has_changed |=
		    update_decorface_colorset(&(decor->BorderStyle.active),
		    cset);
		decor->flags.has_changed |=
...
static Bool
__parse_vector_line(char **ret_action, int *px, int *py, int *pxoff,
    int *pyoff, int *pc, char *action)
{
	Bool            is_valid = True;
	int             offset;
	int             n;
...
refresh_window(Window w, Bool window_update)
{
	...
	w = XCreateWindow(dpy, w, 0, 0, m->coord.w, m->coord.h, 0,
...
ApplyDefaultFontAndColors(void)
{
	...
	if (Scr.StdGC) {
		...
		Scr.StdGC = mvwmlib_XCreateGC(dpy, Scr.NoFocusWin, gcm, &gcv);
	}
	...
	if (Scr.StdReliefGC) {
		...
		Scr.StdReliefGC =
		    mvwmlib_XCreateGC(dpy, Scr.NoFocusWin, gcm, &gcv);
	}
	if (cset >= 0) {
...
FreeDecorFace(Display *dpy, DecorFace *df)
{
	...
	case ShrunkPixmapButton:
		if (df->u.p) {
			PDestroyMvwmPicture(dpy, df->u.p);
		}
		break;
...
ReadDecorFace(char *s, DecorFace *df, int button, int verbose)
{
	...
	if (sscanf(s, "%255s%n", style, &offset) < 1) {
		if (verbose) {
			mvwm_msg(ERR, "ReadDecorFace", "error in face `%s'",
			    s);
		}
		return False;
	}
	...
	if (strncasecmp(style, "--", 2) != 0) {
		...
		if (strncasecmp(style, "Simple", 6) == 0) {
			...
			fpa.mask = (Pdepth <= 8) ? FPAM_DITHER : 0;	/* ? */
			df->u.p =
			    PCacheMvwmPicture(dpy, Scr.NoFocusWin, NULL, file,
			    fpa);
			if (df->u.p == NULL) {
				if (file) {
					if (verbose) {
...
/*
 *
 * Diverts a style definition to an MvwmDecor structure (veliaa@rpi.edu)
 *
 */
void
AddToDecor(F_CMD_ARGS, MvwmDecor * decor)
{
	if (!action) {
		return;
	}
...
/*
 *
 *  InitMvwmDecor -- initializes an MvwmDecor structure to defaults
 *
 */
void
InitMvwmDecor(MvwmDecor * decor)
{
	int             i;
	DecorFace       tmpdf;
	 ...
	 */
	memset(decor, 0, sizeof(MvwmDecor));
	memset(&tmpdf, 0, sizeof(DecorFace));
...
CMD_CursorMove(F_CMD_ARGS)
{
	...
	if (GetTwoArguments(action, &val1, &val2, &val1_unit,
		&val2_unit) != 2) {
		mvwm_msg(ERR, "movecursor", "CursorMove needs 2 arguments");
		return;
	}
	...
	if (m->virtual_scr.EdgeScrollY == 0 && (y >= m->coord.h ||
		y + y_unit >= m->coord.h))
		return;
...
CMD_Exec(F_CMD_ARGS)
{
	...
	if (!(fork())) {	/* child process */
		...
		if (mvwm_setpgrp() == -1) {
			mvwm_msg(ERR, "exec_function", "setpgrp failed (%s)",
			    strerror(errno));
			exit(100);
		}
		if (execl(exec_shell_name, exec_shell_name, "-c", cmd,
			NULL) == -1) {
			mvwm_msg(ERR, "exec_function", "execl failed (%s)",
			    strerror(errno));
			exit(100);
		}
	}
...
CMD_Wait(F_CMD_ARGS)
{
	...
	while (!done && !isTerminated) {
		...
		if (BUSY_WAIT & Scr.BusyCursor) {
			XDefineCursor(dpy, Scr.Root,
			    Scr.MvwmCursors[CRS_WAIT]);
			redefine_cursor = True;
		}
		if (My_XNextEvent(dpy, &e)) {
			dispatch_event(&e);
			if (XFindContext(dpy, e.xmap.window, MvwmContext,
				(caddr_t *) & t) == XCNOENT) {
				t = NULL;
			}
...
CMD_PrintInfo(F_CMD_ARGS)
{
	...
	if (StrEquals(subject, "Colors")) {
		...
		mvwm_msg(ERR, "PrintInfo", "Unknown subject '%s'", action);
	}
	if (subject) {
		free(subject);
	}
...
{
	mvwm_msg(ERR, "iconPath_function",
...
{
	mvwm_msg(ERR, "pixmapPath_function",
...
void
CMD_DefaultColorset(F_CMD_ARGS)
{
	int             cset;
...
CMD_DefaultFont(F_CMD_ARGS)
{
	...
	if (!(new_font = FlocaleLoadFont(dpy, font, "mvwm"))) {
...
/*
 *
 * Changes the window's MvwmDecor pointer (veliaa@rpi.edu)
 *
 */
void
...
/*
 *
 * Destroys an MvwmDecor (veliaa@rpi.edu)
 *
 */
void
...
{
	char           *item;
	MvwmDecor      *decor = Scr.DefaultDecor.next;
	MvwmDecor      *prev = &Scr.DefaultDecor, *found = NULL;
...
{
	MvwmWindow     *fw2;
	MvwmDecor      *decor, *found = NULL;
	MvwmWindow     *hilight = Scr.Hilite;
	char           *item = NULL;
	...
	for (fw2 = Scr.MvwmRoot.next; fw2; fw2 = fw2->next) {
		/*
	...
	border_draw_decorations(hilight, PART_ALL, True, True, CLEAR_ALL,
	    NULL, NULL);
}
...
CMD_BugOpts(F_CMD_ARGS)
{
	...
	/*
	 * mvwm_msg(DBG,"SetGlobalOptions","init action == '%s'\n",action);
	 */
	while (action && *action && *action != '\n') {
		...
		toggle =
		    ParseToggleArgument(SkipNTokens(optstring, 1), NULL, 2,
		    False);
		...
		if (StrEquals(opt, "FlickeringMoveWorkaround")) {
			...
			switch (toggle) {
			case -1:
				bo.do_enable_flickering_qt_dialogs_workaround
				    ^= 1;
				break;
			case 0:
			case 1:
			...
			case 0:
			case 1:
				bo.do_enable_qt_drag_n_drop_workaround =
				    toggle;
				break;
			default:
...
CMD_Emulate(F_CMD_ARGS)
{
	...
	if (!style || StrEquals(style, "mvwm")) {
		...
		mvwm_msg(ERR, "Emulate", "Unknown style '%s'", style);
		return;
	}
	scr_flags.do_need_window_update = 1;
	scr_flags.has_default_font_changed = 1;
...
{
	mvwm_msg(WARN, "ColorLimit", "ColorLimit is obsolete,\n\tuse the "
	    "mvwm -color-limit option");
...
CMD_SetAnimation(F_CMD_ARGS)
{
	...
	if (!opt || sscanf(opt, "%d", &delay) != 1) {
		mvwm_msg(ERR, "SetAnimation",
		    "Improper milli-second delay as first argument");
		return;
	}
	if (delay > 500) {
		mvwm_msg(WARN, "SetAnimation",
...
static Bool
FKeysymToKeycode(Display *dpy, KeySym keysym,
    unsigned int *keycode, unsigned int *modifiers)
{
	...
	*modifiers = 0;
	...
	for (m = 0; m <= 8; ++m) {
		KeySym          ks =
		    mvwm_KeycodeToKeysym(dpy, *keycode, m, 0);
		if (ks == keysym) {
			switch (m) {
			case 0:	/* No modifiers */
				break;
...
__fake_event(F_CMD_ARGS, FakeEventType type)
{
	...
	while (token && action) {
		int             index =
		    GetTokenIndex(token, optlist, 0, NULL);
		int             val, depth;
		XEvent          e;
		Window          w;
		Window          child_w;
		...
		case 3:
			...
			if (type == FakeMouseEvent) {
				if ((GetIntegerArguments(action, &action,
					    &val, 1) != 1) || val < 1
				    || val >
				    NUMBER_OF_EXTENDED_MOUSE_BUTTONS) {
					mvwm_msg(ERR, "__fake_event",
				...
				/*
				 * Do *NOT* use MvwmStringToKeysym() as it is
				 * * case insensitive.
				 */
			...
			for (depth = 1;
			    depth != maxdepth &&
			    w != child_w && child_w != None; depth++) {
				w = child_w;
				if (FQueryPointer(dpy, w, &root, &child_w,
					&rx, &ry, &x, &y,
					&JunkMask) == False) {
					/*
...
CMD_StrokeFunc(F_CMD_ARGS)
{
	...
	for (action = GetNextSimpleOption(action, &opt); opt;
		...
		if (StrEquals("NotStayPressed", opt)) {
			...
			mvwm_msg(WARN, "StrokeFunc", "Unknown option %s",
			    opt);
		}
		if (opt) {
			free(opt);
	...
	if (echo_sequence) {
		...
		num_seq[i++] = '\0';
		mvwm_msg(INFO, "StrokeFunc", "stroke sequence: %s (N%s)",
		    sequence, num_seq);
	}
	if (abort) {
		return;
...
CMD_State(F_CMD_ARGS)
{
	...
	if (state < 0 || state > 31) {
		mvwm_msg(ERR, "CMD_State", "Illegal state %d\n", state);
		return;
	}
