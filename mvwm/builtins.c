...
static void __remove_window_decors(F_CMD_ARGS, MvwmDecor *d)
{
...
static void do_title_style(F_CMD_ARGS, Bool do_add)
{
	char *parm;
	char *prev;
	MvwmDecor *decor = Scr.cur_decor ? Scr.cur_decor : &Scr.DefaultDecor;
...
static char *ReadMultiPixmapDecor(char *s, DecorFace *df)
{
	...
	df->u.mp.pixmaps = pm = xcalloc(TBMP_NUM_PIXMAPS, sizeof(MvwmPicture*));
	...
	while (pm_id >= 0)
	{
		...
		if (pm_id == TBMP_BUTTONS)
		{
			...
			{
				PDestroyMvwmPicture(dpy, pm[TBMP_LEFT_BUTTONS]);
			}
			if (pm[TBMP_RIGHT_BUTTONS])
			{
				PDestroyMvwmPicture(dpy, pm[TBMP_RIGHT_BUTTONS]);
			}
 ...
 *
 */
static void DestroyMvwmDecor(MvwmDecor *decor)
{
	int i;
...
static void SetLayerButtonFlag(
	int layer, int multi, int set, MvwmDecor *decor, TitleButton *tb)
{
	int i;
	int start = 0;
	int add = 2;
...
static void SetMWMButtonFlag(
	mwm_flags flag, int multi, int set, MvwmDecor *decor, TitleButton *tb)
{
	int i;
	int start = 0;
	int add = 2;
...
void refresh_window(Window w, Bool window_update)
{
	...
	w = XCreateWindow(
		dpy, w, 0, 0, m->coord.w, m->coord.h, 0,
...
void ApplyDefaultFontAndColors(void)
{
	...
	{
		Scr.StdReliefGC = mvwmlib_XCreateGC(
	...
	{
		Scr.StdShadowGC = mvwmlib_XCreateGC(
			dpy, Scr.NoFocusWin, gcm, &gcv);
	}
...
Bool ReadDecorFace(char *s, DecorFace *df, int button, int verbose)
{
	...
	if (sscanf(s, "%255s%n", style, &offset) < 1)
	{
		if (verbose)
		{
			mvwm_msg(ERR, "ReadDecorFace", "error in face `%s'", s);
		}
		return False;
	}
	style[255] = 0;
 ...
 *
 */
void AddToDecor(F_CMD_ARGS, MvwmDecor *decor)
{
	if (!action)
	{
		return;
...
void CMD_CursorMove(F_CMD_ARGS)
{
	...
	FWarpPointerUpdateEvpos(
		exc->x.elast, dpy, None, Scr.Root, 0, 0, m->coord.w,
		m->coord.h, x, y);
...
void CMD_Wait(F_CMD_ARGS)
{
	...
	while (!done && !isTerminated)
	{
		...
		{
			XDefineCursor(dpy, Scr.Root, Scr.MvwmCursors[CRS_WAIT]);
	...
	unsigned int *keycode, unsigned int *modifiers)
{
	...
	*modifiers = 0;
	...
	for (m = 0; m <= 8; ++m)
	{
		KeySym ks = mvwm_KeycodeToKeysym(dpy, *keycode, m, 0);
		if (ks == keysym)
		{
			switch (m)
			{
...
void CMD_StrokeFunc(F_CMD_ARGS)
{
	     ...
	     action = GetNextSimpleOption(action, &opt))
	{
		...
		{
			mvwm_msg(WARN,"StrokeFunc","Unknown option %s", opt);
		}
		if (opt)
		{
			free(opt);
