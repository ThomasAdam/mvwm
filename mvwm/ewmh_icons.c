...
ewmh_WMIcon(EWMH_CMD_ARGS)
{
	...
	if (ev != NULL && HAS_EWMH_WM_ICON_HINT(fw) == EWMH_MVWM_ICON) {
		/*
		 * this event has been produced by mvwm itself
		 */
		return 0;
	}
	...
	if (list == NULL || HAS_EWMH_WM_ICON_HINT(fw) != EWMH_TRUE_ICON) {
		...
		if (DO_EWMH_DONATE_ICON(fw) &&
		    (new_list =
			ewmh_SetWmIconFromPixmap(fw, list, &size,
			    False)) != NULL) {
			SET_HAS_EWMH_WM_ICON_HINT(fw, EWMH_MVWM_ICON);
		}
 ...
 */
void
EWMH_DoUpdateWmIcon(MvwmWindow *fw, Bool mini_icon, Bool icon)
{
	CARD32         *list = NULL;
	CARD32         *new_list = NULL;
	CARD32         *dummy = NULL;
 ...
 */
CARD32         *
ewmh_SetWmIconFromPixmap(MvwmWindow *fw, CARD32 *orig_icon, int *orig_size,
    Bool is_mini_icon)
{
	...
	*orig_size = 0;
	...
	image =
	    XGetImage(dpy, pixmap, 0, 0, width, height, AllPlanes, ZPixmap);
	if (image == NULL) {
		mvwm_msg(ERR, "EWMH_SetWmIconFromPixmap",
	...
	*orig_size = (height * width + 2 + keep_length) * sizeof(CARD32);
	new_icon = mvwm_malloc(*orig_size);
	if (keep_length > 0) {
	...
	case 1:
		{
			...
			fg = 0xff000000 +
			    (((colors[0].red >> 8) & 0xff) << 16) +
			    (((colors[0].green >> 8) & 0xff) << 8) +
			    ((colors[0].blue >> 8) & 0xff);
			bg = 0xff000000 +
			    (((colors[1].red >> 8) & 0xff) << 16) +
	...
	default:	/* depth = Pdepth */
		{
			...
			colors = mvwm_malloc(width * height * sizeof(XColor));
			cm = mvwm_malloc(width * height * sizeof(char));
			for (j = 0; j < height; j++) {
				for (i = 0; i < width; i++) {
					if (m_image != NULL &&
					    (XGetPixel(m_image, i, j) == 0)) {
 ...
 */
void
EWMH_DeleteWmIcon(MvwmWindow *fw, Bool mini_icon, Bool icon)
{
	CARD32         *list;
	CARD32         *new_list = NULL;
	int             keep_start = 0, keep_length = 0;
	...
	list =
	    ewmh_AtomGetByName(FW_W(fw), "_NET_WM_ICON",
	...
	if (FMiniIconsSupported && list != NULL) {
		...
		int             k_height =
		    (mini_icon) ? fw->ewmh_icon_height : fw->
	...
	if (keep_length > 0) {
		new_list = mvwm_malloc(keep_length * sizeof(CARD32));
...
int
EWMH_SetIconFromWMIcon(MvwmWindow *fw, CARD32 *list, int size,
    Bool is_mini_icon)
{
	...
	if (!PImageCreatePixmapFromArgbData(dpy, Scr.NoFocusWin, list, start,
		...
		mvwm_msg(ERR, "EWMH_SetIconFromWMIcon",
	...
	if (FMiniIconsSupported && is_mini_icon &&
		...
		fw->mini_icon =
		    PCacheMvwmPictureFromPixmap(dpy, Scr.NoFocusWin, name,
		    pixmap, mask, alpha, width, height, nalloc_pixels,
		    alloc_pixels, no_limit);
		if (fw->mini_icon != NULL) {
			fw->mini_pixmap_file = name;
			BroadcastMvwmPicture(M_MINI_ICON, FW_W(fw),
