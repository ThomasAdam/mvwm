 ...
 * via a _NET_WM_STATE Client message. mvwm must maintain these states
 * too */
ewmh_atom       ewmh_atom_wm_state[] = {
	ENTRY("_NET_WM_STATE_ABOVE", XA_ATOM,
	    ewmh_WMStateStaysOnTop),
	ENTRY("_NET_WM_STATE_BELOW", XA_ATOM,
	    ewmh_WMStateStaysOnBottom),
	ENTRY("_NET_WM_STATE_FULLSCREEN", XA_ATOM,
	    ewmh_WMStateFullScreen),
	ENTRY("_NET_WM_STATE_HIDDEN", XA_ATOM, ewmh_WMStateHidden),
	ENTRY("_NET_WM_STATE_MAXIMIZED_HORIZ", XA_ATOM,
	    ewmh_WMStateMaxHoriz),
	ENTRY("_NET_WM_STATE_MAXIMIZED_HORZ", XA_ATOM,
	    ewmh_WMStateMaxHoriz),
	...
	ENTRY("_NET_WM_STATE_SKIP_PAGER", XA_ATOM,
	    ewmh_WMStateSkipPager),
	ENTRY("_NET_WM_STATE_SKIP_TASKBAR", XA_ATOM,
	    ewmh_WMStateSkipTaskBar),
	ENTRY("_NET_WM_STATE_STAYS_ON_TOP", XA_ATOM,
	    ewmh_WMStateStaysOnTop),
...
/* EWMH_ATOM_LIST_MVWM_ROOT: root atom that should be maintained by mvwm only
 */
ewmh_atom       ewmh_atom_mvwm_root[] = {
...
/* EWMH_ATOM_LIST_MVWM_WIN: window atom that should be maintained by mvwm
 * only */
ewmh_atom       ewmh_atom_mvwm_win[] = {
...
ewmh_atom_list  atom_list[] = {
	...
	L_ENTRY(EWMH_ATOM_LIST_MVWM_ROOT, ewmh_atom_mvwm_root),
	L_ENTRY(EWMH_ATOM_LIST_MVWM_WIN, ewmh_atom_mvwm_win),
...
void           *
ewmh_AtomGetByName(Window win, const char *atom_name,
...
/*
 *  client_root: here the client is mvwm
 */
static int
check_desk(void)
{
...
/*
 *  client_win: here the client is mvwm
 */
void
...
void
EWMH_SetWMDesktop(MvwmWindow *fw)
{
	long            desk = fw->Desk;
...
/*
 *  mvwm must maintain the _NET_WM_STATE
 */

void
EWMH_SetWMState(MvwmWindow *fw, Bool do_restore)
{
...
set_kde_sys_tray(void)
{
	...
	ewmh_ChangeProperty(Scr.Root, "_KDE_NET_SYSTEM_TRAY_WINDOWS",
	    EWMH_ATOM_LIST_MVWM_ROOT, (unsigned char *) wins, i);
	if (wins != NULL) {
		free(wins);
	}
...
void
ewmh_AddToKdeSysTray(MvwmWindow *fw)
{
	int             size = 0;
	CARD32         *val;
	KstItem        *t;
	...
	val =
	    ewmh_AtomGetByName(FW_W(fw), "_KDE_NET_WM_SYSTEM_TRAY_WINDOW_FOR",
...
EWMH_SetClientList(struct monitor *m)
{
	...
	ewmh_ChangeProperty(Scr.Root, "_NET_CLIENT_LIST",
	    EWMH_ATOM_LIST_MVWM_ROOT, (unsigned char *) wl, nbr);
	if (wl != NULL) {
		free(wl);
	}
...
ewmh_SetWorkArea(void)
{
	...
	TAILQ_FOREACH(m, &monitor_q, entry) {
		...
		ewmh_ChangeProperty(Scr.Root, "_NET_WORKAREA",
		    EWMH_ATOM_LIST_MVWM_ROOT, (unsigned char *) &val, i * 4);
	}
...
void
EWMH_GetWorkAreaIntersection(MvwmWindow *fw, int *x, int *y, int *w, int *h,
    int type)
{
...
static float
get_intersection(int x11, int y11, int x12, int y12, int x21, int y21,
    int x22, int y22, Bool use_percent)
{
	float           ret = 0;
	int             xl, xr, yt, yb;
...
static float
ewmh_GetStrutIntersection(struct monitor *m,
    ...
    int left, int right, int top, int bottom, Bool use_percent)
{
	...
	y22 = m->coord.h;
	ret +=
	    get_intersection(x11, y11, x12, y12, x21, y21, x22, y22,
	    use_percent);
	/*
	 * right
	 */
...
float
EWMH_GetBaseStrutIntersection(struct monitor *m,
 ...
 */
void
EWMH_SetFrameStrut(MvwmWindow *fw)
{
	long            val[4];
	size_borders    b;
	...
	ewmh_ChangeProperty(FW_W(fw), "_KDE_NET_WM_FRAME_STRUT",
	    EWMH_ATOM_LIST_MVWM_WIN, (unsigned char *) &val, 4);
...
{
	return is_function_allowed(F_CLOSE, NULL, fw, RQORIG_PROGRAM_US,
	    False);
}
...
ewmh_AllowsFullScreen(EWMH_CMD_ARGS)
{
	if (!is_function_allowed(F_MAXIMIZE, NULL, fw, RQORIG_PROGRAM_US,
		False)
	    || !is_function_allowed(F_MOVE, NULL, fw, RQORIG_PROGRAM_US,
		False)
...
{
	return is_function_allowed(F_MOVE, NULL, fw, RQORIG_PROGRAM_US,
	    False);
}
...
void
EWMH_SetAllowedActions(MvwmWindow *fw)
{
	...
	if (i > 0) {
		ewmh_ChangeProperty(FW_W(fw), "_NET_WM_ALLOWED_ACTIONS",
		    EWMH_ATOM_LIST_MVWM_WIN, (unsigned char *) wm_actions, i);
	} else {
		ewmh_DeleteProperty(FW_W(fw), "_NET_WM_ALLOWED_ACTIONS",
		    EWMH_ATOM_LIST_MVWM_WIN);
	}
...
int
ewmh_HandleDesktop(EWMH_CMD_ARGS)
{
	if (Scr.EwmhDesktop != NULL && FW_W(Scr.EwmhDesktop) != FW_W(fw)) {
		mvwm_msg(WARN, "ewmh_HandleDesktop",
...
int
ewmh_HandleDialog(EWMH_CMD_ARGS)
{
...
int
ewmh_HandleNormal(EWMH_CMD_ARGS)
{
...
int
ewmh_HandleToolBar(EWMH_CMD_ARGS)
{
...
int
ewmh_HandleNotification(EWMH_CMD_ARGS)
{
	/*
	 * fw->ewmh_window_type is generally used by MvwmIdent, but for this
	 * * EWMH type it is not used.  Reporting on unmanaged windows with
	 * * MvwmIdent won't work anyway as the click to the window is eaten.
...
void
ewmh_HandleWindowType(MvwmWindow *fw, window_style *style)
{
	CARD32         *val;
	unsigned int    nitems;
 ...
 */
static int
ksmserver_workarround(MvwmWindow *fw)
{
...
void
EWMH_GetStyle(MvwmWindow *fw, window_style *style)
{
...
static void
ewmh_check_wm_pid(MvwmWindow *fw)
{
	int             size = 0;
	CARD32         *val;
	...
	fw->ewmh_window_type = 0;
	val =
	    ewmh_AtomGetByName(FW_W(fw), "_NET_WM_PID",
	    EWMH_ATOM_LIST_FIXED_PROPERTY, &size);
	if (val != NULL) {
		free(val);
...
void
EWMH_RestoreInitialStates(MvwmWindow *fw, int event_type)
{
	...
	if (HAS_EWMH_WM_ICON_HINT(fw) == EWMH_MVWM_ICON) {
 ...
 * sens) */
void
EWMH_DestroyWindow(MvwmWindow *fw)
{
	if (IS_EWMH_DESKTOP(FW_W(fw))) {
...
set_net_supported(int l)
{
	...
	ewmh_ChangeProperty(Scr.Root, "_NET_SUPPORTED",
	    EWMH_ATOM_LIST_MVWM_ROOT, (unsigned char *) supported, k);
	free(supported);
...
{
	ewmh_ChangeProperty(Scr.Root, "_KDE_NET_SYSTEM_TRAY_WINDOWS",
	    EWMH_ATOM_LIST_MVWM_ROOT, NULL, 0);
...
EWMH_Init(struct monitor *m)
{
	...
	ewmh_ChangeProperty(Scr.Root, "_NET_SUPPORTING_WM_CHECK",
	    EWMH_ATOM_LIST_MVWM_ROOT, (unsigned char *) &val, 1);
	ewmh_ChangeProperty(Scr.NoFocusWin, "_NET_SUPPORTING_WM_CHECK",
	    EWMH_ATOM_LIST_MVWM_ROOT, (unsigned char *) &val, 1);

	names[0] = "mvwm";
	classhints.res_name = "mvwm";
	classhints.res_class = "MVWM";
...
EWMH_fullscreen(MvwmWindow *fw)
{
	...
	if (IS_ICONIFIED(fw)) {
		...
		execute_function_override_window(NULL, NULL, "Iconify off", 0,
		    fw);
	}
	if (IS_SHADED(fw)) {
