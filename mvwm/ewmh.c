...
void EWMH_SetClientList(struct monitor *m)
{
	...
	ewmh_ChangeProperty(
		Scr.Root,"_NET_CLIENT_LIST", EWMH_ATOM_LIST_MVWM_ROOT,
...
void EWMH_SetClientListStacking(struct monitor *m)
{
	...
	for (
		fw = Scr.MvwmRoot.stack_next; fw != &Scr.MvwmRoot;
		fw = fw->stack_next)
	{
		nbr++;
	}
	...
	ewmh_ChangeProperty(
		Scr.Root,"_NET_CLIENT_LIST_STACKING", EWMH_ATOM_LIST_MVWM_ROOT,
		(unsigned char *)wl, nbr);
	if (wl != NULL)
	{
		free (wl);
...
void EWMH_SetFrameStrut(MvwmWindow *fw)
{
	...
	ewmh_ChangeProperty(
		FW_W(fw), "_KDE_NET_WM_FRAME_STRUT", EWMH_ATOM_LIST_MVWM_WIN,
		(unsigned char *)&val, 4);
	ewmh_ChangeProperty(
		FW_W(fw), "_NET_FRAME_EXTENTS", EWMH_ATOM_LIST_MVWM_WIN,
		(unsigned char *)&val, 4);
...
void EWMH_SetAllowedActions(MvwmWindow *fw)
{
	...
	if (i > 0)
	{
		ewmh_ChangeProperty(
			...
			EWMH_ATOM_LIST_MVWM_WIN, (unsigned char *)wm_actions,
			i);
	}
	else
	{
 ...
 */
static
int ksmserver_workarround(MvwmWindow *fw)
{
...
static void ewmh_check_wm_pid(MvwmWindow *fw)
{
	int size = 0;
	CARD32 *val;
...
void set_net_supported(int l)
{
	...
	ewmh_ChangeProperty(
		Scr.Root, "_NET_SUPPORTED", EWMH_ATOM_LIST_MVWM_ROOT,
