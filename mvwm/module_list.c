...
static inline fmodule *module_list_remove(fmodule *module,
    fmodule_list * list);
...
{
	int             mvwm_to_app[2], app_to_mvwm[2];
	int             i, val, nargs = 0;
	char           *cptr = NULL;
	char          **args = NULL;
	char           *arg1 = NULL;
	...
	if (do_listen_only == True) {
		mvwm_to_app[0] = -1;
		mvwm_to_app[1] = -1;
	} else if (pipe(mvwm_to_app) != 0) {
		mvwm_msg(ERR, "executeModule", "Failed to open pipe");
		goto err_exit;
	}
	if (pipe(app_to_mvwm) != 0) {
		mvwm_msg(ERR, "executeModule", "Failed to open pipe2");
		goto err_exit;
	}
	if (mvwm_to_app[0] >= mvwmlib_max_fd ||
	    mvwm_to_app[1] >= mvwmlib_max_fd ||
	    app_to_mvwm[0] >= mvwmlib_max_fd ||
	    app_to_mvwm[1] >= mvwmlib_max_fd) {
		mvwm_msg(ERR, "executeModule", "too many open fds");
		goto err_exit;
	}
	...
	free(cptr);
	sprintf(arg2, "%d", app_to_mvwm[1]);
	sprintf(arg3, "%d", mvwm_to_app[0]);
	...
	for (nargs = 6; action = GetNextToken(action, &token), token; nargs++) {
		...
		if (MOD_ALIAS(module) == NULL) {
			const char     *ptr =
			    skipModuleAliasToken(args[nargs]);
	...
	if (val > 0) {
		/*
		 * This fork remains running mvwm
		 */
		/*
		 * close appropriate descriptors from each pipe so
		 * * that mvwm will be able to tell when the app dies
		 */
		close(app_to_mvwm[1]);
		/*
		...
		/*
		 * add these pipes to mvwm's active pipe list
		 */
		MOD_WRITEFD(module) = mvwm_to_app[1];
		MOD_READFD(module) = app_to_mvwm[0];
		msg_mask_set(&MOD_PIPEMASK(module), DEFAULT_MASK,
		    DEFAULT_MASK);
		free(arg1);
		if (DoingCommandLine) {
			/*
		...
		if (!Pdefault) {
			...
			sprintf(visualid, "MVWM_VISUALID=%lx",
			    XVisualIDFromVisual(Pvisual));
			flib_putenv("MVWM_VISUALID", visualid);
			sprintf(colormap, "MVWM_COLORMAP=%lx", Pcmap);
			flib_putenv("MVWM_COLORMAP", colormap);
		} else {
			flib_unsetenv("MVWM_VISUALID");
			flib_unsetenv("MVWM_COLORMAP");
		}
		...
		execvp(arg1, args);
		mvwm_msg(ERR, "executeModule",
		    "Execution of module failed: %s", arg1);
		perror("");
		close(app_to_mvwm[1]);
		/*
		 * dont't care that this may be -1
		...
		mvwm_msg(ERR, "executeModule", "Fork failed");
		free(arg1);
		for (i = 6; i < nargs; i++) {
			if (args[i] != 0) {
				free(args[i]);
...
/* This used to be marked "mvwm_inline".  I removed this
...
PositiveWrite(fmodule *module, unsigned long *ptr, int size)
{
	...
	/*
	 * a dirty hack to prevent MvwmAnimate triggering during Recapture
	 */
	/*
	 ...
	 */
	{
		...
		c = (mqueue_object_type *) malloc(sizeof(mqueue_object_type) +
		    size);
		if (c == NULL) {
			mvwm_msg(ERR, "PositiveWrite", "malloc failed\n");
			exit(1);
		}
		c->size = size;
		c->done = 0;
	...
	/*
	 * dje, from afterstep, for MvwmAnimate, allows modules to sync with
	 * * mvwm. this is disabled when the server is grabbed, otherwise
	 * * deadlocks happen. M_LOCKONSEND has been replaced by a separated
	 * * mask which defines on which messages the mvwm-to-module
	 ...
	 * * M_DESTROY_WINDOW handling in MvwmEvent.
	 */
	/*
	...
	if (IS_MESSAGE_IN_MASK(&(MOD_SYNCMASK(module)), ptr[1])
		...
		while (!done) {
			...
			do {
				 ...
				 * * descriptor, with a timeout (mvwmSelect <= 0)
				 * * or read() returning wrong size is bad news
				 */
				rc = mvwmSelect(channel + 1, &readSet, NULL,
				    NULL, &timeout);
				/*
			 ...
			 * * MvwmIconMan can tell MvwmAnimate to do an animation
...
int
module_list_len(fmodule_list * list)
{
	int             count = 0;
...
FlushMessageQueue(fmodule *module)
{
	...
	while (fqueue_get_first(&(MOD_PIPEQUEUE(module)),
		(void **) &obj) == 1) {
		dptr = (char *) obj->data;
		while (obj->done < obj->size) {
			 ...
			 */
				...
				if (!FD_ISSET(channel, &writeSet)) {
					 ...
					 * * rid of the offender!
					 */
					mvwm_msg(ERR, "FlushMessageQueue",
...
CMD_ModuleSynchronous(F_CMD_ARGS)
{
	...
	if (token && StrEquals(token, "timeout")) {
		if (GetIntegerArguments(next, &next, &sec, 1) > 0 && sec > 0) {
			...
			mvwm_msg(ERR, "executeModuleSync", "illegal timeout");
			return;
		}
	}
	...
	while (!done) {
		...
		do {
			...
			timeout.tv_usec = 1;
			num_fd =
			    mvwmSelect(mvwmlib_max_fd, &in_fdset, &out_fdset,
			    0, &timeout);
...
CMD_set_nograb_mask(F_CMD_ARGS)
{
	...
	set_message_mask(&(MOD_NOGRABMASK(exc->m.module)),
	    (unsigned long) val);
