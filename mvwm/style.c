...
static int fw_match_style_id(MvwmWindow *fw, style_id_t s_id)
{
		...
		if (do_free_src_and_alloc_copy)
		{
			...
			SSET_MINI_ICON_NAME(
				*merged_style,
				...
				mvwm_strdup(SGET_MINI_ICON_NAME(*add_style)) :
				NULL);
		}
		else
		{
		...
		if (do_free_src_and_alloc_copy)
		{
			...
			SSET_DECOR_NAME(
				*merged_style, (SGET_DECOR_NAME(*add_style)) ?
				mvwm_strdup(SGET_DECOR_NAME(*add_style)) :
				NULL);
		}
		else
		{
		...
		if (do_free_src_and_alloc_copy)
		{
			...
			SSET_WINDOW_FONT(
				*merged_style, (SGET_WINDOW_FONT(*add_style)) ?
				mvwm_strdup(SGET_WINDOW_FONT(*add_style)) :
				NULL);
		}
		else
		{
		...
		if (do_free_src_and_alloc_copy)
		{
			...
			SSET_FORE_COLOR_NAME(
				*merged_style,
				...
				mvwm_strdup(SGET_FORE_COLOR_NAME(*add_style)) :
				NULL);
		}
		else
		{
		...
		if (do_free_src_and_alloc_copy)
		{
			...
			SSET_BACK_COLOR_NAME(
				*merged_style,
				...
				mvwm_strdup(SGET_BACK_COLOR_NAME(*add_style)) :
				NULL);
		}
		else
		{
 ...
 */
void lookup_style(MvwmWindow *fw, window_style *styles)
{
	window_style *nptr;
...
void check_window_style_change(
	MvwmWindow *t, update_win *flags, window_style *ret_style)
{
	int i;
	char *wf;
	char *sf;
...
void update_window_color_style(MvwmWindow *fw, window_style *pstyle)
{
...
void update_window_color_hi_style(MvwmWindow *fw, window_style *pstyle)
{
	int cs = Scr.DefaultColorset;
...
void update_icon_title_cs_style(MvwmWindow *fw, window_style *pstyle)
{
...
void update_icon_title_cs_hi_style(MvwmWindow *fw, window_style *pstyle)
{
...
void update_icon_background_cs_style(MvwmWindow *fw, window_style *pstyle)
{
