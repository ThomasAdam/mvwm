...
static int
fw_match_style_id(MvwmWindow *fw, style_id_t s_id)
{
...
Get_TBLR(char *token, unsigned char *IconFill)
{
	...
	if (StrEquals(token, "B") ||
	    StrEquals(token, "BOT") || StrEquals(token, "BOTTOM")) {
		/*
		...
		*IconFill = ICONFILLHRZ;
	    ...
	    StrEquals(token, "RGT") || StrEquals(token, "RIGHT")) {
		/*
		 * turn on right bit
		 */
		*IconFill = ICONFILLRGT;
	} else if (StrEquals(token, "L") ||
	    StrEquals(token, "LFT") || StrEquals(token, "LEFT")) {
		*IconFill = 0;
	} else {
		/*
    ...
    Bool do_free_src_and_alloc_copy)
{
	...
	if (add_style->flags.use_start_on_desk) {
		...
		SSET_START_PAGE_X(*merged_style,
		    SGET_START_PAGE_X(*add_style));
		SSET_START_PAGE_Y(*merged_style,
...
free_style_mask(window_style *style, style_flags * mask)
{
	...
	blockunmask((char *) &style->flag_default,
	    (char *) &style->flag_default, (char *) pmask,
	    sizeof(style_flags));
	blockunmask((char *) &style->change_mask,
	    (char *) &style->change_mask, (char *) pmask,
	    sizeof(style_flags));
...
style_set_old_focus_policy(window_style *ps, int policy)
{
	...
	if (!FP_DO_GRAB_FOCUS(S_FOCUS_POLICY(SCM(*ps)))) {
		FPS_GRAB_FOCUS(S_FOCUS_POLICY(SCF(*ps)),
		    FP_DO_GRAB_FOCUS(fp));
    ...
    Bool is_reversed, focus_policy_t *f, focus_policy_t *m, focus_policy_t *c)
{
	...
	case 1:
		...
		if (token != NULL && *token != 0) {
			mvwm_msg(ERR, "style_parse_focus_policy_style",
    ...
    window_style *ps)
{
	...
	if (num == 4) {
		...
		if (l > 0 && l < 24) {
			 ...
			 */
			geom_flags =
			    FScreenParseGeometryWithScreen(option,
    ...
    char *prefix, window_style *ps, icon_boxes ** cur_ib)
{
	...
	case 'a':
		if (StrEquals(token, "ACTIVEPLACEMENT")) {
			...
			ps->flag_mask.manual_placement_honors_starts_on_page =
			    1;
			ps->change_mask.
			    manual_placement_honors_starts_on_page = 1;
	...
	case 'b':
		if (StrEquals(token, "BackColor")) {
			...
			if (token) {
				...
				mvwm_msg(ERR, "style_parse_on_estyle_option",
	...
	case 'c':
		if (StrEquals(token, "CascadePlacement")) {
			...
			*val = -1;
			...
			if (token == NULL) {
				mvwm_msg(ERR, "style_parse_one_style_option",
			...
			if (strncasecmp(token, "rgb:", 4) == 0) {
				...
				free(token);
				next =
				    DoGetNextToken(rest, &token, NULL, ",/",
				    &c);
			}
			rest = next;
			...
			FPS_RAISE_FOCUSED_CLIENT_CLICK(S_FOCUS_POLICY(SCF
				(*ps)), on);
			FPS_RAISE_FOCUSED_CLIENT_CLICK(S_FOCUS_POLICY(SCM
				(*ps)), 1);
			FPS_RAISE_FOCUSED_CLIENT_CLICK(S_FOCUS_POLICY(SCC
				(*ps)), 1);
			FPS_RAISE_UNFOCUSED_CLIENT_CLICK(S_FOCUS_POLICY(SCF
				(*ps)), on);
			FPS_RAISE_UNFOCUSED_CLIENT_CLICK(S_FOCUS_POLICY(SCM
				(*ps)), 1);
			FPS_RAISE_UNFOCUSED_CLIENT_CLICK(S_FOCUS_POLICY(SCC
				(*ps)), 1);
	...
	case 'e':
		if (StrEquals(token, "ExactWindowName")) {
			 ...
			 * * TitleFormat %n
			 */
			mvwm_msg(WARN, "style_parse_one_style_option",
	...
	case 'f':
		if (strncasecmp(token, "fp", 2) == 0) {
			 ...
			 */
			found =
			    style_parse_focus_policy_style(token + 2, rest,
			    &rest, (on) ? False : True,
	...
	case 'g':
		if (StrEquals(token, "GrabFocusOff")) {
			...
			FPS_GRAB_FOCUS_TRANSIENT(S_FOCUS_POLICY(SCF(*ps)),
			    !on);
	...
	case 'i':
		if (StrEquals(token, "Icon")) {
			...
			*val = ICON_BACKGROUND_PADDING;
			...
			SSET_ICON_BACKGROUND_PADDING(*ps,
			    (unsigned char) *val);
	...
	case 'm':
		if (StrEquals(token, "ManualPlacement")) {
			...
			if (on != 0) {
				num =
				    GetIntegerArguments(rest, &rest, val, 4);
				for (i = 0; i < num; i++) {
					if (val[i] < 0)
						bad = True;
				}
			...
			}
			{
				...
				p = SGET_PLACEMENT_PERCENTAGE_PENALTY_PTR
				    (*ps);
			...
			num =
			    GetTwoArguments(rest, &val1, &val2, &val1_unit,
			    &val2_unit);
	...
	case 's':
		if (StrEquals(token, "SMARTPLACEMENT")) {
			...
			S_SET_HAS_NO_STICKY_STIPPLED_ICON_TITLE(SCF(*ps),
			    !on);
		 ...
		 */
			...
			if (token) {
				...
				direction =
				    gravity_parse_dir_argument(token, &token,
				    DIR_NONE);
				if (direction >= 0 && direction <= DIR_MASK) {
					SSET_STARTS_SHADED_DIR(*ps,
					    direction);
				} else {
					mvwm_msg(ERR,
			...
			do {
				...
				if (GetIntegerArguments(rest, &rest, &val,
					1) != 1) {
					break;
				}
				if (val >= 0) {
...
parse_and_set_window_style(char *action, char *prefix, window_style *ps)
{
	...
	while (action && *action && *action != '\n') {
		 ...
		 */
		found =
		    style_parse_one_style_option(token, rest, &rest, prefix,
		    ps, &cur_ib);

		if (found == False) {
			mvwm_msg(ERR, "style_parse_and_set_window_style",
 ...
 *
 *  Inputs:
 *	fw     - MvwmWindow structure to match against
 *	styles - callers return area
 */
void
lookup_style(MvwmWindow *fw, window_style *styles)
{
	window_style   *nptr;
...
void
check_window_style_change(MvwmWindow *t, update_win *flags,
...
void
update_window_color_style(MvwmWindow *fw, window_style *pstyle)
{
...
void
update_window_color_hi_style(MvwmWindow *fw, window_style *pstyle)
{
	int             cs = Scr.DefaultColorset;
...
void
update_icon_title_cs_style(MvwmWindow *fw, window_style *pstyle)
{
...
void
update_icon_title_cs_hi_style(MvwmWindow *fw, window_style *pstyle)
{
...
void
update_icon_background_cs_style(MvwmWindow *fw, window_style *pstyle)
{
...
print_styles(int verbose)
{
	...
	fprintf(stderr, "Info on mvwm Styles:\n");
	if (verbose) {
	...
	for (nptr = all_styles; nptr != NULL; nptr = SGET_NEXT_STYLE(*nptr)) {
		...
		if (SGET_BACK_COLOR_NAME_HI(*nptr)) {
			...
			if (verbose > 1) {
				fprintf(stderr, "        Back Color hi: %s\n",
		...
		if (SGET_FORE_COLOR_NAME_HI(*nptr)) {
			...
			if (verbose > 1) {
				fprintf(stderr, "        Fore Color hi: %s\n",
		...
		if (SGET_MINI_ICON_NAME(*nptr)) {
			...
			if (verbose > 1) {
				fprintf(stderr, "        MiniIcon Name: %s\n",
