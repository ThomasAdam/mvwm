...
static void
init_style(MvwmWindow *old_t, MvwmWindow *t, window_style *pstyle,
    short *pbuttons)
{
	/*
	 * copy the window structure because we still need some old values.
	 */
	memcpy(old_t, t, sizeof(MvwmWindow));
	/*
...
static void
apply_window_updates(MvwmWindow *t, update_win *flags, window_style *pstyle,
    MvwmWindow *focus_w)
{
	MvwmWindow      old_t;
	short           buttons;
	...
	if (flags->do_redecorate || flags->do_update_title_dir) {
		...
		if (flags->do_redecorate) {
			 ...
			 */
			gravity_translate_to_northwest_geometry_no_bw(old_t.
			    hints.win_gravity, &old_t, &naked_g, &naked_g);
			/*
		...
		if (flags->do_update_title_dir) {
			...
			dh = b_new.total_size.height -
			    b_old.total_size.height;
		...
		if (IS_MAXIMIZED(t)) {
			if (flags->do_redecorate) {
				int             off_x =
				    old_t.g.normal.x - old_t.g.max.x;
				int             off_y =
				    old_t.g.normal.y - old_t.g.max.y;
				int             new_off_x;
				int             new_off_y;
			...
			if (flags->do_update_title_dir) {
				frame_g = t->g.max;
				gravity_resize(t->hints.win_gravity,
				    &t->g.max, dw, dh);
				gravity_constrain_size(t->hints.win_gravity,
	...
	if (flags->do_setup_frame) {
		...
		frame_force_setup_window(t, frame_g.x, frame_g.y,
		    frame_g.width, frame_g.height, True);
...
void
apply_decor_change(MvwmWindow *fw)
{
	window_style    style;
	update_win      flags;
