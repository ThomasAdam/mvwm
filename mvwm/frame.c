...
static void frame_get_titlebar_dimensions_only(
	MvwmWindow *fw, rectangle *frame_g, size_borders *bs,
...
static void frame_setup_border(
	MvwmWindow *fw, rectangle *frame_g, window_parts setup_parts,
	rectangle *diff_g)
{
	XWindowChanges xwc;
	Window w;
...
static void __frame_setup_window(
	MvwmWindow *fw, rectangle *frame_g, Bool do_send_configure_notify,
...
static Bool frame_is_parent_hidden(
	MvwmWindow *fw, rectangle *frame_g)
{
	size_borders b;
...
static void frame_set_decor_gravities(
	MvwmWindow *fw, frame_decor_gravities_type *grav,
...
static void frame_restore_client_gravities(MvwmWindow *fw)
{
	XSetWindowAttributes xcwa;
	long valuemask;
...
static rectangle *frame_get_hidden_pos(
	MvwmWindow *fw, mr_args_internal *mra, Bool do_unhide,
...
static void frame_prepare_animation_shape(
	MvwmWindow *fw, mr_args_internal *mra, int parent_x, int parent_y)
{
	rectangle parent_g;
	rectangle client_g;
...
static void frame_mrs_prepare_vars(
	MvwmWindow *fw, mr_args_internal *mra)
{
	Bool dummy;
	int i;
...
static void frame_has_handles_and_tiled_border(
	MvwmWindow *fw, int *ret_has_handles, int *ret_has_tiled_border)
{
	DecorFace *df;
...
void frame_reshape_border(MvwmWindow *fw)
{
	int grav;
	int off_x = 0;
	int off_y = 0;
...
void frame_get_titlebar_dimensions(
	MvwmWindow *fw, rectangle *frame_g, rectangle *diff_g,
	frame_title_layout_t *title_layout)
{
	size_borders b;
	int i;
...
void frame_get_sidebar_geometry(
	MvwmWindow *fw, DecorFaceStyle *borderstyle, rectangle *frame_g,
...
int frame_window_id_to_context(
	MvwmWindow *fw, Window w, int *ret_num)
{
	int context = C_ROOT;
...
frame_move_resize_args frame_create_move_resize_args(
	MvwmWindow *fw, frame_move_resize_mode mr_mode,
	rectangle *start_g, rectangle *end_g, int anim_steps, int shade_dir)
{
	...
	/* set some variables */
	mra = mvwm_callow(1, sizeof(mr_args_internal));
...
void frame_free_move_resize_args(
	MvwmWindow *fw, frame_move_resize_args mr_args)
{
	mr_args_internal *mra;
...
void frame_setup_window(
	MvwmWindow *fw, int x, int y, int w, int h,
 ...
 *
 ****************************************************************************/
void frame_setup_shape(MvwmWindow *fw, int w, int h, int shape_mode)
{
	XRectangle rect;
	rectangle r;
	size_borders b;
