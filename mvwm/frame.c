...
static void
frame_get_titlebar_dimensions_only(MvwmWindow *fw, rectangle *frame_g,
...
static void
frame_setup_border(MvwmWindow *fw, rectangle *frame_g,
...
static void
frame_setup_titlebar(MvwmWindow *fw, rectangle *frame_g,
    window_parts setup_parts, rectangle *diff_g)
{
	...
	if (setup_parts & PART_TITLE) {
		XMoveResizeWindow(dpy, FW_W_TITLE(fw),
...
static void
__frame_setup_window(MvwmWindow *fw, rectangle *frame_g,
...
static Bool
frame_is_parent_hidden(MvwmWindow *fw, rectangle *frame_g)
{
	size_borders    b;
 ...
 * pixel. */
static int
frame_get_titlebar_compression(MvwmWindow *fw, rectangle *frame_g)
{
	size_borders    b;
	int             space;
	int             need_space;
    ...
    direction_t title_dir, frame_move_resize_mode rmode, rectangle *delta_g)
{
	...
	get_resize_decor_gravities_one_axis(&grav_x, title_dir_x, rmode,
	    DIR_W, DIR_E, (delta_g->x != 0));
...
static void
frame_set_decor_gravities(MvwmWindow *fw, frame_decor_gravities_type * grav,
...
static void
frame_restore_client_gravities(MvwmWindow *fw)
{
	XSetWindowAttributes xcwa;
	long            valuemask;
...
static rectangle *
frame_get_hidden_pos(MvwmWindow *fw, mr_args_internal * mra, Bool do_unhide,
...
static void
frame_update_hidden_window_pos(MvwmWindow *fw, mr_args_internal * mra,
    Bool do_unhide)
{
	rectangle      *target_g;
	rectangle       hidden_g;
...
static void
frame_prepare_animation_shape(MvwmWindow *fw, mr_args_internal * mra,
...
static void
frame_mrs_prepare_vars(MvwmWindow *fw, mr_args_internal * mra)
{
	Bool            dummy;
	int             i;
	...
	frame_get_sidebar_geometry(fw, NULL, &mra->next_g,
	    &mra->next_sidebar_g, &dummy, &dummy);
	mvwmrect_subtract_rectangles(&mra->dstep_g, &mra->next_g,
	    &mra->current_g);
	...
	mra->step_flags.do_hide_parent = ((!mra->step_flags.was_hidden
		|| mra->flags.do_force) && mra->step_flags.is_hidden);
...
frame_mrs_hide_changing_parts(mr_args_internal * mra)
{
	...
	h = mra->current_g.height;
	if (w > 0 && h > 0) {
		XMoveResizeWindow(dpy, hide_wins.w[3], mra->current_g.width -
...
static void
frame_mrs_hide_unhide_parent(MvwmWindow *fw, mr_args_internal * mra)
{
...
static void
frame_mrs_hide_unhide_parent2(MvwmWindow *fw, mr_args_internal * mra)
{
	XSetWindowAttributes xswa;
...
static void
frame_mrs_setup_draw_decorations(MvwmWindow *fw, mr_args_internal * mra)
{
...
static void
frame_mrs_resize_move_windows(MvwmWindow *fw, mr_args_internal * mra)
{
	/*
	...
	if (!mra->flags.is_shading) {
		...
		if (mra->flags.do_update_shape) {
			...
			FShapeCombineShape(dpy, FW_W_FRAME(fw),
			    FShapeBounding, 0, 0, Scr.NoFocusWin,
...
static void
frame_move_resize_step(MvwmWindow *fw, mr_args_internal * mra)
{
...
static void
frame_has_handles_and_tiled_border(MvwmWindow *fw, int *ret_has_handles,
...
static int
frame_get_shading_laziness(MvwmWindow *fw, mr_args_internal * mra)
{
	int             has_handles;
...
void
frame_reshape_border(MvwmWindow *fw)
{
	int             grav;
	int             off_x = 0;
	int             off_y = 0;
	...
	if (IS_SHADED(fw)) {
		...
		get_relative_geometry(fw->m, new_g, new_g);
...
void
frame_get_titlebar_dimensions(MvwmWindow *fw, rectangle *frame_g,
    rectangle *diff_g, frame_title_layout_t * title_layout)
{
	...
	if (nbuttons > 0 && t_length < MIN_WINDOW_TITLE_LENGTH) {
		...
		nbuttons_big =
		    nbuttons - (MIN_WINDOW_TITLE_LENGTH - t_length);
...
void
frame_get_sidebar_geometry(MvwmWindow *fw, DecorFaceStyle *borderstyle,
...
int
frame_window_id_to_context(MvwmWindow *fw, Window w, int *ret_num)
{
	int             context = C_ROOT;
...
frame_create_move_resize_args(MvwmWindow *fw, frame_move_resize_mode mr_mode,
    rectangle *start_g, rectangle *end_g, int anim_steps, int shade_dir)
{
	...
	mvwmrect_subtract_rectangles(&mra->delta_g, &mra->end_g,
	    &mra->start_g);
	...
	case FRAME_MR_SCROLL:
		whdiff =
		    max(abs(mra->delta_g.width), abs(mra->delta_g.height));
 ...
 */
void
frame_update_move_resize_args(frame_move_resize_args mr_args,
    rectangle *end_g)
{
	mr_args_internal *mra;
...
void
frame_free_move_resize_args(MvwmWindow *fw, frame_move_resize_args mr_args)
{
	mr_args_internal *mra;
...
void
frame_move_resize(MvwmWindow *fw, frame_move_resize_args mr_args)
{
	mr_args_internal *mra;
 ...
 ************************************************************************/
void
frame_setup_window(MvwmWindow *fw, int x, int y, int w, int h,
...
void
frame_setup_window_app_request(MvwmWindow *fw, int x, int y, int w, int h,
    Bool do_send_configure_notify)
{
	rectangle       g;
...
void
frame_force_setup_window(MvwmWindow *fw, int x, int y, int w, int h,
    Bool do_send_configure_notify)
{
	rectangle       g;
 ...
 ****************************************************************************/
void
frame_setup_shape(MvwmWindow *fw, int w, int h, int shape_mode)
{
	XRectangle      rect;
	rectangle       r;
	size_borders    b;
