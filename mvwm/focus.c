 ...
 */
static void __try_program_focus(Window w, const MvwmWindow *fw)
{
...
static Bool __check_allow_focus(
	Window w, MvwmWindow *fw, fpol_set_focus_by_t set_by)
{
	MvwmWindow *sf;
...
static void __update_windowlist(
	MvwmWindow *fw, fpol_set_focus_by_t set_by,
...
static Bool __try_other_screen_focus(const MvwmWindow *fw)
{
...
static void __set_focus_to_fwin(
	Window w, MvwmWindow *fw, sftfwin_args_t *args)
{
	MvwmWindow *sf;
 ...
 *
 */
static void warp_to_mvwm_window(
...
static Bool focus_query_grab_buttons(MvwmWindow *fw, Bool client_entered)
{
...
static MvwmWindow *__restore_focus_after_unmap(
...
static void __focus_grab_one_button(
	MvwmWindow *fw, int button, int grab_buttons)
{
	Bool do_grab;
...
static void __focus_grab_buttons(MvwmWindow *fw, Bool client_entered)
{
	int i;
...
void focus_grab_buttons_on_layer(int layer)
{
	...
	for (
		fw = Scr.MvwmRoot.stack_next;
		fw != &Scr.MvwmRoot && fw->layer >= layer;
		fw = fw->stack_next)
	{
		if (fw->layer == layer)
		{
