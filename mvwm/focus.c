...
/* Last window which Mvwm gave the focus to NOT the window that really has the
 * focus */
static MvwmWindow *ScreenFocus = NULL;
/* Window which had focus before the pointer moved to a different screen. */
static MvwmWindow *LastScreenFocus = NULL;
...
void
_focus_set(Window w, MvwmWindow *fw)
{
 ...
 */
static void
__try_program_focus(Window w, const MvwmWindow *fw)
{
...
static Bool
__try_forbid_user_focus(Window w, MvwmWindow *fw)
{
	if (fw == NULL ||
...
static Bool
__check_allow_focus(Window w, MvwmWindow *fw, fpol_set_focus_by_t set_by)
{
	MvwmWindow     *sf;
...
static void
__update_windowlist(MvwmWindow *fw, fpol_set_focus_by_t set_by,
    int is_focus_by_flip_focus_cmd)
{
	lastFocusType = (is_focus_by_flip_focus_cmd) ? True : False;
	if (fw == NULL || focus_is_focused(fw) || fw == &Scr.MvwmRoot ||
	...
	if (!is_focus_by_flip_focus_cmd &&
		...
		FPOL_SORT_WL_BY_OPEN || set_by == FOCUS_SET_BY_FUNCTION)) {
		/*
		 ...
		 */
		for (fw2 = &Scr.MvwmRoot; fw2 && fw2 != fw; fw2 = fw2->next) {
			/*
			 * nothing
			 */
		}
		if (fw2) {
			 ...
			 */
			fw2->next = Scr.MvwmRoot.next;
			Scr.MvwmRoot.next->prev = fw2;

			/*
			 * make fw the new start of the list
			 */
			Scr.MvwmRoot.next = fw;
			/*
		...
		if (Scr.MvwmRoot.next) {
			Scr.MvwmRoot.next->prev = fw;
		}
		Scr.MvwmRoot.next = fw;
		fw->prev = &Scr.MvwmRoot;
	}
...
static Bool
__try_other_screen_focus(const MvwmWindow *fw)
{
 ...
 */
static void
__set_focus_to_fwin(Window w, MvwmWindow *fw, sftfwin_args_t * args)
{
	MvwmWindow     *sf;
...
static void
set_focus_to_fwin(Window w, MvwmWindow *fw, sftfwin_args_t * args)
{
	MvwmWindow     *sf;
 ...
 */
static void
warp_to_mvwm_window(const exec_context_t *exc, int warp_x, int x_unit,
    int warp_y, int y_unit)
{
	...
	if (t->g.frame.x + t->g.frame.width < 0 ||
		...
		frame_setup_window(t, 0, 0, t->g.frame.width,
...
static Bool
focus_query_grab_buttons(MvwmWindow *fw, Bool client_entered)
{
...
static MvwmWindow *
__restore_focus_after_unmap(const MvwmWindow *fw,
    Bool do_skip_marked_transients)
{
	MvwmWindow     *t = NULL;
	MvwmWindow     *set_focus_to = NULL;
...
__activate_window_by_command(F_CMD_ARGS, int is_focus_by_flip_focus_cmd)
{
	...
	if (!do_not_warp) {
		...
		if (cx < 0 || cx >= m->coord.w || cy < 0 || cy >= m->coord.h) {
			int             dx;
			int             dy;
...
static void
__focus_grab_one_button(MvwmWindow *fw, int button, int grab_buttons)
{
	Bool            do_grab;
	...
	if (do_grab) {
		XGrabButton(dpy, button + 1, AnyModifier, FW_W_PARENT(fw),
...
Bool
focus_does_accept_input_focus(const MvwmWindow *fw)
{
...
Bool
focus_is_focused(const MvwmWindow *fw)
{
	return (fw && fw == ScreenFocus);
}

Bool
focus_query_click_to_raise(MvwmWindow *fw, Bool is_focused, int context)
{
	fpol_context_t *c;
...
Bool
focus_query_click_to_focus(MvwmWindow *fw, int context)
{
	fpol_context_t *c;
...
Bool
focus_query_open_grab_focus(MvwmWindow *fw, MvwmWindow *focus_win)
{
	if (fw == NULL) {
		return False;
	}
	...
	if (IS_TRANSIENT(fw) && FW_W_TRANSIENTFOR(fw) != Scr.Root) {
		...
		if (FP_DO_GRAB_FOCUS(FW_FOCUS_POLICY(fw)) &&
		    (focus_win == NULL ||
			!FP_DO_OVERRIDE_GRAB_FOCUS(FW_FOCUS_POLICY
			    (focus_win)))) {
			return True;
		}
	}
 ...
 * unmapping. */
Bool
focus_query_close_release_focus(const MvwmWindow *fw)
{
...
static void
__focus_grab_buttons(MvwmWindow *fw, Bool client_entered)
{
	int             i;
...
void
focus_grab_buttons(MvwmWindow *fw)
{
	__focus_grab_buttons(fw, False);
}

void
focus_grab_buttons_client_entered(MvwmWindow *fw)
{
...
focus_grab_buttons_on_layer(int layer)
{
	...
	for (fw = Scr.MvwmRoot.stack_next;
	    fw != &Scr.MvwmRoot && fw->layer >= layer; fw = fw->stack_next) {
		if (fw->layer == layer) {
			focus_grab_buttons(fw);
		}
	}
...
void
_SetFocusWindow(MvwmWindow *fw, Bool do_allow_force_broadcast,
...
void
_ReturnFocusWindow(MvwmWindow *fw)
{
	sftfwin_args_t  sf_args;
...
void
restore_focus_after_unmap(const MvwmWindow *fw,
    Bool do_skip_marked_transients)
{
	extern MvwmWindow *colormap_win;
	MvwmWindow     *set_focus_to = NULL;
...
focus_grab_buttons_on_pointer_window(void)
{
	...
	if (!FQueryPointer(dpy, Scr.Root, &JunkRoot, &w, &JunkX, &JunkY,
...
void
set_focus_window(MvwmWindow *fw)
{
	ScreenFocus = fw;
...
void
set_last_screen_focus_window(MvwmWindow *fw)
{
	LastScreenFocus = fw;
...
void
update_last_screen_focus_window(MvwmWindow *fw)
{
...
void
focus_force_refresh_focus(const MvwmWindow *fw)
{
...
void
refresh_focus(const MvwmWindow *fw)
{
...
CMD_WarpToWindow(F_CMD_ARGS)
{
	...
	if (exc->w.wcontext != C_UNMANAGED) {
		if (n == 2) {
			warp_to_mvwm_window(exc, val1, val1_unit, val2,
			    val2_unit);
		} else {
			warp_to_mvwm_window(exc, 0, 0, 0, 0);
		}
	} else {
		int             x = 0;
		int             y = 0;
