...
push_read_file(const char *file)
{
	if (curr_read_depth >= MAX_READ_DEPTH) {
		mvwm_msg(ERR, "Read", "Nested Read limit %d is reached",
		    MAX_READ_DEPTH);
		return 0;
	}
...
parse_filename(char *cmdname, char *action, char **filename, int *quiet_flag)
{
	...
	/*
	 * mvwm_msg(INFO,cmdname,"action == '%s'",action);
	 */
	...
	if (*filename == NULL) {
		mvwm_msg(ERR, cmdname, "missing filename parameter");
		return 0;
	}
	/*
...
run_command_file(char *filename, const exec_context_t *exc)
{
	 ...
	 * *   absolute path.
	 * *
	 * *  - Otherwise, it's assumed to be in MVWM_USERDIR OR MVWM_DATADIR,
	...
	if (full_filename[0] == '/') {
		...
		/*
		 * It's a relative path.  Check in either MVWM_USERDIR or
		 * MVWM_DATADIR.
		 *
		 */
		full_filename = CatString3(mvwm_userdir, "/", filename);

		if ((f = fopen(full_filename, "r")) == NULL) {
			full_filename =
			    CatString3(MVWM_DATADIR, "/", filename);
			f = fopen(full_filename, "r");
		}
	}
		...
		if (!read_quietly) {
			if (filename[0] == '/') {
				...
				mvwm_msg(ERR, "Read",
				    ...
				    MVWM_DATADIR, filename, mvwm_userdir);
			}
		}
		if (cond_rc != NULL) {
			cond_rc->rc = COND_RC_ERROR;
...
CMD_PipeRead(F_CMD_ARGS)
{
	...
	if (f == NULL) {
		...
		if (!read_quietly) {
			mvwm_msg(ERR, "PipeRead", "command '%s' not run",
			    command);
		}
		free(command);
		cursor_control(False);
