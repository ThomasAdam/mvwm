...
/* ---------------------------- local functions ---------------------------- */
static void
clear_menu_item_background(MenuPaintItemParameters *mpip, int x, int y, int w,
    int h)
{
		...
		case PIXMAP_STRETCH_Y:
			{
				...
				} else if (cs->pixmap_type ==
				    PIXMAP_STRETCH_X) {
					gcv.ts_x_origin = x;
					gcm |= GCTileStipXOrigin;
				}
				bgc =
				    mvwmlib_XCreateGC(dpy, mpip->w, gcm,
				    &gcv);
				XFillRectangle(dpy, mpip->w, bgc, x, y, width,
				    height);
				XFreeGC(dpy, bgc);
				break;
			}
...
menuitem_free(struct MenuItem *mi)
{
	...
	if (MI_PICTURE(mi)) {
		PDestroyMvwmPicture(dpy, MI_PICTURE(mi));
	}
	for (i = 0; i < MAX_MENU_ITEM_MINI_ICONS; i++) {
		if (MI_MINI_ICON(mi)[i]) {
			PDestroyMvwmPicture(dpy, MI_MINI_ICON(mi)[i]);
		}
	}
	free(mi);
...
menuitem_clone(struct MenuItem *mi)
{
	...
	if (MI_PICTURE(mi) != NULL) {
		MI_PICTURE(new_mi) = PCloneMvwmPicture(MI_PICTURE(mi));
	}
	for (i = 0; i < MAX_MENU_ITEM_MINI_ICONS; i++) {
		if (MI_MINI_ICON(mi)[i] != NULL) {
			MI_MINI_ICON(new_mi)[i] =
			    PCloneMvwmPicture(MI_MINI_ICON(mi)[i]);
		}
	}
...
menuitem_paint(struct MenuItem *mi, struct MenuPaintItemParameters *mpip)
{
	...
	if (is_item_selected &&
		 ...
		 * Undo the hilighting.
		 */
		x1 = min(MDIM_HILIGHT_X_OFFSET(*dim),
		    MDIM_ITEM_X_OFFSET(*dim));
		x2 = max(MDIM_HILIGHT_X_OFFSET(*dim) +
	...
	for (i = MAX_MENU_ITEM_LABELS; i-- > 0;) {
		if (!empty_inter && MI_LABEL(mi)[i] && *(MI_LABEL(mi)[i])) {
			...
			b.width = text_width =
			    FlocaleTextWidth(font, fws->str,
			    strlen(fws->str));

			if (!item_cleared && mpip->ev) {
				int             s_x, s_w;
				if (frect_get_seg_intersection(mpip->ev->
					xexpose.x, mpip->ev->xexpose.width,
					fws->x, text_width, &s_x, &s_w)) {
					b.x = s_x;
					b.width = s_w;
			...
			if (draw_string) {
				if (!item_cleared && xft_clear) {
					clear_menu_item_background(mpip, b.x,
		...
		if (draw_picture) {
			if (!item_cleared &&
				...
				(tmp_cs >= 0 &&
				    ...
				    100))) {
				clear_menu_item_background(mpip, b.x, b.y,
				    b.width, b.height);
			}
	...
	for (i = 0; i < mpip->used_mini_icons; i++) {
		...
		if (MI_MINI_ICON(mi)[i]) {
			...
			if (!item_cleared && mpip->ev) {
				if (!frect_get_intersection(mpip->ev->xexpose.
					x, mpip->ev->xexpose.y,
			...
			if (draw_picture) {
				...
				PGraphicsRenderPicture(dpy, mpip->w,
				    MI_MINI_ICON(mi)[i], &fra, mpip->w,
				    tmp_gc, Scr.MonoGC, Scr.AlphaGC,
				    b.x - MDIM_ICON_X_OFFSET(*dim)[k],
				    b.y - y, b.width, b.height, b.x, b.y,
				    b.width, b.height, False);
			}
		}
	}
