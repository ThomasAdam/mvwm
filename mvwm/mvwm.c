	...
	if (restart)
	{
		...
		if (command)
		{
			...
			else
			{
				...
				if (n == 1 && strcmp(my_argv[0], "mvwm2") == 0)
				{
					str = "mvwm2";
				}
...
static void CreateGCs(void)
{
	...
	Scr.AlphaGC = mvwmlib_XCreateGC(dpy, Scr.ScratchAlphaPixmap, gcm, &gcv);
	return;
}
...
static int MvwmErrorHandler(Display *dpy, XErrorEvent *event)
{
	...
	mvwm_msg(ERR, "MvwmErrorHandler", "Request %d, Error %d, EventType: %d",
		 event->request_code,
		 event->error_code,
		 last_event_type);
 ...
 *
 ************************************************************************/
void DestroyAllButtons(MvwmDecor *decor)
{
	TitleButton *tbp;
	DecorFace *face;
	int i;
...
void ResetAllButtons(MvwmDecor *decor)
{
	TitleButton *tbp;
	DecorFace *face;
	int i;
...
int main(int argc, char **argv)
{
	...
	Scr.ColorLimit = PictureInitColors(
		PICTURE_CALLED_BY_MVWM, True, &colorLimitop, True, True);
	...
	else
	{
		...
		if (
			!run_command_file(CatString3(
				mvwm_userdir, "/", MVWM_CONFIG), exc) &&
			!run_command_file(CatString3(
				MVWM_DATADIR, "/", MVWM_CONFIG), exc) &&
			!run_command_file(CatString3(
				mvwm_userdir, "/", MVWM2RC), exc) &&
			!run_command_file(CatString3(
				home_dir, "/", MVWM2RC), exc) &&
			!run_command_file(CatString3(
				MVWM_DATADIR, "/", MVWM2RC), exc) &&
			!run_command_file(CatString3(
				MVWM_DATADIR, "/system", MVWM2RC), exc) &&
			!run_command_file(CatString3(
				MVWM_CONFDIR, "/system", MVWM2RC), exc))
		{
			mvwm_msg(
				ERR, "main", "Cannot read startup config file,"
				" tried: \n\t%s/%s\n\t%s/%s\n\t%s/%s\n\t"
				"%s/%s\n\t%s/%s\n\t%s/system%s\n\t%s/system%s",
				mvwm_userdir, MVWM_CONFIG,
				MVWM_DATADIR, MVWM_CONFIG,
				mvwm_userdir, MVWM2RC,
				home_dir, MVWM2RC,
				MVWM_DATADIR, MVWM2RC,
				MVWM_DATADIR, MVWM2RC,
				MVWM_CONFDIR, MVWM2RC);
		}
	}
	exc_destroy_context(exc);
