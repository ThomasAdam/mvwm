...
static volatile sig_atomic_t mvwmRunState = MVWM_RUNNING;
...
int             master_pid;	/* process number of 1st mvwm process */
...
Bool            fMvwmInStartup = True;	/* Set to False when startup has finished */
Bool            DoingCommandLine = False;	/* Set True before each cmd line arg */

XContext        MvwmContext;	/* context for mvwm windows */
XContext        MenuContext;	/* context for mvwm menus */
...
char           *mvwm_userdir;
char const     *Mvwm_VersionInfo;
char const     *Mvwm_LicenseInfo;
char const     *Mvwm_SupportInfo;
 ...
 *
 *  Procedure:
 *      Done - tells mvwm to clean up and exit
 *
 */
...
Done(int restart, char *command)
{
	...
	if (functions_is_complex_function(exit_func_name)) {
		...
		char           *action =
		    mvwm_strdup(CatString2("Function ", exit_func_name));
	...
	if (restart) {
		...
		if (command) {
			...
			const char     *error_msg;
			int             n =
			    parse_command_args(command, my_argv, MAX_ARG_SIZE,
			    &error_msg);
			...
			if (n <= 0) {
				...
				/*
				 * Warn against an old 'Restart mvwm2' usage
				 */
				if (n == 1
				    && strcmp(my_argv[0], "mvwm2") == 0) {
					str = "mvwm2";
				}
				/*
				...
				execvp(my_argv[0], my_argv);
				mvwm_msg(ERR, "Done", "Call of '%s' failed!"
				    " (restarting '%s' instead)",
				    my_argv[0], g_argv[0]);
				perror("  system error description");
			}
		...
		mvwm_msg(ERR, "Done", "Call of '%s' failed!", g_argv[0]);
		perror("  system error description");
	} else {
		CloseICCCM2();
		catch_exit();
	 ...
	 * * Otherwise mvwm ignores map requests while it still has
...
InstallSignals(void)
{
	...
	/*
	 * All signals whose handlers call mvwmSetTerminate()
	...
	sigact.sa_handler = mvwmReapChildren;
	sigaction(SIGCHLD, &sigact, NULL);
#else
#ifdef USE_BSD_SIGNALS
	mvwmSetSignalMask(sigmask(SIGUSR1) | sigmask(SIGINT) | sigmask(SIGHUP)
...
#endif
	signal(SIGCHLD, mvwmReapChildren);
#ifdef HAVE_SIGINTERRUPT
	...
	/*
	 * When mvwm restarts, the SIGCHLD handler is automatically reset
	 * * to the default handler. This means that Zombies left over from
	 * * the previous instance of mvwm could still be roaming the process
...
CreateGCs(void)
{
	...
	Scr.ScratchGC1 = mvwmlib_XCreateGC(dpy, Scr.NoFocusWin, gcm, &gcv);
	Scr.ScratchGC2 = mvwmlib_XCreateGC(dpy, Scr.NoFocusWin, gcm, &gcv);
	Scr.ScratchGC3 = mvwmlib_XCreateGC(dpy, Scr.NoFocusWin, gcm, &gcv);
	Scr.ScratchGC4 = mvwmlib_XCreateGC(dpy, Scr.NoFocusWin, gcm, &gcv);
	Scr.TitleGC = mvwmlib_XCreateGC(dpy, Scr.NoFocusWin, gcm, &gcv);
	Scr.BordersGC = mvwmlib_XCreateGC(dpy, Scr.NoFocusWin, gcm, &gcv);
	Scr.TransMaskGC = mvwmlib_XCreateGC(dpy, Scr.NoFocusWin, gcm, &gcv);
	Scr.ScratchMonoPixmap = XCreatePixmap(dpy, Scr.Root, 1, 1, 1);
	Scr.MonoGC = mvwmlib_XCreateGC(dpy, Scr.ScratchMonoPixmap, gcm, &gcv);
	Scr.ScratchAlphaPixmap =
	    XCreatePixmap(dpy, Scr.Root, 1, 1, FRenderGetAlphaDepth());
	Scr.AlphaGC =
	    mvwmlib_XCreateGC(dpy, Scr.ScratchAlphaPixmap, gcm, &gcv);
	return;
}
...
static void
init_screen_flags(void)
{
	memset(&scr_flags, 0, sizeof(scr_flags));
	scr_flags.is_pointer_on_this_screen =
...
init_scr(void)
{
	...
	FW_W(&Scr.MvwmRoot) = Scr.Root;
	Scr.MvwmRoot.next = 0;
	init_stack_and_layers();
	Scr.root_pushes = 0;
	Scr.mvwm_pushes = 0;
	Scr.pushed_window = &Scr.MvwmRoot;
	Scr.MvwmRoot.number_cmap_windows = 0;
	Scr.MvwmRoot.attr_backup.colormap = Pcmap;
	 ...
	 * * initially
	 */
	Scr.cascade_window = &Scr.MvwmRoot;
	Scr.buttons2grab = 0;
	/*
	 * ewmh desktop
	 */
	Scr.EwmhDesktop = NULL;
	InitMvwmDecor(&Scr.DefaultDecor);
...
setVersionInfo(void)
{
	...
	sprintf(license_str,
	    ...
	    "You may\nredistribute copies of mvwm under "
	...
	if (support_len > 0) {
		...
		Mvwm_SupportInfo =
		    mvwm_strdup(CatString2("with support for:", support_str));
	} else {
		Mvwm_SupportInfo = "with no optional feature support";
	}
...
#define RC_DEFAULTS_COMPLETE ((char *)-1)
	...
	const char     *defaults[][3] = {
		...
		{"DestroyFunc MvwmMakeMissingDesktopMenu", "", ""},
		{"AddToFunc   MvwmMakeMissingDesktopMenu I PipeRead 'mvwm-menu-desktop --enable-mini-icons --mvwm-icons'", "", ""},
		{
			    "AddToMenu MenuMvwmRoot \"",
			    _("Builtin Menu"),
		    "\" Title"},
		{"+ MissingSubmenuFunction MvwmMakeMissingDesktopMenu", "",
			    ""},
		{"+ \"&1. XTerm\" Exec xterm", "", ""},
		{
			    "+ \"&2. ",
			    _("Setup Form"),
		    "\" Module MvwmForm MvwmForm-Setup"},
		{
			    "+ \"&3. ",
			    _("Setup 95 Script"),
		    "\" Module MvwmScript MvwmScript-Setup95"},
		{
			    "+ \"&4. ",
			    _("Issue mvwm commands"),
		    "\" Module MvwmConsole"},
		{
			    "+ \"&D. ",
			    _("Desktop Menu"),
		...
		{"Read " MVWM_DATADIR "/ConfigMvwmDefaults", "", ""},
		{NULL, NULL, NULL}
	};
	...
	for (i = 0; defaults[i][0] != NULL; i++) {
		...
		exc = exc_create_context(&ecc, ECC_TYPE | ECC_WCONTEXT);
		cmd =
		    CatString3(defaults[i][0], defaults[i][1],
		    defaults[i][2]);
...
{
	mvwm_msg(ERR, "CatchRedirectError", "another WM is running");
	exit(1);
...
MvwmErrorHandler(Display *dpy, XErrorEvent * event)
{
	...
	mvwm_msg(ERR, "MvwmErrorHandler", "*** internal error ***");
	mvwm_msg(ERR, "MvwmErrorHandler",
...
#define start_func_name "StartFunction"
	...
	if (functions_is_complex_function(init_func_name)) {
		char           *action =
		    mvwm_strdup(CatString2("Function ", init_func_name));
 ...
 ************************************************************************/
void
DestroyAllButtons(MvwmDecor * decor)
{
	TitleButton    *tbp;
	DecorFace      *face;
	int             i;
...
void
ResetAllButtons(MvwmDecor * decor)
{
	TitleButton    *tbp;
	DecorFace      *face;
	int             i;
...
main(int argc, char **argv)
{
	...
	FlocaleInit(LC_CTYPE, "", "", "mvwm");
	FGettextInit("mvwm", LOCALEDIR, "mvwm");
	 ...
	 */
	flib_putenv("MVWM_MODULEDIR", "MVWM_MODULEDIR=" MVWM_MODULEDIR);
	 ...
	 */
	mvwm_userdir = getenv("MVWM_USERDIR");
	if (mvwm_userdir == NULL) {
		char           *s;
	...
	/*
	 * Create MVWM_USERDIR directory if needed
	 */
	if (access(mvwm_userdir, F_OK) != 0) {
		mkdir(mvwm_userdir, 0777);
	}
	if (access(mvwm_userdir, W_OK) != 0) {
		mvwm_msg(ERR, "main", "No write permissions in `%s/'.\n",
		    mvwm_userdir);
	}
	...
	for (i = 1; i < argc; i++) {
		 ...
		 */
		    ...
		    strcmp(argv[i], "--version") == 0) {
			printf("%s\n%s\n\n%s\n", Mvwm_VersionInfo,
			    Mvwm_SupportInfo, Mvwm_LicenseInfo);
			exit(0);
		} else {
			usage(0);
	...
	if (single_screen_num >= 0) {
		...
		if (!dn) {
			...
			if (!(dpy = XOpenDisplay(dn))) {
				mvwm_msg(ERR, "main", "can't open display %s"
	...
	if (fcntl(x_fd, F_SETFD, 1) == -1) {
		mvwm_msg(ERR, "main", "close-on-exec failed");
		exit(1);
	}
#endif
	...
	if (strncmp(display_string, "DISPLAY=:", 9) == 0) {
		...
		sprintf(rdisplay_string, "HOSTDISPLAY=%s",
		    XDisplayString(dpy));
		flib_putenv("HOSTDISPLAY", rdisplay_string);
		free(rdisplay_string);
	}
	...
	if (Scr.Root == None) {
		mvwm_msg(ERR, "main", "Screen %d is not a valid screen",
		    (int) Scr.screen);
		exit(1);
	}

	{
		...
		if (Pdepth > 0) {
			if (Pvisual->class == DirectColor) {
				Pcmap =
				    XCreateColormap(dpy, Scr.Root, Pvisual,
				    AllocAll);
			} else {
				Pcmap =
				    XCreateColormap(dpy, Scr.Root, Pvisual,
	...
	PUseDynamicColors = 0;
	Scr.ColorLimit =
	    PictureInitColors(PICTURE_CALLED_BY_MVWM, True, &colorLimitop,
	    True, True);
	...
	Scr.MvwmCursors = CreateCursors(dpy);
	XDefineCursor(dpy, Scr.Root, Scr.MvwmCursors[CRS_ROOT]);
	/*
	...
	Scr.NoFocusWin =
	    XCreateWindow(dpy, Scr.Root, -10, -10, 10, 10, 0, Pdepth,
	    InputOutput, Pvisual,
	...
	{
		/*
		 * do not grab the pointer earlier because if mvwm exits with
		 * * the pointer grabbed while a different display is visible,
		 * * XFree 4.0 freezes.
		 */
		Cursor          cursor = XCreateFontCursor(dpy, XC_watch);
		XGrabPointer(dpy, Scr.Root, 0, 0, GrabModeAsync,
	...
	restart_state_filename =
	    mvwm_strdup(CatString3(mvwm_userdir, "/.fs-restart-",
		getenv("HOSTDISPLAY")));
	...
	if (visualClass != -1 || visualId != -1) {
		/*
		 * this is so that menus use the (non-default) mvwm colormap
		 */
		FW_W(&Scr.MvwmRoot) = Scr.NoFocusWin;
		Scr.MvwmRoot.number_cmap_windows = 1;
		Scr.MvwmRoot.cmap_windows = &Scr.NoFocusWin;
	}
	...
	if (num_config_commands > 0) {
		 ...
		 * *   ~/.mvwm/config
		 * *   /usr/local/share/mvwm/config
		 * * and for compatibility:
		 * *   ~/.mvwm/.mvwm2rc
		 * *   /usr/local/share/mvwm/system.mvwm2rc
		 * * and for compatibility to be discontinued:
		 * *   ~/.mvwm2rc,
		 * *   /usr/local/share/mvwm/.mvwm2rc
		 * *   /usr/local/etc/system.mvwm2rc
		 */
		if (!run_command_file(CatString3(mvwm_userdir, "/",
			    MVWM_CONFIG), exc)
		    && !run_command_file(CatString3(MVWM_DATADIR, "/",
			    MVWM_CONFIG), exc)
		    && !run_command_file(CatString3(mvwm_userdir, "/",
			    MVWM2RC), exc)
		    && !run_command_file(CatString3(home_dir, "/", MVWM2RC),
			exc)
		    && !run_command_file(CatString3(MVWM_DATADIR, "/",
			    MVWM2RC), exc)
		    && !run_command_file(CatString3(MVWM_DATADIR, "/system",
			    MVWM2RC), exc)
		    && !run_command_file(CatString3(MVWM_CONFDIR, "/system",
			    MVWM2RC), exc)) {
			mvwm_msg(ERR, "main",
			    "Cannot read startup config file,"
			    " tried: \n\t%s/%s\n\t%s/%s\n\t%s/%s\n\t"
			    "%s/%s\n\t%s/%s\n\t%s/system%s\n\t%s/system%s",
			    mvwm_userdir, MVWM_CONFIG, MVWM_DATADIR,
			    MVWM_CONFIG, mvwm_userdir, MVWM2RC, home_dir,
			    MVWM2RC, MVWM_DATADIR, MVWM2RC, MVWM_DATADIR,
			    MVWM2RC, MVWM_CONFDIR, MVWM2RC);
		}
	}
	exc_destroy_context(exc);
	...
	if (Pdepth < 2) {
		Scr.gray_pixmap =
		    XCreatePixmapFromBitmapData(dpy, Scr.NoFocusWin, g_bits,
		    g_width, g_height, PictureBlackPixel(),
		    PictureWhitePixel(), Pdepth);
		Scr.light_gray_pixmap =
		    XCreatePixmapFromBitmapData(dpy, Scr.NoFocusWin, l_g_bits,
		    l_g_width, l_g_height, PictureBlackPixel(),
		    PictureWhitePixel(), Pdepth);
		Scr.sticky_gray_pixmap =
		    XCreatePixmapFromBitmapData(dpy, Scr.NoFocusWin, s_g_bits,
		    s_g_width, s_g_height, PictureBlackPixel(),
		    PictureWhitePixel(), Pdepth);
	}
	...
	default:
		DBUG("main", "Unknown mvwm run-state");
	}
