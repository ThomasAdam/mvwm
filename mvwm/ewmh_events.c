...
int
ewmh_DesktopGeometry(EWMH_CMD_ARGS)
{
	char            action[256];
	...
	if (width <= 0 || height <= 0) {
		mvwm_msg(WARN, "ewmh_DesktopGeometry",
		    "The application window (id %#lx)\n"
...
int
ewmh_NumberOfDesktops(EWMH_CMD_ARGS)
{
	...
	if (d > 0 && (d <= ewmhc.MaxDesktops || ewmhc.MaxDesktops == 0)) {
		...
		mvwm_msg(WARN, "ewmh_NumberOfDesktops",
		    "The application window (id %#lx)\n"
...
int
ewmh_ActiveWindow(EWMH_CMD_ARGS)
{
	if (ev == NULL) {
		return 0;
	}
...
int
ewmh_CloseWindow(EWMH_CMD_ARGS)
{
	if (ev == NULL) {
		return 0;
	}
...
int
ewmh_MoveResizeWindow(EWMH_CMD_ARGS)
{
	...
	if (fw == NULL) {
		...
		do_reconfigure =
		    ! !is_function_allowed(func, NULL, fw, RQORIG_PROGRAM,
		    False);
	}
	if (do_reconfigure == 1) {
...
int
ewmh_RestackWindow(EWMH_CMD_ARGS)
{
	XConfigureRequestEvent cre;
	int             do_restack;
...
ewmh_WMDesktop(EWMH_CMD_ARGS)
{
	...
	if (style != NULL && ev == NULL) {
		...
		val =
		    ewmh_AtomGetByName(FW_W(fw), "_NET_WM_DESKTOP",
		...
		fprintf(stderr, "ewmh WM_DESKTOP hint for window 0x%lx  "
		    "(%i,%lu,%u)\n", FW_W(fw),
...
ewmh_MoveResize(EWMH_CMD_ARGS)
{
	...
	if (move) {
		...
		if (!is_function_allowed(F_RESIZE, NULL, fw,
...
ewmh_WMState(EWMH_CMD_ARGS)
{
	...
	if (ev != NULL) {
		...
		val =
		    ewmh_AtomGetByName(FW_W(fw), "_NET_WM_STATE",
		    EWMH_ATOM_LIST_CLIENT_WIN, &size);
...
#if DEBUG_EWMH_INIT_STATE
		if (size != 0) {
			fprintf(stderr, "Window 0x%lx has an init"
			    " _NET_WM_STATE hint\n", FW_W(fw));
		}
#endif
	...
	if (maximize != 0) {
		int             max_vert =
		    (maximize & EWMH_MAXIMIZE_VERT) ? 100 : 0;
		int             max_horiz =
		    (maximize & EWMH_MAXIMIZE_HORIZ) ? 100 : 0;
		char            cmd[256];
		...
		if (maximize & EWMH_MAXIMIZE_REMOVE) {
			...
			sprintf(cmd, "Maximize on %i %i", max_horiz,
			    max_vert);
		}
		execute_function_override_window(NULL, NULL, cmd, 0, fw);
	}
...
int
ewmh_WMStateFullScreen(EWMH_CMD_ARGS)
{
	if (ev == NULL && style == NULL) {
...
int
ewmh_WMStateHidden(EWMH_CMD_ARGS)
{
	if (ev == NULL && style == NULL) {
...
int
ewmh_WMStateMaxHoriz(EWMH_CMD_ARGS)
{
	...
	if (ev == NULL && style == NULL) {
		 ...
		 * * make any sense in mvwm, so just claim we're never maximized
		 */
		return 0;
#endif
	}
...
int
ewmh_WMStateMaxVert(EWMH_CMD_ARGS)
{
...
int
ewmh_WMStateModal(EWMH_CMD_ARGS)
{
	if (ev == NULL && style == NULL) {
		unsigned long   do_restore = any;
	...
	if (ev == NULL && style != NULL) {
		...
		if (IS_TRANSIENT(fw)) {
			...
			if (!FP_DO_GRAB_FOCUS_TRANSIENT(S_FOCUS_POLICY(SCF
				    (*style)))) {
				FPS_GRAB_FOCUS_TRANSIENT(S_FOCUS_POLICY(SCF
					(*style)), 0);
				FPS_GRAB_FOCUS_TRANSIENT(S_FOCUS_POLICY(SCM
					(*style)), 1);
				FPS_GRAB_FOCUS_TRANSIENT(S_FOCUS_POLICY(SCC
					(*style)), 1);
			}
		}
		return 0;
...
int
ewmh_WMStateShaded(EWMH_CMD_ARGS)
{
	...
	if (ev != NULL) {
		...
		if (!IS_SHADED(fw) &&
			...
			execute_function_override_window(NULL, NULL,
			    "Windowshade on", 0, fw);
		} else if (IS_SHADED(fw) && (cmd_arg == NET_WM_STATE_TOGGLE
			|| cmd_arg == NET_WM_STATE_REMOVE)) {
			execute_function_override_window(NULL, NULL,
...
int
ewmh_WMStateSkipPager(EWMH_CMD_ARGS)
{
	if (ev == NULL && style == NULL) {
		unsigned long   do_restore = any;
...
int
ewmh_WMStateSkipTaskBar(EWMH_CMD_ARGS)
{
	if (ev == NULL && style == NULL) {
		unsigned long   do_restore = any;
...
int
ewmh_WMStateStaysOnTop(EWMH_CMD_ARGS)
{
	if (ev == NULL && style == NULL) {
		unsigned long   do_restore = any;
...
int
ewmh_WMStateStaysOnBottom(EWMH_CMD_ARGS)
{
	if (ev == NULL && style == NULL) {
		unsigned long   do_restore = any;
...
int
ewmh_WMStateSticky(EWMH_CMD_ARGS)
{
 ...
 */
int
ewmh_WMIconGeometry(EWMH_CMD_ARGS)
{
	int             size;
	CARD32         *val;
	...
	if (val != NULL && size < 4 * sizeof(CARD32)) {
		mvwm_msg(WARN, "ewmh_WMIconGeometry",
		    "The application window (id %#lx)\n"
...
/**** for animation ****/
void
EWMH_GetIconGeometry(MvwmWindow *fw, rectangle *icon_rect)
{
