...
int             GetContext(MvwmWindow **ret_fw, MvwmWindow *t,
...
void            SendConfigureNotify(MvwmWindow *fw, int x, int y, int w,
    int h, int bw, Bool send_for_frame_too);
...
void            events_handle_configure_request(XConfigureRequestEvent cre,
    MvwmWindow *fw, Bool force_use_grav, int force_gravity);
Bool            test_button_event(Display *display, XEvent *event, char *arg);
Bool            test_typed_window_event(Display *display, XEvent *event,
    char *arg);
