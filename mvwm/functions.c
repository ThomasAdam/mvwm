...
static MvwmFunction *find_complex_function(const char *function_name)
{
	MvwmFunction *func;
...
static void __run_complex_function_items(
	cond_rc_t *cond_rc, char cond, MvwmFunction *func,
 ...
 */
static MvwmFunction *NewMvwmFunction(const char *name)
{
	MvwmFunction *tmp;
...
static void DestroyFunction(MvwmFunction *func)
{
...
void execute_function_override_window(
	cond_rc_t *cond_rc, const exec_context_t *exc, char *action,
	FUNC_FLAGS_TYPE exec_flags, MvwmWindow *fw)
{
	const exec_context_t *exc2;
	exec_context_changes_t ecc;
 ...
 */
void AddToFunction(MvwmFunction *func, char *action)
{
	FunctionItem *tmp;
	char *token = NULL;
	char condition;
