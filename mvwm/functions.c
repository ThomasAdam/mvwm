...
/*
 * mvwm built-in functions and complex functions
 */
...
DeferExecution(exec_context_changes_t *ret_ecc,
    exec_context_change_mask_t * ret_mask, cursor_t cursor,
    int trigger_evtype, int do_allow_unmanaged)
{
	int             done;
	int             finished = 0;
...
find_builtin_function(char *func)
{
	...
	ret_func =
	    (func_t *) bsearch(temp, func_table, nfuncs, sizeof(func_t),
	    func_comp);
	free(temp);
...
static void
__execute_function(cond_rc_t *cond_rc, const exec_context_t *exc,
    char *action, FUNC_FLAGS_TYPE exec_flags, char *args[],
    Bool has_ref_window_moved)
{
	...
	if (func_depth > MAX_FUNCTION_DEPTH) {
		mvwm_msg(ERR, "__execute_function",
		    "Function '%s' called with a depth of %i, "
		    "stopping function execution!", action, func_depth);
		func_depth--;
		return;
	}
	if (args) {
	...
	if (function) {
		char           *tmp = function;
		function =
		    expand_vars(function, arguments, False, False, func_rc,
		    exc);
		free(tmp);
	}
	...
	if (!(exec_flags & FUNC_DONT_EXPAND_COMMAND)) {
		expaction = expand_vars(taction, arguments, (bif) ?
	...
	mvwm_msg(INFO, "LOG", "%c: %s", (char) exc->type, expaction);
#endif
	...
	if (expaction[0] == '*') {
		if (Scr.cur_decor && Scr.cur_decor != &Scr.DefaultDecor) {
			mvwm_msg(WARN, "__execute_function",
		...
		if (bif && bif->func_t != F_FUNCTION) {
			...
			if (!bif && desperate) {
				if (executeModuleDesperate(func_rc, exc,
					runaction) == NULL && *function != 0
				    && !set_silent) {
					mvwm_msg(ERR, "__execute_function",
...
static void
__run_complex_function_items(cond_rc_t *cond_rc, char cond,
    MvwmFunction * func, const exec_context_t *exc, char *args[],
    Bool has_ref_window_moved)
{
	char            c;
	FunctionItem   *fi;
	...
	if (!(!has_ref_window_moved && PressedW
		&& XTranslateCoordinates(dpy, PressedW, Scr.Root, 0, 0, &x0,
		    &y0, &JunkChild))) {
		x0 = y0 = 0;
	}
	...
	for (fi = func->first_item; fi != NULL && cond_rc->break_levels == 0;) {
		...
		if (c == cond) {
			...
			if (!has_ref_window_moved && PressedW
			    && XTranslateCoordinates(dpy, PressedW, Scr.Root,
				0, 0, &x, &y, &JunkChild)) {
    ...
    char *action, Bool *desperate, Bool has_ref_window_moved)
{
	...
	if (func == NULL) {
		if (*desperate == 0) {
			mvwm_msg(ERR, "ComplexFunction",
			    "No such function %s", action);
		}
		return;
	}
	if (!depth) {
	...
	*desperate = 0;
	...
	for (fi = func->first_item;
	    fi != NULL && cond_rc->break_levels == 0; fi = fi->next_item) {
		/*
	...
	case ButtonRelease:
		...
		break;
	default:
		if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild, &x,
	...
	if (type == CF_CLICK) {
		...
		if (HaveDoubleClick) {
			type =
			    CheckActionType(x, y, &d, True, False, &button2);
			switch (type) {
			case CF_HOLD:
			case CF_MOTION:
			case CF_CLICK:
	...
	exc2 = exc_clone_context(exc, &ecc, mask);
	__run_complex_function_items(cond_rc, type, func, exc2, arguments,
...
/*
 * create a new MvwmFunction
 */
static MvwmFunction *
NewMvwmFunction(const char *name)
{
	MvwmFunction   *tmp;
...
static void
DestroyFunction(MvwmFunction * func)
{
...
void
execute_function_override_window(cond_rc_t *cond_rc,
    const exec_context_t *exc, char *action, FUNC_FLAGS_TYPE exec_flags,
    MvwmWindow *fw)
{
	const exec_context_t *exc2;
...
find_func_t(char *action, short *func_t, unsigned char *flags)
{
	...
	if (action) {
		...
		while (!matched && (mlen = strlen(func_table[j].keyword)) > 0) {
			if (mlen == len &&
			    strncasecmp(action, func_table[j].keyword,
				mlen) == 0) {
				matched = True;
				/*
				 * found key word
...
/*
 *  add an item to a MvwmFunction
 *
 *  Inputs:
 *      func      - pointer to the MvwmFunction to add the item
 *      action    - the definition string from the config line
 */
void
AddToFunction(MvwmFunction * func, char *action)
{
	FunctionItem   *tmp;
	char           *token = NULL;
	char            condition;
...
CMD_Plus(F_CMD_ARGS)
{
	if (Scr.last_added_item.type == ADDED_MENU) {
		...
		MvwmDecor      *tmp = &Scr.DefaultDecor;
...
CMD_EchoFuncDefinition(F_CMD_ARGS)
{
	...
	if (!token) {
		mvwm_msg(ERR, "EchoFuncDefinition", "Missing argument");

		return;
	}
	bif = find_builtin_function(token);
	if (bif != NULL) {
		mvwm_msg(INFO, "EchoFuncDefinition",
	...
	mvwm_msg(INFO, "EchoFuncDefinition", "definition of function '%s':",
	    token);
	for (fi = func->first_item; fi != NULL; fi = fi->next_item) {
		mvwm_msg(INFO, "EchoFuncDefinition", "  %c %s", fi->condition,
		    (fi->action == 0) ? "(null)" : fi->action);
	}
	mvwm_msg(INFO, "EchoFuncDefinition", "end of definition");
	free(token);
