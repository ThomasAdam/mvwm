...
/* When mvwm destroys pixmaps it puts them on a list and only destroys them
 ...
 * reference pixmaps that MvwmTheme has destroyed, bad things would happen */
struct junklist
{
...
get_root_pixmap(Atom prop)
{
	...
	ret =
	    XGetWindowProperty(dpy, Scr.Root, prop, 0L, 1L, False, XA_PIXMAP,
...
static char    *
get_simple_color(char *string, char **color, colorset_t *cs,
    int supplied_color, int special_flag, char *special_string)
{
	char           *rest;
...
static void
SafeDestroyPicture(Display *dpy, MvwmPicture *picture)
{
	/*
	 ...
	 */
	PDestroyMvwmPicture(dpy, picture);
...
free_colorset_background(colorset_t *cs, Bool do_free_args)
{
	...
	if (cs->pixels && cs->nalloc_pixels) {
		PictureFreeColors(dpy, Pcmap, cs->pixels, cs->nalloc_pixels,
		    0, False);
		free(cs->pixels);
		cs->pixels = NULL;
		cs->nalloc_pixels = 0;
...
parse_pixmap(Window win, GC gc, colorset_t *cs, Bool *pixmap_is_a_bitmap)
{
	 ...
	 * load the file
	 */
	cs->picture = PCacheMvwmPicture(dpy, win, NULL, cs->pixmap_args, fpa);
	if (cs->picture == NULL) {
		mvwm_msg(ERR, name, "can't load picture %s", cs->pixmap_args);
		return;
	}
    ...
    int *has_shape_changed)
{
	...
	*has_shape_changed = True;
	 ...
	 * load the shape mask
	 */
	picture = PCacheMvwmPicture(dpy, win, NULL, token, fpa);
	if (!picture) {
		mvwm_msg(ERR, name, "can't load picture %s", token);
	} else if (picture->depth != 1 && picture->mask == None) {
		mvwm_msg(ERR, name, "shape pixmap must be of depth 1");
		...
		if (mask != None) {
			cs->shape_mask =
			    XCreatePixmap(dpy, mask, picture->width,
			    picture->height, 1);
...
parse_colorset(int n, char *line)
{
	...
	while (line && *line) {
		 ...
		 */
		line =
		    GetQuotedString(line, &optstring, ",", NULL, NULL, NULL);
		if (!optstring)
			break;
		...
		case 4:	/* Back */
		case 5:	/* bg */
			get_simple_color(args, &bg, cs, BG_SUPPLIED,
			    BG_AVERAGE, "average");
			has_bg_changed = True;
			break;
		case 6:	/* Hilight */
		...
		case 23:	/* Transparent */
			...
			/*
			 * This is only allowable when the root depth == mvwm
			...
			if (Pdepth != DefaultDepth(dpy, (DefaultScreen(dpy)))) {
				mvwm_msg(ERR, name, "can't do Transparent "
				    "when root_depth!=mvwm_depth");
				break;
			}
			has_pixmap_changed = True;
			free_colorset_background(cs, True);
		...
		default:
			...
			if (option[0] && StrEquals(&option[1], "Gradient")) {
				...
				mvwm_msg(WARN, name, "bad colorset pixmap "
	...
	if (has_bg_changed ||
		...
		if ((cs->color_flags & BG_AVERAGE) && average_pix) {
			 ...
			 */
			colors =
			    mvwm_malloc(cs->width * cs->height * sizeof(XColor));
			/*
			 * get the pixmap and mask into an image
			 */
			image =
			    XGetImage(dpy, average_pix, 0, 0, cs->width,
			    cs->height, AllPlanes, ZPixmap);
			if (cs->mask != None) {
				mask_image =
				    XGetImage(dpy, cs->mask, 0, 0, cs->width,
				    cs->height, AllPlanes, ZPixmap);
			}
	 ...
	 * ---------- tint the bg colour ----------
	 */
	if (has_bg_tint_changed || (has_bg_changed
		&& cs->bg_tint_percent > 0)) {
		if (cs->bg_tint_percent == 0) {
			...
			PictureFreeColors(dpy, Pcmap, &cs->bg, 1, 0, True);
			cs->bg =
			    GetTintedPixel(cs->bg_saved, cs->bg_tint,
			    cs->bg_tint_percent);
			if (old_bg != cs->bg) {
				have_pixels_changed = True;
				has_bg_changed = True;
	 ...
	 */
	if (has_fg_tint_changed || (has_fg_changed
		&& cs->fg_tint_percent > 0)) {
	...
	/*
	 * ---------- send new colorset to mvwm and clean up ----------
	 */
	/*
...
alloc_colorset(int n)
{
	...
	while (nColorsets <= n) {
		...
		if (PictureUseBWOnly()) {
			...
			ncs->pixmap =
			    XCreatePixmapFromBitmapData(dpy, Scr.NoFocusWin,
...
{
	mvwm_msg(WARN, "CMD_ReadWriteColors", "ReadWriteColors is obsolete");
