...
/*
 * Mvwm trivia: There were 97 commands in the mvwm command table
 * when the F_CMD_ARGS macro was written.
 * dje 12/19/98.
 */

/* Macro for args passed to mvwm commands... */
#define F_CMD_ARGS \
...
#define CR_MOTION_METHOD_MASK      0x3
	...
	/* mvwm places the window itself */
	unsigned is_placed_by_mvwm : 1;
	...
	/* the window is a torn out mvwm menu */
...
#define EWMH_MVWM_ICON   2 /* the ewmh icon has been set by mvwm */
...
typedef struct MvwmWindow
{
	...
	int icon_name_count;
	/* next mvwm window */
	struct MvwmWindow *next;
	/* prev mvwm window */
	struct MvwmWindow *prev;
	/* next (lower) mvwm window in stacking order*/
	struct MvwmWindow *stack_next;
	/* prev (higher) mvwm window in stacking order */
	struct MvwmWindow *stack_prev;
