...
static int visibleCompare(const  MvwmWindow **a, const MvwmWindow **b)
{
	return strcasecmp((*a)->visible_name, (*b)->visible_name);
}

static int iconCompare(const MvwmWindow **a, const MvwmWindow **b)
{
...
static int classCompare(const MvwmWindow **a, const MvwmWindow **b)
{
...
static int resourceCompare(const MvwmWindow **a, const MvwmWindow **b)
{
	int result = strcasecmp((*a)->class.res_class, (*b)->class.res_class);
	if (result)
	{
...
static int classIconCompare(const MvwmWindow **a, const MvwmWindow **b)
{
	int result = strcasecmp((*a)->class.res_class, (*b)->class.res_class);
	if (result)
	{
...
static int resourceIconCompare(const MvwmWindow **a, const MvwmWindow **b)
{
	int result = strcasecmp((*a)->class.res_class, (*b)->class.res_class);
	if (result)
	{
...
static int compareReverse(const  MvwmWindow **a, const  MvwmWindow **b)
{
	return -compare(a, b);
}
...
void CMD_WindowList(F_CMD_ARGS)
{
	...
	if (flags & (SHOW_ALPHABETIC | SORT_BYCLASS | SORT_BYRESOURCE))
	{
		 ...
		 * is selected. */
		int (*sort)(const MvwmWindow **a, const MvwmWindow **b);
	     ...
	     sor_keyname != sor_default_keyname))
	{
		MST_SELECT_ON_RELEASE_KEY(mr) =
			XKeysymToKeycode(
				dpy, MvwmStringToKeysym(dpy, sor_keyname));
	}
