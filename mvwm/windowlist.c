...
static char    *
get_desk_title(struct monitor *m, int desk, unsigned long flags,
    Bool is_top_title)
{
	char           *desk_name;
	char           *tlabel;
	...
	if (desk_name != NULL) {
		tlabel = mvwm_malloc(strlen(desk_name) + 50);
	} else {
		tlabel = mvwm_malloc(50);
	}
...
static int
visibleCompare(const MvwmWindow **a, const MvwmWindow **b)
{
	return strcasecmp((*a)->visible_name, (*b)->visible_name);
}

static int
iconCompare(const MvwmWindow **a, const MvwmWindow **b)
{
	return strcasecmp((*a)->visible_icon_name, (*b)->visible_icon_name);
}

/* Which of the compare functions to sort on. */
static int      (*compare) (const MvwmWindow **a, const MvwmWindow **b);

static int
classCompare(const MvwmWindow **a, const MvwmWindow **b)
{
	int             result =
...
static int
resourceCompare(const MvwmWindow **a, const MvwmWindow **b)
{
	int             result =
	    strcasecmp((*a)->class.res_class, (*b)->class.res_class);
	if (result) {
...
static int
classIconCompare(const MvwmWindow **a, const MvwmWindow **b)
{
	int             result =
	    strcasecmp((*a)->class.res_class, (*b)->class.res_class);
	if (result) {
...
static int
resourceIconCompare(const MvwmWindow **a, const MvwmWindow **b)
{
	int             result =
	    strcasecmp((*a)->class.res_class, (*b)->class.res_class);
	if (result) {
...
static int
compareReverse(const MvwmWindow **a, const MvwmWindow **b)
{
	return -compare(a, b);
}
...
CMD_WindowList(F_CMD_ARGS)
{
	...
	MvwmWindow     *t;
	MvwmWindow    **windowList;
	MvwmWindow    **iconifiedList = NULL;
	int             numWindows;
	int             ii;
	char            tname[128];
	char            loc[64];
	...
	if (action && *action) {
		...
		opts =
		    get_menu_options(action, w, fw, NULL, NULL, NULL, &mops);
		...
		while (opts && *opts) {
			 ...
			 */
				...
				sor_keyname = NULL;
				opts =
				    GetNextSimpleOption(opts, &sor_keyname);
				...
				default_action = mvwm_strdup(tok);
			} else {
				mvwm_msg(ERR, "WindowList",
	...
	if (flags & (SHOW_ALPHABETIC | SORT_BYCLASS | SORT_BYRESOURCE)) {
		...
		case SORT_BYRESOURCE | SHOW_ICONNAME | SORT_BYCLASS | SHOW_ALPHABETIC:
			compare =
			    resourceIconCompare;
			break;
	...
	while (next_desk != INT_MAX) {
		...
		for (ii = 0; ii < numWindows; ii++) {
			...
			if (first_desk && flags & TITLE_FOR_ALL_DESKS) {
				tlabel =
				    get_desk_title(m, t->Desk, flags, False);
			...
			t_hot = mvwm_malloc(strlen(name) + 80);
			if (use_hotkey) {
				/*
				 * Generate label
				 */
			...
			tfunc = mvwm_malloc(strlen(ffunc) + 36);
			/*
	...
	if (sor_keyname &&
		...
		MST_SELECT_ON_RELEASE_KEY(mr) =
		    XKeysymToKeycode(dpy, MvwmStringToKeysym(dpy,
			sor_keyname));
	}
	...
	mp.pexc = &exc2;
	mp.flags.has_default_action = (default_action
	    && *default_action != 0);
