 ...
 * MvwmWindow structure reflect the desired geometry of the window or its
...
static void     border_rotate_titlebar_descr(MvwmWindow *fw,
    titlebar_descr *td);

/* for grouping the MultiPixmap stuff */
static Bool
border_mp_get_use_title_style_parts_and_geometry(titlebar_descr *td,
    MvwmPicture **pm, MvwmAcs *acs, unsigned short sf, int is_left,
...
static Bool
is_button_toggled(MvwmWindow *fw, int button)
{
	mwm_flags       mf;
...
static void
get_common_decorations(common_decorations_type *cd, MvwmWindow *t,
...
static window_parts
border_get_changed_border_parts(MvwmWindow *fw, rectangle *old_sidebar_g,
...
static int
border_get_parts_and_pos_to_draw(common_decorations_type *cd, MvwmWindow *fw,
    ...
    border_relief_descr * br)
{
	...
	if (has_x_marks_old != has_x_marks) {
		draw_parts |=
		    (PART_FRAME & (~(PART_BORDER_N | PART_BORDER_S)));
	}
	if (has_y_marks_old != has_y_marks) {
		draw_parts |=
		    (PART_FRAME & (~(PART_BORDER_W | PART_BORDER_E)));
	}
	draw_parts |=
	    border_get_changed_border_parts(fw, &sidebar_g_old,
	    &br->sidebar_g, cs);
...
static window_parts
border_get_tb_parts_to_draw(MvwmWindow *fw, titlebar_descr *td,
    rectangle *old_g, rectangle *new_g, window_parts force_draw_parts)
{
	...
	if ((draw_parts & PART_TITLE) == PART_NONE &&
		...
		if (DFS_USE_BORDER_STYLE(df->style) &&
		    (((td->old_layout.title_g.x != td->layout.title_g.x ||
				...
				|| CSET_PIXMAP_IS_TILED(td->cd->
				    bg_border_cs)))
			|| (old_g->width != new_g->width
			    && CSET_PIXMAP_IS_X_STRETCHED(td->cd->
				bg_border_cs))
			|| (old_g->height != new_g->height
			    && CSET_PIXMAP_IS_Y_STRETCHED(td->cd->
				bg_border_cs))
			|| ((old_g->x != new_g->x || old_g->y != new_g->y)
			    && CSET_IS_TRANSPARENT_ROOT(td->cd->
				bg_border_cs)))) {
			draw_parts |= PART_TITLE;
		}
		if ((draw_parts & PART_TITLE) == PART_NONE &&
	...
	for (i = 0; i < NUMBER_OF_TITLE_BUTTONS; i++) {
		...
		mask = (1 << i);
		bs = &TB_STATE(GetDecor(fw,
			buttons[i]))[td->tbstate.bstate[i]].style;
		...
		if (DFS_USE_BORDER_STYLE(*bs) &&
			...
			df = &TB_STATE(GetDecor(fw,
				titlebar))[td->tbstate.bstate[i]];
			for (tdf = df; tdf != NULL; tdf = tdf->next) {
				int             cs;
    ...
    MvwmWindow *fw, Bool do_hilight)
{
	...
	if (transparent_gc == None && !HAS_NO_BORDER(fw)
		...
		xgcv.plane_mask = 0;
		transparent_gc =
		    mvwmlib_XCreateGC(dpy, Scr.NoFocusWin,
...
static void
trim_border_layout(MvwmWindow *fw, DecorFaceStyle *borderstyle,
...
static void
border_fetch_mwm_layout(MvwmWindow *fw, DecorFaceStyle *borderstyle,
    border_relief_size_descr *ret_size_descr)
{
	/*
...
static void
border_fetch_mvwm_layout(MvwmWindow *fw, DecorFaceStyle *borderstyle,
    border_relief_size_descr *ret_size_descr)
{
	/*
	 * Mvwm borders look like this:
	 * *
    ...
    MvwmWindow *fw, Bool do_hilight)
{
	...
	if (HAS_MWM_BORDER(fw)) {
		...
		border_fetch_mvwm_layout(fw, borderstyle, ret_size_descr);
	}
...
border_get_border_marks_descr(common_decorations_type *cd,
    border_relief_descr * br, MvwmWindow *fw)
{
	int             inset;
    ...
    rectangle *part_g, Pixmap dest_pix, Bool is_inverted)
{
	...
	for (i = 0; i < 7; i++) {
		if (ulgc[i] != 0x7f && w[i] > 0) {
			do_relieve_rectangle(dpy, dest_pix, off_x, off_y,
    ...
    int height, int cs)
{
	...
	p = CreateBackgroundPixmap(dpy, w, my_w, my_h, &Colorset[cs],
	    Pdepth, Scr.BordersGC, False);
	if (p && td->td_is_rotated) {
		Pixmap          tmp;
		tmp =
		    CreateRotatedPixmap(dpy, p, my_w, my_h, Pdepth,
...
border_get_frame_pixmap(common_decorations_type *cd, rectangle *frame_g)
{
	...
	if (cd->bg_border_cs < 0 || CSET_IS_TRANSPARENT(cd->bg_border_cs)) {
		...
		dcd->frame_pixmap =
		    CreateBackgroundPixmap(dpy, Scr.NoFocusWin,
	...
	*free_bg_pixmap = False;
	...
	if (cd->texture_pixmap) {
		...
		if (CSET_IS_TRANSPARENT_ROOT(cd->bg_border_cs)) {
			bg->pixmap.p =
			    CreateBackgroundPixmap(dpy, w, part_g->width,
			    part_g->height, cs_t, Pdepth, Scr.BordersGC,
			    False);
		} else {
			/*
			 * FIXME
...
static void
border_draw_one_border_part(common_decorations_type *cd, MvwmWindow *fw,
...
static void
border_draw_all_border_parts(common_decorations_type *cd, MvwmWindow *fw,
			...
			for (j = 0; j < UTS_TBMP_NUM_PIXMAPS; j++) {
				...
				if (tsdf->u.mp.acs[j].cs >= 0 ||
					...
					dcd->bar_pixmaps[bs].bps[i].created =
					    0;
					dcd->bar_pixmaps[bs].bps[i].mp_part =
					    j;
					i++;
				}
			}
...
border_get_bar_pixmaps(dynamic_common_decorations *dcd, rectangle *bar_g,
    ButtonState bs, int cset, MvwmPicture *mp_pic, int mp_part, int stretch,
    MvwmPicture **mp_ret_pic)
{
	ButtonState     b;
	int             i, j;
	int             count = dcd->bar_pixmaps[bs].count;
		...
		while (b < BS_MaxButtonState) {
			...
			if (j < c && dcd->bar_pixmaps[b].bps[j].p) {
				...
				if (mp_pic && mp_ret_pic) {
					*mp_ret_pic =
					    dcd->bar_pixmaps[bs].
					    bps[i].mp_created_pic =
					    dcd->bar_pixmaps[b].
					    bps[j].mp_created_pic;
				}
				break;
			}
			b++;
	...
	if (dcd->bar_pixmaps[bs].bps[i].p == None) {
		if (cset >= 0) {
			dcd->bar_pixmaps[bs].bps[i].p =
			    CreateBackgroundPixmap(dpy, Scr.NoFocusWin,
			    bar_g->width, bar_g->height, &Colorset[cset],
			    Pdepth, Scr.BordersGC, False);
			...
			if (stretch) {
				dcd->bar_pixmaps[bs].bps[i].mp_created_pic =
				    ...
				    bar_g->height, Scr.BordersGC, Scr.MonoGC,
				    Scr.AlphaGC);
			} else {
				dcd->bar_pixmaps[bs].bps[i].mp_created_pic =
...
border_mp_render_into_pixmap(titlebar_descr *td, common_decorations_type *cd,
    MvwmPicture **src, MvwmAcs *acs, Pixel *pixels,
    unsigned short solid_flags, unsigned short stretch_flags, int part,
    Pixmap dest, Window w, rectangle *full_g, rectangle *title_g,
    ButtonState bs, rectangle *g)
{
	...
	if (solid_flags & (1 << part)) {
		...
		if (CSET_IS_TRANSPARENT_ROOT(acs[part].cs)) {
			...
			bg.pixmap.p =
			    border_get_bar_pixmaps(dcd, full_g, bs,
		...
		if (!bg.pixmap.p) {
			...
			p = CreateBackgroundPixmap(dpy, w, g->width,
			    g->height, &Colorset[acs[part].cs], Pdepth,
			    Scr.BordersGC, False);
			bg.pixmap.p = p;
...
static int
border_mp_get_length(titlebar_descr *td, MvwmPicture **pm, MvwmAcs *acs,
...
static void
border_mp_get_titlebar_descr(MvwmWindow *fw, titlebar_descr *td,
    DecorFace *df)
{
	DecorFace      *tsdf;
	MvwmPicture   **pm;
	...
	for (tsdf = df; tsdf != NULL; tsdf = tsdf->next) {
		...
		add =
		    border_mp_get_length(td, pm, acs, sf, TBMP_LEFT_OF_TEXT);
		if (add > left_of_text
		    && add + left_end + right_of_text + right_end +
		...
		if (add > right_of_text
		    && add + left_end + left_of_text + right_end +
		    under_width + 2 * TBMP_MIN_RL_TITLE_LENGTH <=
		    fw->title_length) {
			right_of_text = add;
		}
...
static void
border_mp_get_extreme_geometry(titlebar_descr *td, MvwmPicture **pm,
    MvwmAcs *acs, unsigned short sf, rectangle *left_of_text_g,
...
static void
border_mp_draw_mp_titlebar(MvwmWindow *fw, titlebar_descr *td, DecorFace *df,
    Pixmap dest_pix, Window w)
{
	...
	if (td->length > 0) {
		    ...
		    td->under_text_g.width > 0 && td->under_text_g.height > 0)
		{
			border_mp_render_into_pixmap(td, td->cd, pm, acs,
			    pixels, solid_flags, stretch_flags,
			    TBMP_UNDER_TEXT, dest_pix, w, NULL,
			    &td->layout.title_g, bs, &td->under_text_g);
		}
 ...
 */
static void
border_draw_decor_to_pixmap(MvwmWindow *fw, Pixmap dest_pix, Window w,
    ...
    int use_title_style, int is_toggled, int left1right0)
{
	...
	case SolidButton:
		...
		dest_g.x = 0;
		dest_g.y = 0;
		border_fill_pixmap_background(dest_pix, &dest_g, solid_bg,
		    cd);
		break;
	case VectorButton:
	...
	case MultiPixmap:	/* for UseTitleStyle only */
		{
			...
			if (sf & (1 << part)) {
				bg.flags.use_pixmap = 0;
				bg.pixel = pixels[part];
				border_fill_pixmap_background(dest_pix,
				    &dest_g, &bg, cd);
				break;
			}
			cs = acs[part].cs;
			...
			if (cs >= 0 && use_title_style && g.width > 0
			    && g.height > 0 && !CSET_IS_TRANSPARENT_ROOT(cs)
			    && (bg.pixmap.p =
				border_get_bar_pixmaps(dcd, &g, bs, cs, NULL,
				...
				tmp =
				    border_create_root_transparent_pixmap(td,
				    w, w_g->width, w_g->height, cs);
				bg.pixmap.p = tmp;
				...
				tmp =
				    CreateBackgroundPixmap(dpy, w, w_g->width,
				    w_g->height, &Colorset[cs], Pdepth,
				    Scr.BordersGC, False);
				bg.pixmap.p = tmp;
				GetWindowBackgroundPixmapSize(&Colorset[cs],
				    w_g->width, w_g->height, &bg_w, &bg_h);
				bg.pixmap.g.width = bg_w;
				bg.pixmap.g.height = bg_h;
				bg.pixmap.shape = None;
				bg.pixmap.alpha = None;
	...
	case ColorsetButton:
		{
			...
			if (use_title_style &&
			    !CSET_IS_TRANSPARENT_ROOT(cs) &&
			    (bg.pixmap.p =
				border_get_bar_pixmaps(&(cd->dynamic_cd),
				    &(td->bar_g), bs, cs, NULL, TBMP_NONE, 0,
				    NULL))
			    != None) {
				...
				tmp =
				    CreateBackgroundPixmap(dpy, w, w_g->width,
				    w_g->height, cs_t, Pdepth, Scr.BordersGC,
				    False);
				if (tmp == None) {
					break;
				}
				bg.pixmap.p = tmp;
				GetWindowBackgroundPixmapSize(cs_t,
				    w_g->width, w_g->height, &bg_w, &bg_h);
				bg.pixmap.g.width = bg_w;
				bg.pixmap.g.height = bg_h;
				bg.pixmap.g.x = 0;
			...
			bg.pixmap.flags.is_tiled = 1;
			bg.pixmap.fra.mask = FRAM_HAVE_ADDED_ALPHA;
			bg.pixmap.fra.added_alpha_percent =
	...
	default:
		mvwm_msg(ERR, "DrawButton", "unknown button type: %i", type);
		break;
	}
...
static void
border_set_button_pixmap(MvwmWindow *fw, titlebar_descr *td, int button,
    Pixmap *dest_pix, Window w)
{
	pixmap_background_type bg;
	unsigned int    mask;
	...
	if (!DFS_USE_BORDER_STYLE(df->style)) {
		...
		border_get_border_background(&bg, td->cd, button_g,
	...
	for (; df; df = df->next) {
		 ...
		 */
		bg.pixel = df->u.back;
		border_draw_decor_to_pixmap(fw, *dest_pix, w, &bg, button_g,
		    df, td, bs, False, (td->tbstate.toggled_bmask & mask),
		    is_left_button);
	}
	/*
	...
	switch (DFS_BUTTON_RELIEF(TB_STATE(GetDecor(fw,
			buttons[button]))[bs].style)) {
	...
	case DFS_BUTTON_IS_UP:
		do_relieve_rectangle(dpy, *dest_pix, 0, 0,
		    button_g->width - 1, button_g->height - 1,
...
static void
border_draw_one_button(MvwmWindow *fw, titlebar_descr *td, int button)
{
	Pixmap          p;
	...
	p = border_create_decor_pixmap(td->cd,
	    &(td->layout.button_g[button]));
	/*
	 * set the background tile
	 */
...
static void
border_draw_title_stick_lines(MvwmWindow *fw, titlebar_descr *td,
    title_draw_descr * tdd, Pixmap dest_pix)
{
	...
	right_x = ((under_text_length == 0) ?
	    td->offset + td->length : under_text_offset + under_text_length)
	...
	if (right_w < WINDOW_TITLE_STICK_MIN_WIDTH) {
		...
		right_x = fw->title_length - WINDOW_TITLE_STICK_MIN_WIDTH - 1
		    ...
		    (right_length - WINDOW_TITLE_STICK_MIN_WIDTH) / 2 : 0);
	}
...
static void
border_draw_title_mono(MvwmWindow *fw, titlebar_descr *td,
    title_draw_descr * tdd, FlocaleWinString *fstr, Pixmap dest_pix)
{
	int             has_vt;

	has_vt = HAS_VERTICAL_TITLE(fw);
	XFillRectangle(dpy, dest_pix, td->cd->relief_gc,
...
static void
border_draw_title_relief(MvwmWindow *fw, titlebar_descr *td,
    title_draw_descr * tdd, Pixmap dest_pix)
{
	int             reverse = 0;
	rotation_t      rotation;
...
static void
border_draw_title_deep(MvwmWindow *fw, titlebar_descr *td,
...
static void
border_get_titlebar_draw_descr(MvwmWindow *fw, titlebar_descr *td,
    title_draw_descr * tdd, Pixmap dest_pix)
{
	...
	NewFontAndColor(fw->title_font, td->cd->fore_color,
	    td->cd->back_color);
	tdd->tstyle =
...
static void
border_set_title_pixmap(MvwmWindow *fw, titlebar_descr *td, Pixmap *dest_pix,
    Window w)
{
	pixmap_background_type bg;
...
static void
border_draw_title(MvwmWindow *fw, titlebar_descr *td)
{
	Pixmap          p;
...
static void
border_draw_buttons(MvwmWindow *fw, titlebar_descr *td)
{
	int             i;
...
static void
border_setup_use_title_style(MvwmWindow *fw, titlebar_descr *td)
{
	int             i;
	DecorFace      *df, *tsdf;
	ButtonState     bs, tsbs;
...
static void
border_rotate_titlebar_descr(MvwmWindow *fw, titlebar_descr *td)
{
	rotation_t      rotation;
	int             i, tmpi;
...
#define ROTATE_RECTANGLE(rot, r, vs_frame, vs_titlebar, vs_title) \
	...
	case ROTATION_180:
		td->offset =
		    td->layout.title_g.width - td->offset - td->length;
		tmpi = td->left_end_length;
		td->left_end_length = td->right_end_length;
		td->right_end_length = tmpi;
		tmpi = td->left_of_text_length;
	...
	ROTATE_RECTANGLE(rotation, (&td->under_text_g), False, False, True)
	    ...
	    ROTATE_RECTANGLE(rotation, (&td->right_main_g), False, False,
	    True)
...
static void
border_get_titlebar_descr_state(MvwmWindow *fw, window_parts pressed_parts,
    ...
    border_titlebar_state *tbstate)
{
	...
	for (i = 0; i < NUMBER_OF_TITLE_BUTTONS; i++) {
		...
		tbstate->bstate[i] =
		    border_flags_to_button_state(tbstate->
		    pressed_bmask & mask, tbstate->lit_bmask & mask,
		    tbstate->toggled_bmask & mask);
	}
	tbstate->tstate =
	    border_flags_to_button_state(tbstate->is_title_pressed,
	    tbstate->is_title_lit, 0);
}

static window_parts
border_get_titlebar_descr(common_decorations_type *cd, MvwmWindow *fw,
    ...
    titlebar_descr *ret_td)
{
	...
	if (fw->visible_name != (char *) NULL) {
		ret_td->length =
		    ...
		    (ret_td->has_vt) ? -strlen(fw->visible_name) : strlen(fw->
			visible_name));
		if (ret_td->length >
	 ...
	 */
	draw_parts =
	    border_get_tb_parts_to_draw(fw, ret_td, old_g, new_g,
	    force_draw_parts);
...
static void
border_draw_titlebar(common_decorations_type *cd, MvwmWindow *fw,
    window_parts pressed_parts, int pressed_button,
    window_parts force_draw_parts, clear_window_parts clear_parts,
    rectangle *old_g, rectangle *new_g, Bool do_hilight)
{
 ...
 */
static void
border_draw_border_parts(common_decorations_type *cd, MvwmWindow *fw,
    window_parts pressed_parts, window_parts force_draw_parts,
    ...
    Bool do_hilight)
{
	...
	if (HAS_NO_BORDER(fw)) {
		...
		fw->decor_state.parts_inverted &=
		    ~(PART_FRAME | PART_HANDLES);
		return;
	}
	...
	draw_parts =
	    border_get_parts_and_pos_to_draw(cd, fw, pressed_parts,
...
DecorFace      *
border_get_border_style(MvwmWindow *fw, Bool has_focus)
{
	DecorFace      *df;
...
int
border_is_using_border_style(MvwmWindow *fw, Bool has_focus)
{
	ButtonState     bs;
	int             is_pressed;
	int             is_toggled;
	...
	for (i = 0; i < NUMBER_OF_TITLE_BUTTONS; i++) {
		...
		if (DFS_USE_BORDER_STYLE(TB_STATE(GetDecor(fw,
				buttons[i]))[bs].style)) {
			return 1;
		}
	}
...
void
border_get_part_geometry(MvwmWindow *fw, window_parts part,
...
void
border_draw_decorations(MvwmWindow *fw, window_parts draw_parts,
    Bool has_focus, Bool do_force, clear_window_parts clear_parts,
    rectangle *old_g, rectangle *new_g)
{
	...
	m = fw->m != NULL ? fw->m : monitor_get_current();
...
void
border_undraw_decorations(MvwmWindow *fw)
{
 ...
 */
void
border_redraw_decorations(MvwmWindow *fw)
{
	MvwmWindow     *u = Scr.Hilite;
 ...
 */
unsigned int
border_get_transparent_decorations_part(MvwmWindow *fw)
{
	DecorFace      *df, *tdf;
	...
	border_get_titlebar_descr_state(fw, pressed_parts & PART_TITLEBAR,
	    item, CLEAR_ALL, (Scr.Hilite == fw), &tbstate);
	...
	for (tdf = df; tdf != NULL; tdf = tdf->next) {
		if (DFS_FACE_TYPE(tdf->style) == ColorsetButton &&
			...
			for (i = 0; i < TBMP_NUM_PIXMAPS; i++) {
				if (CSET_IS_TRANSPARENT_ROOT(tdf->u.mp.acs[i].
					cs)) {
					draw_parts |= PART_TITLE;
					break;
				}
...
CMD_BorderStyle(F_CMD_ARGS)
{
	...
	char           *prev;
	MvwmDecor      *decor =
	    Scr.cur_decor ? Scr.cur_decor : &Scr.DefaultDecor;
	...
	for (prev = action; (parm = PeekToken(action, &action));
		...
		if (StrEquals(parm, "active") || StrEquals(parm, "inactive")) {
			...
			if (ReadDecorFace(prev, &decor->BorderStyle.active,
				-1, True)) {
				ReadDecorFace(prev,
			...
			if (ReadDecorFace(prev, &tmpdf, -1, True)) {
				FreeDecorFace(dpy,
				    &decor->BorderStyle.active);
				decor->BorderStyle.active = tmpdf;
				ReadDecorFace(prev,
				    &decor->BorderStyle.inactive, -1, False);
				decor->BorderStyle.active.flags.has_changed =
				    1;
				decor->BorderStyle.inactive.flags.
				    has_changed = 1;
			}
			break;
		}
