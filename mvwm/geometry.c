...
void
gravity_get_naked_geometry(int gravity, MvwmWindow *t, rectangle *dest_g,
    rectangle *orig_g)
{
	int             xoff;
	int             yoff;
...
void
gravity_add_decoration(int gravity, MvwmWindow *t, rectangle *dest_g,
    rectangle *orig_g)
{
	size_borders    b;
...
void
get_relative_geometry(struct monitor *m, rectangle *rel_g, rectangle *abs_g)
{
...
void
get_absolute_geometry(struct monitor *m, rectangle *abs_g, rectangle *rel_g)
{
...
void
gravity_translate_to_northwest_geometry(int gravity, MvwmWindow *t,
...
void
gravity_translate_to_northwest_geometry_no_bw(int gravity, MvwmWindow *t,
    rectangle *dest_g, rectangle *orig_g)
{
...
void
get_title_geometry(MvwmWindow *fw, rectangle *ret_g)
{
	size_borders    b;
	size_borders    nt;
	int             w;
...
void
get_title_gravity_factors(MvwmWindow *fw, int *ret_fx, int *ret_fy)
{
	switch (GET_TITLE_DIR(fw)) {
	case DIR_N:
		*ret_fx = 0;
...
Bool
get_title_button_geometry(MvwmWindow *fw, rectangle *ret_g, int context)
{
	int             bnum;
	...
	XTranslateCoordinates(dpy, FW_W_FRAME(fw), Scr.Root, ret_g->x,
	    ret_g->y, &ret_g->x, &ret_g->y, &JunkChild);

	return True;
}

void
get_title_font_size_and_offset(MvwmWindow *fw, direction_t title_dir,
...
void
get_icon_corner(MvwmWindow *fw, rectangle *ret_g)
{
	switch (GET_TITLE_DIR(fw)) {
	case DIR_N:
	case DIR_W:
...
void
get_shaded_geometry(MvwmWindow *fw, rectangle *small_g, rectangle *big_g)
{
	size_borders    b;
	/*
...
void
get_shaded_geometry_with_dir(MvwmWindow *fw, rectangle *small_g,
...
void
get_unshaded_geometry(MvwmWindow *fw, rectangle *ret_g)
{
...
void
get_shaded_client_window_pos(MvwmWindow *fw, rectangle *ret_g)
{
	rectangle       big_g;
	size_borders    b;
	struct monitor *m = fw->m;
...
void
get_window_borders(const MvwmWindow *fw, size_borders *borders)
{
...
void
get_window_borders_no_title(const MvwmWindow *fw, size_borders *borders)
{
	borders->top_left.width = fw->boundary_width;
	borders->bottom_right.width = fw->boundary_width;
	borders->top_left.height = fw->boundary_width;
...
void
set_window_border_size(MvwmWindow *fw, int used_width)
{
	if (used_width <= 0) {
		fw->boundary_width = 0;
...
Bool
is_window_border_minimal(MvwmWindow *fw)
{
	size_borders    nt;
...
void
get_client_geometry(MvwmWindow *fw, rectangle *ret_g)
{
 ...
 * state */
void
update_relative_geometry(MvwmWindow *fw)
{
	struct monitor *m = fw->m;
	get_relative_geometry(m,
	    &fw->g.frame, (IS_MAXIMIZED(fw)) ? &fw->g.max : &fw->g.normal);
	if (IS_SHADED(fw)) {
...
void
update_absolute_geometry(MvwmWindow *fw)
{
	rectangle      *dest_g;
	rectangle       frame_g;
	struct monitor *m = fw->m;
 ...
 * more apart. */
void
maximize_adjust_offset(MvwmWindow *fw)
{
	int             off_x;
	int             off_y;
	int             dh;
...
static void
__cs_handle_aspect_ratio(size_rect * ret_s, MvwmWindow *fw, size_rect s,
    ...
    int xmotion, int ymotion, int flags)
{
	...
	rmin =
	    (double) fw->hints.min_aspect.x / (double) fw->hints.min_aspect.y;
	rmax =
	    (double) fw->hints.max_aspect.x / (double) fw->hints.max_aspect.y;
	do {
		double          r;
		...
		if (r < rmin && (flags & CS_ROUND_UP) && xmotion == 0) {
			 ...
			 */
			delta =
			    MAKEMULT(s.height * rmin - s.width, inc.width);
		...
		if (r < rmin) {
			 ...
			 */
			delta =
			    MAKEMULT(s.height - s.width / rmin, inc.height);
			if (s.height - delta >= min.height) {
				s.height -= delta;
			} else {
				delta =
				    MAKEMULT(s.height * rmin - s.width,
				    inc.width);
				if (s.width + delta <= max.width) {
					s.width += delta;
				}
		...
		if (r > rmax && (flags & CS_ROUND_UP) && ymotion == 0) {
			 ...
			 */
			delta =
			    MAKEMULT(s.width / rmax - s.height, inc.height);
		...
		if (r > rmax) {
			 ...
			 */
			delta =
			    MAKEMULT(s.width - s.height * rmax, inc.width);
			if (s.width - delta >= min.width) {
				s.width -= delta;
			} else {
				delta =
				    MAKEMULT(s.width / rmax - s.height,
				    inc.height);
				if (s.height + delta <= max.height) {
					s.height += delta;
				}
 ...
 */
void
constrain_size(MvwmWindow *fw, const XEvent *e, int *widthp, int *heightp,
    int xmotion, int ymotion, int flags)
{
	size_rect       min;
	size_rect       max;
 ...
 */
void
gravity_constrain_size(int gravity, MvwmWindow *t, rectangle *rect, int flags)
{
...
Bool
get_visible_icon_title_geometry(MvwmWindow *fw, rectangle *ret_g)
{
...
Bool
get_icon_title_geometry(MvwmWindow *fw, rectangle *ret_g)
{
...
Bool
get_visible_icon_picture_geometry(MvwmWindow *fw, rectangle *ret_g)
{
...
Bool
get_icon_picture_geometry(MvwmWindow *fw, rectangle *ret_g)
{
	if (fw->icon_g.picture_w_g.width == 0) {
		memset(ret_g, 0, sizeof(*ret_g));
		return False;
...
Bool
get_visible_icon_geometry(MvwmWindow *fw, rectangle *ret_g)
{
...
void
get_icon_geometry(MvwmWindow *fw, rectangle *ret_g)
{
	/*
	 * valid geometry?
	 */
...
Bool
get_visible_window_or_icon_geometry(MvwmWindow *fw, rectangle *ret_g)
{
	if (IS_ICONIFIED(fw)) {
...
void
move_icon_to_position(MvwmWindow *fw)
{
	...
	if (!HAS_NO_ICON_TITLE(fw)) {
		...
		XMoveWindow(dpy, FW_W_ICON_TITLE(fw),
...
void
broadcast_icon_geometry(MvwmWindow *fw, Bool do_force)
{
	rectangle       g;
	Bool            rc;
...
void
modify_icon_position(MvwmWindow *fw, int dx, int dy)
{
 ...
 * layout */
void
set_icon_position(MvwmWindow *fw, int x, int y)
{
	if (fw->icon_g.picture_w_g.width > 0) {
...
void
set_icon_picture_size(MvwmWindow *fw, int w, int h)
{
	if (fw->icon_g.picture_w_g.width > 0) {
...
void
resize_icon_title_height(MvwmWindow *fw, int dh)
{
	if (!HAS_NO_ICON_TITLE(fw)) {
...
void
get_page_offset_rectangle(int *ret_page_x, int *ret_page_y, rectangle *r)
{
	 ...
	 */
	*ret_page_x =
	    truncate_to_multiple(r->x + r->width / 2 + xoff,
	    m->coord.w) - xoff;
	*ret_page_y =
	    truncate_to_multiple(r->y + r->height / 2 + yoff,
	    m->coord.h) - yoff;
...
void
get_page_offset(int *ret_page_x, int *ret_page_y, MvwmWindow *fw)
{
	rectangle       r;
...
void
get_page_offset_check_visible(int *ret_page_x, int *ret_page_y,
    MvwmWindow *fw)
{
