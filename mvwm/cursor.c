...
/* CreateCursors - Loads mvwm cursors */
Cursor         *
...
CMD_CursorStyle(F_CMD_ARGS)
{
	...
	if (!cname) {
		mvwm_msg(ERR, "CursorStyle", "Bad cursor style");
	...
	if (StrEquals("POSITION", cname)) {
		...
		mvwm_msg(ERR, "CursorStyle", "Unknown cursor name %s", cname);
	...
	if (newcursor) {
		my_nc = mvwmCursorNameToIndex(newcursor);
	} else {
	...
	if (my_nc > -1) {
		...
		if ((nc < 0) || (nc >= XC_num_glyphs) || ((nc % 2) != 0)) {
			mvwm_msg(ERR, "CursorStyle", "Bad cursor number %s",
			    newcursor);
			free(newcursor);
		...
		if (StrEquals("none", newcursor) ||
			...
			XFillRectangle(dpy, Scr.ScratchMonoPixmap, Scr.MonoGC,
			    0, 0, 1, 1);
			cursor =
			    XCreatePixmapCursor(dpy, Scr.ScratchMonoPixmap,
	...
	if (!cursor) {
		mvwm_msg(ERR, "CursorStyle", "Cannot load cursor: %s",
		    newcursor);
		free(newcursor);
	 ...
	 */
	if (Scr.MvwmCursors[index]) {
		XFreeCursor(dpy, Scr.MvwmCursors[index]);
	}
	Scr.MvwmCursors[index] = cursor;
	...
	if (fore && back) {
		...
		XRecolorCursor(dpy, Scr.MvwmCursors[index], &(colors[0]),
		    &(colors[1]));
	}
	if (fore) {
		free(fore);
		...
		if (index == CRS_DEFAULT) {
			...
			if (IS_ICONIFIED(fw2)) {
				...
				if (FW_W_ICON_PIXMAP(fw2) != None) {
					SafeDefineCursor(FW_W_ICON_PIXMAP
					    (fw2),
					    Scr.MvwmCursors[CRS_DEFAULT]);
				}
			}
	 ...
	 */
	SetMenuCursor(Scr.MvwmCursors[CRS_MENU]);

	SafeDefineCursor(Scr.PanFrameTop.win, Scr.MvwmCursors[CRS_TOP_EDGE]);
	SafeDefineCursor(Scr.PanFrameBottom.win,
	    Scr.MvwmCursors[CRS_BOTTOM_EDGE]);
	SafeDefineCursor(Scr.PanFrameLeft.win,
	    Scr.MvwmCursors[CRS_LEFT_EDGE]);
	SafeDefineCursor(Scr.PanFrameRight.win,
	    Scr.MvwmCursors[CRS_RIGHT_EDGE]);
	/*
	 * migo (04/Nov/1999): don't annoy users which use xsetroot
	 */
	if (index == CRS_ROOT) {
		SafeDefineCursor(Scr.Root, Scr.MvwmCursors[CRS_ROOT]);
	}

	return;
}

/* Defines in which cases mvwm "grab" the cursor during execution of certain
 * functions. */
void
CMD_BusyCursor(F_CMD_ARGS)
{
	...
	while (action && *action != '\0') {
		...
		default:
			mvwm_msg(ERR, "BusyCursor", "unknown context '%s'",
			    option);
			break;
		}
		free(option);
