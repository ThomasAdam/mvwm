...
void CMD_CursorStyle(F_CMD_ARGS)
{
	...
	if (fore && back)
	{
		...
		XRecolorCursor(
			dpy, Scr.MvwmCursors[index], &(colors[0]),
			&(colors[1]));
	}
	if (fore)
	{
	...
	for (fw2 = Scr.MvwmRoot.next; fw2; fw2 = fw2->next)
	{
		...
		{
			SafeDefineCursor(
				FW_W_BUTTON(fw2, i), Scr.MvwmCursors[CRS_SYS]);
		}
		...
		{
			SafeDefineCursor(
				FW_W_FRAME(fw2), Scr.MvwmCursors[CRS_DEFAULT]);
			SafeDefineCursor(
				FW_W_PARENT(fw2), Scr.MvwmCursors[CRS_DEFAULT]);
			if (IS_ICONIFIED(fw2))
			{
	...
	SafeDefineCursor(
		Scr.PanFrameBottom.win, Scr.MvwmCursors[CRS_BOTTOM_EDGE]);
	SafeDefineCursor(Scr.PanFrameLeft.win, Scr.MvwmCursors[CRS_LEFT_EDGE]);
	SafeDefineCursor(
		Scr.PanFrameRight.win, Scr.MvwmCursors[CRS_RIGHT_EDGE]);
