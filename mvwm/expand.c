...
extern char const *const Mvwm_VersionInfo;
    ...
    const exec_context_t *exc)
{
	 ...
	 */
	switch ((i =
		GetTokenIndex(var_name, partial_function_vars, -1, &rest))) {
	case VAR_FG_CS:
	case VAR_BG_CS:
	case VAR_HILIGHT_CS:
	case VAR_SHADOW_CS:
	...
	case VAR_W_MINIICONFILE:
		if (fw && !IS_EWMH_DESKTOP(FW_W(fw))) {
			 ...
			 */
			allocated_string =
			    PictureFindImageFile(t, NULL, R_OK);
	...
	case VAR_IP_HEIGHT:
		if (!fw || IS_EWMH_DESKTOP(FW_W(fw))) {
			...
			if (get_visible_icon_picture_geometry(fw,
				&g) == False) {
				return -1;
			}
			is_numeric = True;
	...
	while (i < l) {
		if (input[i] == '$' && (!ismod || !isalpha(input[i + 1]))) {
			...
			case 'v':
				...
				sprintf(&out[j], "%s", (Mvwm_VersionInfo) ?
				    Mvwm_VersionInfo : "");
				j += strlen(&out[j]);
				i++;
				break;
