...
menu_func(F_CMD_ARGS, Bool fStaysUp)
{
	...
	if (menu == NULL) {
		if (menu_name) {
			mvwm_msg(ERR, "menu_func", "No such menu %s",
			    menu_name);
			free(menu_name);
		}
		return;
	...
	if (menu_name &&
	    set_repeat_data(menu_name,
		(fStaysUp) ? REPEAT_MENU : REPEAT_POPUP, NULL)) {
		free(menu_name);
	}
...
CMD_AddToMenu(F_CMD_ARGS)
{
	...
	if (mr && MR_MAPPED_COPIES(mr) != 0) {
		mvwm_msg(ERR, "add_item_to_menu", "menu %s is in use", token);
		return;
	}
...
CMD_DestroyMenuStyle(F_CMD_ARGS)
{
	...
	if (name == NULL) {
		mvwm_msg(ERR, "DestroyMenuStyle", "needs one parameter");
		return;
	}
...
CMD_ChangeMenuStyle(F_CMD_ARGS)
{
	...
	ms = menustyle_find(name);
	if (ms == NULL) {
		mvwm_msg(ERR, "ChangeMenuStyle", "cannot find style %s",
		    name);
		return;
	}
