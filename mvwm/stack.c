...
static void     __raise_or_lower_window(MvwmWindow *t, stack_mode_t mode,
    Bool allow_recursion, Bool is_new_window, Bool is_client_request);
static void     raise_or_lower_window(MvwmWindow *t, stack_mode_t mode,
    Bool allow_recursion, Bool is_new_window, Bool is_client_request);
#if 0
...
static int      collect_transients_recursive(MvwmWindow *t,
    MvwmWindow *list_head, int layer, stack_mode_t mode,
...
dump_stack_ring(void)
{
	...
	for (t1 = Scr.MvwmRoot.stack_next; t1 != &Scr.MvwmRoot;
	    t1 = t1->stack_next) {
...
verify_stack_ring_consistency(void)
{
	...
	for (t1 = t2->stack_next; t1 != &Scr.MvwmRoot;
	...
	for (t1 = Scr.MvwmRoot.stack_next; t1 != &Scr.MvwmRoot;
		...
		if (i == nchildren) {
			fprintf(stderr, "vsrc: window already died:"
			    " fw=%p w=0x%08x '%s'\n",
			    t1, (int) FW_W_FRAME(t1), t1->name.name);
		} else if (i >= last_index) {
			fprintf(stderr, "vsrc: window is at wrong position"
			    " in stack ring: fw=%p f=0x%08x '%s'\n",
			    t1, (int) FW_W_FRAME(t1), t1->name.name);
			dump_stack_ring();
			fprintf(stderr, "dumping X stacking order:\n");
			for (i = nchildren; i-- > 0;) {
				for (t1 = Scr.MvwmRoot.stack_next;
				    t1 != &Scr.MvwmRoot;
				    t1 = t1->stack_next) {
					/*
...
static void
add_windowlist_to_stack_ring_after(MvwmWindow *list_head,
...
static MvwmWindow *
get_transientfor_top_mvwmwindow(MvwmWindow *t)
{
	MvwmWindow     *s;
 ...
 */
static void
raise_over_unmanaged(MvwmWindow *t)
{
	int             i;
	...
	if (!is_above_unmanaged(t, &OR_Above)) {
		for (count = 0, t2 = Scr.MvwmRoot.stack_next;
...
static Bool
__is_restack_transients_needed(MvwmWindow *t, stack_mode_t mode)
{
...
static Bool
__must_move_transients(MvwmWindow *t, stack_mode_t mode)
{
	if (IS_ICONIFIED(t)) {
		return False;
	}
...
static Window
__get_stacking_sibling(MvwmWindow *fw, Bool do_stack_below)
{
	Window          w;
...
static void
__sort_transient_ring(MvwmWindow *ring)
{
	MvwmWindow     *s;
	MvwmWindow     *t;
	MvwmWindow     *u;
...
static void
__restack_window_list(MvwmWindow *r, MvwmWindow *s, int count,
...
MvwmWindow     *
__get_window_to_insert_after(MvwmWindow *fw, stack_mode_t mode)
{
	int             test_layer;
	MvwmWindow     *s;
...
static void
__mark_group_member(MvwmWindow *fw, MvwmWindow *start, MvwmWindow *end)
{
	MvwmWindow     *t;
...
static Bool
__mark_transient_subtree_test(MvwmWindow *s, MvwmWindow *start,
    MvwmWindow *end, int mark_mode, Bool do_ignore_icons,
...
static Bool
is_transient_subtree_straight(MvwmWindow *t, int layer, stack_mode_t mode,
...
static Bool
__is_restack_needed(MvwmWindow *t, stack_mode_t mode,
...
static Bool
__restack_window(MvwmWindow *t, stack_mode_t mode, Bool do_restack_transients,
    Bool is_new_window, Bool is_client_request)
{
	...
	if (do_restack_transients) {
		...
		count =
		    collect_transients_recursive(t, &tmp_r, t->layer, mode,
		    False);
		if (count == 0) {
...
static Bool
__raise_lower_recursion(MvwmWindow *t, stack_mode_t mode,
    Bool is_client_request)
{
	MvwmWindow     *t2;

	for (t2 = Scr.MvwmRoot.stack_next; t2 != &Scr.MvwmRoot;
	    t2 = t2->stack_next) {
...
static void
__raise_or_lower_window(MvwmWindow *t, stack_mode_t mode,
    Bool allow_recursion, Bool is_new_window, Bool is_client_request)
{
	MvwmWindow     *t2;
...
static void
raise_or_lower_window(MvwmWindow *t, stack_mode_t mode, Bool allow_recursion,
    Bool is_new_window, Bool is_client_request)
{
	MvwmWindow     *fw;
...
static Bool
overlap(MvwmWindow *r, MvwmWindow *s)
{
	rectangle       g1;
	rectangle       g2;
	Bool            rc;
...
ResyncXStackingOrder(void)
{
	...
	for (count = 0, t = Scr.MvwmRoot.next; t != NULL;
	    count++, t = t->next) {
		/*
		 * nothing to do here
		 */
...
static void
BroadcastRestack(MvwmWindow *s1, MvwmWindow *s2)
{
	MvwmWindow     *fw;
	int             num;
	int             i;
	...
	for (fw = s1, num = 1; fw != s2 && fw != &Scr.MvwmRoot;
...
static int
collect_transients_recursive(MvwmWindow *t, MvwmWindow *list_head, int layer,
...
static Bool
is_above_unmanaged(MvwmWindow *fw, Window *umtop)
{
	/*
...
static Bool
is_on_top_of_layer_ignore_rom(MvwmWindow *fw)
{
	MvwmWindow     *t;
	Bool            ontop = True;
	...
	if (DO_RAISE_TRANSIENT(fw)) {
		mark_transient_subtree(fw, fw->layer, MARK_RAISE, True,
		    False);
	}
...
static Bool
__is_on_top_of_layer(MvwmWindow *fw, Bool client_entered)
{
	Window          junk;
	Bool            ontop = False;
	if (bo.do_raise_over_unmanaged) {
...
void
remove_window_from_stack_ring(MvwmWindow *t)
{
...
void
add_window_to_stack_ring_after(MvwmWindow *t, MvwmWindow *add_after_win)
{
	if (IS_SCHEDULED_FOR_DESTROY(t)) {
		return;
	}
	if (t == add_after_win || t == add_after_win->stack_next) {
		 ...
		 */
		mvwm_msg(ERR, "add_window_to_stack_ring_after",
...
MvwmWindow     *
get_next_window_in_stack_ring(const MvwmWindow *t)
{
	return t->stack_next;
}

MvwmWindow     *
get_prev_window_in_stack_ring(const MvwmWindow *t)
{
	return t->stack_prev;
}

MvwmWindow     *
get_transientfor_mvwmwindow(const MvwmWindow *t)
{
	MvwmWindow     *s;
...
Bool
position_new_window_in_stack_ring(MvwmWindow *t, Bool do_lower)
{
	 ...
	 */
	raise_or_lower_window(t, (do_lower) ? SM_LOWER : SM_RAISE, False,
	    True, False);
...
void
RaiseWindow(MvwmWindow *t, Bool is_client_request)
{
	BroadcastPacket(M_RAISE_WINDOW, 3, (long) FW_W(t),
...
void
LowerWindow(MvwmWindow *t, Bool is_client_request)
{
	BroadcastPacket(M_LOWER_WINDOW, 3, (long) FW_W(t),
	    (long) FW_W_FRAME(t), (unsigned long) t);
...
void
RestackWindow(MvwmWindow *t, Bool is_client_request)
{
...
Bool
HandleUnusualStackmodes(unsigned int stack_mode, MvwmWindow *r, Window rw,
    MvwmWindow *s, Window sw)
{
	int             do_restack = 0;
	MvwmWindow     *t;
	...
	switch (stack_mode) {
	case TopIf:
		for (t = r->stack_prev; t != &Scr.MvwmRoot && !do_restack;
		    t = t->stack_prev) {
		...
		break;
	case BottomIf:
		for (t = r->stack_next; t != &Scr.MvwmRoot && !do_restack;
		    t = t->stack_next) {
			do_restack = ((s == NULL || s == t) && overlap(t, r));
		}
		if (do_restack) {
...
void
BroadcastRestackThisWindow(MvwmWindow *t)
{
...
int
compare_window_layers(MvwmWindow *t, MvwmWindow *s)
{
	return t->layer - s->layer;
}

void
set_default_layer(MvwmWindow *t, int layer)
{
...
void
set_layer(MvwmWindow *t, int layer)
{
	t->layer = layer;
	return;
}
 ...
 */
void
mark_transient_subtree(MvwmWindow *t, int layer, int mark_mode,
    Bool do_ignore_icons, Bool use_window_group_hint)
{
	MvwmWindow     *s;
	MvwmWindow     *start;
...
void
new_layer(MvwmWindow *fw, int layer)
{
	MvwmWindow     *s;
	MvwmWindow     *target;
	MvwmWindow     *prev;
	 ...
	 */
	for (target = Scr.MvwmRoot.stack_next; target != &Scr.MvwmRoot;
...
Bool
is_on_top_of_layer(MvwmWindow *fw)
{
	return __is_on_top_of_layer(fw, False);
}

Bool
is_on_top_of_layer_and_above_unmanaged(MvwmWindow *fw)
{
