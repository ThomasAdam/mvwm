...
CMD_WindowShade(F_CMD_ARGS)
{
	...
	if (StrEquals("Last", token)) {
		 ...
		 * * mvwm to reuse the last used shading direction.
	...
	mr_args =
	    frame_create_move_resize_args(fw, resize_mode, &start_g, &end_g,
	    fw->shade_anim_steps, shade_dir);
...
CMD_WindowShadeAnimate(F_CMD_ARGS)
{
	...
	mvwm_msg(ERR, "CMD_WindowShadeAnimate",
