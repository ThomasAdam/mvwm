...
static void
fake_map_unmap_notify(const MvwmWindow *fw, int event_type)
{
	XEvent          client_event;
...
static Bool
test_map_request(Display *display, XEvent *event, XPointer arg)
{
...
static Bool
test_withdraw_request(Display *display, XEvent *event, XPointer arg)
{
	check_if_event_args *cie_args;
	Bool            rc;
	...
	if (event->type == UnmapNotify &&
	    ...
	    event->xunmap.event == FW_W(&Scr.MvwmRoot)) {
...
Bool
test_typed_window_event(Display *display, XEvent *event, XPointer arg)
{
	test_typed_window_event_args *ta =
	    (test_typed_window_event_args *) arg;
...
static inline void
__handle_cr_on_icon(XConfigureRequestEvent * cre, MvwmWindow *fw)
{
	XWindowChanges  xwc;
	unsigned long   xwcm;
...
static inline void
__handle_cr_on_shaped(MvwmWindow *fw)
{
	/*
...
static inline void
__handle_cr_restack(int *ret_do_send_event, XConfigureRequestEvent * cre,
    MvwmWindow *fw)
{
	XWindowChanges  xwc;
	unsigned long   xwcm;
	...
	if (cre->value_mask & CWSibling) {
		if (XFindContext(dpy, cre->above, MvwmContext,
			(caddr_t *) & fw2) == XCNOENT) {
			fw2 = NULL;
		}
		if (fw2 == fw) {
	 ...
	 */
		...
		xwcm = CWSibling | CWStackMode;
		if (FW_W_ICON_TITLE(fw) != None) {
			XConfigureWindow(dpy, FW_W_ICON_TITLE(fw), xwcm,
			    &xwc);
		}
		if (FW_W_ICON_PIXMAP(fw) != None) {
			XConfigureWindow(dpy, FW_W_ICON_PIXMAP(fw), xwcm,
			    &xwc);
		}
		/*
		 * 2. for fw2
...
static inline void
__cr_get_static_position(rectangle *ret_g, MvwmWindow *fw,
...
static inline void
__cr_get_grav_position(rectangle *ret_g, MvwmWindow *fw,
    XConfigureRequestEvent * cre, size_borders *b)
{
	int             grav_x;
	int             grav_y;
...
static inline void
__cr_detect_icccm_move(MvwmWindow *fw, XConfigureRequestEvent * cre,
    size_borders *b)
{
	rectangle       grav_g;
	rectangle       static_g;
	...
	if (HAS_EWMH_WM_PID(fw)) {
		if (bo.do_debug_cr_motion_method == 1) {
			fprintf(stderr, "_cdim: +++ has ewmh_wm_pid: icccm"
			    " %p '%s'\n", fw, fw->visible_name);
		}
	...
	if (fw->ewmh_window_type != EWMH_WINDOW_TYPE_NONE_ID) {
		if (bo.do_debug_cr_motion_method == 1) {
			fprintf(stderr, "_cdim: +++ has ewmh_window_type:"
			    " icccm %p '%s'\n", fw, fw->visible_name);
		}
		SET_CR_MOTION_METHOD(fw, CR_MOTION_METHOD_USE_GRAV);
		SET_CR_MOTION_METHOD_DETECTED(fw, 1);
	...
	if (FShapesSupported && fw->wShaped) {
		if (bo.do_debug_cr_motion_method == 1) {
			fprintf(stderr, "_cdim: --- shaped window %p "
			    "'%s'\n", fw, fw->visible_name);
		}
		/*
	...
	if (!has_x && !has_y) {
		if (bo.do_debug_cr_motion_method == 1) {
			fprintf(stderr, "_cdim: --- not moved %p '%s'\n",
			    fw, fw->visible_name);
		}
		return;
	}
	...
	if (bo.do_debug_cr_motion_method == 1) {
		fprintf(stderr, "s %3d/%3d %2d/%2d, g %3d/%3d %2d/%2d: ",
	...
	if ((cre->value_mask & (CWX | CWY)) == (CWX | CWY) &&
	    (has_x || has_y) &&
	    cre->width == mon->coord.w && cre->height == mon->coord.h) {
		if (grav_g.x == -b->top_left.width &&
			...
			SET_CR_MOTION_METHOD_DETECTED(fw, 1);
			if (bo.do_debug_cr_motion_method == 1) {
				fprintf(stderr, "+++ fullscreen icccm %p"
				    " '%s'\n", fw, fw->visible_name);
			}
			return;
	...
	if (has_x && dg_g.x == 0 && ds_g.x != 0 &&
		...
		SET_CR_MOTION_METHOD_DETECTED(fw, 1);
		if (bo.do_debug_cr_motion_method == 1) {
			fprintf(stderr, "+++ travelling icccm %p '%s'\n",
			    fw, fw->visible_name);
		}
		return;
	}
	if (has_x && dg_g.x != 0 && ds_g.x == 0 &&
		...
		SET_CR_MOTION_METHOD_DETECTED(fw, 1);
		if (bo.do_debug_cr_motion_method == 1) {
			fprintf(stderr, "+++ travelling traditional %p"
			    " '%s'\n", fw, fw->visible_name);
		}
		return;
	}
	...
	if (m != CR_MOTION_METHOD_AUTO) {
		...
		if (m == my || my == CR_MOTION_METHOD_AUTO) {
			...
			SET_CR_MOTION_METHOD_DETECTED(fw, 1);
			if (bo.do_debug_cr_motion_method == 1) {
				fprintf(stderr, "+++ near border %s %p "
...
static inline int
__merge_cr_moveresize(const evh_args_t *ea, XConfigureRequestEvent * cre,
    MvwmWindow *fw, size_borders *b)
{
	int             cn_count = 0;
	XEvent          e;
	...
	for (cn_count = 0; 1;) {
		...
		FCheckPeekIfEvent(dpy, &e, test_resizing_event,
		    (XPointer) &args);
...
static inline int
__handle_cr_on_client(int *ret_do_send_event, XConfigureRequestEvent cre,
    const evh_args_t *ea, MvwmWindow *fw, Bool force, int force_gravity)
{
	rectangle       current_g;
	rectangle       new_g;
	rectangle       d_g;
...
void
__handle_configure_request(XConfigureRequestEvent cre, const evh_args_t *ea,
    MvwmWindow *fw, Bool force, int force_gravity)
{
	int             do_send_event = 0;
	int             cn_count = 0;
...
static Bool
__predicate_button_click(Display *display, XEvent *event, XPointer arg)
{
	if (event->type == ButtonPress || event->type == ButtonRelease) {
		return True;
	}
...
__test_for_motion(int x0, int y0)
{
	 ...
	 * * to check for a button press in the future to avoid a mvwm
	 * * lockup! (olicha 2004-01-31)
	 */

	for (x = x0, y = y0;
	    FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild, &JunkX,
		&JunkY, &x, &y, &mask) == True; usleep(20000)) {
		...
		if (FCheckPeekIfEvent(dpy, &e, __predicate_button_click,
			NULL)) {
			/*
			 * click in the future
			 */
...
/* Helper function for __handle_focus_raise_click(). */
static void
__check_click_to_focus_or_raise(hfrc_ret_t * ret_args,
...
/*  RBW -- Dang! This works without the one in HandleEnterNotify!  */
	...
	if (ret_args->do_focus || ret_args->do_raise) {
		if (!((ret_args->do_focus &&
			...
			|| (ret_args->do_raise
			    &&
			    FP_DO_ALLOW_FUNC_RAISE_CLICK(FW_FOCUS_POLICY
				(fw))))) {
...
__handle_bpress_action(const exec_context_t *exc, char *action)
{
	 ...
	 */
	PressedW = None;
	if (check_if_mvwm_window_exists(exc->w.fw)) {
		part = border_context_to_parts(exc->w.wcontext);
		do_force = (part & PART_TITLEBAR) ? True : False;
		border_draw_decorations(exc->w.fw, part,
		    (Scr.Hilite == exc->w.fw), do_force, CLEAR_ALL, NULL,
		    NULL);
	}
...
__handle_bpress_on_root(const exec_context_t *exc)
{
	 ...
	 */
	action =
	    CheckBinding(Scr.AllBindings, exc->x.etrigger->xbutton.button,
...
{
	FOCUS_SET(Scr.StolenFocusWin, Scr.StolenFocusMvwmWin);
...
HandleConfigureRequest(const evh_args_t *ea)
{
	 ...
	 * * window may be wrong.
	 */
	if (XFindContext(dpy, cre.window, MvwmContext, (caddr_t *) & fw) ==
	    XCNOENT) {
		fw = NULL;
	}
...
HandleEnterNotify(const evh_args_t *ea)
{
	...
	ewp = &te->xcrossing;
	ENTER_DBG((stderr,
		"++++++++ en (%d): fw %p w 0x%08x sw 0x%08x mode 0x%x detail 0x%x '%s'\n",
		++ecount, fw, (int) ewp->window, (int) ewp->subwindow,
		ewp->mode, ewp->detail, fw ? fw->visible_name : "(none)"));
	...
	if (!fMvwmInStartup && !is_tear_off_menu &&
	...
	if (is_pan_frame(ewp->window)) {
		...
		if (edge_command && ewp->mode == NotifyUngrab &&
			...
			scr_flags.is_pointer_on_this_screen = 1;
			e = *te;
			HandlePaging(&e, m->virtual_scr.EdgeScrollX,
	...
	if (FP_DO_FOCUS_ENTER(FW_FOCUS_POLICY(fw))) {
		...
		if (ewp->window == FW_W(fw)) {
			 ...
			 * *sure yet.
			 */
			SetFocusWindowClientEntered(fw, True,
			    FOCUS_SET_BY_ENTER);
#else
		 ...
		 * * focused mvwm window.  Motif apps may lose the input focus
	...
	/*
	 * We get an EnterNotify with mode == UnGrab when mvwm releases the
...
HandleFocusIn(const evh_args_t *ea)
{
	...
	Pixel           fc = 0;
	Pixel           bc = 0;
	MvwmWindow     *ffw_old = get_focus_window();
	MvwmWindow     *sf;
	...
	/**/ if (XFindContext(dpy, w, MvwmContext,
		(caddr_t *) & fw) == XCNOENT) {
		fw = NULL;
	}
	...
	if (fw && Scr.focus_in_requested_window != fw &&
		...
		if (DEBUG_GLOBALLY_ACTIVE) {
			fprintf(stderr, "prevented globally active fw %p (%s)"
	...
	if (!fw) {
		if (w != Scr.NoFocusWin) {
			...
			Scr.StolenFocusMvwmWin = ffw_old;
			focus_w = w;
		...
		/*
		 * Not very useful if no window that mvwm and its modules know
		 * * about has the focus.
		 */
	     ...
	     * * was focused. Otherwise mvwm would believe that Scr.Hilite
...
__handle_key(const evh_args_t *ea, Bool is_press)
{
	...
	int             kcontext;
	const XEvent   *te = ea->exc->x.etrigger;
	const MvwmWindow *const fw = ea->exc->w.fw;
	 ...
	 */
	kc = XKeysymToKeycode(dpy, mvwm_KeycodeToKeysym(dpy, te->xkey.keycode,
		0, 0));
...
HandleLeaveNotify(const evh_args_t *ea)
{
	...
	ENTER_DBG((stderr,
		"-------- ln (%d): fw %p w 0x%08x sw 0x%08x mode 0x%x detail 0x%x '%s'\n",
	...
	if (lwp->window == Scr.Root &&
		 ...
		 */
		BroadcastPacket(MX_LEAVE_WINDOW, 3, (long) Scr.Root,
		    (long) NULL, (long) NULL);
	...
	if (lwp->window == Scr.Root &&
	    /*
	     * domivogt (16-May-2000): added this test because somehow mvwm
		...
		if (lwp->mode == NotifyNormal) {
			if (lwp->detail != NotifyInferior) {
				MvwmWindow     *sf = get_focus_window();
    ...
    MvwmWindow *ReuseWin, initial_window_options_t *win_opts)
{
	...
	ew = ea->exc->w.w;
	if (ReuseWin == NULL) {
		if (XFindContext(dpy, ew, MvwmContext, (caddr_t *) & fw) ==
		    XCNOENT) {
			fw = NULL;
		}
	...
	if (!fw || (fw && DO_REUSE_DESTROYED(fw))) {
		...
		if (FCheckIfEvent(dpy, &dummy, test_withdraw_request,
			(XPointer) &args)) {
			/*
			...
			if (XGetWindowProperty(dpy, ew, _XA_WM_STATE, 0L, 3L,
				False, _XA_WM_STATE, &atype, &aformat,
				&nitems, &bytes_remain, &prop)
			    == Success) {
				if (prop != NULL) {
					XFree(prop);
					XDeleteProperty(dpy, ew,
					    _XA_WM_STATE);
				} else {
					XPropertyEvent  ev;
	...
	if (IS_ICONIFIED(fw)) {
		...
		default:
			...
			if ((initial_map_command != NULL) &&
				...
				execute_function_override_window(NULL,
				    ea->exc, (char *) initial_map_command, 0,
				    fw);
			}
		...
		case IconicState:
			...
			if (win_opts->flags.is_iconified_by_parent ||
			    ((tmp = get_transientfor_mvwmwindow(fw)) &&
				IS_ICONIFIED(tmp))) {
...
HandlePropertyNotify(const evh_args_t *ea)
{
	...
	if (te->xproperty.window == Scr.Root &&
		 ...
		 */
		/*
		 * _XA_XSETROOT_ID is used by mvwm-root, xli and more (xv sends
	...
	case XA_WM_NAME:
		 ...
		 */
		if (!IS_ICONIFIED(fw)) {
			border_draw_decorations(fw, PART_TITLE,
	...
	case XA_WM_NORMAL_HINTS:
		 ...
		 * * request, due to XSizeHints disallowing resize -- MVWM would
		 * * always use old values if the application decided to toggle
		 * * such things, and MVWM would then never resize the window.
		 * *
...
HandleReparentNotify(const evh_args_t *ea)
{
	...
	if (te->xreparent.parent != FW_W_FRAME(fw)) {
		...
		if (!IS_TEAR_OFF_MENU(fw)) {
			...
			XSelectInput(dpy, te->xreparent.window,
			    XEVMASK_MENUW);
		}
...
HandleUnmapNotify(const evh_args_t *ea)
{
	 ...
	 * unmapped (which is the case for mvwm for IconicState).
	 * Unfortunately, we looked for the MvwmContext using that field, so
	 * try the window field also. */
	weMustUnmap = 0;
	if (!fw) {
		weMustUnmap = 1;
		if (XFindContext(dpy, te->xunmap.window, MvwmContext,
			(caddr_t *) & fw) == XCNOENT) {
			return;
		}
	}
	...
	if (weMustUnmap) {
		 ...
		 */
		FCheckIfEvent(dpy, &dummy, test_map_request,
		    (XPointer) &args);
		/*
	 ...
	 *
	 * We need to reparent the window back to the root (so that mvwm exiting
 ...
 *  before sending. */
void
SendConfigureNotify(MvwmWindow *fw, int x, int y, int w, int h, int bw,
...
InitEventHandlerJumpTable(void)
{
	...
	if (register_event_group(0, LASTEvent, EventHandlerJumpTable)) {
		 ...
		 * should never happen
		 */
		mvwm_msg(ERR, "InitEventHandlerJumpTable",
...
dispatch_event(XEvent *e)
{
	...
	if (w == Scr.Root ||
	    XFindContext(dpy, w, MvwmContext, (caddr_t *) & fw) == XCNOENT) {
		fw = NULL;
	}
...
void
events_handle_configure_request(XConfigureRequestEvent cre, MvwmWindow *fw,
...
My_XNextEvent(Display *dpy, XEvent *event)
{
	 ...
	 * * mvwm to die, some might be harmless. Harmless interruptions
	...
	do {
		...
		num_fd =
		    mvwmSelect(mvwmlib_max_fd, &in_fdset, &out_fdset, 0,
		    timeoutP);
 ...
 *
 *  Procedure:
 *      Find the Mvwm context for the event.
 *
 */
int
GetContext(MvwmWindow **ret_fw, MvwmWindow *t, const XEvent *e, Window *w)
{
	int             context;
	Window          win;
	Window          subw = None;
	...
	case KeyRelease:
		...
		if (win == Scr.Root && subw != None) {
			 ...
			 */
			win = subw;
			XTranslateCoordinates(dpy, Scr.Root, subw, x, y, &x,
			    &y, &subw);
			XFindContext(dpy, win, MvwmContext, (caddr_t *) & t);
		}
		is_key_event = True;
		/*
		 * fall through
	...
	case ButtonRelease:
		...
		if (t && win == FW_W_FRAME(t) && subw != None) {
			...
			if (win == FW_W_PARENT(t)) {
				win = subw;
				XTranslateCoordinates(dpy, FW_W_PARENT(t),
				    subw, x, y, &x, &y, &subw);
			}
		}
		break;
...
CoerceEnterNotifyOnCurrentWindow(void)
{
	...
	if (XFindContext(dpy, child, MvwmContext,
		(caddr_t *) & fw) == XCNOENT) {
		fw = NULL;
	} else {
...
int
flush_property_notify(Atom atom, Window w)
{
	XEvent          e;
	int             count;
	 ...
	 */
	while (FCheckIfEvent(dpy, &e, test_typed_window_event,
		(XPointer) &args))
		count++;
	...
	for (count = 0, use_wait_cursor = 0; mask != 0; count++) {
		...
		if (FCheckMaskEvent(dpy, evmask, &e)) {
			...
			if (FQueryPointer(dpy, Scr.Root, &JunkRoot,
				&JunkChild, &JunkX, &JunkY, &JunkX, &JunkY,
				&mask) == False) {
				/*
...
Bool
is_resizing_event_pending(MvwmWindow *fw)
{
	XEvent          e;
	check_if_event_args args;
