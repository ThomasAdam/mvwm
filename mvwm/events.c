...
static void fake_map_unmap_notify(const MvwmWindow *fw, int event_type)
{
	XEvent client_event;
...
static inline void _handle_cr_on_icon(XEvent *e, MvwmWindow *fw)
{
	XConfigureRequestEvent *cre = &e->xconfigurerequest;
	XWindowChanges xwc;
	unsigned long xwcm;
...
static inline void _handle_cr_on_shaped(MvwmWindow *fw)
{
...
static inline void _handle_cr_restack(
	int *ret_do_send_event, XEvent *e, MvwmWindow *fw)
{
	XConfigureRequestEvent *cre = &e->xconfigurerequest;
	XWindowChanges xwc;
	unsigned long xwcm;
...
static inline void _cr_get_static_position(
	rectangle *ret_g, MvwmWindow *fw, XEvent *e, size_borders *b)
{
	XConfigureRequestEvent *cre = &e->xconfigurerequest;
...
static inline void _cr_detect_icccm_move(
	MvwmWindow *fw, XEvent *e, size_borders *b)
{
	XConfigureRequestEvent *cre = &e->xconfigurerequest;
	rectangle grav_g;
	rectangle static_g;
...
static inline int _merge_cr_moveresize(
	const evh_args_t *ea, XEvent *ev, MvwmWindow *fw, size_borders *b)
{
	_merge_cr_args args;
...
void _handle_configure_request(
	XEvent *e, const evh_args_t *ea, MvwmWindow *fw,
...
void HandleFocusIn(const evh_args_t *ea)
{
	...
	/**/
	if (XFindContext(dpy, w, MvwmContext, (caddr_t *) &fw) == XCNOENT)
	{
		fw = NULL;
	}
...
void __handle_key(const evh_args_t *ea, Bool is_press)
{
	...
	kc = XKeysymToKeycode(dpy, mvwm_KeycodeToKeysym(dpy, te->xkey.keycode, 0, 0));
...
void HandleMapRequestKeepRaised(
	const evh_args_t *ea, Window KeepRaised, MvwmWindow *ReuseWin,
	initial_window_options_t *win_opts)
{
	Bool is_on_this_page = False;
...
void HandleUnmapNotify(const evh_args_t *ea)
{
	...
	if (!fw)
	{
		weMustUnmap = 1;
		if (XFindContext(
			    dpy, te->xunmap.window, MvwmContext,
			    (caddr_t *)&fw) == XCNOENT)
		{
			return;
		}
...
void SendConfigureNotify(
	MvwmWindow *fw, int x, int y, int w, int h, int bw,
...
int register_event_group(int event_base, int event_count, PFEH *jump_table)
{
	...
	group = mvwm_malloc(sizeof *group);
...
void events_handle_configure_request(
	XEvent *cre, MvwmWindow *fw, Bool force, int force_gravity)
{
...
int My_XNextEvent(Display *dpy, XEvent *event)
{
	...
	do
	{
		...
		num_fd = mvwmSelect(
			mvwmlib_max_fd, &in_fdset, &out_fdset, 0, timeoutP);
 ...
 *
 */
int GetContext(MvwmWindow **ret_fw, MvwmWindow *t, const XEvent *e, Window *w)
{
	int context;
	Window win;
	Window subw = None;
...
void CoerceEnterNotifyOnCurrentWindow(void)
{
	...
	if (XFindContext(dpy, child, MvwmContext, (caddr_t *)&fw) == XCNOENT)
	{
		fw = NULL;
	}
	else
...
Bool is_resizing_event_pending(MvwmWindow *fw)
{
	XEvent e;
	check_if_event_args args;
