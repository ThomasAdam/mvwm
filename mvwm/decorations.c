 ...
 * hints from a window, and makes necessary adjustments for mvwm.
 *
 */
 ...
 */
void
SelectDecor(MvwmWindow *t, window_style *pstyle, short *buttons)
{
	int             decor;
	int             i;
	int             border_width;
	...
	*buttons = (1 << NUMBER_OF_TITLE_BUTTONS) - 1;
	 ...
	 */
	if (decor & (MWM_DECOR_MENU | MWM_DECOR_MINIMIZE |
		MWM_DECOR_MAXIMIZE)) {
		decor |= MWM_DECOR_TITLE;
	}
	...
	SET_HAS_NO_BORDER(t, S_HAS_NO_BORDER(SFC(*sflags))
	    || used_width <= 0);
	if (HAS_NO_BORDER(t)) {
		used_width = 0;
	}
	...
	if (!(decor & MWM_DECOR_MINIMIZE)) {
		...
		int             i;
		for (i = 0; i < NUMBER_OF_TITLE_BUTTONS; ++i) {
			if (TB_HAS_MWM_DECOR_MINIMIZE(GetDecor(t,
				    buttons[i]))) {
				*buttons &= ~(1 << i);
			}
		}
	...
	if (!(decor & MWM_DECOR_MAXIMIZE)) {
		...
		int             i;
		for (i = 0; i < NUMBER_OF_TITLE_BUTTONS; ++i) {
			if (TB_HAS_MWM_DECOR_MAXIMIZE(GetDecor(t,
				    buttons[i]))) {
				*buttons &= ~(1 << i);
			}
		}
...
static Bool
__is_resize_allowed(const MvwmWindow *t, int functions,
...
*/
Bool
is_function_allowed(int function, char *action_string, const MvwmWindow *t,
