		...
		if (binding->Context & C_ICON) {
			if (FW_W_ICON_TITLE(t) != None) {
				GrabWindowKeyOrButton(dpy, FW_W_ICON_TITLE(t),
				    binding, C_ICON, GetUnusedModifiers(),
				    None, do_grab);
			}
			if (FW_W_ICON_PIXMAP(t) != None) {
				GrabWindowKeyOrButton(dpy,
				    FW_W_ICON_PIXMAP(t), binding, C_ICON,
				    GetUnusedModifiers(), None, do_grab);
			}
		}
		if (IS_EWMH_DESKTOP(FW_W(t)) &&
    ...
    unsigned short *buttons_grabbed, Bool is_silent)
{
	 ...
	 */
	if (p == NULL) {
		mvwm_msg(ERR, "ParseBinding", "empty %s binding, ignored\n",
		    tline);
	...
	if (wcontext_string_to_wcontext(context_string, &context)
	    && !is_silent) {
		mvwm_msg(WARN, "ParseBinding", "Illegal context in line %s",
		    tline);
	}
	if (modifiers_string_to_modmask(modifier_string, &modifier) &&
	    !is_silent) {
		mvwm_msg(WARN, "ParseBinding", "Illegal modifier in line %s",
		    tline);
	}

	if (BIND_IS_KEY_BINDING(type)) {
		keysym = MvwmStringToKeysym(dpy, key_string);
		/*
	...
	if ((modifier & AnyModifier) && (modifier & (~AnyModifier))) {
		mvwm_msg(WARN, "ParseBinding", "Binding specified AnyModifier"
...
binding_cmd(F_CMD_ARGS, binding_t type)
{
	...
	count =
	    ParseBinding(dpy, &Scr.AllBindings, action, type,
	    &Scr.nr_left_buttons, &Scr.nr_right_buttons, &btg,
	    scr_flags.are_functions_silent);
