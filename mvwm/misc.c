...
change_grab_cursor(int cursor)
{
	if (cursor != None) {
		XChangeActivePointerGrab(dpy, GRAB_EVMASK,
		    Scr.MvwmCursors[cursor], CurrentTime);
	}
...
GrabEm(int cursor, int grab_context)
{
	...
	if (grab_context <= GRAB_STARTUP || grab_context >= GRAB_MAXVAL) {
		mvwm_msg(ERR, "GrabEm", "Bug: Called with illegal context %d",
		    grab_context);
		return False;
	}
	...
	while (i < rep &&
	    (val =
		XGrabPointer(dpy, grab_win, True, GRAB_EVMASK, GrabModeAsync,
		    GrabModeAsync, None,
		    (grab_context ==
			GRAB_FREEZE_CURSOR) ? None : Scr.MvwmCursors[cursor],
		    CurrentTime) != GrabSuccess)) {
		switch (val) {
...
UngrabEm(int ungrab_context)
{
	if (ungrab_context <= GRAB_ALL || ungrab_context >= GRAB_MAXVAL) {
		mvwm_msg(ERR, "UngrabEm",
		    "Bug: Called with illegal context %d", ungrab_context);
		return False;
	}
...
#ifndef mvwm_msg	/* Some ports (i.e. VMS) define their own version */
/*
** mvwm_msg: used to send output from mvwm to files and or stderr/stdout
**
...
*/
static char    *mvwm_msg_strings[] = {
	"<<DEBUG>> ", "", "", "<<WARNING>> ", "<<DEPRECATED>> ", "<<ERROR>> "
};

void
mvwm_msg(mvwm_msg_t type, char *id, char *msg, ...)
{
	va_list         args;
	char           *mfmt;
	char            mvwm_id[20];
	...
	if (Scr.NumberOfScreens > 1) {
		sprintf(&mvwm_id[strlen(mvwm_id)], ".%d", (int) Scr.screen);
	}
	...
	if (type == ERR) {
		...
		sprintf(tmp, "[%s][%s]: %s",
		    mvwm_id, id, mvwm_msg_strings[(int) type]);
		va_start(args, msg);
		...
		fprintf(stderr, "%s[%s][%s]: %s",
		    time_str, mvwm_id, id, mvwm_msg_strings[(int) type]);
 ...
 */
void
Keyboard_shortcuts(XEvent *ev, MvwmWindow *fw, int *x_defect, int *y_defect,
    int ReturnEvent)
{
	int             x_move_size = 0;
	int             y_move_size = 0;
	...
	mvwmlib_keyboard_shortcuts(dpy, Scr.screen, ev, x_move_size,
...
/*
 *
 * Check if the given MvwmWindow structure still points to a valid window.
 *
 */

Bool
check_if_mvwm_window_exists(MvwmWindow *fw)
{
	MvwmWindow     *t;
...
IsRectangleOnThisPage(struct monitor *m, const rectangle *rec, int desk)
{
	...
	return (desk == m->virtual_scr.CurrentDesk &&
	    ...
	    (rec->y < 0 || rec->y < m->coord.h)) ? True : False;
}

/* returns the MvwmWindow that contains the pointer or NULL if none */
MvwmWindow     *
get_pointer_mvwm_window(void)
{
	int             x, y;
	Window          win;
	Window          ancestor;
	...
	if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &win, &JunkX, &JunkY,
		&x, &y, &JunkMask) == False) {
		/*
	...
	for (t = NULL; win != Scr.Root && win != None; win = ancestor) {
		...
		if (XFindContext(dpy, win, MvwmContext, (caddr_t *) & t) !=
		    XCNOENT) {
			/*
