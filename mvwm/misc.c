...
static void change_grab_cursor(int cursor)
{
	if (cursor != None)
	{
		XChangeActivePointerGrab(
			dpy, GRAB_EVMASK, Scr.MvwmCursors[cursor], CurrentTime);
	}
...
Bool GrabEm(int cursor, int grab_context)
{
	...
	while (i < rep &&
	       (val = XGrabPointer(
		      ...
		      None : Scr.MvwmCursors[cursor], CurrentTime) !=
		GrabSuccess))
	{
		switch (val)
		{
...
void mvwm_msg(mvwm_msg_t type, char *id, char *msg, ...)
{
	va_list args;
	char *mfmt;
	char mvwm_id[20];
...
void mvwm_msg_report_app_and_workers(void)
{
	fprintf(
		stderr,
...
void Keyboard_shortcuts(
	XEvent *ev, MvwmWindow *fw, int *x_defect, int *y_defect,
	int ReturnEvent)
{
	int x_move_size = 0;
	int y_move_size = 0;
...
Bool check_if_mvwm_window_exists(MvwmWindow *fw)
{
	MvwmWindow *t;
...
MvwmWindow *get_pointer_mvwm_window(void)
{
	int x,y;
	Window win;
	Window ancestor;
