...
/*
 *
 * mvwm per-screen data include file
 *
 */
...
typedef struct MvwmDecor
{
	...
	struct MvwmDecor *next;       /* additional user-defined styles */
	struct
	{
		unsigned has_changed : 1;
...
typedef struct ScreenInfo
{
	...
	/* the head of the mvwm window list */
	MvwmWindow MvwmRoot;
	/* the root window */
	Window Root;
	...
	int root_pushes;
	/* current push level to install mvwm colormap windows */
	int mvwm_pushes;
	/* saved window to install when pushes drops to zero */
	const MvwmWindow *pushed_window;
	Cursor *MvwmCursors;
	...
	/* the mvwm window that is highlighted except for networking delays,
	 * this is the window which REALLY has the focus */
	MvwmWindow *Hilite;
	/* None, if the focus is nowhere or on an mvwm managed window. Set to
	...
	MvwmWindow *StolenFocusMvwmWin;
	MvwmWindow *focus_in_pending_window;
	MvwmWindow *focus_in_requested_window;
	unsigned short buttons2grab;
	int NumBoxes;
	/* values used for CascadePlacement */
	int cascade_x;
	int cascade_y;
	MvwmWindow *cascade_window;
...
extern struct bug_opts bo;
extern struct global_style_opts gso;
extern struct screen_flags scr_flags;
