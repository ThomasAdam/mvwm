...
colormap_handle_colormap_notify(const evh_args_t *ea)
{
	...
	while (FCheckTypedEvent(dpy, ColormapNotify, &evdummy)) {
		if (XFindContext(dpy, cevent->window, MvwmContext,
			(caddr_t *) & fw) == XCNOENT) {
			fw = NULL;
		}
		if ((fw) && (cevent->new)) {
...
/*  Procedure:
 *      InstallWindowColormaps - install the colormaps for one mvwm window
 *
 *  Inputs:
 ...
 */
void
InstallWindowColormaps(const MvwmWindow *fw)
{
	int             i;
	...
	/*
	 * If no window, then install mvwm colormap
	 */
	if (!fw) {
		fw = &Scr.MvwmRoot;
	}
	...
	/*
	 * Don't load any new colormap if root/mvwm colormap(s) has been
	 * * force loaded.
	 */
	if (Scr.root_pushes || Scr.mvwm_pushes || client_controls_colormaps) {
		return;
	}
 ...
 *
 * {Uni,I}nstall{Root,Mvwm}Colormap calls may be freely intermixed
 */
void
 ...
 * or the mvwm colormap if mvwm has a menu posted
 */
void
...
/*  Procedures:
 *      {Uni/I}nstallMvwmColormap - Force (un)loads mvwm colormap(s)
 *      This is used to ensure the mvwm colormap is installed during
 *      menu operations
 */
void
InstallMvwmColormap(void)
{
	if (last_cmap != Pcmap) {
		last_cmap = Pcmap;
		XInstallColormap(dpy, last_cmap);
 ...
 */
void
FetchWmColormapWindows(MvwmWindow *fw)
{
