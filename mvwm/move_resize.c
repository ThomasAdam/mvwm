...
float           rgpctMovementDefault[32] = {
	-.01, 0, .01, .03, .08, .18, .3, .45, .60, .75, .85, .90, .94, .97,
	    .99, 1.0
	    /*
	     * must end in 1.0
	     */
...
draw_move_resize_grid(int x, int y, int width, int height)
{
	...
	if (move_resize_grid.flags.is_enabled
		...
		nrects +=
		    get_outline_rects(&(rects[0]), move_resize_grid.geom.x,
 ...
 */
int
GetMoveArguments(char **paction, int w, int h, int *pFinalX, int *pFinalY,
...
static int
ParseOneResizeArgument(char *arg, int scr_size, int base_size, int size_inc,
    int add_size, int *ret_size)
{
	float           factor;
	int             val;
	int             add_base_size = 0;
...
static int
GetResizeArguments(char **paction, int x, int y, int w_base, int h_base,
    int w_inc, int h_inc, size_borders *sb, int *pFinalW, int *pFinalH,
    direction_t * ret_dir, Bool *is_direction_fixed, Bool *do_warp_to_border,
    Bool *automatic_border_direction, Bool *detect_automatic_direction)
{
	int             n;
	char           *naction;
	char           *token;
	...
	*detect_automatic_direction = False;
	...
	for (;; token = PeekToken(naction, &naction)) {
		if (StrEquals(token, "frame")) {
			...
			*ret_dir =
			    gravity_parse_dir_argument(naction, &naction,
			    DIR_NONE);
			if (*ret_dir != DIR_NONE) {
	...
	*paction = naction;
	...
	n = 0;
	n += ParseOneResizeArgument(s1, m->coord.w, w_base, w_inc, w_add,
	    pFinalW);
	n += ParseOneResizeArgument(s2, m->coord.h, h_base, h_inc, h_add,
	    pFinalH);
	if (s1 != NULL) {
		free(s1);
	}
...
static int
GetResizeMoveArguments(char **paction, int w_base, int h_base, int w_inc,
    int h_inc, size_borders *sb, int *pFinalX, int *pFinalY, int *pFinalW,
    int *pFinalH, Bool *fWarp, Bool *fPointer)
{
	...
	if (GetResizeArguments(&action, *pFinalX, *pFinalY, w_base, h_base,
		w_inc, h_inc, sb, pFinalW, pFinalH, &dir, &dummy, &dummy,
		&dummy, &dummy) < 2) {
		return 0;
	}
	if (GetMoveArguments(&action, *pFinalW, *pFinalH, pFinalX, pFinalY,
		fWarp, NULL, True) < 2) {
		return 0;
	}
	*paction = action;
...
position_geometry_window(const XEvent *eventp)
{
	...
	/*
	 * Probably should remove this positioning code from {builtins,mvwm}.c?
	 */
...
resize_geometry_window(void)
{
	...
	if (cset >= 0) {
		SetWindowBackground(dpy, Scr.SizeWindow, w, h,
 ...
 *
 *  Inputs:
 *      tmp_win - the current mvwm window
 *      x, y    - position of the window
 *
 */

static void
DisplayPosition(const MvwmWindow *tmp_win, const XEvent *eventp, int x, int y,
    int Init)
{
	char            str[100];
	int             offset;
 ...
 */
static void
DisplaySize(const MvwmWindow *tmp_win, const XEvent *eventp, int width,
...
resize_move_window(F_CMD_ARGS)
{
	...
	n = GetResizeMoveArguments(&action,
	...
	if (IS_SHADED(fw)) {
		frame_setup_window(fw, FinalX, FinalY, FinalW,
    ...
    int *FinalY, Bool do_start_at_pointer)
{
	...
	if (do_start_at_pointer) {
		if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild,
			&DragX, &DragY, &JunkX, &JunkY, &JunkMask) == False) {
			/*
		 ...
		 * * faked.
		 */
		fev_get_evpos_or_query(dpy, Scr.Root, exc->x.elast, &DragX,
		    &DragY);
	}
 ...
 * entry */
static void
AnimatedMoveAnyWindow(MvwmWindow *fw, Window w, int startX, int startY,
    int endX, int endY, Bool fWarpPointerToo, int cmsDelay,
    float *ppctMovement, MenuRepaintTransparentParameters *pmrtp)
{
	...
	do {
		...
		if (fWarpPointerToo == True) {
			if (FQueryPointer(dpy, Scr.Root, &JunkRoot,
				&JunkChild, &JunkX, &JunkY, &pointerX,
			...
			FWarpPointer(dpy, None, Scr.Root, 0, 0, 0, 0,
			    pointerX, pointerY);
		}
		if (fw && !IS_SHADED(fw) && !bo.do_disable_configure_notify) {
			...
			mvwm_msg(DBG, "AnimatedMoveAnyWindow",
		...
		if (FCheckMaskEvent(dpy,
			 ...
			 */
			if (pmrtp != NULL) {
				update_transparent_menu_bg(pmrtp, lastX,
				    lastY, currentX, currentY, endX, endY);
			}
...
void
AnimatedMoveMvwmWindow(MvwmWindow *fw, Window w, int startX, int startY,
    int endX, int endY, Bool fWarpPointerToo, int cmsDelay,
    float *ppctMovement)
{
...
placement_binding(int button, KeySym keysym, int modifier, char *action)
{
	if (keysym != 0) {
		 ...
		 * fixme
		 */
		mvwm_msg(ERR, "placement_binding",
 ...
 */
void
__move_icon(MvwmWindow *fw, int x, int y, int old_x, int old_y,
    Bool do_move_animated, Bool do_warp_pointer)
{
	...
	if (do_move_animated) {
		AnimatedMoveOfWindow(tw, -1, -1, tx, ty, do_warp_pointer, -1,
		    NULL, NULL);
		do_warp_pointer = 0;
	}
	if (has_icon_title) {
	...
	if (do_warp_pointer) {
		FWarpPointer(dpy, None, None, 0, 0, 0, 0, x - old_x,
		    y - old_y);
	}
...
__move_window(F_CMD_ARGS, Bool do_animate, int mode)
{
	...
	if (mode == MOVE_PAGE) {
		...
		s.width = m->coord.w;
		s.height = m->coord.h;
		mvwmrect_move_into_rectangle(&r, &s);
		FinalX = r.x;
		FinalY = r.y;
		...
		do_animate = False;
		FScreenGetScrRect(NULL, FSCREEN_CURRENT, &s.x, &s.y, &s.width,
		    &s.height);
		page_x = m->virtual_scr.Vx;
		page_y = m->virtual_scr.Vy;
		r.x = x;
		 ...
		 * move to page first
		 */
		mvwmrect_move_into_rectangle(&r, &p);
		/*
		 * then move to screen
		 */
		mvwmrect_move_into_rectangle(&r, &s);
		...
		FinalX = x;
		FinalY = y;
		n = GetMoveArguments(&action, width, height, &FinalX, &FinalY,
		    &fWarp, &fPointer, True);
	...
	if (w == FW_W_FRAME(fw)) {
		...
		dy = FinalY - fw->g.frame.y;
		if (do_animate) {
			AnimatedMoveMvwmWindow(fw, w, -1, -1, FinalX, FinalY,
			    fWarp, -1, NULL);
		}
...
static void
DoSnapAttract(MvwmWindow *fw, int Width, int Height, int *px, int *py)
{
	int             nyt, nxl;
	rectangle       self;
	int             score_x;
	...
	if (fw->snap_attraction.proximity > 0 &&
	    (fw->snap_attraction.
		mode & (SNAP_ICONS | SNAP_WINDOWS | SNAP_SAME))) {
		MvwmWindow     *tmp;
		int             maskout = (SNAP_SCREEN | SNAP_SCREEN_WINDOWS |
		    SNAP_SCREEN_ICONS | SNAP_SCREEN_ALL);

		for (tmp = Scr.MvwmRoot.next; tmp; tmp = tmp->next) {
			rectangle       other;
	...
	if (fw->snap_attraction.proximity > 0 && (
		 ...
		 * vertically
		 */
		if (!(m->coord.w < (*px) || (*px + self.width) < 0)) {
			if (*py + self.height >=
			    m->coord.h &&
			    *py + self.height <
		 ...
		 * horizontally
		 */
		if (!(m->coord.h < (*py) || (*py + self.height) < 0)) {
	...
	if (fw->edge_resistance_move > 0) {
		 ...
		 * snap to right edge
		 */
		if (*px + Width > m->coord.w &&
		    *px + Width < m->coord.w + fw->edge_resistance_move) {
			*px = m->coord.w - Width;
		}
		/*
		 * snap to left edge
		 ...
		 * snap to bottom edge
		 */
		if (*py + Height > m->coord.h &&
		    *py + Height < m->coord.h + fw->edge_resistance_move) {
			*py = m->coord.h - Height;
		}
		/*
		 * snap to top edge
    ...
    int Height, int *FinalX, int *FinalY, Bool do_move_opaque, int cursor)
{
	...
	struct monitor *m = fw->m;
	int             dx =
	    m->virtual_scr.EdgeScrollX ? m->virtual_scr.EdgeScrollX : m->
	    coord.w;
	int             dy =
	    m->virtual_scr.EdgeScrollY ? m->virtual_scr.EdgeScrollY : m->
	    coord.h;
	 ...
	 */
	memcpy(&fw_copy, fw, sizeof(MvwmWindow));
	/*
	...
	while (!is_finished && bad_window != FW_W(fw)) {
		...
		if (rc == -1) {
			 ...
			 * * events are not reported to mvwm when the pointer
		...
		if (e.type == MotionNotify ||
			...
			while (FPending(dpy) > 0 &&
			    FCheckMaskEvent(dpy, ButtonMotionMask |
		...
		case Expose:
			...
			dispatch_event(&e);
			if (!do_move_opaque) {
				draw_move_resize_grid(xl, yt, Width - 1,
				    Height - 1);
			}
			break;
		...
		if (do_move_opaque && !IS_ICONIFIED(fw) &&
			...
			if (!sent_cn || cnx != xl || cny != yt) {
				...
				cny = yt;
				sent_cn = True;
				SendConfigureNotify(fw, xl, yt, Width, Height,
				    0, False);
#ifdef MVWM_DEBUG_MSGS
				mvwm_msg(DBG, "frame_setup_window",
				    "Sent ConfigureNotify (w %d, h %d)",
				    Width, Height);
#endif
			}
		...
		if (do_move_opaque) {
			...
			if (xl != old_xl || yt != old_yt) {
				...
				if (draw_parts != PART_NONE) {
					border_draw_decorations(fw,
					    draw_parts,
					    ((fw == get_focus_window()))? True
					    : False, True, CLEAR_ALL, NULL,
					    NULL);
				}
	...
	if (is_aborted || bad_window == FW_W(fw)) {
		if (vx != fw->m->virtual_scr.Vx
		    || vy != fw->m->virtual_scr.Vy) {
...
CMD_XorValue(F_CMD_ARGS)
{
	...
	gcm =
	    GCFunction | GCLineWidth | GCForeground | GCFillStyle |
	    GCSubwindowMode;
	...
	if (Scr.XorGC) {
		...
		Scr.XorGC = mvwmlib_XCreateGC(dpy, Scr.Root, gcm, &gcv);
	}
...
CMD_XorPixmap(F_CMD_ARGS)
{
	...
	PictureUseDefaultVisual();
	xp = PGetMvwmPicture(dpy, Scr.Root, NULL, PixmapName, fpa);
	if (xp == NULL) {
		mvwm_msg(ERR, "SetXORPixmap", "Can't find pixmap %s",
		    PixmapName);
		free(PixmapName);
		PictureUseMvwmVisual();
		return;
	 ...
	 */
	XorPixmap =
	    XCreatePixmap(dpy, Scr.Root, xp->width, xp->height, Pdepth);
	XCopyArea(dpy, xp->picture, XorPixmap, DefaultGC(dpy, Scr.screen), 0,
	    0, xp->width, xp->height, 0, 0);
	/*
	 * destroy picture and free colors
	 */
	PDestroyMvwmPicture(dpy, xp);
	PictureUseMvwmVisual();
	 ...
	 */
	gcm =
	    GCFunction | GCLineWidth | GCTile | GCFillStyle | GCSubwindowMode;
	gcv.subwindow_mode = IncludeInferiors;
	gcv.function = GXxor;
	/*
...
static void
__resize_get_dir_from_window(int *ret_xmotion, int *ret_ymotion,
    MvwmWindow *fw, Window context_w)
{
...
static void
__resize_get_dir_proximity(int *ret_xmotion, int *ret_ymotion, MvwmWindow *fw,
    ...
    int *ymotionp, Bool do_resize_opaque, Bool is_direction_fixed)
{
	...
	DisplaySize(exc->w.fw, exc->x.elast, drag->width, drag->height, False,
	    False);
...
__resize_window(F_CMD_ARGS)
{
	...
	MvwmWindow     *fw = exc->w.fw;
	struct monitor *m = fw->m;
	Bool            is_finished = False, is_done = False, is_aborted =
	    False;
	...
	if (fev_get_evpos_or_query(dpy, Scr.Root, exc->x.etrigger, &px,
		&py) == False
	    || XTranslateCoordinates(dpy, Scr.Root, ResizeWindow, px, py, &px,
		&py, &JunkChild) == False) {
		/*
		 * pointer is on a different screen - that's okay here
		 */
	...
	get_window_borders(fw, &b);
	n = GetResizeArguments(&action, fw->g.frame.x, fw->g.frame.y,
	    fw->hints.base_width, fw->hints.base_height,
	    fw->hints.width_inc, fw->hints.height_inc,
	...
	if (do_resize_opaque) {
		mr_args =
		    frame_create_move_resize_args(fw, FRAME_MR_OPAQUE,
	...
	if (dir == DIR_NONE && detect_automatic_direction == True) {
		dir =
		    __resize_get_dir_from_resize_quadrant(orig->width,
		    orig->height, px, py);
	}
	...
	if (xmotion == 0 && ymotion == 0) {
		__resize_get_dir_from_window(&xmotion, &ymotion, fw,
		    PressedW);
	}
	...
	if (PressedW != Scr.Root && xmotion == 0 && ymotion == 0 &&
	    !called_from_title) {
		__resize_get_dir_proximity(&xmotion, &ymotion, fw,
		    orig->width, orig->height, px, py, &warp_x, &warp_y,
	...
	if (exc->x.elast->type == KeyPress) {
		...
		if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild,
			&stashed_x, &stashed_y, &JunkX, &JunkY,
			&JunkMask) == False) {
			/*
			 * pointer is on a different screen
			 */
	...
	while (!is_finished && bad_window != FW_W(fw)) {
		...
		if (ev.type == MotionNotify ||
			 ...
			 * * too to get an idea where the pointer might be.
			 */
			while (FCheckMaskEvent(dpy, ButtonMotionMask |
		 ...
		 */
		if (ev.type == KeyPress) {
			Keyboard_shortcuts(&ev, fw, NULL, NULL,
			    ButtonRelease);
			if (ev.type == ButtonRelease) {
				do_send_cn = True;
			}
	...
	if (is_aborted || bad_window == FW_W(fw)) {
		...
		if (do_resize_opaque) {
			...
			ymotion = 1;
			g = sorig;
			__resize_step(exc, sorig.x, sorig.y, &xo, &yo, &g,
			    orig, &xmotion, &ymotion, do_resize_opaque, True);
		}
...
Bool
is_window_sticky_across_pages(MvwmWindow *fw)
{
...
Bool
is_window_sticky_across_desks(MvwmWindow *fw)
{
...
static void
move_sticky_window_to_same_page(struct monitor *m,
...
static void
MaximizeHeight(MvwmWindow *win, int win_width, int win_x, int *win_height,
...
static void
MaximizeWidth(MvwmWindow *win, int *win_width, int *win_x, int win_height,
...
static void
unmaximize_mvwm_window(MvwmWindow *fw)
{
	rectangle       new_g;
	...
	if (fw->fullscreen.is_iconified) {
		execute_function_override_window(NULL, NULL, "Iconify on", 0,
		    fw);
...
static void
maximize_mvwm_window(MvwmWindow *fw, rectangle *geometry)
{
	SET_MAXIMIZED(fw, 1);
	...
	fprintf(stderr,
	    ...
	    fw->g.max.x, fw->g.max.y, fw->g.max.width, fw->g.max.height,
	    fw->g.max_offset.x, fw->g.max_offset.y, fw->g.normal.x,
	    fw->g.normal.y, fw->g.normal.width, fw->g.normal.height);
#endif
...
CMD_Maximize(F_CMD_ARGS)
{
	...
	if (!is_screen_given) {
		...
		fscr.xypos.y =
		    fw->g.frame.y + fw->g.frame.height / 2 - page_y;
		FScreenGetScrRect(&fscr, FSCREEN_XYPOS, &scr_x, &scr_y,
		    &scr_w, &scr_h);
	}

	if (!ignore_working_area) {
		EWMH_GetWorkAreaIntersection(fw, &scr_x, &scr_y, &scr_w,
		    &scr_h, EWMH_MAXIMIZE_MODE(fw));
	}
#if 0
	fprintf(stderr, "%s: page=(%d,%d), scr=(%d,%d, %dx%d)\n",
	    __FUNCTION__, page_x, page_y, scr_x, scr_y, scr_w, scr_h);
#endif
	...
	if (do_forget == True) {
		...
		if (grow_up || grow_down) {
			MaximizeHeight(fw, new_g.width, new_g.x,
			    &new_g.height, &new_g.y, grow_up, grow_down,
			    page_y + scr_y, page_y + scr_y + scr_h, layers);
		} else if (val2 > 0) {
			new_g.height = val2 * val2_unit / 100;
			new_g.y = page_y + scr_y;
		}
		if (grow_left || grow_right) {
			MaximizeWidth(fw, &new_g.width, &new_g.x,
			    new_g.height, new_g.y, grow_left, grow_right,
			    page_x + scr_x, page_x + scr_x + scr_w, layers);
		} else if (val1 > 0) {
...
static void
__handle_stick_exit(MvwmWindow *fw, int do_not_draw, int do_silently)
{
	if (do_not_draw == 0) {
