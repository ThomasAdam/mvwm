...
static void DisplayPosition(
	const MvwmWindow *tmp_win, const XEvent *eventp, int x, int y,int Init)
{
	char str[100];
	int offset;
...
static void DisplaySize(
	const MvwmWindow *tmp_win, const XEvent *eventp, int width,
...
static void AnimatedMoveAnyWindow(
	MvwmWindow *fw, Window w, int startX, int startY, int endX,
...
static void DoSnapAttract(
	MvwmWindow *fw, int Width, int Height, int *px, int *py)
{
	int nyt,nxl;
	rectangle self;
	int score_x;
...
void CMD_XorPixmap(F_CMD_ARGS)
{
	...
	if (xp == NULL)
	{
		mvwm_msg(ERR,"SetXORPixmap","Can't find pixmap %s", PixmapName);
		free(PixmapName);
		PictureUseMvwmVisual();
		return;
	}
...
static void __resize_get_dir_from_window(
	int *ret_xmotion, int *ret_ymotion, MvwmWindow *fw, Window context_w)
{
...
static void __resize_get_dir_proximity(
	int *ret_xmotion, int *ret_ymotion, MvwmWindow *fw, int x_off,
...
Bool is_window_sticky_across_pages(MvwmWindow *fw)
{
...
Bool is_window_sticky_across_desks(MvwmWindow *fw)
{
...
static void MaximizeHeight(
	MvwmWindow *win, int win_width, int win_x, int *win_height,
...
static void unmaximize_mvwm_window(
	MvwmWindow *fw, cmdparser_context_t *pc)
{
	rectangle new_g;
...
static void maximize_mvwm_window(
	MvwmWindow *fw, rectangle *geometry)
{
	SET_MAXIMIZED(fw, 1);
...
static void __handle_stick_exit(
	MvwmWindow *fw, int do_not_draw, int do_silently)
{
	if (do_not_draw == 0)
	{
		border_draw_decorations(
