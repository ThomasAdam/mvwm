...
static int      do_all_iconboxes(MvwmWindow *t, icon_boxes ** icon_boxes_ptr);
static void     GetIconFromFile(MvwmWindow *fw);
static void     GetIconWindow(MvwmWindow *fw);
static void     GetIconBitmap(MvwmWindow *fw);

static void
clear_icon_dimensions(MvwmWindow *fw)
{
	int             px;
	int             py;
	int             tx;
...
int
get_visible_icon_window_count(MvwmWindow *fw)
{
	int             count = 0;
...
void
setup_icon_title_size(MvwmWindow *fw)
{
	if (HAS_NO_ICON_TITLE(fw)) {
		...
		fw->icon_g.title_text_width =
		    FlocaleTextWidth(fw->icon_font, fw->visible_icon_name,
 ...
 */
static void
SetIconPixmapSize(Pixmap *icon, int width, int height, int depth,
    int newWidth, int newHeight, Bool force_centering, int resize_type,
    int *nrx, int *nry, int freeOldPixmap)
{
	Pixmap          oldPixmap;
	...
	*nry = 0;
	...
	case ICON_RESIZE_TYPE_ADJUSTED:
		if (newWidth != width || newHeight != height) {
			*icon =
			    CreateStretchPixmap(dpy, oldPixmap, width, height,
			    depth, newWidth, newHeight, gc);
		}
		break;
 ...
 */
void
GetIconPicture(MvwmWindow *fw, Bool no_icon_window)
{
	char            icon_order[4];
	int             i;

	/*
	 * First, see if it was specified in the .mvwmrc
	 */
	if (ICON_OVERRIDE_MODE(fw) == ICON_OVERRIDE) {
		/*
		 * try mvwm provided icons before application provided icons
		 */
		icon_order[0] = 0;
		icon_order[1] = 1;
		icon_order[2] = 2;
		...
		if (fw->wmhints && (fw->wmhints->flags & IconPixmapHint) &&
			 ...
			 * * first, then mvwm provided icons.
			 */
			icon_order[0] = 1;
			icon_order[1] = 2;
			icon_order[2] = 3;
			 ...
			 * * then mvwm provided default icon
			 */
			icon_order[0] = 1;
			icon_order[1] = 2;
			icon_order[2] = 3;
			 ...
			 * * first, then mvwm provided icons and then application
		...
		/*
		 * use application provided icon rather than mvwm provided
		 * * icon
		 */
		icon_order[0] = 1;
		icon_order[1] = 2;
	...
	if ((IS_ICON_OURS(fw)) && fw->icon_g.picture_w_g.height > 0 &&
		...
		if (resize) {
			...
			if (fw->icon_maskPixmap) {
				...
				fw->icon_maskPixmap =
				    XCreatePixmap(dpy, fw->iconPixmap,
 ...
 */
static void
set_icon_pixmap_background(MvwmWindow *fw)
{
 ...
 */
void
CreateIconWindow(MvwmWindow *fw, int def_x, int def_y)
{
	/*
	 ...
	 */
	/*
	 * this does not happen if mvwm is using a non-default visual (with
	...
	if (IS_ICON_OURS(fw) && fw->icon_g.picture_w_g.width > 0 &&
	    fw->icon_g.picture_w_g.height > 0) {
		/*
		 * use mvwm's visuals in these cases
		 */
		if (Pdefault || fw->iconDepth == 1 || fw->iconDepth == Pdepth
			...
			/*
			 * client supplied icon pixmap and mvwm is using
			 * * another visual.
			 * * use it as the background pixmap, don't try to put
			 * * relief on it because mvwm will not have the correct
	...
	if (old_icon_pixmap_w != None &&
		...
		XDeleteContext(dpy, old_icon_pixmap_w, MvwmContext);
		XFlush(dpy);
	...
	if (FShapesSupported) {
		if (IS_ICON_SHAPED(fw) && fw->icon_background_cs < 0) {
			/*
			 * when mvwm is using the non-default visual client
	...
	if (FW_W_ICON_PIXMAP(fw) != None &&
	    FW_W_ICON_PIXMAP(fw) != old_icon_pixmap_w) {
		XSaveContext(dpy, FW_W_ICON_PIXMAP(fw), MvwmContext,
		    (caddr_t) fw);
 ...
 */
static void
DrawIconTitleWindow(MvwmWindow *fw, XEvent *pev, Pixel BackColor, GC Shadow,
    GC Relief, int cs, int title_cs)
{
	...
	if (is_stippled) {
		...
		for (i = min; w_stipple > 0 && i <= max;
			...
			RelieveRectangle(dpy, FW_W_ICON_TITLE(fw), x_stipple,
...
static void
DrawIconPixmapWindow(MvwmWindow *fw, Bool reset_bg, XEvent *pev, GC Shadow,
    GC Relief, int cs)
{
	XRectangle      r, clip;
	Bool            cleared = False;
	...
	if (!pev) {
		...
		if (reset_bg && (fw->iconDepth == 1 || fw->iconDepth == Pdepth
	...
	if (fw->iconPixmap != None) {
		if (fw->iconDepth == 1 || fw->iconDepth == Pdepth || Pdefault
			...
			if (pev) {
				if (!frect_get_intersection(pev->xexpose.x,
					pev->xexpose.y, pev->xexpose.width,
					pev->xexpose.height, r.x, r.y,
					r.width, r.height, &clip)) {
					draw_icon = False;
				}
			} else {
				clip.x = r.x;
			...
			/*
			 * it's a client pixmap and mvwm is not using
	...
	/*
	 * only relieve unshaped icons or icons with a bg that share mvwm's
	 * * visual
	 */
	if ((fw->iconPixmap != None) &&
...
void
DrawIconWindow(MvwmWindow *fw, Bool draw_title, Bool draw_pixmap,
    Bool focus_change, Bool reset_bg, XEvent *pev)
{
	...
	if (draw_pixmap && FW_W_ICON_PIXMAP(fw) != None) {
		if (pev && pev->xexpose.window != FW_W_ICON_PIXMAP(fw)) {
			...
			DrawIconPixmapWindow(fw, reset_bg, pev, Shadow,
			    Relief, cs);
		}
	}
 ...
 */
void
ChangeIconPixmap(MvwmWindow *fw)
{
	rectangle       g;
 ...
 */
void
AutoPlaceIcon(MvwmWindow *t, initial_window_options_t *win_opts,
    Bool do_move_immediately)
{
	...
	if (IS_ICON_STICKY_ACROSS_PAGES(t) || IS_STICKY_ACROSS_PAGES(t)) {
		...
		base_x =
		    ((t->g.frame.x + t->m->virtual_scr.Vx +
			(t->g.frame.width >> 1)) / t->m->coord.w) *
		    t->m->coord.w;
		base_y =
		    ((t->g.frame.y + t->m->virtual_scr.Vy +
			(t->g.frame.height >> 1)) / t->m->coord.h) *
		    t->m->coord.h;
		/*
...
#define HRZ_FILL icon_boxes_ptr->IconFlags & ICONFILLHRZ
		...
		fscr.xypos.y =
		    t->g.frame.y + (t->g.frame.height / 2) - base_y;
		get_icon_geometry(t, &g);
		/*
		...
		while (do_all_iconboxes(t, &icon_boxes_ptr)) {
			...
			while ((dim[1].step < 0	/* filling reversed */
				? (dim[1].start_at + dim[1].icon_dimension - dim[1].nom_dimension > dim[1].end_at)	/* check back edge */
				: (dim[1].start_at + dim[1].nom_dimension < dim[1].end_at))	/* check front edge */
			    &&(!loc_ok)
				...
				while ((dim[2].step < 0	/* filling reversed */
					? (dim[2].start_at + dim[2].icon_dimension - dim[2].nom_dimension > dim[2].end_at)	/* check back edge */
					: (dim[2].start_at + dim[2].nom_dimension < dim[2].end_at))	/* check front edge */
				    &&(!loc_ok)
				    && (!loc_ok_wrong_screen2)) {	/* nothing found yet */
					...
					test_fw = Scr.MvwmRoot.next;
					while ((test_fw != (MvwmWindow *) 0)
					    && (loc_ok == True
...
*/
static int
do_all_iconboxes(MvwmWindow *t, icon_boxes ** icon_boxes_ptr)
{
...
GetIconFromFile(MvwmWindow *fw)
{
	...
	if (!PImageLoadPixmapFromFile(dpy, Scr.NoFocusWin, path,
		...
		&fw->iconDepth, &fw->icon_nalloc_pixels,
		&fw->icon_alloc_pixels, &fw->icon_no_limit, fpa)) {
		mvwm_msg(ERR, "GetIconFromFile", "Failed to load %s", path);
		free(path);
		return;
	}
 ...
 */
void
DeIconify(MvwmWindow *fw)
{
	MvwmWindow     *t, *tmp, *ofw;
	...
	for (ofw = NULL; fw != ofw && IS_ICONIFIED_BY_PARENT(fw);) {
		t = get_transientfor_mvwmwindow(fw);
		if (t == NULL) {
			break;
		}
		ofw = fw;
		...
		if (t == fw || IS_IN_TRANSIENT_SUBTREE(t)) {
			...
			if (t == fw) {
				BroadcastPacket(M_DEICONIFY, 11,
				    ...
				    (long) t->g.frame.x, (long) t->g.frame.y,
 ...
 */
void
Iconify(MvwmWindow *fw, initial_window_options_t *win_opts)
{
	MvwmWindow     *t;
	MvwmWindow     *sf;
	XWindowAttributes winattrs = { 0 };
 ...
 */
void
SetMapStateProp(const MvwmWindow *fw, int state)
{
	/*
