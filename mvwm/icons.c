...
static void clear_icon_dimensions(MvwmWindow *fw)
{
	int px;
	int py;
	int tx;
 ...
 *
 */
static void set_icon_pixmap_background(MvwmWindow *fw)
{
...
void CreateIconWindow(MvwmWindow *fw, int def_x, int def_y)
{
	...
	{
		XSaveContext(
			dpy, FW_W_ICON_TITLE(fw), MvwmContext, (caddr_t)fw);
		XDefineCursor(
			dpy, FW_W_ICON_TITLE(fw), Scr.MvwmCursors[CRS_DEFAULT]);
		GrabAllWindowKeysAndButtons(
			dpy, FW_W_ICON_TITLE(fw), Scr.AllBindings, C_ICON,
			GetUnusedModifiers(), Scr.MvwmCursors[CRS_DEFAULT],
			True);
	...
	{
		XSaveContext(
			dpy, FW_W_ICON_PIXMAP(fw), MvwmContext, (caddr_t)fw);
		XDefineCursor(
			dpy, FW_W_ICON_PIXMAP(fw),
...
void DrawIconTitleWindow(
	MvwmWindow *fw, XEvent *pev, Pixel BackColor, GC Shadow, GC Relief,
	int cs, int title_cs)
{
...
void DrawIconPixmapWindow(
	MvwmWindow *fw, Bool reset_bg, XEvent *pev, GC Shadow, GC Relief, int cs)
{
	XRectangle r,clip;
	Bool cleared = False;
...
void DrawIconWindow(
	MvwmWindow *fw, Bool draw_title, Bool draw_pixmap, Bool focus_change,
 ...
 */
void AutoPlaceIcon(
	MvwmWindow *t, initial_window_options_t *win_opts,
 ...
 *
 */
void SetMapStateProp(const MvwmWindow *fw, int state)
{
