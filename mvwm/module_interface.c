...
/*
 *
 * code for talking with mvwm modules.
 *
 */
#include "config.h"
    ...
    unsigned long num, va_list ap)
{
	 ...
	 */
	if (num > MvwmPacketMaxSize) {
		num = MvwmPacketMaxSize;
	}
	*(bp++) = START_FLAG;
	*(bp++) = event_type;
	*(bp++) = num + MvwmPacketHeaderSize;
	*(bp++) = fev_get_evtime();
    ...
    unsigned long num, va_list ap)
{
	...
	*(bp++) = fev_get_evtime();
	...
	for (; num > 0; --num) {
		...
		bodylen += addlen;
		if (bodylen >= MvwmPacketMaxSize_byte) {
			mvwm_msg(ERR, "make_new_vpacket",
			    "packet too long %ld %ld", (long) bodylen,
			    (long) MvwmPacketMaxSize_byte);
			break;
		}
		if (arglen > 0) {
			...
			*bp = (unsigned long) *tmp;
			...
			mvwm_msg(ERR, "make_new_vpacket",
			    ...
			    " mvwm-workers@mvwm.org. aborting...", arglen);
			abort();
		}
	}
...
SendPacket(fmodule *module, unsigned long event_type, unsigned long num_datum,
    ...)
{
	unsigned long   body[MvwmPacketMaxSize];
	va_list         ap;
	...
	PositiveWrite(module, body,
	    (num_datum + MvwmPacketHeaderSize) * sizeof(body[0]));
...
{
	unsigned char   body[MvwmPacketMaxSize_byte];
	va_list         ap;
	unsigned long   plen;
...
action_flags   *
__get_allowed_actions(const MvwmWindow *fw)
{
...
void
SendConfig(fmodule *module, unsigned long event_type, const MvwmWindow *t)
{
	const MvwmWindow **t1 = &t;
...
void
BroadcastConfig(unsigned long event_type, const MvwmWindow *t)
{
	const MvwmWindow **t1 = &t;
    ...
    int num, ...)
{
	 ...
	 * * name string.
	 */
	*len = MvwmPacketHeaderSize + num +
	    (strlen(name) / sizeof(unsigned long)) + 1;
	/*
	 * truncate long packets
	 */
	if (*len > MvwmPacketMaxSize) {
	...
	strncpy((char *) &body[MvwmPacketHeaderSize + num], name,
	    (*len - MvwmPacketHeaderSize - num) * sizeof(unsigned long) - 1);
	body[2] = *len;
...
void
BroadcastWindowIconNames(MvwmWindow *fw, Bool window, Bool icon)
{
	if (window) {
...
void
SendMvwmPicture(fmodule *module, unsigned long event_type,
    unsigned long data1, unsigned long data2, unsigned long data3,
...
void
BroadcastMvwmPicture(unsigned long event_type, unsigned long data1,
    unsigned long data2, unsigned long data3, MvwmPicture *picture,
    char *name)
{
	unsigned long  *body;
...
module_input_execute(struct fmodule_input *input)
{
	...
	memset(&e, 0, sizeof(e));
	if (XFindContext(dpy, input->window, MvwmContext,
...
CMD_Send_WindowList(F_CMD_ARGS)
{
	...
	for (t = Scr.MvwmRoot.next; t != NULL; t = t->next) {
		...
		if (FMiniIconsSupported && t->mini_icon != NULL) {
			SendMvwmPicture(mod, M_MINI_ICON, FW_W(t),
