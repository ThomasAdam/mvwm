...
action_flags *__get_allowed_actions(const MvwmWindow *fw)
{
...
void SendConfig(fmodule *module, unsigned long event_type, const MvwmWindow *t)
{
...
void BroadcastConfig(unsigned long event_type, const MvwmWindow *t)
{
	const MvwmWindow **t1 = &t;
...
void BroadcastWindowIconNames(MvwmWindow *fw, Bool window, Bool icon)
{
	if (window)
	{
		BroadcastName(
...
void BroadcastMvwmPicture(
	unsigned long event_type, unsigned long data1, unsigned long data2,
	unsigned long data3, MvwmPicture *picture, char *name)
{
	unsigned long *body;
