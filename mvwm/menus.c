...
__menu_execute_function(const exec_context_t **pexc, char *action)
{
	 ...
	 */
	if (!check_if_mvwm_window_exists((*pexc)->w.fw)) {
		ecc.w.fw = NULL;
		ecc.w.w = None;
		ecc.w.wcontext = 0;
		exc =
		    exc_clone_context(*pexc, &ecc,
 ...
 * fw - the MvwmWindow structure to check against allowed functions */
static void
get_menu_repaint_transparent_parameters(MenuRepaintTransparentParameters
    *pmrtp, MenuRoot *mr, MvwmWindow *fw)
{
	pmrtp->mr = mr;
	pmrtp->fw = fw;
...
static void
animated_move_back(MenuRoot *mr, Bool do_warp_pointer, MvwmWindow *fw)
{
	...
	if (menu_get_geometry(mr, &JunkRoot, &act_x, &act_y, &JunkWidth,
		...
		AnimatedMoveOfWindow(MR_WINDOW(mr), act_x, act_y,
		    act_x - MR_XANIMATION(mr), act_y, do_warp_pointer, -1,
		    NULL, (transparent_bg) ? &mrtp : NULL);
		MR_XANIMATION(mr) = 0;
	}
...
move_any_menu(MenuRoot *mr, MenuParameters *pmp, int endX, int endY)
{
	if (MR_IS_TEAR_OFF_MENU(mr)) {
		 ...
		 * * are used.
		 */
		AnimatedMoveMvwmWindow(pmp->tear_off_root_menu_window,
		...
		if (ST_HAS_MENU_CSET(MR_STYLE(mr)) &&
			...
			repaint_transparent_menu(&mrtp, False, endX, endY,
			    endX, endY, True);
		} else {
			XMoveWindow(dpy, MR_WINDOW(mr), endX, endY);
		}
    ...
    Window p_child, int p_rx, int p_ry)
{
	...
	if (XFindContext(dpy, Child, MenuContext,
		(caddr_t *) & mr) == XCNOENT) {
		return NULL;
	}
	/*
	 ...
	 */
	if (!XTranslateCoordinates(dpy, Scr.Root, MR_WINDOW(mr), root_x,
		root_y, &x, &y, &JunkChild)) {
		return NULL;
	}
    ...
    string_context_t * context)
{
	...
	for (s = instring; *s != '\0'; s++) {
		if (type < 0) {
			...
			s[0] = '\0';
			is_valid =
			    string_defs[type].string_handler(string,
...
__scan_for_pixmap(char *name, char type, string_context_t * context)
{
	...
	case '%':
		...
		while (current_mini_icon < MAX_MENU_ITEM_MINI_ICONS) {
			if (MI_MINI_ICON(SCTX_GET_MI(*context))
			    [current_mini_icon]) {
				current_mini_icon++;
			} else {
				break;
	...
	fpa.mask = 0;
	p = PCacheMvwmPicture(dpy, Scr.NoFocusWin, NULL, name, fpa);
	if (!p) {
		mvwm_msg(WARN, "scanForPixmap",
		    "Couldn't load image from %s", name);
		mvwm_msg(WARN, "scanForPixmap",
		    "Check that MVWM has support for the filetype it's "
		    "being asked to load.");
...
calculate_item_sizes(MenuSizingParameters * msp)
{
	...
	for (mi = MR_FIRST_ITEM(msp->menu); mi != NULL; mi = MI_NEXT_ITEM(mi)) {
		if (MI_IS_TITLE(mi)) {
			menuitem_get_size(mi, &mipst,
			    MST_PTITLEFONT(msp->menu), do_reverse_icon_order);
		} else {
...
size_menu_horizontally(MenuSizingParameters * msp)
{
	...
	int            *item_order[MAX_MENU_ITEM_LABELS +
	    MAX_MENU_ITEM_MINI_ICONS + 1 /* triangle */  +
	    2 /* relief markers */ ];
	int             used_objects = 0;
	int             left_objects = 0;
	int             right_objects = 0;
	...
	if (d > 0) {
		...
		int             n =
		    1 + used_objects - left_objects - right_objects;
	...
	for (mi = MR_FIRST_ITEM(msp->menu); mi != NULL; mi = MI_NEXT_ITEM(mi)) {
		for (i = 0; i < MAX_MENU_ITEM_LABELS; i++) {
			...
			if (!MI_IS_TITLE(mi) || !MI_IS_TITLE_CENTERED(mi)) {
				 ...
				 */
				MI_LABEL_OFFSET(mi)[i] =
				    menudim_middle_x_offset(&MR_DIM(msp->
					menu)) - MI_LABEL_OFFSET(mi)[i] / 2;
			}
		}	/* for */
	}	/* for */
...
calc_more_item_height(MenuSizingParameters * msp)
{
	...
	height =
	    ...
	    MST_ITEM_GAP_BELOW(msp->menu) + MST_RELIEF_THICKNESS(msp->menu);
	...
	for (cItems = 0, mi = MR_FIRST_ITEM(msp->menu); mi != NULL;
		...
		if (menu_height > MR_SCREEN_HEIGHT(msp->menu)) {
			...
			if (MI_PREV_ITEM(mi) == NULL ||
			    menu_height > MR_SCREEN_HEIGHT(msp->menu)) {
				mvwm_msg(ERR, "size_menu_vertically",
			...
			tempname = mvwm_malloc((10 + strlen(t)) * sizeof(char));
			 ...
			 * * strlen("Popup ")==6 )
			 */
			t = mvwm_malloc(strlen(MR_NAME(msp->menu)) + 2);
			...
			if (MR_MISSING_SUBMENU_FUNC(msp->menu)) {
				MR_MISSING_SUBMENU_FUNC(menuContinuation) =
				    mvwm_strdup(MR_MISSING_SUBMENU_FUNC(msp->
					menu));
			}
			/*
			 ...
			 */
			AddToMenu(msp->menu, gettext("More&..."), tempname,
...
make_menu_window(MenuRoot *mr, Bool is_tear_off)
{
	...
	if (MR_WINDOW(mr) != None) {
		...
		attributes.cursor = Scr.MvwmCursors[CRS_MENU];
		 ...
		 * * window or leave it as an orphan if mvwm dies or is
		 * * restarted.
		 */
		if (is_tear_off) {
...
{
	dest_mr->s = mvwm_malloc(sizeof(MenuRootStatic));
	/*
	 * copy everything
	 */
	...
	if (MR_NAME(src_mr) != NULL) {
		MR_NAME(dest_mr) = mvwm_strdup(MR_NAME(src_mr));
	}
	MR_COPIES(dest_mr) = 1;
	...
	if (MR_MISSING_SUBMENU_FUNC(src_mr)) {
		MR_MISSING_SUBMENU_FUNC(dest_mr) =
		    mvwm_strdup(MR_MISSING_SUBMENU_FUNC(src_mr));
	}
	if (MR_HAS_SIDECOLOR(src_mr)) {
		MR_SIDECOLOR(dest_mr) =
		    mvwmlib_clone_color(MR_SIDECOLOR(src_mr));
	}
	MR_SIDEPIC(dest_mr) = PCloneMvwmPicture(MR_SIDEPIC(src_mr));
...
paint_menu_gradient_background(MenuRoot *mr, XEvent *pevent)
{
	...
	case H_GRADIENT:
		if (MR_IS_BACKGROUND_SET(mr) == False) {
			...
			pmapgc = mvwmlib_XCreateGC(dpy, pmap, gcm, &gcv);
			dw = (float)
	...
	case V_GRADIENT:
		if (MR_IS_BACKGROUND_SET(mr) == False) {
			...
			pmap =
			    XCreatePixmap(dpy, MR_WINDOW(mr), best_tile_width,
			    MR_HEIGHT(mr), Pdepth);
			pmapgc = mvwmlib_XCreateGC(dpy, pmap, gcm, &gcv);
	...
	case B_GRADIENT:
		{
			...
			XSetClipRectangles(dpy, Scr.TransMaskGC, 0, 0, &r, 1,
			    Unsorted);
			numLines = MR_WIDTH(mr) + MR_HEIGHT(mr) - 2 * bw;
			for (i = 0; i < numLines; i++) {
				if ((int) (i * ST_FACE(ms).u.grad.npixels /
					numLines) > cindex) {
					/*
					 * pick the next colour (skip if necc.)
					 */
					cindex =
					    i * ST_FACE(ms).u.grad.npixels /
					    numLines;
					XSetForeground(dpy, Scr.TransMaskGC,
					    ST_FACE(ms).u.grad.xcs[cindex].
					    pixel);
				}
...
get_menu_paint_item_parameters(MenuPaintItemParameters *mpip, MenuRoot *mr,
    MenuItem *mi, MvwmWindow *fw, XEvent *pevent, Bool do_redraw_menu_border)
{
	mpip->ms = MR_STYLE(mr);
	mpip->w = MR_WINDOW(mr);
 ...
 */
static void
paint_menu(MenuRoot *mr, XEvent *pevent, MvwmWindow *fw)
{
	MenuItem       *mi;
	MenuStyle      *ms = MR_STYLE(mr);
	int             bw = MST_BORDER_WIDTH(mr);
	...
	if (fw && !check_if_mvwm_window_exists(fw)) {
		fw = NULL;
	}
	...
	if (ms && ST_HAS_MENU_CSET(ms)) {
		if (MR_IS_BACKGROUND_SET(mr) == False) {
			SetWindowBackground(dpy, MR_WINDOW(mr), MR_WIDTH(mr),
			    ...
			    Pdepth, FORE_GC(ST_MENU_INACTIVE_GCS(ms)), True);
			MR_IS_BACKGROUND_SET(mr) = True;
		}
	} else if (ms) {
		int             type;
...
static void
select_menu_item(MenuRoot *mr, MenuItem *mi, Bool select, MvwmWindow *fw)
{
		...
		case GradientMenu:
			if (select == True) {
				...
				MR_STORED_ITEM(mr).stored =
				    XCreatePixmap(dpy, Scr.NoFocusWin, mw, ih,
				    Pdepth);
	...
	if (MR_IS_PAINTED(mr)) {
		...
		get_menu_paint_item_parameters(&mpip, mr, mi, fw, NULL,
		    False);
		menuitem_paint(mi, &mpip);
	}
...
get_right_popup_x_position(MenuRoot *mr, MenuRoot *submenu, int x)
{
	if (MST_USE_LEFT_SUBMENUS(mr)) {
		...
		return (x +
		    MR_WIDTH(mr) * MST_POPUP_OFFSET_PERCENT(mr) / 100 +
    ...
    Bool *pprefer_left_submenus)
{
	...
	if (!menu_get_geometry(mr, &JunkRoot, &menu_x, &menu_y, &JunkWidth,
		...
		*pprefer_left_submenus = False;
		mvwm_msg(ERR, "get_prefered_popup_position",
    ...
    Bool allow_popup_offset_tolerance)
{
	...
	if (!menu_get_geometry(mr, &JunkRoot, &prior_x, &prior_y,
    ...
    Bool *ret_overlap, Window popdown_window)
{
	...
	if (MR_POPUP_ACTION(mr)) {
		 ...
		 * * overwritten
		 */
		menu_name = mvwm_strdup(MR_NAME(mr));
	...
	if (!pops->flags.has_poshints && fw && parent_menu == NULL &&
		...
		has_context =
		    get_title_button_geometry(fw, &button_g, context);
		if (has_context) {
			fscreen_scr_arg fscr;
	 ...
	 */
	{
		 ...
		 * clip to screen
		 */
		FScreenClipToScreen(&fscr, FSCREEN_XYPOS, &x, &y,
	...
	if (parent_menu) {
		...
		x_overlap =
		    do_menus_overlap(parent_menu, x, y, MR_WIDTH(mr),
	...
	if (parent_menu == NULL ||
		...
		x_clipped_overlap =
		    do_menus_overlap(parent_menu, x, y, MR_WIDTH(mr),
		    MR_HEIGHT(mr), bwp, bwp, bwp, True);
		if (x_clipped_overlap && (!pops->flags.has_poshints
			 ...
			 */
			left_x =
			    get_left_popup_x_position(parent_menu, mr,
			    prev_x);
			right_x =
			    get_right_popup_x_position(parent_menu, mr,
			    prev_x);
			...
			if (MST_IS_ANIMATED(mr)) {
				...
				if (ST_HAS_MENU_CSET(MR_STYLE(parent_menu)) &&
				    CSET_IS_TRANSPARENT(ST_CSET_MENU(MR_STYLE
					    (parent_menu)))) {
					transparent_bg = True;
			 ...
			 */
				...
				if (x + MR_WIDTH(mr) > scr_x + scr_w) {
					...
					if (prev_width >=
						...
						c = PARENT_MENU_FORCE_VISIBLE_WIDTH + prev_x;
					} else {
						c = prev_x + prev_width;
					}
	...
	if (ret_overlap) {
		*ret_overlap =
		    do_menus_overlap(parent_menu, x, y, MR_WIDTH(mr),
		    MR_HEIGHT(mr), 0, 0, 0, False) ? True : False;
	}
    ...
    MenuParameters *pmp)
{
	...
	if (*fSubmenuOverlaps && parent) {
		 ...
		 */
		event.type = Expose;
		if (!menu_get_geometry(*pmr, &JunkRoot, &mr_x, &mr_y,
...
static void
__mloop_init(MenuParameters *pmp, MenuReturn *pmret,
    ...
    MenuOptions * pops)
{
	...
	if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild,
		&(msi->x_init), &(msi->y_init), &JunkX, &JunkY,
		&JunkMask) == False) {
		/*
		 * pointer is on a different screen
		 */
...
static void
__mloop_get_event_timeout_loop(MenuParameters *pmp,
    mloop_evh_input_t * in, mloop_evh_data_t * med, mloop_static_info_t * msi)
{
			...
			if (!m || med->mi != MR_PARENT_ITEM(m)) {
				...
				if (MST_DO_POPUP_IMMEDIATELY(pmp->menu)) {
					...
					in->mif.is_popped_up_by_timeout =
					    True;
				} else if (!MST_DO_POPUP_IMMEDIATELY(pmp->
					menu)
				    && in->mif.
...
__mloop_get_event(MenuParameters *pmp, MenuReturn *pmret,
    mloop_evh_input_t * in, mloop_evh_data_t * med, mloop_static_info_t * msi)
{
	XEvent          e = *(*pmp->pexc)->x.elast;
	...
	if (pmp->event_propagate_to_submenu) {
		...
		if ((*pmp->pexc)->x.elast->type == KeyPress) {
			...
			if (FQueryPointer(dpy, Scr.Root, &JunkRoot,
				&JunkChild, &e.xkey.x_root, &e.xkey.y_root,
    ...
    mloop_static_info_t * msi)
{
	...
	case ButtonRelease:
		...
		case MENU_SELECTED:
			...
			if (is_double_click(msi->t0, med->mi, pmp, pmret,
				pdkp, in->mif.has_mouse_moved)) {
	...
	case KeyPress:
		...
		if (pmret->flags.is_menu_posted) {
			...
			e.xkey.y_root =
			    menuitem_middle_y_offset(med->mi,
			    MR_STYLE(pmp->menu));
			fev_fake_event(&e);
		}
	...
	case MotionNotify:
		{
			...
			med->mi =
			    find_entry(pmp, &med->x_offset, &med->mrMi,
			    p_child, p_rx, p_ry);
			if (pmret->flags.is_menu_posted
			...
			if (!in->mif.is_release_first
			    && !in->mif.is_motion_faked
    ...
    MenuOptions * pops, Bool *pdoes_submenu_overlap)
{
	...
	if (!is_submenu_mapped(pmp->menu, med->mi)) {
		 ...
		 */
		pop_menu_up(&in->mrPopup, pmp, pmp->menu, med->mi, pmp->pexc,
		    x, y, prefer_left_submenus, in->mif.do_popup_and_warp,
		    pops, pdoes_submenu_overlap,
...
__mloop_get_mi_actions(MenuParameters *pmp, MenuReturn *pmret,
    double_keypress * pdkp, mloop_evh_input_t * in, mloop_evh_data_t * med,
    mloop_static_info_t * msi, MenuRoot *mrMiPopup,
...
static mloop_ret_code_t
__mloop_do_popup(MenuParameters *pmp, MenuReturn *pmret,
    mloop_evh_input_t * in, mloop_evh_data_t * med,
    ...
    Bool *pdoes_popdown_submenu_overlap)
{
	...
	if (med->mrMi == pmp->menu && mrMiPopup == NULL &&
		...
		menu_name =
		    PeekToken(SkipNTokens(MI_ACTION(med->mi), 1), NULL);
		if (!menu_name) {
			menu_name = "";
		}
		is_complex_function =
	...
	if (in->mif.do_popdown) {
		if (in->mrPopdown) {
			if (in->mrPopdown != in->mrPopup) {
				...
				pop_menu_down_and_repaint_parent(&in->
				    mrPopdown, pdoes_popdown_submenu_overlap,
				    pmp);
			}
			in->mrPopdown = NULL;
		}
...
static mloop_ret_code_t
__mloop_do_menu(MenuParameters *pmp, MenuReturn *pmret,
    double_keypress * pdkp, mloop_evh_input_t * in, mloop_evh_data_t * med,
    MenuOptions * pops, Bool *pdoes_submenu_overlap)
{
	MenuParameters  mp;
...
__mloop_handle_action_with_mi(MenuParameters *pmp, MenuReturn *pmret,
    double_keypress * pdkp, mloop_evh_input_t * in, mloop_evh_data_t * med,
    mloop_static_info_t * msi, MenuOptions * pops,
    Bool *pdoes_submenu_overlap, Bool *pdoes_popdown_submenu_overlap)
{
	MenuItem       *tmi;
	MenuRoot       *tmrMi;
	...
	if (med->mi != MR_SELECTED_ITEM(pmp->menu) && med->mrMi == pmp->menu) {
		...
		*pdoes_submenu_overlap = *pdoes_popdown_submenu_overlap;
		select_menu_item(pmp->menu, med->mi, True,
		    (*pmp->pexc)->w.fw);
	}
	mrMiPopup = mr_popup_for_mi(pmp->menu, med->mi);
	/*
    ...
    mloop_evh_data_t * med, MenuOptions * pops)
{
	...
	if (pmp->ret_paction) {
		if (pmret->rc == MENU_EXEC_CMD) {
			*pmp->ret_paction = mvwm_strdup(*pmp->ret_paction);
		} else {
			if (*pmp->ret_paction) {
				free(*pmp->ret_paction);
			}
			*pmp->ret_paction = (med->mi) ?
			    mvwm_strdup(MI_ACTION(med->mi)) : NULL;
		}
	}
...
menu_tear_off(MenuRoot *mr_to_copy)
{
	...
	if (MR_WINDOW(mr_to_copy) != None) {
		discard_window_events(MR_WINDOW(mr_to_copy),
	...
	menuclasshints.res_name =
	    (name != NULL) ? name : mvwm_strdup(MR_NAME(mr));
	menuclasshints.res_class = mvwm_strdup("mvwm_menu");
	 ...
	 */
	XSetWMProperties(dpy, MR_WINDOW(mr), &menunametext, &menunametext,
	    NULL, 0, &menusizehints, NULL, &menuclasshints);
...
menu_enter_tear_off_menu(const exec_context_t *exc)
{
	...
	if (XFindContext(dpy, FW_W(exc->w.fw), MenuContext,
		(caddr_t *) & mr) == XCNOENT) {
		return;
	}
	ecc.w.fw = NULL;
...
void
menu_close_tear_off_menu(MvwmWindow *fw)
{
	MenuRoot       *mr;
	MenuParameters  mp;
	const exec_context_t *exc;
	...
	if (XFindContext(dpy, FW_W(fw), MenuContext,
		(caddr_t *) & mr) == XCNOENT) {
		return;
	}
	memset(&mp, 0, sizeof(mp));
...
Bool
menu_redraw_transparent_tear_off_menu(MvwmWindow *fw, Bool pr_only)
{
	MenuRoot       *mr;
	MenuStyle      *ms = NULL;
	int             cs;
	...
	if (!(IS_TEAR_OFF_MENU(fw) &&
		...
		XCNOENT && (ms = MR_STYLE(mr)) && ST_HAS_MENU_CSET(ms))) {
		return False;
	}
	...
	return UpdateBackgroundTransparency(dpy, MR_WINDOW(mr), MR_WIDTH(mr),
	    ...
	    Pdepth, FORE_GC(MST_MENU_INACTIVE_GCS(mr)), True);
}
...
do_menu(MenuParameters *pmp, MenuReturn *pmret)
{
	...
	if (!pmp->flags.is_already_mapped) {
		...
		if (pmp->flags.is_submenu) {
			 ...
			 */
			get_prefered_popup_position(pmp->parent_menu,
			    pmp->menu, &x, &y, &prefer_left_submenus);
		} else {
			fscreen_scr_arg fscr;
			...
			fscr.xypos.x = x;
			fscr.xypos.y = y;
			FScreenGetScrRect(&fscr, FSCREEN_XYPOS, &scr_x,
			    &scr_y, &scr_w, &scr_h);
			x -= menudim_middle_x_offset(&MR_DIM(pmp->menu));
			y -= menuitem_middle_y_offset(MR_FIRST_ITEM(pmp->
				menu), MR_STYLE(pmp->menu));
			if (x < scr_x) {
				x = scr_x;
			}
			if (y < scr_y) {
	...
	for (do_menu_interaction = True; do_menu_interaction == True;) {
		...
		if (!pmp->flags.is_submenu && x_start >= 0 && y_start >= 0 &&
			 ...
			 * * end it
			 */
			FWarpPointer(dpy, 0, Scr.Root, 0, 0, mon->coord.w,
		...
		if (!pmp->flags.is_submenu) {
			if (pmret->rc == MENU_DONE) {
				...
				last_saved_pos_hints.
				    flags.do_ignore_pos_hints = False;
				last_saved_pos_hints.
				    flags.is_last_menu_pos_hints_valid =
				    False;
			}
			if (indirect_depth == 0) {
				last_saved_pos_hints.
...
Bool
menu_expose(XEvent *event, MvwmWindow *fw)
{
	MenuRoot       *mr = NULL;
 ...
 */
void
repaint_transparent_menu(MenuRepaintTransparentParameters *prtm,
    Bool first, int x, int y, int end_x, int end_y, Bool is_bg_set)
{
	...
	for (mi = MR_FIRST_ITEM(mr); mi != NULL; mi = MI_NEXT_ITEM(mi)) {
		...
		get_menu_paint_item_parameters(&mpip, mr, NULL, prtm->fw,
		    NULL, True);
		mpip.flags.is_first_item = (MR_FIRST_ITEM(mr) == mi);
		menuitem_paint(mi, &mpip);
	}
...
DestroyMenu(MenuRoot *mr, Bool do_recreate, Bool is_command_request)
{
	...
	if (MR_MAPPED_COPIES(mr) > 0 &&
		 ...
		 */
		mvwm_msg(ERR, "DestroyMenu", "Menu %s is in use",
		    MR_NAME(mr));
		return False;
	}
	...
	if (MR_COPIES(mr) == 0) {
		...
		free(MR_NAME(mr));
		if (MR_SIDEPIC(mr)) {
			PDestroyMvwmPicture(dpy, MR_SIDEPIC(mr));
		}
 ...
 */
void
AddToMenu(MenuRoot *mr, char *item, char *action, Bool fPixmapsOk,
    Bool fNoPlus, Bool is_continuation_item)
{
	MenuItem       *tmp;
	char           *start;
	char           *end;
	...
	for (i = 0; i < MAX_MENU_ITEM_LABELS; i++, start = end) {
		...
		if (*end) {
			 ...
			 */
			MI_LABEL(tmp)[i] = mvwm_malloc(end - start + 1);
...
UpdateMenuColorset(int cset)
{
	...
	for (t = Scr.MvwmRoot.next; t != NULL; t = t->next) {
		...
		if (IS_TEAR_OFF_MENU(t) &&
		    XFindContext(dpy, FW_W(t), MenuContext,
			(caddr_t *) & mr) != XCNOENT && (ms = MR_STYLE(mr))) {
			if (ST_HAS_MENU_CSET(ms) && ST_CSET_MENU(ms) == cset) {
...
change_mr_menu_style(MenuRoot *mr, char *stylename)
{
	...
	if (MR_MAPPED_COPIES(mr) != 0) {
		mvwm_msg(ERR, "ChangeMenuStyle", "menu %s is in use",
		    MR_NAME(mr));
	} else {
		MR_STYLE(mr) = ms;
		MR_IS_UPDATED(mr) = 1;
...
add_another_menu_item(char *action)
{
	...
	if (MR_MAPPED_COPIES(mr) != 0) {
		mvwm_msg(ERR, "add_another_menu_item", "menu is in use");
		return;
	}
 ...
 */
char           *
get_menu_options(char *action, Window w, MvwmWindow *fw, XEvent *e,
    MenuRoot *mr, MenuItem *mi, MenuOptions * pops)
{
	...
	while (action != NULL && *action != 0 && once_more) {
		...
		if (StrEquals(tok, "context")) {
			...
			arg.mouse_ev = NULL;
			arg.name = screen;
			FScreenGetScrRect(&arg, FSCREEN_BY_NAME, &sx, &sy,
			    &sw, &sh);
		...
		if (fRectangleContext) {
			...
			!XGetGeometry(dpy, context_window, &JunkRoot, &JunkX,
			    ...
			    (unsigned int *) &JunkBW,
			    (unsigned int *) &JunkDepth) ||
			!XTranslateCoordinates(dpy, context_window, Scr.Root,
			    0, 0, &x, &y, &JunkChild))) {
			/*
			...
			if (!pops->pos_hints.has_screen_origin) {
				...
				if (fXineramaRoot) {
					...
					if (!fev_get_evpos_or_query(dpy,
						context_window, e,
						&pops->
						pos_hints.screen_origin_x,
						&pops->
						pos_hints.screen_origin_y)) {
						pops->
						    pos_hints.screen_origin_x
						    = 0;
						pops->
		 ...
		 */
		taction =
		    get_one_menu_position_argument(naction, x, width,
		...
		naction =
		    get_one_menu_position_argument(taction, y, height,
...
/*!!!static*/
mloop_trigger_type_t __mloop_get_trigger(mloop_trigger_t * ret_trigger,
    mloop_timeouts_t * io_timeouts,
    const mloop_get_trigger_input_t * const in,
    {
	    if (in_out->in_flags->do_propagate_event_down) {
	    return MTR_PROPAGATE_XEVENT_DOWN;}
	    else
    if (in_out->in_flags->do_propagate_event_up) {
	    if (a != b) {
	 ...
	 * !!!static
	 */
    void __menu_loop_new(MenuParameters *pmp, MenuReturn *pmret,
...
#endif
	    mloop_evh_data_t med;
	    mloop_static_info_t msi; MenuOptions mops; Bool is_finished;
	    /*
	     * !!!init menu loop
	     */
	    __mloop_init(pmp, pmret, &mei, &med, &msi, &mops);
...
case MTR_XEVENT:
			     ...
			     * !!!handle event
			     */
break; case MTR_FAKE_ENTER_ITEM:
			    /*
			     * !!!fake enter item
			     */
break; case MTR_PROPAGATE_XEVENT_UP:
			    /*
			     * !!!handle propagation
			     */
break; case MTR_PROPAGATE_XEVENT_DOWN:
			    /*
			     * !!!handle propagation
			     */
break; case MTR_POPUP_TIMEOUT:
			    /*
			     * !!!handle popup
			     */
break; case MTR_POPDOWN_TIMEOUT:
			    /*
			     * !!!handle popdown
			     */
		    break;}
		    ...
		    switch (mloop_ret) {
case MENU_MLOOP_RET_LOOP:
continue; case MENU_MLOOP_RET_END:
is_finished = True; break; default:
		    break;}
		    /*
		    ...
		    if (med.mi != NULL) {
		    mloop_ret =
			    __mloop_handle_action_with_mi(pmp, pmret,
				pdkp, &mei, &med, &msi, &mops);}
		    else
		    {
		    mloop_ret =
			    __mloop_handle_action_without_mi(pmp,
				pmret, pdkp, &mei, &med, &msi, &mops);}
		    if (mloop_ret == MENU_MLOOP_RET_END) {
		    is_finished = True;}
		    XFlush(dpy);
#endif
	    }
    __mloop_exit(pmp, pmret, pdkp, &mei, &med, &msi, &mops); return;}
#endif
