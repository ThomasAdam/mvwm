...
static void get_menu_repaint_transparent_parameters(
	MenuRepaintTransparentParameters *pmrtp, MenuRoot *mr, MvwmWindow *fw)
{
	pmrtp->mr = mr;
	pmrtp->fw = fw;
...
static void animated_move_back(
	MenuRoot *mr, Bool do_warp_pointer, MvwmWindow *fw)
{
		...
		if (menu_height > MR_SCREEN_HEIGHT(msp->menu))
		{
			...
			if (MR_MISSING_SUBMENU_FUNC(msp->menu))
			{
				MR_MISSING_SUBMENU_FUNC(menuContinuation) =
					mvwm_strdup(MR_MISSING_SUBMENU_FUNC(
							   msp->menu));
			}
...
static void get_menu_paint_item_parameters(
	...
	MvwmWindow *fw, XEvent *pevent, Bool do_redraw_menu_border)
{
	mpip->ms = MR_STYLE(mr);
	mpip->w = MR_WINDOW(mr);
...
static void paint_menu(
	MenuRoot *mr, XEvent *pevent, MvwmWindow *fw)
{
	MenuItem *mi;
	MenuStyle *ms = MR_STYLE(mr);
	int bw = MST_BORDER_WIDTH(mr);
...
static void select_menu_item(
	MenuRoot *mr, MenuItem *mi, Bool select, MvwmWindow *fw)
{
...
void menu_close_tear_off_menu(MvwmWindow *fw)
{
	MenuRoot *mr;
	MenuParameters mp;
	const exec_context_t *exc;
...
Bool menu_redraw_transparent_tear_off_menu(MvwmWindow *fw, Bool pr_only)
{
	MenuRoot *mr;
	MenuStyle *ms = NULL;
	int cs;
...
Bool menu_expose(XEvent *event, MvwmWindow *fw)
{
	MenuRoot *mr = NULL;
...
Bool DestroyMenu(MenuRoot *mr, Bool do_recreate, Bool is_command_request)
{
	    ...
	    (is_command_request || MR_COPIES(mr) == 1))
	{
		...
		mvwm_msg(ERR, "DestroyMenu", "Menu %s is in use", MR_NAME(mr));
		return False;
	}
...
char *get_menu_options(
	char *action, Window w, MvwmWindow *fw, XEvent *e, MenuRoot *mr,
