 ...
 */
static MvwmWindow *
Circulate(MvwmWindow *sf, char *action, int Direction, char **restofline)
{
	int             pass = 0;
	MvwmWindow     *fw, *found = NULL;
	...
	for (pass = 0; pass < 3 && !found; pass++) {
		while (fw && !found && fw != &Scr.MvwmRoot) {
			/*
		...
		if (fw == NULL || fw == &Scr.MvwmRoot) {
			if (Direction > 0) {
				/*
				 * Go to end of list
				 */
				for (fw = Scr.MvwmRoot.next; fw && fw->next;
				    fw = fw->next) {
					/*
					 * nop
					 */
...
select_cmd(F_CMD_ARGS)
{
	...
	if (MatchesConditionMask(fw, &mask) && restofline) {
		...
		execute_function_override_wcontext(cond_rc, exc, restofline,
		    0, C_WINDOW);
	...
	while (condition) {
		...
		if (StrEquals(cond, "AcceptsFocus")) {
			...
			if (on) {
				if (mask->placed_by_button_mask &
				    mask->
				    placed_by_button_set_mask & ~(1 << 2)) {
					mvwm_msg(WARN, "PlacedByButton3",
					    "Condition always False.");
				}
		...
		} else if (StrEquals(cond, "PlacedByMvwm")) {
			SET_PLACED_BY_MVWM(mask, on);
			SETM_PLACED_BY_MVWM(mask, 1);
			...
			mask->my_flags.needs_current_desk_and_global_page =
			    on;
		...
		if (tmp && *tmp) {
			mvwm_msg(OLD, "CreateConditionMask",
			...
			if (next_condition && *next_condition) {
				next_condition =
				    GetNextFullOption(next_condition,
				    &allocated_condition);
			}
 ...
 */
Bool
MatchesConditionMask(MvwmWindow *fw, WindowConditionMask *mask)
{
	int             does_match;
	int             is_on_desk;
	int             is_on_page;
	...
	if (!IS_UNCLOSABLE(mask) &&
		...
		&& !is_function_allowed(F_DELETE, NULL, fw, RQORIG_PROGRAM_US,
		    False)
		&& !is_function_allowed(F_DESTROY, NULL, fw,
	...
	if (mask->my_flags.do_check_desk ||
		...
		is_on_desk = (fw->Desk == fw->m->virtual_scr.CurrentDesk);
	}
	is_on_page = 1;
	if (mask->my_flags.do_check_page ||
		...
		if (FScreenIsEnabled() && !mask->my_flags.do_not_check_screen) {
			is_on_page =
			    ! !FScreenIsRectangleOnScreen(NULL,
			    FSCREEN_CURRENT, &(fw->g.frame));
		} else {
			is_on_page = ! !IsRectangleOnThisPage(fw->m,
	...
	if (mask->my_flags.do_check_global_page ||
	    mask->my_flags.do_check_desk_and_global_page) {
		is_on_global_page = ! !IsRectangleOnThisPage(fw->m,
		    &(fw->g.frame), fw->m->virtual_scr.CurrentDesk);
	}
	...
	if (mask->my_flags.do_check_desk_and_page) {
		...
		is_on_desk_and_global_page = (is_on_desk
		    && is_on_global_page);
...
direction_cmd(F_CMD_ARGS, Bool is_scan)
{
	...
	if (dir == -1 || dir > DIR_ALL_MASK) {
		mvwm_msg(ERR, "Direction", "Invalid direction %s",
		    (tmp) ? tmp : "");
		if (cond_rc != NULL) {
			cond_rc->rc = COND_RC_ERROR;
		}
	...
	if (is_scan) {
		...
		tmp = PeekToken(action, &action);
		if (!tmp) {
			mvwm_msg(ERR, "Direction",
			    "Missing minor direction %s", (tmp) ? tmp : "");
			if (cond_rc != NULL) {
				cond_rc->rc = COND_RC_ERROR;
			}
			return;
		...
		if (dir2 == -1 || dir2 > DIR_NW ||
			...
			mvwm_msg(ERR, "Direction",
			    "Invalid minor direction %s", (tmp) ? tmp : "");
			if (cond_rc != NULL) {
				cond_rc->rc = COND_RC_ERROR;
			}
			return;
	...
	fw_best = NULL;
	best_score = -1;
	for (tfw = Scr.MvwmRoot.next; tfw != NULL; tfw = tfw->next) {
		/*
	...
	if (fw_best) {
		...
		execute_function_override_window(cond_rc, exc, restofline, 0,
		    fw_best);
	} else if (cond_rc != NULL) {
		cond_rc->rc = COND_RC_NO_MATCH;
	}
...
static int
__rc_matches_rcstring_consume(char **ret_rest, cond_rc_t *cond_rc,
    char *action)
{
	...
	if (flags == NULL) {
		...
		if (StrEquals(flags, "1") || StrEquals(flags, "match")) {
			 ...
			 */
			fprintf(stderr, "Unrecognised condition \"%s\" in"
			    " TestRc command.\n", flags);
		}
	}
	if (orig_flags != NULL) {
...
CMD_PointerWindow(F_CMD_ARGS)
{
	...
	ecc.w.fw = get_pointer_mvwm_window();
...
CMD_All(F_CMD_ARGS)
{
	...
	if (!use_stack) {
		...
		for (t = Scr.MvwmRoot.stack_next; t && t != &Scr.MvwmRoot;
		    t = t->stack_next) {
			if (MatchesConditionMask(t, &mask)) {
				g[num++] = t;
				does_any_window_match = True;
...
CMD_WindowId(F_CMD_ARGS)
{
	...
	if (!t) {
		/*
		 * The window is not managed by mvwm. Still some functions may
		 * * work on it.
		 */
		if (use_condition) {
			if (cond_rc != NULL) {
			...
			if (action != NULL) {
				...
				exc2 = exc_clone_context(exc, &ecc,
				    ECC_FW | ECC_W | ECC_WCONTEXT);
				execute_function(cond_rc, exc2, action,
				    FUNC_IS_UNMANAGED);
				exc_destroy_context(exc2);
			}
		} else {
...
/* match_version() - compare $version against this version of mvwm
 * using the operator specified by $operator. */
static Bool
match_version(char *version, char *operator)
{
	static int      mvwm_version = -1;
	...
	if (v < 0) {
		fprintf(stderr, "match_version: Invalid version: %s\n",
		    version);
		return False;
	}
	if (strcmp(operator, ">=") == 0) {
		return mvwm_version >= v;
		...
		return (v == mvwm_version);
	} else if (strcmp(operator, "!=") == 0) {
		return (v != mvwm_version);
	} else {
		fprintf(stderr, "match_version: Invalid operator: %s\n",
		    operator);
	}
...
CMD_Test(F_CMD_ARGS)
{
	...
	while (condition) {
		...
		if (StrEquals(cond, "True")) {
			...
			if (dirname) {
				dir =
				    gravity_parse_dir_argument(dirname, NULL,
				    DIR_NONE);
				if (dir == DIR_NONE) {
