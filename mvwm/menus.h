...
Bool            menu_redraw_transparent_tear_off_menu(MvwmWindow *fw,
    Bool pr_only);
void            do_menu(struct MenuParameters *pmp, struct MenuReturn *pret);
char           *get_menu_options(char *action, Window w, MvwmWindow *fw,
    XEvent *e, struct MenuRoot *mr, struct MenuItem *mi,
...
void            update_transparent_menu_bg(struct
    MenuRepaintTransparentParameters *prtm, int current_x, int current_y,
    int step_x, int step_y, int end_x, int end_y);
void            repaint_transparent_menu(struct
    MenuRepaintTransparentParameters *prtmp, Bool first, int x, int y,
    int end_x, int end_y, Bool is_bg_set);
Bool            menu_expose(XEvent *event, MvwmWindow *fw);
int             menu_binding(Display *dpy, binding_t type, int button,
    KeySym keysym, int context, int modifier, char *action, char *menuStyle);
#endif /* _MENUS_ */
