...
__drag_viewport(const exec_context_t *exc, int scroll_speed)
{
	...
	while (!is_finished) {
		 ...
		 */
		if (e.type == KeyPress) {
			Keyboard_shortcuts(&e, NULL, NULL, NULL,
			    ButtonRelease);
		}
		switch (e.type) {
		case KeyPress:
...
UnmapDesk(struct monitor *m, int desk, Bool grab)
{
	...
	for (t = get_prev_window_in_stack_ring(&Scr.MvwmRoot);
	    t != &Scr.MvwmRoot; t = get_prev_window_in_stack_ring(t)) {
		if (t->m != m)
			continue;
...
{
	MvwmWindow     *t;
	MvwmWindow     *FocusWin = NULL;
	MvwmWindow     *StickyWin = NULL;
	MvwmWindow     *sf = get_focus_window();
	...
	for (t = get_next_window_in_stack_ring(&Scr.MvwmRoot);
	    t != &Scr.MvwmRoot; t = get_next_window_in_stack_ring(t)) {
		if (t->m != m)
			continue;
    ...
    Bool do_continue_previous, int delay)
{
	...
	*delta_y = 0;
	...
	if (mon->virtual_scr.Vx + *delta_x < 0) {
		...
		if (!(scr_flags.do_edge_wrap_x)) {
			*delta_x =
			    mon->virtual_scr.VxMax - mon->virtual_scr.Vx;
			*xl = x - *delta_x;
		} else {
			*delta_x -= mon->virtual_scr.VxMax + mon->coord.w;
			*xl = x + *delta_x % mon->coord.w - HorWarpSize;
	...
	if (mon->virtual_scr.Vy + *delta_y < 0) {
		...
		if (!(scr_flags.do_edge_wrap_y)) {
			*delta_y =
			    mon->virtual_scr.VyMax - mon->virtual_scr.Vy;
			*yt = y - *delta_y;
		} else {
			*delta_y -= mon->virtual_scr.VyMax + mon->coord.h;
			*yt = y + *delta_y % mon->coord.h - VertWarpSize;
...
checkPanFrames(void)
{
	...
	if (do_unmap_l) {
		...
		if (edge_thickness != last_edge_thickness) {
			XResizeWindow(dpy, Scr.PanFrameLeft.win,
	...
	if (do_unmap_t) {
		...
		if (edge_thickness != last_edge_thickness) {
			XResizeWindow(dpy, Scr.PanFrameTop.win, m->coord.w,
			    edge_thickness);
		}
		if (!Scr.PanFrameTop.isMapped) {
...
initPanFrames(void)
{
	...
	attributes.cursor = Scr.MvwmCursors[CRS_TOP_EDGE];
	/*
	...
	TAILQ_FOREACH(m, &monitor_q, entry) {
		...
		attributes.cursor = Scr.MvwmCursors[CRS_LEFT_EDGE];
		Scr.PanFrameLeft.win =
		    XCreateWindow(dpy, Scr.Root, 0, 0, edge_thickness,
		    m->coord.h, 0, CopyFromParent, InputOnly, CopyFromParent,
		    valuemask, &attributes);
		attributes.cursor = Scr.MvwmCursors[CRS_RIGHT_EDGE];
		Scr.PanFrameRight.win =
		    XCreateWindow(dpy, Scr.Root, m->coord.w - edge_thickness,
		    0, edge_thickness, m->coord.h, 0, CopyFromParent,
		    InputOnly, CopyFromParent, valuemask, &attributes);
		attributes.cursor = Scr.MvwmCursors[CRS_BOTTOM_EDGE];
		Scr.PanFrameBottom.win =
		    XCreateWindow(dpy, Scr.Root, 0,
		    m->coord.h - edge_thickness, m->coord.w, edge_thickness,
		    0, CopyFromParent, InputOnly, CopyFromParent, valuemask,
		    &attributes);
		Scr.PanFrameTop.isMapped = Scr.PanFrameLeft.isMapped =
		    Scr.PanFrameRight.isMapped = Scr.PanFrameBottom.isMapped =
		    False;
	}
...
MoveViewport(struct monitor *m, int newx, int newy, Bool grab)
{
	...
	if (deltax || deltay) {
		BroadcastPacket(M_NEW_PAGE, 7, (long) m->virtual_scr.Vx,
		...
		for (t1 = get_prev_window_in_stack_ring(&Scr.MvwmRoot);
		    t1 != &Scr.MvwmRoot;
 ...
 */
void
do_move_window_to_desk(MvwmWindow *fw, int desk)
{
	if (fw == NULL) {
		return;
	}
...
Bool
get_page_arguments(struct monitor *m, char *action, int *page_x, int *page_y)
{
	int             val[2];
	int             suffix[2];
	char           *token;
...
CMD_EdgeResistance(F_CMD_ARGS)
{
	...
	if (n == 1) {
		...
		if (n == 2) {
			sprintf(stylecmd2, "Style * EdgeMoveResistance %d",
			    val[1]);
		} else {
			sprintf(stylecmd2, "Style * EdgeMoveResistance %d %d",
			    val[1], val[2]);
		}
		mvwm_msg(OLD, "CMD_EdgeResistance",
		...
		mvwm_msg(ERR, "CMD_EdgeResistance",
...
void
CMD_XineramaPrimaryScreen(F_CMD_ARGS)
{
	if (FScreenIsEnabled()) {
		scr_flags.do_need_window_update = True;
		scr_flags.has_xinerama_state_changed = True;
...
CMD_Scroll(F_CMD_ARGS)
{
	...
	if (((val1 <= -100000) || (val1 >= 100000))
		...
		xpixels = (m->virtual_scr.VxMax / m->coord.w + 1) *
		    m->coord.w;
		x %= xpixels;
		y += m->coord.h * (1 + ((x - m->virtual_scr.VxMax -
			    1) / xpixels));
		if (y > m->virtual_scr.VyMax) {
	...
	if (((val2 <= -100000) || (val2 >= 100000))
	    && (y > m->virtual_scr.VyMax)) {
		int             ypixels =
		    (m->virtual_scr.VyMax / m->coord.h + 1) * m->coord.h;
		y %= ypixels;
		x += m->coord.w * (1 + ((y - m->virtual_scr.VyMax -
			    1) / ypixels));
		if (x > m->virtual_scr.VxMax) {
...
CMD_DesktopName(F_CMD_ARGS)
{
	...
	if (!fMvwmInStartup) {
		...
		if (action != NULL && *action && *action != '\n') {
			...
			sprintf(msg, "DesktopName %d %s %d", desk,
