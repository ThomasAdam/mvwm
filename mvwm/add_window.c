...
static void delete_client_context(MvwmWindow *fw)
{
	MvwmWindow *cw;
	...
	if (XFindContext(
		    dpy, FW_W(fw), MvwmContext, (caddr_t *)&cw) !=
	    XCNOENT && cw == fw)
	{
...
static void CaptureOneWindow(
	const exec_context_t *exc, MvwmWindow *fw, Window window,
	Window keep_on_top_win, Window parent_win, Bool is_recapture)
{
	...
	if (XFindContext(dpy, window, MvwmContext, (caddr_t *)&fw) != XCNOENT)
	{
...
static void setup_window_structure(
	MvwmWindow **pfw, Window w, MvwmWindow *ReuseWin)
{
...
static void setup_name_count(MvwmWindow *fw, Bool is_icon)
{
	MvwmWindow *t;
	int count = 0;
	int win_count;
...
static char *interpolate_titleformat_name(MvwmWindow *fw, window_style *style,
		Bool is_icon)
{
...
static void setup_class_and_resource(MvwmWindow *fw)
{
...
static void setup_window_attr(
	MvwmWindow *fw, XWindowAttributes *ret_attr)
{
...
static void destroy_window_font(MvwmWindow *fw)
{
...
static void destroy_icon_font(MvwmWindow *fw)
{
...
static void adjust_mvwm_internal_windows(MvwmWindow *fw)
{
	if (fw == Scr.Hilite)
	{
		Scr.Hilite = NULL;
...
static void broadcast_mini_icon(MvwmWindow *fw)
{
	if (!FMiniIconsSupported)
	{
		return;
...
static void setup_mini_icon(MvwmWindow *fw, window_style *pstyle)
{
	...
	{
		fpa.mask = 0;
		fw->mini_icon = PCacheMvwmPicture(
...
void setup_icon_background_parameters(MvwmWindow *fw, window_style *pstyle)
{
...
void setup_icon_title_parameters(MvwmWindow *fw, window_style *pstyle)
{
...
static void setup_title_window(
	MvwmWindow *fw, int valuemask, XSetWindowAttributes *pattributes)
{
...
static void destroy_title_window(MvwmWindow *fw, Bool do_only_delete_context)
{
	...
	short buttons)
{
	...
	for (i = 0; i < NUMBER_OF_TITLE_BUTTONS; i++)
	{
		...
		if (FW_W_BUTTON(fw, i) == None && has_button)
		{
			...
			XSaveContext(
				dpy, FW_W_BUTTON(fw, i), MvwmContext,
				(caddr_t)fw);
		}
...
static void destroy_button_windows(MvwmWindow *fw, Bool do_only_delete_context)
{
	int i;
	Bool is_deleted = False;
...
static void setup_parent_window(MvwmWindow *fw)
{
	size_borders b;
...
static void setup_resize_handle_cursors(MvwmWindow *fw)
{
	unsigned long valuemask;
	XSetWindowAttributes attributes;
	int i;
...
static void setup_resize_handle_windows(MvwmWindow *fw)
{
	unsigned long valuemask;
	XSetWindowAttributes attributes;
	int i;
	...
	for (i = 0; i < 4; i++)
	{
		...
		XSaveContext(
			dpy, FW_W_CORNER(fw, i), MvwmContext, (caddr_t)fw);
		attributes.win_gravity = s_grav[i];
		FW_W_SIDE(fw, i) = XCreateWindow(
			dpy, FW_W_FRAME(fw), -1, -1, 1, 1, 0, Pdepth,
			InputOutput, Pvisual, valuemask, &attributes);
		XSaveContext(dpy, FW_W_SIDE(fw, i), MvwmContext, (caddr_t)fw);
	}
...
static void destroy_resize_handle_windows(
	MvwmWindow *fw, Bool do_only_delete_context)
{
	int i;
...
static void change_resize_handle_windows(MvwmWindow *fw)
{
...
static void setup_frame_stacking(MvwmWindow *fw)
{
	int i;
	int n;
...
static void get_default_window_attributes(
	MvwmWindow *fw, unsigned long *pvaluemask,
...
static void setup_auxiliary_windows(
	MvwmWindow *fw, Bool setup_frame_and_parent, short buttons)
{
...
static void destroy_auxiliary_windows(
	MvwmWindow *fw, Bool destroy_frame_and_parent)
{
...
static void setup_icon(MvwmWindow *fw, window_style *pstyle)
{
...
static void destroy_icon(MvwmWindow *fw)
{
...
static void setup_icon_boxes(MvwmWindow *fw, window_style *pstyle)
{
	icon_boxes *ib;
...
static void destroy_icon_boxes(MvwmWindow *fw)
{
	if (fw->IconBoxes)
	{
...
static void setup_layer(MvwmWindow *fw, window_style *pstyle)
{
	MvwmWindow *tf;
	int layer;
...
static void destroy_mini_icon(MvwmWindow *fw)
{
	if (fw->mini_icon)
	{
...
static void setup_key_and_button_grabs(MvwmWindow *fw)
{
#ifdef BUGS_ARE_COOL
...
static void __add_window_handle_x_resources(MvwmWindow *fw)
{
	int client_argc = 0;
...
void setup_window_name(MvwmWindow *fw)
{
...
void setup_title_geometry(
	MvwmWindow *fw, window_style *pstyle)
{
	int width;
	int offset;
...
void setup_window_font(
	MvwmWindow *fw, window_style *pstyle, Bool do_destroy)
{
	/* get rid of old font */
	if (do_destroy)
	{
	...
	if (!IS_WINDOW_FONT_LOADED(fw))
	{
		if (S_HAS_WINDOW_FONT(SCF(*pstyle)) &&
		    ...
		    (fw->title_font =
		     FlocaleLoadFont(dpy, SGET_WINDOW_FONT(*pstyle), "mvwm")))
		{
	...
	MvwmWindow *fw, window_style *pstyle, Bool do_destroy)
{
	...
	if (!IS_ICON_FONT_LOADED(fw))
	{
		if (S_HAS_ICON_FONT(SCF(*pstyle)) && SGET_ICON_FONT(*pstyle) &&
		    (fw->icon_font =
		     FlocaleLoadFont(dpy, SGET_ICON_FONT(*pstyle), "mvwm")))
		{
...
void setup_style_and_decor(
	MvwmWindow *fw, window_style *pstyle, short *buttons)
{
...
Bool validate_transientfor(MvwmWindow *fw)
{
	...
	else if (XFindContext(dpy, w, MvwmContext, (caddr_t *)&cw) != XCNOENT)
	{
		if (cw == fw)
		{
		...
		while (XFindContext(
			       dpy, FW_W_TRANSIENTFOR(cw), MvwmContext,
			       (caddr_t *)&cw) != XCNOENT &&
		       IS_TRANSIENT(cw))
		{
 ...
 *
 */
void FetchWmProtocols(MvwmWindow *tmp)
{
...
void GetWindowSizeHints(MvwmWindow *fw)
{
 ...
 *
 */
void free_window_names(MvwmWindow *fw, Bool nukename, Bool nukeicon)
{
	if (!fw)
	{
		return;
 ...
 *
 */
void destroy_window(MvwmWindow *fw)
{
...
void RestoreWithdrawnLocation(
	MvwmWindow *fw, Bool is_restart_or_recapture, Window parent)
{
	int w2,h2;
	unsigned int mask;
	XWindowChanges xwc;
