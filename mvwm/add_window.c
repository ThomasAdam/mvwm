...
static void
delete_client_context(MvwmWindow *fw)
{
	MvwmWindow     *cw;
	 ...
	 * * by another MvwmWindow structure in the mean time.
	 */
	if (XFindContext(dpy, FW_W(fw), MvwmContext, (caddr_t *) & cw) !=
	    XCNOENT && cw == fw) {
		XDeleteContext(dpy, FW_W(fw), MvwmContext);
	}
...
static void
CaptureOneWindow(const exec_context_t *exc, MvwmWindow *fw, Window window,
    Window keep_on_top_win, Window parent_win, Bool is_recapture)
{
	...
	if (IS_SCHEDULED_FOR_DESTROY(fw)) {
		/*
		 * Mvwm might crash in complex functions if we really try to
	...
	if (XFindContext(dpy, window, MvwmContext,
		(caddr_t *) & fw) != XCNOENT) {
		 ...
		 */
		if (check_if_mvwm_window_exists(fw)) {
			if (!fMvwmInStartup) {
...
hide_screen(Bool do_hide, Window *ret_hide_win, Window *ret_parent_win)
{
	...
	if (do_hide) {
		...
		xswa.cursor = Scr.MvwmCursors[CRS_WAIT];
		...
		hide_win =
		    XCreateWindow(dpy, Scr.Root, 0, 0, m->coord.w, m->coord.h,
		    0, Pdepth, InputOutput, Pvisual, valuemask, &xswa);
		if (hide_win) {
			/*
 ...
 *		put a mvwm frame on the window
 *
 *  Returned Value:
...
static void
do_recapture(F_CMD_ARGS, Bool fSingle)
{
...
static void
setup_window_structure(MvwmWindow **pfw, Window w, MvwmWindow *ReuseWin)
{
	MvwmWindow      save_state;
	MvwmWindow     *savewin = NULL;

	/*
	 * Allocate space for the MvwmWindow struct, or reuse an
	 ...
	 */
	memset(*pfw, '\0', sizeof(MvwmWindow));
	FW_W(*pfw) = w;
	if (savewin != NULL) {
		...
		SET_PLACED_BY_MVWM(*pfw, IS_PLACED_BY_MVWM(savewin));
		SET_HAS_EWMH_WM_ICON_HINT(*pfw,
		    HAS_EWMH_WM_ICON_HINT(savewin));
		(*pfw)->ewmh_mini_icon_width = savewin->ewmh_mini_icon_width;
		(*pfw)->ewmh_mini_icon_height =
...
static void
setup_name_count(MvwmWindow *fw, Bool is_icon)
{
	MvwmWindow     *t;
	int             count = 0;
	int             win_count;
	...
	while (!done) {
		...
		for (t = Scr.MvwmRoot.next; t != NULL; t = t->next) {
			if (t == fw) {
				continue;
			}
...
static char    *
interpolate_titleformat_name(MvwmWindow *fw, window_style *style,
    Bool is_icon)
{
	...
	if (is_icon) {
		...
		format = (style->flags.has_title_format_string) ?
		    SGET_TITLE_FORMAT_STRING(*style) : DEFAULT_TITLE_FORMAT;
	}
	...
	while (*format) {
		...
		case 'n':
			if (strlen(stringbuf) +
			    strlen(fw->name.name) > MAX_VISIBLE_NAME_LEN) {
				mvwm_msg(WARN,
...
static void
setup_class_and_resource(MvwmWindow *fw)
{
	/*
...
static void
setup_window_attr(MvwmWindow *fw, XWindowAttributes *ret_attr)
{
	if (XGetWindowAttributes(dpy, FW_W(fw), ret_attr) == 0) {
		/*
		 * can't happen because mvwm has grabbed the server and does
...
static void
destroy_window_font(MvwmWindow *fw)
{
...
static void
destroy_icon_font(MvwmWindow *fw)
{
...
static void
adjust_mvwm_internal_windows(MvwmWindow *fw)
{
	if (fw == Scr.Hilite) {
		Scr.Hilite = NULL;
	}
	...
	if (fw == Scr.StolenFocusMvwmWin) {
		Scr.StolenFocusWin = None;
		Scr.StolenFocusMvwmWin = NULL;
	}
...
static void
broadcast_mini_icon(MvwmWindow *fw)
{
	if (!FMiniIconsSupported) {
		return;
	}
	if (fw->mini_pixmap_file && fw->mini_icon) {
		BroadcastMvwmPicture(M_MINI_ICON,
...
static void
setup_mini_icon(MvwmWindow *fw, window_style *pstyle)
{
	...
	if (fw->mini_pixmap_file) {
		fpa.mask = 0;
		fw->mini_icon =
		    PCacheMvwmPicture(dpy, Scr.NoFocusWin, NULL,
...
/*
 * Copy icon size limits from window_style structure to MvwmWindow
 * structure.
 */
void
setup_icon_size_limits(MvwmWindow *fw, window_style *pstyle)
{
...
void
setup_icon_background_parameters(MvwmWindow *fw, window_style *pstyle)
{
...
void
setup_icon_title_parameters(MvwmWindow *fw, window_style *pstyle)
{
...
void
setup_numeric_vals(MvwmWindow *fw, window_style *pstyle)
{
	/****** window shading ******/
...
static void
setup_frame_window(MvwmWindow *fw)
{
	...
	/*
	 * This adds preliminary support for ARGB windows in mvwm. It should
	...
	attributes.cursor = Scr.MvwmCursors[CRS_DEFAULT];
	 ...
	 */
	FW_W_FRAME(fw) =
	    XCreateWindow(dpy, Scr.Root, fw->g.frame.x, fw->g.frame.y,
	    fw->g.frame.width, fw->g.frame.height, 0, depth, InputOutput,
	    visual, valuemask, &attributes);
	XSaveContext(dpy, FW_W(fw), MvwmContext, (caddr_t) fw);
	XSaveContext(dpy, FW_W_FRAME(fw), MvwmContext, (caddr_t) fw);

	return;
}

static void
setup_title_window(MvwmWindow *fw, int valuemask,
    XSetWindowAttributes *pattributes)
{
	valuemask |= CWCursor | CWEventMask;
	pattributes->cursor = Scr.MvwmCursors[CRS_TITLE];
	...
	XSaveContext(dpy, FW_W_TITLE(fw), MvwmContext, (caddr_t) fw);

	return;
}

static void
destroy_title_window(MvwmWindow *fw, Bool do_only_delete_context)
{
	if (!do_only_delete_context) {
		XDestroyWindow(dpy, FW_W_TITLE(fw));
		FW_W_TITLE(fw) = None;
	}
	XDeleteContext(dpy, FW_W_TITLE(fw), MvwmContext);
	XFlush(dpy);
	FW_W_TITLE(fw) = None;
...
static void
change_title_window(MvwmWindow *fw, int valuemask,
    XSetWindowAttributes *pattributes)
{
...
static void
setup_button_windows(MvwmWindow *fw, int valuemask,
    XSetWindowAttributes *pattributes, short buttons)
{
	...
	valuemask |= CWCursor | CWEventMask;
	pattributes->cursor = Scr.MvwmCursors[CRS_SYS];
	...
	for (i = 0; i < NUMBER_OF_TITLE_BUTTONS; i++) {
		...
		if (FW_W_BUTTON(fw, i) == None && has_button) {
			FW_W_BUTTON(fw, i) =
			    XCreateWindow(dpy, FW_W_FRAME(fw), 0, 0, 1, 1, 0,
			    Pdepth, InputOutput, Pvisual,
			    valuemask, pattributes);
			XSaveContext(dpy, FW_W_BUTTON(fw, i), MvwmContext,
			    (caddr_t) fw);
			...
			XDeleteContext(dpy, FW_W_BUTTON(fw, i), MvwmContext);
			is_deleted = True;
...
static void
destroy_button_windows(MvwmWindow *fw, Bool do_only_delete_context)
{
	int             i;
	Bool            is_deleted = False;
...
static void
change_button_windows(MvwmWindow *fw, int valuemask,
    XSetWindowAttributes *pattributes, short buttons)
{
	if (HAS_TITLE(fw)) {
...
static void
setup_parent_window(MvwmWindow *fw)
{
	size_borders    b;
	...
	FW_W_PARENT(fw) =
	    XCreateWindow(dpy, FW_W_FRAME(fw), b.top_left.width,
	...
	XSaveContext(dpy, FW_W_PARENT(fw), MvwmContext, (caddr_t) fw);

	return;
}

static void
setup_resize_handle_cursors(MvwmWindow *fw)
{
	unsigned long   valuemask;
	XSetWindowAttributes attributes;
	int             i;
	...
	for (i = 0; i < 4; i++) {
		if (HAS_HANDLES(fw)) {
			attributes.cursor = Scr.MvwmCursors[CRS_TOP_LEFT + i];
		}
		XChangeWindowAttributes(dpy, FW_W_CORNER(fw, i), valuemask,
		    &attributes);
		if (HAS_HANDLES(fw)) {
			attributes.cursor = Scr.MvwmCursors[CRS_TOP + i];
		}
...
static void
setup_resize_handle_windows(MvwmWindow *fw)
{
	unsigned long   valuemask;
	XSetWindowAttributes attributes;
	int             i;
	...
	for (i = 0; i < 4; i++) {
		...
		FW_W_CORNER(fw, i) =
		    XCreateWindow(dpy, FW_W_FRAME(fw), -1, -1, 1, 1, 0,
		    Pdepth, InputOutput, Pvisual, valuemask, &attributes);
		XSaveContext(dpy, FW_W_CORNER(fw, i), MvwmContext,
		    (caddr_t) fw);
		attributes.win_gravity = s_grav[i];
		FW_W_SIDE(fw, i) =
		    XCreateWindow(dpy, FW_W_FRAME(fw), -1, -1, 1, 1, 0,
		    Pdepth, InputOutput, Pvisual, valuemask, &attributes);
		XSaveContext(dpy, FW_W_SIDE(fw, i), MvwmContext,
		    (caddr_t) fw);
	}
...
static void
destroy_resize_handle_windows(MvwmWindow *fw, Bool do_only_delete_context)
{
	int             i;

	for (i = 0; i < 4; i++) {
		XDeleteContext(dpy, FW_W_SIDE(fw, i), MvwmContext);
		XDeleteContext(dpy, FW_W_CORNER(fw, i), MvwmContext);
		if (!do_only_delete_context) {
...
static void
change_resize_handle_windows(MvwmWindow *fw)
{
...
static void
setup_frame_stacking(MvwmWindow *fw)
{
	int             i;
	int             n;
...
static void
get_default_window_attributes(MvwmWindow *fw, unsigned long *pvaluemask,
    XSetWindowAttributes *pattributes)
{
	*pvaluemask |=
	    CWBackingStore | CWCursor | CWSaveUnder | CWBorderPixel |
	...
	pattributes->cursor = Scr.MvwmCursors[CRS_DEFAULT];
...
static void
setup_auxiliary_windows(MvwmWindow *fw, Bool setup_frame_and_parent,
    short buttons)
{
	...
	if (HAS_TITLE(fw)) {
		...
		setup_button_windows(fw, valuemask_save, &attributes,
		    buttons);
	}
...
static void
destroy_auxiliary_windows(MvwmWindow *fw, Bool destroy_frame_and_parent)
{
	if (destroy_frame_and_parent) {
		XDeleteContext(dpy, FW_W_FRAME(fw), MvwmContext);
		XDeleteContext(dpy, FW_W_PARENT(fw), MvwmContext);
...
static void
setup_icon(MvwmWindow *fw, window_style *pstyle)
{
...
destroy_icon(MvwmWindow *fw)
{
	...
	if (FW_W_ICON_TITLE(fw)) {
		...
		XDeleteContext(dpy, FW_W_ICON_TITLE(fw), MvwmContext);
		XFlush(dpy);
	}
	if (FW_W_ICON_PIXMAP(fw) != None) {
		...
		XDeleteContext(dpy, FW_W_ICON_PIXMAP(fw), MvwmContext);
	}
	clear_icon(fw);
	XFlush(dpy);
...
static void
setup_icon_boxes(MvwmWindow *fw, window_style *pstyle)
{
	icon_boxes     *ib;
...
static void
destroy_icon_boxes(MvwmWindow *fw)
{
	if (fw->IconBoxes) {
...
static void
setup_layer(MvwmWindow *fw, window_style *pstyle)
{
	MvwmWindow     *tf;
	int             layer;
	...
	} else if ((tf = get_transientfor_mvwmwindow(fw)) != NULL) {
		/*
...
static void
destroy_mini_icon(MvwmWindow *fw)
{
	if (fw->mini_icon) {
		PDestroyMvwmPicture(dpy, fw->mini_icon);
		fw->mini_icon = 0;
	}
...
static void
setup_key_and_button_grabs(MvwmWindow *fw)
{
#ifdef BUGS_ARE_COOL
	/*
...
static void
__add_window_handle_x_resources(MvwmWindow *fw)
{
	int             client_argc = 0;
	...
	if (XGetCommand(dpy, FW_W(fw), &client_argv, &client_argc)) {
		if (client_argc > 0 && client_argv != NULL) {
			 ...
			 */
			MergeCmdLineResources(&db, table, 2,
			    fw->class.res_name, &client_argc, client_argv,
			    True);
		}
	}
	 ...
	 */
	if (GetResourceString(db, "mvwmstyle", fw->class.res_name, &rm_value)
	    && rm_value.size != 0) {
		char           *style_name;
		int             name_len;
...
void
setup_visible_name(MvwmWindow *fw, Bool is_icon)
{
	char           *ext_name;
	window_style    style;
...
void
setup_window_name(MvwmWindow *fw)
{
...
void
setup_title_geometry(MvwmWindow *fw, window_style *pstyle)
{
	int             width;
	int             offset;
...
void
setup_window_font(MvwmWindow *fw, window_style *pstyle, Bool do_destroy)
{
	/*
	 * get rid of old font
	 */
	...
	if (!IS_WINDOW_FONT_LOADED(fw)) {
		if (S_HAS_WINDOW_FONT(SCF(*pstyle)) &&
		    ...
		    (fw->title_font =
			FlocaleLoadFont(dpy, SGET_WINDOW_FONT(*pstyle),
			    "mvwm"))) {
...
void
setup_icon_font(MvwmWindow *fw, window_style *pstyle, Bool do_destroy)
{
	int             height = 0;
	...
	if (!IS_ICON_FONT_LOADED(fw)) {
		if (S_HAS_ICON_FONT(SCF(*pstyle)) && SGET_ICON_FONT(*pstyle)
		    && (fw->icon_font =
			FlocaleLoadFont(dpy, SGET_ICON_FONT(*pstyle),
			    "mvwm"))) {
...
void
setup_style_and_decor(MvwmWindow *fw, window_style *pstyle, short *buttons)
{
	/*
	...
	if (!IS_DECOR_CHANGED(fw)) {
		MvwmDecor      *decor = &Scr.DefaultDecor;
	...
	if (FHaveShapeExtension) {
		if (fw->wShaped) {
			set_window_border_size(fw,
			    fw->unshaped_boundary_width);
...
void
change_icon_boxes(MvwmWindow *fw, window_style *pstyle)
{
...
void
setup_frame_size_limits(MvwmWindow *fw, window_style *pstyle)
{
...
void
setup_placement_penalty(MvwmWindow *fw, window_style *pstyle)
{
...
void
setup_frame_attributes(MvwmWindow *fw, window_style *pstyle)
{
	XSetWindowAttributes xswa;
...
void
change_auxiliary_windows(MvwmWindow *fw, short buttons)
{
	unsigned long   valuemask_save = 0;
	XSetWindowAttributes attributes;
...
void
increase_icon_hint_count(MvwmWindow *fw)
{
	if (fw->wmhints &&
...
void
change_icon(MvwmWindow *fw, window_style *pstyle)
{
	destroy_icon(fw);
...
void
change_mini_icon(MvwmWindow *fw, window_style *pstyle)
{
	MvwmPicture    *old_mi = fw->mini_icon;
	...
	if (old_mi != NULL && fw->mini_icon == 0) {
		 ...
		 */
		BroadcastMvwmPicture(M_MINI_ICON, FW_W(fw), FW_W_FRAME(fw),
...
void
setup_focus_policy(MvwmWindow *fw)
{
...
Bool
validate_transientfor(MvwmWindow *fw)
{
	XWindowAttributes wa;
	MvwmWindow     *cw;
	Window          w;
	...
	if (w == None || w == FW_W(fw) || w == IS_EWMH_DESKTOP(w)) {
		...
		return False;
	} else if (XFindContext(dpy, w, MvwmContext,
		(caddr_t *) & cw) != XCNOENT) {
		if (cw == fw) {
			/*
		 ...
		 */
		while (XFindContext(dpy, FW_W_TRANSIENTFOR(cw), MvwmContext,
			(caddr_t *) & cw) != XCNOENT && IS_TRANSIENT(cw)) {
...
Bool
setup_transientfor(MvwmWindow *fw)
{
	Bool            rc;
 ...
 *
 *  Procedure:
 *	AddWindow - add a new window to the mvwm list
 *
 */
MvwmWindow     *
AddWindow(const char **ret_initial_map_command, const exec_context_t *exc,
    MvwmWindow *ReuseWin, initial_window_options_t *win_opts)
{
	/*
	 * new mvwm window structure
	 */
	register MvwmWindow *fw;
	MvwmWindow     *tmp;
	/*
	...
	if (XGetGeometry(dpy, w, &JunkRoot, &JunkX, &JunkY,
		...
		if (bo.do_display_new_window_names) {
			mvwm_msg(INFO, "AddWindow", "new window disappeared");
		}
		free(fw);
		MyXUngrabServer(dpy);
		return NULL;
	...
	if (SIS_UNMANAGED(sflags)) {
		if (bo.do_display_new_window_names) {
			mvwm_msg(INFO, "AddWindow",
			    "new window is unmanaged:\n" "  name:      %s\n"
			    "  icon name: (unknown)\n" "  resource:  %s\n"
			    "  class:     %s", fw->name.name,
	...
	if (bo.do_display_new_window_names) {
		mvwm_msg(INFO, "AddWindow", "new window:\n"
		    "  name:      %s\n"
		    "  icon name: %s\n"
		    "  resource:  %s\n"
		    "  class:     %s",
	...
	*ret_initial_map_command =
	...
	/*
	 * add the window to the end of the mvwm list
	 */
	fw->next = Scr.MvwmRoot.next;
	fw->prev = &Scr.MvwmRoot;
	while (fw->next != NULL) {
		fw->prev = fw->next;
	 ...
	 * This chain is anchored at both ends on Scr.MvwmRoot, there are
	 * no null pointers.
	 */
	add_window_to_stack_ring_after(fw, &Scr.MvwmRoot);
	...
	if (used_sm) {
		 ...
		 */
		gravity_translate_to_northwest_geometry_no_bw(fw->hints.
		    win_gravity, fw, &fw->g.normal, &fw->g.normal);
		...
		do_resize_too =
		    setup_window_placement(fw, &style, &attr_g, win_opts,
		    PLACE_INITIAL);
		wattr.x = attr_g.x;
		wattr.y = attr_g.y;
		...
		fw->g.frame.width = wattr.width + b.total_size.width;
		fw->g.frame.height = wattr.height + b.total_size.height;
		gravity_constrain_size(fw->hints.win_gravity, fw,
		    &fw->g.frame, 0);
	...
	/****** make sure the window is not destroyed when mvwm dies ******/
	if (!IS_TEAR_OFF_MENU(fw)) {
		/*
		 * menus were created by mvwm itself, don't add them to the
		 * * save set
		 */
	...
	if (is_resizing_event_pending(fw) == True) {
		...
		mr_args =
		    frame_create_move_resize_args(fw,
	 ...
	 * * of MvwmIconBox which can't handle M_ICON_FILE after M_RES_NAME
	 */
	/****** icon and mini icon ******/
	/*
	...
	if (!(fw->hints.flags & USPosition) || used_sm) {
		...
		if ((stick_page &&
			!IsRectangleOnThisPage(mon, &fw->g.frame,
			    mon->virtual_scr.CurrentDesk)) || (stick_desk
			 ...
			 * * e.g. if we want a sticky 'panel' in MvwmButtons but
			...
			handle_stick(NULL, exc2, "", stick_page, stick_desk,
			    1, 0);
	...
	if (do_resize_too) {
		...
		memset(&e, 0, sizeof(e));
		FWarpPointer(dpy, Scr.Root, Scr.Root, 0, 0, mon->coord.w,
		    mon->coord.h,
	...
	if (HAS_EWMH_INIT_MAXVERT_STATE(fw) == EWMH_STATE_HAS_HINT ||
		...
		if (is_function_allowed(F_MAXIMIZE, NULL, fw,
			...
			execute_function_override_window(NULL, NULL, cmd, 0,
			    fw);
		}
	}
 ...
 *
 *  Inputs:
 *	tmp - the mvwm window structure to use
 *
 */
void
FetchWmProtocols(MvwmWindow *tmp)
{
	...
	if (XGetWMProtocols(dpy, FW_W(tmp), &protocols, &n)) {
		...
		if ((XGetWindowProperty(dpy, FW_W(tmp), _XA_WM_PROTOCOLS, 0L,
			    10L, False, _XA_WM_PROTOCOLS, &atype, &aformat,
			    &nitems, &bytes_remain,
...
void
GetWindowSizeHints(MvwmWindow *fw)
{
	long            supplied = 0;
	char           *broken_cause = "";
...
#define minAspectY fw->hints.min_aspect.y
		...
		if ((maxAspectX < 0) || (maxAspectY < 0) ||
			...
			mvwm_msg(WARN, "GetWindowSizeHints",
			    "The applicaton window (window id %#lx)\n"
			    "  \"%s\" has broken aspect ratio: "
			    "%d/%d - %d/%d\n"
			    "    mvwm is ignoring this aspect ratio.  ",
 ...
 */
void
free_window_names(MvwmWindow *fw, Bool nukename, Bool nukeicon)
{
	if (!fw) {
		return;
	}
...
destroy_window(MvwmWindow *fw)
{
	...
	if ((scr_flags.is_executing_complex_function ||
		...
		adjust_mvwm_internal_windows(fw);
 ...
 *
 *  Puts windows back where they were before mvwm took over
 *
 */
void
RestoreWithdrawnLocation(MvwmWindow *fw, Bool is_restart_or_recapture,
    Window parent)
{
	int             w2, h2;
	unsigned int    mask;
 ...
 *
 *  Procedure:
 *	Reborder - Removes mvwm border windows
 *
 */
void
Reborder(void)
{
	 ...
	 * force reinstall
	 */
	InstallWindowColormaps(&Scr.MvwmRoot);
	 ...
	 * * restart.
	 */
	for (fw = get_prev_window_in_stack_ring(&Scr.MvwmRoot);
	    fw != &Scr.MvwmRoot; fw = get_prev_window_in_stack_ring(fw)) {
		if (!IS_ICONIFIED(fw)
...
CaptureAllWindows(const exec_context_t *exc, Bool is_recapture)
{
	...
	if (!(scr_flags.are_windows_captured)) {	/* initial capture? */
		 ...
		 */
		for (i = 0; i < nchildren; i++) {
			if (XFindContext(dpy, children[i], MvwmContext,
				(caddr_t *) & fw) != XCNOENT) {
				CaptureOneWindow(exc, fw, children[i],
				    keep_on_top_win, parent_win,
				    is_recapture);
			}
		}
		 ...
		 */
		if (focus_w) {
			for (t = Scr.MvwmRoot.next; t && FW_W(t) != focus_w;
			    t = t->next) {
				;
			}
			if (t) {
