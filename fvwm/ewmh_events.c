...
ewmh_CurrentDesktop(EWMH_CMD_ARGS)
{
	...
	if (ev->xclient.data.l[0] < 0 || ev->xclient.data.l[0] > 0x7fffffff) {
		fvwm_msg(WARN, "ewmh_CurrentDesktop",
...
ewmh_DesktopGeometry(EWMH_CMD_ARGS)
{
	...
	if (width <= 0 || height <= 0) {
		fvwm_msg(WARN, "ewmh_DesktopGeometry",
		    "The application window (id %#lx)\n"
...
ewmh_DesktopViewPort(EWMH_CMD_ARGS)
{
	...
	if (ev->xclient.data.l[0] < 0 ||
		...
		fvwm_msg(WARN, "ewmh_DesktopViewPort",
		    "The application window (id %#lx)\n"
...
ewmh_NumberOfDesktops(EWMH_CMD_ARGS)
{
	...
	if (d > 0 && (d <= ewmhc.MaxDesktops || ewmhc.MaxDesktops == 0)) {
		...
		fvwm_msg(WARN, "ewmh_NumberOfDesktops",
		    "The application window (id %#lx)\n"
...
ewmh_WMStateMaxHoriz(EWMH_CMD_ARGS)
{
	...
	if (ev == NULL && style == NULL) {
		 ...
		 */
		/* TA:  Oh, but it does!  This is wrong!!! FIXME */
		return 0;
#endif
	}
...
ewmh_WMIconGeometry(EWMH_CMD_ARGS)
{
	...
	if (val != NULL && size < 4 * sizeof(CARD32)) {
		fvwm_msg(WARN, "ewmh_WMIconGeometry",
		    "The application window (id %#lx)\n"
