...
ReadMultiPixmapDecor(char *s, DecorFace *df)
{
	...
	df->u.mp.pixmaps = pm =
	    xcalloc(TBMP_NUM_PIXMAPS, sizeof(FvwmPicture *));
...
ReadDecorFace(char *s, DecorFace *df, int button, int verbose)
{
	...
	if (strncasecmp(style, "--", 2) != 0) {
		...
		if (strncasecmp(style, "Simple", 6) == 0) {
			...
			fpa.mask = (Pdepth <= 8) ? FPAM_DITHER : 0;	/* ? */
			df->u.p =
			    PCacheFvwmPicture(dpy, Scr.NoFocusWin, NULL, file,
			    fpa);
			if (df->u.p == NULL) {
				if (file) {
					if (verbose) {
    ...
    unsigned int *keycode, unsigned int *modifiers)
{
	...
	*modifiers = 0;
	...
	for (m = 0; m <= 8; ++m) {
		KeySym          ks =
		    fvwm_KeycodeToKeysym(dpy, *keycode, m, 0);
		if (ks == keysym) {
			switch (m) {
			case 0:	/* No modifiers */
				break;
