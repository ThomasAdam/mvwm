...
setup_resize_handle_windows(FvwmWindow *fw)
{
	...
	for (i = 0; i < 4; i++) {
		...
		XSaveContext(dpy, FW_W_CORNER(fw, i), FvwmContext,
		    (caddr_t) fw);
		attributes.win_gravity = s_grav[i];
		FW_W_SIDE(fw, i) =
		    XCreateWindow(dpy, FW_W_FRAME(fw), -1, -1, 1, 1, 0,
		    Pdepth, InputOutput, Pvisual, valuemask, &attributes);
		XSaveContext(dpy, FW_W_SIDE(fw, i), FvwmContext,
		    (caddr_t) fw);
	}
...
void
setup_visible_name(FvwmWindow *fw, Bool is_icon)
{
	char           *ext_name;
	window_style    style;
...
validate_transientfor(FvwmWindow *fw)
{
	...
	if (w == None || w == FW_W(fw) || w == IS_EWMH_DESKTOP(w)) {
		...
		return False;
	} else if (XFindContext(dpy, w, FvwmContext,
		(caddr_t *) & cw) != XCNOENT) {
		if (cw == fw) {
			/*
