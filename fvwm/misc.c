...
GrabEm(int cursor, int grab_context)
{
	...
	if (grab_context <= GRAB_STARTUP || grab_context >= GRAB_MAXVAL) {
		fvwm_msg(ERR, "GrabEm", "Bug: Called with illegal context %d",
		    grab_context);
		return False;
	}
	...
	while (i < rep &&
		...
		XGrabPointer(dpy, grab_win, True, GRAB_EVMASK, GrabModeAsync,
		    GrabModeAsync, None,
		    (grab_context ==
			GRAB_FREEZE_CURSOR) ? None : Scr.FvwmCursors[cursor],
		    CurrentTime) != GrabSuccess)) {
		switch (val) {
...
void
fvwm_msg(fvwm_msg_t type, char *id, char *msg, ...)
{
	va_list         args;
	char           *mfmt;
	char            fvwm_id[20];
