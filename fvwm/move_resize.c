...
static void
DisplayPosition(const FvwmWindow *tmp_win, const XEvent *eventp, int x, int y,
    int Init)
{
	char            str[100];
	int             offset;
...
placement_binding(int button, KeySym keysym, int modifier, char *action)
{
	if (keysym != 0) {
		 ...
		 * fixme
		 */
		fvwm_msg(ERR, "placement_binding",
...
__move_window(F_CMD_ARGS, Bool do_animate, int mode)
{
	...
	if (w == FW_W_FRAME(fw)) {
		...
		dy = FinalY - fw->g.frame.y;
		if (do_animate) {
			AnimatedMoveFvwmWindow(fw, w, -1, -1, FinalX, FinalY,
			    fWarp, -1, NULL);
		}
...
CMD_SnapAttraction(F_CMD_ARGS)
{
	...
	fvwm_msg(OLD, "CMD_SnapAttraction",
...
CMD_XorPixmap(F_CMD_ARGS)
{
	...
	if (xp == NULL) {
		fvwm_msg(ERR, "SetXORPixmap", "Can't find pixmap %s",
		    PixmapName);
		free(PixmapName);
		PictureUseFvwmVisual();
		return;
...
__resize_get_dir_from_window(int *ret_xmotion, int *ret_ymotion,
    FvwmWindow *fw, Window context_w)
{
