...
static int      collect_transients_recursive(FvwmWindow *t,
    FvwmWindow *list_head, int layer, stack_mode_t mode,
    ...
    Bool do_broadcast_all, Bool do_lower)
{
	...
	for (t = r->stack_next, i = 0; t != s; t = t->stack_next) {
		if (i > count) {
			fvwm_msg(ERR, "__restack_window_list",
...
static Bool
__mark_transient_subtree_test(FvwmWindow *s, FvwmWindow *start,
    FvwmWindow *end, int mark_mode, Bool do_ignore_icons,
...
static void
__raise_or_lower_window(FvwmWindow *t, stack_mode_t mode,
    Bool allow_recursion, Bool is_new_window, Bool is_client_request)
{
	FvwmWindow     *t2;
