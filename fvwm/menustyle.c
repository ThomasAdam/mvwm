...
menustyle_copy_face(MenuFace *destmf, MenuFace *origmf)
{
	...
	switch (origmf->type) {
	case SolidMenu:
		fvwmlib_copy_color(dpy, &destmf->u.back, &origmf->u.back,
		    False, True);
...
menustyle_free(MenuStyle *ms)
{
	...
	if (ST_HAS_STIPPLE_FORE(ms)) {
		fvwmlib_free_colors(dpy, &ST_MENU_STIPPLE_COLORS(ms).fore, 1,
		    True);
	}
	if (ST_HAS_ACTIVE_BACK(ms)) {
		fvwmlib_free_colors(dpy, &ST_MENU_ACTIVE_COLORS(ms).back, 1,
		    True);
	}
	if (ST_HAS_ACTIVE_FORE(ms)) {
		fvwmlib_free_colors(dpy, &ST_MENU_ACTIVE_COLORS(ms).fore, 1,
		    True);
	}
...
menustyle_parse_style(F_CMD_ARGS)
{
	...
	if (ms && ST_USAGE_COUNT(ms) != 0) {
		fvwm_msg(ERR, "NewMenuStyle", "menu style %s is in use",
		    name);
		return ms;
	}
	...
	while (!is_initialised || (action && *action)) {
		...
		case 3:	/* Foreground */
			fvwmlib_free_colors(dpy, &ST_MENU_COLORS(tmpms).fore,
			    1, True);
			if (arg1) {
		...
		case 4:	/* Background */
			fvwmlib_free_colors(dpy, &ST_MENU_COLORS(tmpms).back,
			    1, True);
			if (arg1) {
		...
		case 31:	/* SidePic */
			...
			if (arg1) {
				fpa.mask = (Pdepth <= 8) ? FPAM_DITHER : 0;
				ST_SIDEPIC(tmpms) =
				    PCacheFvwmPicture(dpy, Scr.NoFocusWin,
				    NULL, arg1, fpa);
				if (!ST_SIDEPIC(tmpms)) {
...
menustyle_copy(MenuStyle *origms, MenuStyle *destms)
{
	 ...
	 * side color
	 */
	fvwmlib_copy_color(dpy, &ST_SIDE_COLOR(destms),
