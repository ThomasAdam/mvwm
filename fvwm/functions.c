    ...
    Bool has_ref_window_moved)
{
	...
	if (func_depth > MAX_FUNCTION_DEPTH) {
		fvwm_msg(ERR, "__execute_function",
	...
	if (expaction[0] == '*') {
		if (Scr.cur_decor && Scr.cur_decor != &Scr.DefaultDecor) {
			fvwm_msg(WARN, "__execute_function",
...
__run_complex_function_items(cond_rc_t *cond_rc, char cond,
    FvwmFunction * func, const exec_context_t *exc, char *args[],
    Bool has_ref_window_moved)
{
	char            c;
	FunctionItem   *fi;
...
CMD_EchoFuncDefinition(F_CMD_ARGS)
{
	...
	if (bif != NULL) {
		fvwm_msg(INFO, "EchoFuncDefinition",
	...
	fvwm_msg(INFO, "EchoFuncDefinition", "definition of function '%s':",
	    token);
	for (fi = func->first_item; fi != NULL; fi = fi->next_item) {
		fvwm_msg(INFO, "EchoFuncDefinition", "  %c %s", fi->condition,
