...
fw_match_style_id(FvwmWindow *fw, style_id_t s_id)
{
	if (SID_GET_HAS_NAME(s_id)) {
		if (matchWildcards(SID_GET_NAME(s_id), fw->class.res_class) == 1)
			return 1;

		if (matchWildcards(SID_GET_NAME(s_id), fw->class.res_name) == 1)
			return 1;
    ...
    Bool is_reversed, focus_policy_t *f, focus_policy_t *m, focus_policy_t *c)
{
	...
	case 1:
		...
		if (token != NULL && *token != 0) {
			fvwm_msg(ERR, "style_parse_focus_policy_style",
	...
	case 'b':
		if (StrEquals(token, "BackColor")) {
			...
			if (token) {
				...
				fvwm_msg(ERR, "style_parse_on_estyle_option",
...
parse_and_set_window_style(char *action, char *prefix, window_style *ps)
{
	...
	while (action && *action && *action != '\n') {
		...
		if (found == False) {
			fvwm_msg(ERR, "style_parse_and_set_window_style",
