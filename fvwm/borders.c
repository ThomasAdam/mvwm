...
typedef struct
{
	...
	unsigned			 has_an_upsidedown_rotation:1;
} titlebar_descr;
...
static void     border_rotate_titlebar_descr(FvwmWindow *, titlebar_descr *);

/* for grouping the MultiPixmap stuff */
static Bool
border_mp_get_use_title_style_parts_and_geometry(titlebar_descr *,
    FvwmPicture **, FvwmAcs *, unsigned short, int, rectangle *, int *);
    ...
    FvwmWindow *fw, Bool do_hilight)
{
	...
	if (transparent_gc == None && !HAS_NO_BORDER(fw)
		...
		xgcv.plane_mask = 0;
		transparent_gc =
		    fvwmlib_XCreateGC(dpy, Scr.NoFocusWin,
...
border_mp_render_into_pixmap(titlebar_descr *td, common_decorations_type *cd,
    FvwmPicture **src, FvwmAcs *acs, Pixel *pixels,
...
static void
border_mp_get_titlebar_descr(FvwmWindow *fw, titlebar_descr *td,
    DecorFace *df)
{
	DecorFace		*tsdf;
	FvwmPicture		**pm;
...
void
border_draw_decorations(FvwmWindow *fw, window_parts draw_parts,
