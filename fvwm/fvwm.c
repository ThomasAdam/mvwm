	...
	if (restart) {
		...
		if (command) {
			...
			if (n <= 0) {
				...
				execvp(my_argv[0], my_argv);
				fvwm_msg(ERR, "Done", "Call of '%s' failed!"
				    " (restarting '%s' instead)",
				    my_argv[0], g_argv[0]);
				perror("  system error description");
			}
...
InstallSignals(void)
{
	...
	fvwmSetSignalMask(sigmask(SIGUSR1) | sigmask(SIGINT) | sigmask(SIGHUP)
...
CreateGCs(void)
{
	...
	Scr.AlphaGC =
	    fvwmlib_XCreateGC(dpy, Scr.ScratchAlphaPixmap, gcm, &gcv);
	return;
}
...
main(int argc, char **argv)
{
	...
	PUseDynamicColors = 0;
	Scr.ColorLimit =
	    PictureInitColors(PICTURE_CALLED_BY_FVWM, True, &colorLimitop,
	    True, True);
	...
	if (num_config_commands > 0) {
		...
		if (!run_command_file(CatString3(fvwm_userdir, "/",
			    FVWM_CONFIG), exc)
		    && !run_command_file(CatString3(FVWM_DATADIR, "/",
			    FVWM_CONFIG), exc)
		    && !run_command_file(CatString3(fvwm_userdir, "/",
			    FVWM2RC), exc)
		    && !run_command_file(CatString3(home_dir, "/", FVWM2RC),
			exc)
		    && !run_command_file(CatString3(FVWM_DATADIR, "/",
			    FVWM2RC), exc)
		    && !run_command_file(CatString3(FVWM_DATADIR, "/system",
			    FVWM2RC), exc)
		    && !run_command_file(CatString3(FVWM_CONFDIR, "/system",
			    FVWM2RC), exc)) {
			fvwm_msg(ERR, "main",
